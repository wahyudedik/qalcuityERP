...
@endphp
<tr class="{{ $isGroup ? 'bg-gray-50/50' : '' }} hover:bg-gray-50 transition">
    {{-- Kode --}}
    <td class="px-4 py-2.5 text-xs text-gray-500 font-mono">{{ $item->code }}</td>
    ...
    <td class="px-4 py-2.5">
        ...
        @if ($item->category)
            <span
                class="ml-1.5 text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 capitalize">{{ $item->category }}</span>
        @endif
    </td>
    ...
    {{-- Satuan --}}
    <td class="px-4 py-2.5 text-xs text-gray-500">{{ $item->unit }}</td>
    ...
    {{-- Jumlah RAB --}}
    <td
        class="px-4 py-2.5 text-right text-sm font-mono {{ $isGroup ? 'font-semibold text-gray-900' : 'text-gray-700' }}">
        Rp {{ number_format($item->subtotal, 0, ',', '.') }}
    </td>

    {{-- Realisasi --}}
    <td
        class="px-4 py-2.5 text-right text-sm font-mono {{ $overBudget ? 'text-red-500 font-semibold' : 'text-gray-600' }}">
