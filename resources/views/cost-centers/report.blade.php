...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('cost-centers.index') }}" class="px-4 py-2 rounded-xl text-sm border border-gray-200 dark:border-white/10 text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-white/10 transition">
            ← Kembali
        </a>
    </div>
    ...
    <form method="GET" class="flex gap-3 flex-wrap items-end">
        <div>
            ...
            <input type="date" name="from" value="{{ $from }}" class="px-3 py-2 rounded-xl text-sm border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-900 dark:text-white">
        </div>
        <div>
            <label class="block text-xs font-medium text-gray-500 dark:text-slate-400 mb-1">Sampai</label>
            <input type="date" name="to" value="{{ $to }}" class="px-3 py-2 rounded-xl text-sm border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-900 dark:text-white">
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-5 py-3 font-medium text-gray-900 dark:text-white">{{ $row['label'] }}</td>
                        ...
                        <td class="px-5 py-3 text-right font-semibold font-mono {{ $row['profit'] >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-500' }}">
                ...
                <tfoot class="bg-gray-50 dark:bg-white/5 font-bold border-t-2 border-gray-200 dark:border-white/20">
                    <tr>
                        ...
                        <td class="px-5 py-3 text-right font-mono {{ $totals['profit'] >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-500' }}">
