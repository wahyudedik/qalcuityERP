            ...
            <div class="lg:w-72 shrink-0">
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    ...
                    <form method="POST" action="{{ route('accounting.coa.store') }}" class="space-y-3">
                        ...
                        <div>
                            ...
                            <select name="parent_id"
                                ...
                                @foreach ($headers ?? [] as $h)
                                    <option value="{{ $h->id }}">{{ $h->code }} - {{ $h->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <button type="submit"
                            class="w-full py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Tambah
                            Akun</button>
                    </form>
                    <form method="POST" action="{{ route('accounting.coa.seed') }}" class="mt-3">
                        @csrf
                        <button type="submit" data-confirm="Load COA default Indonesia?"
                    ...
                    <form method="GET" class="flex gap-2 flex-1">
                        ...
                        <select name="type"
                            ...
                            @foreach (['asset' => 'Aset', 'liability' => 'Kewajiban', 'equity' => 'Ekuitas', 'revenue' => 'Pendapatan', 'expense' => 'Beban'] as $v => $l)
                                <option value="{{ $v }}" @selected(request('type') === $v)>{{ $l }}
                                </option>
                            @endforeach
                        </select>
                        <button type="submit"
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse($accounts as $acc)
                                    <tr class="hover:bg-gray-50 {{ $acc->is_header ? 'font-semibold' : '' }}">
                                        ...
                                        <td class="px-4 py-3 text-gray-900">
                                            ...
                                            @if ($acc->parent)
                                                <span class="text-xs text-gray-400">({{ $acc->parent?->code }})</span>
                                            @endif
                                        </td>
                                        <td class="px-4 py-3 hidden sm:table-cell">
                                            @php
                                                ...
                                                $tc = $typeColors[$acc->type] ?? 'gray';
                                            @endphp
                                            <span
                                                class="px-2 py-0.5 rounded-full text-xs bg-{{ $tc }}-100 text-{{ $tc }}-700 $tc }}-500/20 $tc }}-400">
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            <div class="flex items-center justify-center gap-1">
                                                <button
                                                    onclick="openEditCoa({{ $acc->id }},'{{ addslashes($acc->code) }}','{{ addslashes($acc->name) }}','{{ $acc->type }}','{{ $acc->normal_balance }}',{{ $acc->parent_id ?? 'null' }},{{ $acc->is_header ? 'true' : 'false' }},{{ $acc->is_active ? 'true' : 'false' }})"
                                                    class="text-xs px-2 py-1 border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50">Edit</button>
                                                <form method="POST"
                                                    action="{{ route('accounting.coa.destroy', $acc) }}"
                                                    class="inline">
                                                    @csrf @method('DELETE')
                                                    <button type="submit"
                                                        data-confirm="Hapus akun {{ addslashes($acc->name) }}?"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-4 py-12 text-center text-gray-400">Belum ada
                                            akun. Klik "COA Default Indonesia" untuk memuat data awal.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($bankAccounts as $ba)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 font-mono text-xs text-gray-700">{{ $ba->account_number }}
                                    </td>
                                    <td class="px-4 py-3 hidden sm:table-cell text-gray-600">{{ $ba->account_name }}
                                    </td>
                                    <td class="px-4 py-3 text-right hidden md:table-cell text-gray-900">Rp
                                        {{ number_format($ba->balance, 0, ',', '.') }}</td>
                                    <td class="px-4 py-3 text-center">
                                        <span
                                            class="px-2 py-0.5 rounded-full text-xs {{ $ba->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex items-center justify-center gap-1">
                                            <button
                                                onclick="openEditBank({{ $ba->id }},'{{ addslashes($ba->bank_name) }}','{{ addslashes($ba->account_name) }}',{{ $ba->balance }})"
                                                class="text-xs px-2 py-1 border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50">Edit</button>
                                            <form method="POST" action="{{ route('bank-accounts.toggle', $ba) }}"
                                                class="inline">
                                                @csrf @method('PATCH')
                                                <button type="submit"
                                                    class="text-xs px-2 py-1 {{ $ba->is_active ? 'text-amber-500' : 'text-green-500' }} hover:underline">
                                            ...
                                            <form method="POST" action="{{ route('bank-accounts.destroy', $ba) }}"
                                                class="inline">
                                                @csrf @method('DELETE')
                                                <button type="submit" data-confirm="Hapus rekening ini?"
                                                    data-confirm-type="danger"
                                                    class="text-xs px-2 py-1 text-red-500 hover:text-red-700">✕</button>
                                            </form>
                                        </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-12 text-center text-gray-400">Belum ada rekening
                                        bank.</td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div class="flex flex-col lg:flex-row gap-5">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    ...
                    <form method="POST" action="{{ route('taxes.store') }}" class="space-y-3">
                        ...
                        <div>
                            ...
                            <input type="number" name="rate" required min="0" max="100"
                        ...
                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="is_withholding" id="is_withholding" value="1"
                                class="rounded">
            ...
            <div class="flex-1">
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($taxes as $tax)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 text-center hidden md:table-cell">
                                        ...
                                        <span class="text-xs text-amber-500">WHT</span>@else<span
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex items-center justify-center gap-1">
                                            <button
                                                onclick="openEditTax({{ $tax->id }},'{{ addslashes($tax->name) }}','{{ $tax->code }}','{{ $tax->tax_type }}','{{ $tax->type }}',{{ $tax->rate }},'{{ $tax->account_code }}',{{ $tax->is_withholding ? 'true' : 'false' }})"
                                                class="text-xs px-2 py-1 border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50">Edit</button>
                                            <form method="POST" action="{{ route('taxes.destroy', $tax) }}"
                                                class="inline">
                                                @csrf @method('DELETE')
                                                <button type="submit" data-confirm="Hapus tarif pajak ini?"
                                                    data-confirm-type="danger"
                                                    class="text-xs px-2 py-1 text-red-500 hover:text-red-700">✕</button>
                                            </form>
                                        </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-12 text-center text-gray-400">Belum ada tarif
                                        pajak.</td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="mt-3 flex justify-end">
                    <a href="{{ route('taxes.efaktur') }}"
                        class="text-xs px-3 py-2 border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50">
                        Export e-Faktur CSV
                    </a>
                </div>
            </div>
        ...
        <div class="flex flex-col lg:flex-row gap-5">
            <div class="lg:w-72 shrink-0">
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    ...
                    <form method="POST" action="{{ route('settings.accounting.currencies.store') }}"
                        class="space-y-3">
                        @csrf
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">Kode *</label>
                        ...
                        <div>
                            ...
                            <input type="number" name="rate_to_idr" required min="0" step="0.0001"
                                placeholder="16000"
                                class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="is_active" id="cur_active" value="1" checked
                                class="rounded">
                            <label for="cur_active" class="text-xs text-gray-600">Aktif</label>
                        </div>
                        <button type="submit"
                            class="w-full py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Tambah Mata
                            Uang</button>
                    </form>
                </div>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($currencies as $cur)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <span
                                            class="px-2 py-0.5 rounded-full text-xs {{ $cur->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        @if (!$cur->is_base)
                                            <div class="flex items-center justify-center gap-1">
                                                <button
                                                    onclick="openEditCurrency({{ $cur->id }},'{{ $cur->name }}','{{ $cur->symbol }}',{{ $cur->rate_to_idr }},{{ $cur->is_active ? 'true' : 'false' }})"
                                                    class="text-xs px-2 py-1 border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50">Edit</button>
                                                <form method="POST"
                                                    action="{{ route('settings.accounting.currencies.destroy', $cur) }}"
                                                    class="inline">
                                                    @csrf @method('DELETE')
                                                    <button type="submit"
                                                        data-confirm="Hapus mata uang {{ $cur->code }}?"
                                                        data-confirm-type="danger"
                                                        class="text-xs px-2 py-1 text-red-500 hover:text-red-700">✕</button>
                                                </form>
                                            </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-12 text-center text-gray-400">Belum ada mata
                                        uang.</td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form id="form-edit-coa" method="POST" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Kode *</label>
                        <input type="text" name="code" id="edit-coa-code" required
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Tipe *</label>
                        <select name="type" id="edit-coa-type" required
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="asset">Aset</option>
                            <option value="liability">Kewajiban</option>
                            <option value="equity">Ekuitas</option>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Nama Akun *</label>
                    <input type="text" name="name" id="edit-coa-name" required
                        class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Saldo Normal *</label>
                        <select name="normal_balance" id="edit-coa-normal" required
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="debit">Debit</option>
                            <option value="credit">Kredit</option>
                        </select>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Akun Induk</label>
                        <select name="parent_id" id="edit-coa-parent"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="">-- Tidak ada --</option>
                            @foreach ($headers ?? [] as $h)
                                <option value="{{ $h->id }}">{{ $h->code }} - {{ $h->name }}
                ...
                <div class="flex items-center gap-4">
                    <label class="flex items-center gap-2 text-xs text-gray-600">
                        <input type="checkbox" name="is_header" id="edit-coa-header" value="1"
                            class="rounded"> Header
                    </label>
                    <label class="flex items-center gap-2 text-xs text-gray-600">
                        <input type="checkbox" name="is_active" id="edit-coa-active" value="1"
                            class="rounded"> Aktif
                    </label>
                </div>
                <div class="flex gap-3 pt-2">
                    <button type="button" onclick="document.getElementById('modal-edit-coa').classList.add('hidden')"
                        class="flex-1 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
                    <button type="submit"
    ...
    <div id="modal-edit-bank" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-edit-bank').classList.add('hidden')"
                    class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form id="form-edit-bank" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Nama Bank *</label>
                    <input type="text" name="bank_name" id="edit-bank-name" required
                        class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Nama Pemilik *</label>
                    <input type="text" name="account_name" id="edit-bank-owner" required
                        class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Saldo</label>
                    <input type="number" name="balance" id="edit-bank-balance" min="0" step="0.01"
                        class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="flex gap-3 pt-2">
                    <button type="button"
                        onclick="document.getElementById('modal-edit-bank').classList.add('hidden')"
                        class="flex-1 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
                    <button type="submit"
                        class="flex-1 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Simpan</button>
                </div>
    ...
    <div id="modal-edit-tax" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-edit-tax').classList.add('hidden')"
                    class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form id="form-edit-tax" method="POST" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Nama *</label>
                        <input type="text" name="name" id="edit-tax-name" required
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Kode *</label>
                        <input type="text" name="code" id="edit-tax-code" required
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Jenis Pajak *</label>
                        <select name="tax_type" id="edit-tax-type" required
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="ppn">PPN</option>
                            <option value="pph21">PPh 21</option>
                            <option value="pph23">PPh 23</option>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Tipe *</label>
                        <select name="type" id="edit-tax-calc" required
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="percentage">Persentase</option>
                            <option value="fixed">Nominal Tetap</option>
                        </select>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Tarif *</label>
                        <input type="number" name="rate" id="edit-tax-rate" required min="0"
                            step="0.01"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Kode Akun</label>
                        <input type="text" name="account_code" id="edit-tax-account"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                <div class="flex items-center gap-4">
                    <label class="flex items-center gap-2 text-xs text-gray-600">
                        <input type="checkbox" name="is_withholding" id="edit-tax-wht" value="1"
                            class="rounded"> WHT
                    </label>
                    <label class="flex items-center gap-2 text-xs text-gray-600">
                        <input type="checkbox" name="is_active" id="edit-tax-active" value="1"
                            class="rounded"> Aktif
                    </label>
                </div>
                <div class="flex gap-3 pt-2">
                    <button type="button" onclick="document.getElementById('modal-edit-tax').classList.add('hidden')"
                        class="flex-1 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
                    <button type="submit"
                        class="flex-1 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Simpan</button>
                </div>
    ...
    <div id="modal-edit-currency" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-sm shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-edit-currency').classList.add('hidden')"
                    class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form id="form-edit-currency" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Nama *</label>
                    <input type="text" name="name" id="edit-cur-name" required
                        class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Simbol *</label>
                        <input type="text" name="symbol" id="edit-cur-symbol" required
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Kurs ke IDR *</label>
                        <input type="number" name="rate_to_idr" id="edit-cur-rate" required min="0"
                            step="0.0001"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                <label class="flex items-center gap-2 text-xs text-gray-600">
                    <input type="checkbox" name="is_active" id="edit-cur-active" value="1" class="rounded">
                    Aktif
                </label>
                <div class="flex gap-3 pt-2">
                    <button type="button"
                        onclick="document.getElementById('modal-edit-currency').classList.add('hidden')"
                        class="flex-1 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
                    <button type="submit"
                        class="flex-1 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Simpan</button>
                </div>
