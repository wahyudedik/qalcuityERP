...
<?php $component->withAttributes([]); ?>
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
        ...
        <a href="<?php echo e(route('helpdesk.index')); ?>" class="px-3 py-2 text-sm text-gray-500 dark:text-slate-400">← Helpdesk</a>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
            <div class="flex items-start justify-between mb-2">
                <span class="px-2 py-0.5 rounded-full text-xs bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400"><?php echo e(ucfirst($a->category)); ?></span>
            ...
            <h3 class="font-semibold text-gray-900 dark:text-white mb-2"><?php echo e($a->title); ?></h3>
            <p class="text-xs text-gray-500 dark:text-slate-400 line-clamp-3"><?php echo e(Str::limit(strip_tags($a->body), 150)); ?></p>
            <p class="text-xs text-gray-400 mt-2"><?php echo e($a->views); ?> views · <?php echo e($a->created_at->format('d/m/Y')); ?></p>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
        <div class="col-span-full text-center py-12 text-gray-400 dark:text-slate-500">Belum ada artikel.</div>
        <?php endif; ?>
    </div>
    ...
    <div id="modal-add-kb" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-lg shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/10">
                <h3 class="font-semibold text-gray-900 dark:text-white">Buat Artikel</h3>
                <button onclick="document.getElementById('modal-add-kb').classList.add('hidden')" class="text-gray-400 hover:text-gray-600 dark:hover:text-white">✕</button>
            </div>
            <form method="POST" action="<?php echo e(route('helpdesk.kb.store')); ?>" class="p-6 space-y-4">
                ...
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Judul *</label><input type="text" name="title" required class="<?php echo e($cls); ?>"></div>
                ...
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Isi Artikel *</label><textarea name="body" required rows="6" class="<?php echo e($cls); ?>"></textarea></div>
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-add-kb').classList.add('hidden')" class="px-4 py-2 text-sm border border-gray-200 dark:border-white/10 rounded-xl text-gray-600 dark:text-slate-300">Batal</button>
