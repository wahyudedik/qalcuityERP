    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
                <div class="flex items-start justify-between gap-4 mb-4">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $t->subject }}</h2>
                        <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">{{ $t->ticket_number }} · {{ $t->created_at->format('d/m/Y H:i') }} · oleh {{ $t->creator->name ?? '-' }}</p>
                    </div>
                    @php
                    ...
                    <div class="flex gap-2">
                        <span class="px-2 py-0.5 rounded-full text-xs bg-{{ $pc }}-100 text-{{ $pc }}-700 dark:bg-{{ $pc }}-500/20 dark:text-{{ $pc }}-400">{{ ucfirst($t->priority) }}</span>
                        <span class="px-2 py-0.5 rounded-full text-xs bg-{{ $sc }}-100 text-{{ $sc }}-700 dark:bg-{{ $sc }}-500/20 dark:text-{{ $sc }}-400">{{ ucfirst(str_replace('_', ' ', $t->status)) }}</span>
                    </div>
                </div>
            ...
            <div class="space-y-3">
                ...
                <div class="bg-white dark:bg-[#1e293b] rounded-2xl border {{ $reply->is_internal ? 'border-amber-200 dark:border-amber-500/30' : 'border-gray-200 dark:border-white/10' }} p-4">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $reply->user->name ?? 'System' }}</span>
                    ...
                    <div class="text-sm text-gray-700 dark:text-slate-300">{!! nl2br(e($reply->body)) !!}</div>
                </div>
                @endforeach
            </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4 space-y-2 text-sm">
                ...
                <div><span class="text-gray-500 dark:text-slate-400">Customer:</span> <span class="text-gray-900 dark:text-white">{{ $t->customer->name ?? $t->contact_name ?? '-' }}</span></div>
                <div><span class="text-gray-500 dark:text-slate-400">Email:</span> <span class="text-gray-900 dark:text-white">{{ $t->contact_email ?? $t->customer->email ?? '-' }}</span></div>
                <div><span class="text-gray-500 dark:text-slate-400">Kategori:</span> <span class="text-gray-900 dark:text-white">{{ ucfirst($t->category) }}</span></div>
                @if($t->contract)<div><span class="text-gray-500 dark:text-slate-400">Kontrak:</span> <a href="{{ route('contracts.show', $t->contract) }}" class="text-blue-500 hover:underline">{{ $t->contract->contract_number }}</a></div>@endif
            </div>
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4 space-y-2 text-sm">
                ...
                <div class="flex justify-between"><span class="text-gray-500 dark:text-slate-400">Response Due:</span>
                    <span class="{{ $t->sla_response_due && $t->sla_response_due->isPast() && !$t->first_responded_at ? 'text-red-500' : 'text-gray-900 dark:text-white' }}">{{ $t->sla_response_due?->format('d/m H:i') ?? '-' }}</span>
                </div>
                <div class="flex justify-between"><span class="text-gray-500 dark:text-slate-400">First Response:</span>
                    <span class="text-gray-900 dark:text-white">{{ $t->first_responded_at?->format('d/m H:i') ?? '-' }}
                ...
                <div class="flex justify-between"><span class="text-gray-500 dark:text-slate-400">Resolve Due:</span>
                    <span class="{{ $t->isOverdue() ? 'text-red-500 font-semibold' : 'text-gray-900 dark:text-white' }}">{{ $t->sla_resolve_due?->format('d/m H:i') ?? '-' }}</span>
                </div>
                <div class="flex justify-between"><span class="text-gray-500 dark:text-slate-400">Resolved:</span>
                    <span class="text-gray-900 dark:text-white">{{ $t->resolved_at?->format('d/m H:i') ?? '-' }}
