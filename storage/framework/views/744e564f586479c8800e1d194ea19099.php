...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <form method="POST" action="<?php echo e(route('settings.modules.update')); ?>">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
            <h3 class="text-sm font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-4"><?php echo e($groupName); ?></h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <?php $m = $meta[$key]; $isEnabled = in_array($key, $enabled); ?>
                <label class="module-card flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition
                    ...
                    <div class="shrink-0">
                        <div class="w-10 h-5 rounded-full transition-colors <?php echo e($isEnabled ? 'bg-blue-600' : 'bg-gray-300 dark:bg-white/20'); ?> relative toggle-track">
                            <div class="absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform <?php echo e($isEnabled ? 'translate-x-5' : 'translate-x-0.5'); ?> toggle-thumb"></div>
                        </div>
                    </div>
                </label>
