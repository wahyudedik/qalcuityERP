    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-5 gap-4">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
                ...
                <p class="text-lg font-bold text-green-500 mt-1">Rp <?php echo e(number_format($summary['credit'], 0, ',', '.')); ?></p>
            </div>
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
                <p class="text-xs text-gray-500 dark:text-slate-400">Total Debit</p>
                <p class="text-lg font-bold text-red-500 mt-1">Rp <?php echo e(number_format($summary['debit'], 0, ',', '.')); ?></p>
            </div>
        </div>
            ...
            <form method="GET" class="flex flex-wrap gap-3 items-end">
                <div>
                    ...
                    <select name="account_id" class="bg-gray-50 dark:bg-[#0f172a] border border-gray-200 dark:border-white/10 rounded-xl px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="<?php echo e($acc->id); ?>" <?php if(request('account_id') == $acc->id): echo 'selected'; endif; ?>><?php echo e($acc->bank_name); ?> — <?php echo e($acc->account_number); ?></option>
                ...
                <div>
                    ...
                    <input type="date" name="from" value="<?php echo e(request('from')); ?>" class="bg-gray-50 dark:bg-[#0f172a] border border-gray-200 dark:border-white/10 rounded-xl px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 dark:text-slate-400 mb-1">Sampai</label>
                    <input type="date" name="to" value="<?php echo e(request('to')); ?>" class="bg-gray-50 dark:bg-[#0f172a] border border-gray-200 dark:border-white/10 rounded-xl px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                ...
                <a href="<?php echo e(route('bank.reconciliation')); ?>" class="px-4 py-2 border border-gray-200 dark:border-white/10 rounded-xl text-sm text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-white/5">Reset</a>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
            ...
            <form method="POST" action="<?php echo e(route('bank.import')); ?>" enctype="multipart/form-data" class="flex flex-wrap gap-3 items-end">
                <?php echo csrf_field(); ?>
                <div>
        ...
        <div id="ai-banner" class="bg-purple-50 dark:bg-purple-500/10 border border-purple-200 dark:border-purple-500/20 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm font-semibold text-purple-700 dark:text-purple-300"><?php echo e($unmatchedCount); ?> transaksi belum dicocokkan</p>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-white/10 flex items-center justify-between flex-wrap gap-2">
                ...
                <div class="flex gap-2 text-xs">
                    <span class="px-2 py-1 bg-green-500/20 text-green-400 rounded-full"><?php echo e($statements->where('status','matched')->count()); ?> matched</span>
                    <span class="px-2 py-1 bg-amber-500/20 text-amber-400 rounded-full"><?php echo e($statements->where('status','unmatched')->count()); ?> unmatched</span>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr id="row-<?php echo e($stmt->id); ?>" class="hover:bg-gray-50 dark:hover:bg-white/5 transition">
                            ...
                            <td class="px-4 py-3 text-gray-900 dark:text-white max-w-xs">
                                ...
                                <p class="text-xs text-gray-400 dark:text-slate-500">Ref: <?php echo e($stmt->reference); ?></p>
                                <?php endif; ?>
                            </td>
                            <td class="px-4 py-3 hidden sm:table-cell">
                                <span class="px-2 py-0.5 rounded-full text-xs font-medium <?php echo e($stmt->type === 'credit' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'); ?>">
                                    <?php echo e($stmt->type === 'credit' ? 'Kredit' : 'Debit'); ?>

                                </span>
                            </td>
                            <td class="px-4 py-3 text-right font-medium <?php echo e($stmt->type === 'credit' ? 'text-green-400' : 'text-red-400'); ?>">
                                Rp <?php echo e(number_format($stmt->amount, 0, ',', '.')); ?>

                            </td>
                            <td class="px-4 py-3 text-center">
                                <span id="status-<?php echo e($stmt->id); ?>" class="px-2 py-0.5 rounded-full text-xs font-medium
                                    <?php echo e($stmt->status === 'matched' ? 'bg-green-500/20 text-green-400' : 'bg-amber-500/20 text-amber-400'); ?>">
                                    <?php echo e($stmt->status === 'matched' ? 'Matched' : 'Unmatched'); ?>
                            ...
                            <td class="px-4 py-3 min-w-[200px]">
                                ...
                                <?php else: ?>
                                <div id="ai-cell-<?php echo e($stmt->id); ?>" class="text-xs text-slate-400 italic">Menunggu...</div>
                                <?php endif; ?>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <?php if($stmt->status === 'unmatched'): ?>
                                <div class="flex flex-col items-center gap-1">
                                    <button onclick="openMatchModal(<?php echo e($stmt->id); ?>)"
                                        id="btn-detail-<?php echo e($stmt->id); ?>"
                                        class="text-xs text-purple-400 hover:text-purple-300 hover:underline hidden">
                                        Detail AI
                                    </button>
                                    <button onclick="openManualMatch(<?php echo e($stmt->id); ?>, '<?php echo e(addslashes($stmt->description)); ?>', <?php echo e($stmt->amount); ?>)"
                ...
                <div class="space-y-2 max-h-60 overflow-y-auto" id="manual-erp-list">
                    ...
                    <button onclick="applyManualMatch(<?php echo e($erp['id']); ?>)"
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="text-sm font-medium text-gray-900 dark:text-white truncate"><?php echo e($erp['number']); ?></p>
                            <p class="text-xs text-gray-400 truncate"><?php echo e($erp['date']); ?> · <?php echo e(Str::limit($erp['description'], 50)); ?></p>
                        </div>
                        <span class="text-sm font-medium shrink-0 ml-3 <?php echo e($erp['type'] === 'debit' ? 'text-green-500' : 'text-red-500'); ?>">
