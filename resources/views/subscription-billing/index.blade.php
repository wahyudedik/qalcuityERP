<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('subscription-billing.plans') }}"
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    @forelse($subscriptions as $s)
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            ...
                            <td class="px-4 py-3 text-gray-700 dark:text-slate-300">{{ $s->customer->name ?? '-' }}
                            </td>
                            ...
                            <td
                                class="px-4 py-3 text-center hidden md:table-cell text-xs {{ $s->next_billing_date->isPast() ? 'text-red-500 font-semibold' : 'text-gray-500 dark:text-slate-400' }}">
                                {{ $s->next_billing_date->format('d/m/Y') }}</td>
                            <td class="px-4 py-3 text-center"><span
                                    class="px-2 py-0.5 rounded-full text-xs bg-{{ $sc }}-100 text-{{ $sc }}-700 dark:bg-{{ $sc }}-500/20 dark:text-{{ $sc }}-400">{{ $sl }}</span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    @if (in_array($s->status, ['active', 'trial']) && $s->next_billing_date->lte(today()))
                                        @canmodule('subscription_billing', 'create')
                                        <form method="POST" action="{{ route('subscription-billing.generate', $s) }}"
                                            class="inline">@csrf
                                            <button type="submit"
        ...
        @if ($subscriptions->hasPages())
            <div class="px-4 py-3 border-t border-gray-100 dark:border-white/5">{{ $subscriptions->links() }}</div>
        @endif
    </div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="{{ route('subscription-billing.store') }}" class="p-6 space-y-4">
                ...
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Customer *</label>
                    <select name="customer_id" required class="{{ $cls }}">
                ...
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Plan *</label>
                    <select name="plan_id" required class="{{ $cls }}">
                        <option value="">-- Pilih --</option>
                        @foreach ($plans as $p)
                            <option value="{{ $p->id }}">{{ $p->name }} — Rp
                                {{ number_format($p->price, 0, ',', '.') }}/{{ $p->cycleLabel() }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Mulai
                            *</label><input type="date" name="start_date" required value="{{ date('Y-m-d') }}"
                            class="{{ $cls }}"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Diskon
                            ...
                            step="0.01" value="0" class="{{ $cls }}"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Harga
                            ...
                            step="1000" placeholder="Kosong = pakai harga plan" class="{{ $cls }}">
                    </div>
