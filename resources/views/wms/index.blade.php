<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row gap-4 mb-6">
        <form method="GET" class="flex gap-2">
            <select name="warehouse_id" onchange="this.form.submit()" class="px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#1e293b] text-gray-900 dark:text-white">
                @foreach($warehouses as $w)<option value="{{ $w->id }}" @selected($warehouseId==$w->id)>{{ $w->name }}</option>@endforeach
            </select>
        </form>
    ...
    <div class="flex flex-wrap gap-2 mb-4">
        <a href="?warehouse_id={{ $warehouseId }}" class="px-3 py-1.5 text-xs rounded-xl {{ !request('zone_id') ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-slate-300' }}">Semua</a>
        @foreach($zones as $z)
        <a href="?warehouse_id={{ $warehouseId }}&zone_id={{ $z->id }}" class="px-3 py-1.5 text-xs rounded-xl {{ request('zone_id')==$z->id ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-slate-300' }}">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
        ...
        @endphp
        <div class="rounded-xl border {{ $bc }} p-3 text-center">
            <p class="font-mono text-xs font-bold text-gray-900 dark:text-white">{{ $bin->code }}</p>
            <p class="text-[10px] text-gray-400 dark:text-slate-500">{{ $bin->zone->name ?? '-' }}</p>
            @if($occupied)
            <div class="mt-1 space-y-0.5">
                @foreach($bin->stocks->where('quantity', '>', 0)->take(3) as $bs)
                <p class="text-[10px] text-gray-600 dark:text-slate-300 truncate">{{ $bs->product->name ?? '?' }}: {{ number_format($bs->quantity, 0) }}</p>
                @endforeach
                @if($bin->stocks->where('quantity', '>', 0)->count() > 3)
                <p class="text-[10px] text-gray-400">+{{ $bin->stocks->where('quantity', '>', 0)->count() - 3 }}</p>
                @endif
            </div>
            @else
    ...
    @if($bins->hasPages())<div class="mt-4">{{ $bins->links() }}</div>@endif
    ...
    <div id="modal-zone" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-sm shadow-xl">
            ...
            <form method="POST" action="{{ route('wms.zones.store') }}" class="p-6 space-y-3">
                ...
                <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Kode *</label><input type="text" name="code" required maxlength="10" placeholder="Z01" class="{{ $cls }}"></div>
                <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Nama *</label><input type="text" name="name" required placeholder="Zona Dry Storage" class="{{ $cls }}"></div>
                <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Tipe *</label>
                    <select name="type" required class="{{ $cls }}"><option value="general">General</option><option value="cold">Cold Storage</option><option value="hazmat">Hazmat</option><option value="staging">Staging</option><option value="returns">Returns</option></select>
                </div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-sm shadow-xl">
            ...
            <form method="POST" action="{{ route('wms.bins.store') }}" class="p-6 space-y-3">
                ...
                <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Zone</label>
                    <select name="zone_id" class="{{ $cls }}"><option value="">-- Tanpa Zone --</option>
                        @foreach($zones as $z)<option value="{{ $z->id }}">{{ $z->code }} — {{ $z->name }}</option>@endforeach
                    </select>
                </div>
                <div class="grid grid-cols-3 gap-2">
                    <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Aisle</label><input type="text" name="aisle" maxlength="10" placeholder="01" class="{{ $cls }}"></div>
                    <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Rack</label><input type="text" name="rack" maxlength="10" placeholder="01" class="{{ $cls }}"></div>
                    <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Shelf</label><input type="text" name="shelf" maxlength="10" placeholder="01" class="{{ $cls }}"></div>
                </div>
                <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Tipe</label>
                    <select name="bin_type" class="{{ $cls }}"><option value="storage">Storage</option><option value="picking">Picking</option><option value="staging">Staging</option><option value="returns">Returns</option></select>
                </div>
                <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Kapasitas Maks (0=unlimited)</label><input type="number" name="max_capacity" min="0" value="0" class="{{ $cls }}"></div>
    ...
    <div id="modal-bulk" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-sm shadow-xl">
            ...
            <form method="POST" action="{{ route('wms.bins.bulk') }}" class="p-6 space-y-3">
                ...
                <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Zone</label>
                    <select name="zone_id" class="{{ $cls }}"><option value="">-- Tanpa Zone --</option>
                        @foreach($zones as $z)<option value="{{ $z->id }}">{{ $z->code }}</option>@endforeach
                    </select>
                </div>
                <div class="grid grid-cols-2 gap-2">
                    <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Aisle Dari</label><input type="number" name="aisle_from" required min="1" value="1" class="{{ $cls }}"></div>
                    <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Aisle Sampai</label><input type="number" name="aisle_to" required min="1" value="3" class="{{ $cls }}"></div>
                    <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Rack Dari</label><input type="number" name="rack_from" required min="1" value="1" class="{{ $cls }}"></div>
                    <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Rack Sampai</label><input type="number" name="rack_to" required min="1" value="5" class="{{ $cls }}"></div>
                    <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Shelf Dari</label><input type="number" name="shelf_from" required min="1" value="1" class="{{ $cls }}"></div>
                    <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Shelf Sampai</label><input type="number" name="shelf_to" required min="1" value="4" class="{{ $cls }}"></div>
                </div>
                <div><select name="bin_type" class="{{ $cls }}"><option value="storage">Storage</option><option value="picking">Picking</option></select></div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-sm shadow-xl">
            ...
            <form method="POST" action="{{ route('wms.putaway') }}" class="p-6 space-y-3">
                ...
                <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Produk *</label>
                    <select name="product_id" required class="{{ $cls }}"><option value="">-- Pilih --</option></select>
                </div>
                <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Bin Lokasi *</label>
                    <select name="bin_id" required class="{{ $cls }}"><option value="">-- Pilih --</option>
                        @foreach($bins as $b)<option value="{{ $b->id }}">{{ $b->code }}</option>@endforeach
                    </select>
                </div>
                <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Qty *</label><input type="number" name="quantity" required min="0.001" step="0.001" class="{{ $cls }}"></div>
