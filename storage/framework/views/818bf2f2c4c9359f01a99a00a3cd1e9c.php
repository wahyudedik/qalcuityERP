...
<?php $component->withAttributes([]); ?>
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-3 bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 text-sm px-4 py-3 rounded-xl">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
            <h2 class="font-semibold text-gray-900 dark:text-white mb-4">Buat Pengingat Baru</h2>
            <form method="POST" action="<?php echo e(route('reminders.store')); ?>"
                  class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <?php echo csrf_field(); ?>
                <div class="sm:col-span-2">
                    <label class="block text-xs font-medium text-gray-500 dark:text-slate-400 mb-1">Judul *</label>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-slate-400 mb-1">Waktu Pengingat *</label>
                    <input type="datetime-local" name="remind_at" required
                        min="<?php echo e(now()->addMinutes(5)->format('Y-m-d\TH:i')); ?>"
                        class="w-full bg-gray-50 dark:bg-[#0f172a] border border-gray-200 dark:border-white/10 rounded-xl px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-slate-400 mb-1">Channel</label>
                    <select name="channel"
                        class="w-full bg-gray-50 dark:bg-[#0f172a] border border-gray-200 dark:border-white/10 rounded-xl px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-blue-500">
                ...
                <div class="sm:col-span-2 lg:col-span-4">
                    <label class="block text-xs font-medium text-gray-500 dark:text-slate-400 mb-1">Catatan</label>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
            ...
            <div class="divide-y divide-gray-100 dark:divide-white/5">
                ...
                <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-4 <?php echo e($isOverdue ? 'bg-amber-50 dark:bg-amber-500/5' : ''); ?>">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="font-medium text-gray-900 dark:text-white text-sm <?php echo e($reminder->status === 'done' ? 'line-through opacity-60' : ''); ?>">
                            <?php echo e($reminder->title); ?>

                        </p>
                        <?php if($reminder->notes): ?>
                        <p class="text-xs text-gray-400 dark:text-slate-500 mt-0.5"><?php echo e($reminder->notes); ?></p>
                        <?php endif; ?>
                        <div class="flex items-center gap-3 mt-1">
                            <span class="text-xs <?php echo e($isOverdue ? 'text-amber-500 font-medium' : 'text-gray-400 dark:text-slate-500'); ?>">
