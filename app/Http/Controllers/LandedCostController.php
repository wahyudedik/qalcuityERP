...
class LandedCostController extends Controller
{
    ...
    {
        abort_if($landedCost->tenant_id !== $this->tid(), 403);
        if ($landedCost->status === 'posted') return back()->with('error', 'Sudah diposting, tidak bisa dialokasi ulang.');
    ...
    {
        abort_if($landedCost->tenant_id !== $this->tid(), 403);
        if ($landedCost->status !== 'allocated') return back()->with('error', 'Alokasi dulu sebelum posting.');

        $totalCost = (float) $landedCost->total_additional_cost;
        if ($totalCost <= 0) return back()->with('error', 'Total biaya = 0.');
    ...
    {
        abort_if($landedCost->tenant_id !== $this->tid(), 403);
        if ($landedCost->status === 'posted') return back()->with('error', 'Landed Cost yang sudah diposting tidak bisa dihapus.');
        $landedCost->delete();
