...
<div class="container-fluid">
    <div class="flex items-start justify-between mb-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('iot.devices.edit', $device) }}" class="px-3 py-1.5 border border-gray-400 text-gray-600 hover:bg-gray-50 rounded-lg text-xs transition">Edit</a>
            <form action="{{ route('iot.devices.destroy', $device) }}" method="POST"
                onsubmit="return confirm('Hapus device ini?')">
                @csrf @method('DELETE')
                <button class="px-3 py-1.5 border border-red-500 text-red-600 hover:bg-red-50 rounded-lg text-xs transition">Hapus</button>
            </form>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 g-3 mb-4">
            ...
            <div class="bg-white rounded-2xl border border-amber-300 border-0 shadow-sm bg-warning-subtle">
                <div class="p-5">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="font-semibold mb-1"><i class="fas fa-key mr-1"></i> Device Token (Firmware Secret)</div>
                            <code class="fs-6" id="deviceToken">{{ $device->device_token }}</code>
                        </div>
                        <div class="flex gap-2">
                            <button class="px-3 py-1.5 border border-gray-400 text-gray-600 hover:bg-gray-50 rounded-lg text-xs transition"
                                onclick="navigator.clipboard.writeText('{{ $device->device_token }}');this.textContent='Tersalin!'">
                                Salin
                            </button>
                            <button class="px-3 py-1.5 border border-red-500 text-red-600 hover:bg-red-50 rounded-lg text-xs transition" id="btnRegenToken">Regenerate</button>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm text-center py-3">
                <div class="mb-1">
                    @if($device->is_connected)
                        <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700 fs-6">Online</span>
                    @else
                        <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700 fs-6">Offline</span>
                    @endif
                </div>
        ...
        <div class="w-full md:w-1/4">
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm text-center py-3">
                <div class="font-bold">{{ $device->last_seen_at ? $device->last_seen_at->diffForHumans() : 'Belum pernah' }}</div>
                <div class="text-gray-500 text-sm">Terakhir Online</div>
            </div>
        </div>
        <div class="w-full md:w-1/4">
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm text-center py-3">
                <div class="font-bold">{{ \App\Models\IotDevice::targetModules()[$device->target_module] ?? '-' }}</div>
                ...
                <table class="w-full text-sm text-left w-full text-sm text-left-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-{{ $log->status === 'received' ? 'emerald-500' : 'secondary'  }}-subtle
