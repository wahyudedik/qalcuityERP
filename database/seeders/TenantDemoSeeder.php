...
class TenantDemoSeeder extends Seeder
{
    ...
    private function seedPurchaseRequisitions(): void
    {
        ...
        $prId = DB::table('purchase_requisitions')->insertGetId([
            ...
            'status' => 'approved',
            'purpose' => 'Kebutuhan stok Q2 2026',
        ...
        foreach (array_slice($this->productIds, 0, 2) as $pid) {
            ...
            DB::table('purchase_requisition_items')->insert([
                ...
                'unit' => $product->unit,
                'estimated_price' => $product->price_buy,
    ...
    private function seedGoodsReceipts(): void
    {
        ...
        $grId = DB::table('goods_receipts')->insertGetId([
            ...
            'warehouse_id' => $this->warehouseId,
            'number' => 'GR/MBI/2026/001',
            'receipt_date' => Carbon::parse($po->date)->addDays(5)->format('Y-m-d'),
            'status' => 'confirmed',
    ...
    private function seedPurchaseReturns(): void
    {
        ...
        $prId = DB::table('purchase_returns')->insertGetId([
            ...
            'warehouse_id' => $this->warehouseId,
            'created_by' => $this->adminId,
            'number' => 'PR-RET/MBI/2026/001',
            'return_date' => Carbon::now()->subDays(5)->format('Y-m-d'),
            'reason' => 'Barang cacat / tidak sesuai spesifikasi',
            'status' => 'completed',
            'subtotal' => 500000,
            'tax_amount' => 0,
            'total' => 500000,
    ...
    private function seedLandedCost(): void
    {
        ...
        $lcId = DB::table('landed_costs')->insertGetId([
            ...
            'total_additional_cost' => 2500000,
            'allocation_method' => 'by_value',
            'status' => 'posted',
            'notes' => 'Biaya pengiriman dari supplier',
            'user_id' => $this->adminId,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    ...
    private function seedConsignment(): void
    {
        ...
        if (!empty($this->productIds)) {
            ...
            $shipId = DB::table('consignment_shipments')->insertGetId([
                ...
                'warehouse_id' => $this->warehouseId,
                'user_id' => $this->adminId,
                'number' => 'CS/MBI/2026/001',
                'ship_date' => Carbon::now()->subDays(10)->format('Y-m-d'),
                'status' => 'shipped',
                'notes' => 'Pengiriman konsinyasi pertama',
                'total_cost' => $product->price_buy * 5,
                'total_retail' => $product->price_sell * 5,
                'created_at' => now(),
                'updated_at' => now(),
            ]);
            DB::table('consignment_shipment_items')->insert([
                'consignment_shipment_id' => $shipId,
                'product_id' => $this->productIds[0],
                'quantity_sent' => 5,
                'quantity_sold' => 2,
                'quantity_returned' => 0,
                'cost_price' => $product->price_buy,
                'retail_price' => $product->price_sell,
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        }
    ...
    private function seedQuotations(): void
    {
        ...
        $qtId = DB::table('quotations')->insertGetId([
            ...
            'total' => 21090000,
            'status' => 'sent',
            'notes' => 'Penawaran harga laptop untuk kebutuhan kantor',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        $product = DB::table('products')->find($this->productIds[0]);
    ...
    private function seedDeliveryOrders(): void
    {
        ...
        $doId = DB::table('delivery_orders')->insertGetId([
            ...
            'warehouse_id' => $this->warehouseId,
            'created_by' => $this->adminId,
            'number' => 'DO/MBI/2026/001',
            'delivery_date' => Carbon::parse($so->date)->addDays(3)->format('Y-m-d'),
            'status' => 'delivered',
            'shipping_address' => 'Jakarta',
            'courier' => 'JNE',
            'tracking_number' => 'JNE' . rand(100000000, 999999999),
            'notes' => 'Dikirim via JNE Regular',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    ...
    private function seedDownPayments(): void
    {
        ...
        DB::table('down_payments')->insert([
            ...
            'number' => 'DP/MBI/2026/001',
            'amount' => round($so->total * 0.3),
            'applied_amount' => round($so->total * 0.3),
            'remaining_amount' => 0,
            'payment_date' => Carbon::parse($so->date)->addDay()->format('Y-m-d'),
            'payment_method' => 'transfer',
            'status' => 'applied',
            'created_by' => $this->adminId,
            'notes' => 'DP 30% sesuai kesepakatan',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    }
    ...
    private function seedSalesReturns(): void
    {
        ...
        $srId = DB::table('sales_returns')->insertGetId([
            ...
            'warehouse_id' => $this->warehouseId,
            'created_by' => $this->adminId,
            'number' => 'SR/MBI/2026/001',
            'return_date' => Carbon::now()->subDays(3)->format('Y-m-d'),
            'reason' => 'Produk tidak sesuai spesifikasi yang dipesan',
            'status' => 'approved',
            'subtotal' => 280000,
            'tax_amount' => 0,
            'total' => 280000,
        ...
        if ($soItem) {
            DB::table('sales_return_items')->insert([
                ...
                'total' => $soItem->price,
                'condition' => 'damaged',
                'notes' => 'Tidak sesuai spesifikasi',
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        }
    ...
    private function seedBulkPayments(): void
    {
        ...
        $bpId = DB::table('bulk_payments')->insertGetId([
            'tenant_id' => $this->tenantId,
            'number' => 'BP/MBI/2026/001',
            'type' => 'receivable',
            'party_id' => $invoices->first()->customer_id,
            'party_type' => 'App\\Models\\Customer',
            ...
            'overpayment' => 0,
            'status' => 'draft',
            'notes' => 'Pembayaran batch invoice',
            'created_by' => $this->adminId,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    ...
    private function seedZeroInputLogs(): void
    {
        ...
        DB::table('zero_input_logs')->insert([
            ...
            'user_id' => $this->adminId,
            'channel' => 'text',
            'raw_input' => 'Beli 5 laptop asus dari PT Asus seharga 7.5 juta per unit',
    ...
    private function seedCommission(): void
    {
        ...
        if ($so) {
            ...
            DB::table('commission_calculations')->insert([
                ...
                'user_id' => $this->staffId,
                'total_sales' => $so->total,
                'total_orders' => 1,
                'commission_amount' => round($so->total * 0.025),
                'bonus_amount' => 0,
                'total_payout' => round($so->total * 0.025),
                'status' => 'draft',
                'period' => '2026-03',
                'created_at' => now(),
                'updated_at' => now(),
        ...
        if (!empty($this->customerIds)) {
            foreach (
                ...
                $ticketId = DB::table('helpdesk_tickets')->insertGetId([
                    'tenant_id' => $this->tenantId,
                    'customer_id' => $this->customerIds[$i] ?? $this->customerIds[0],
                    'assigned_to' => $this->staffId,
                    'created_by' => $this->adminId,
    ...
    private function seedQualityControl(): void
    {
        ...
        DB::table('quality_check_standards')->insertOrIgnore([
            'tenant_id' => $this->tenantId,
            'name' => 'Standar Elektronik Konsumen',
            'code' => 'STD-ELEC-001',
    ...
    private function seedFleet(): void
    {
        ...
        if ($vehicle && $driver) {
            DB::table('fleet_trips')->insert([
                ...
                'destination' => 'Gedung Sudirman, Jakarta',
                'odometer_start' => $vehicle->odometer,
                'odometer_end' => $vehicle->odometer + 45,
            ...
            DB::table('fleet_fuel_logs')->insert([
                ...
                'fuel_type' => 'pertalite',
                'station' => 'SPBU Pertamina Sudirman',
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        }
    ...
    private function seedProjects(): void
    {
        ...
        $projectId = DB::table('projects')->insertGetId([
            'tenant_id' => $this->tenantId,
            'name' => 'Implementasi ERP PT Teknologi Nusantara',
            'number' => 'PRJ-2026-001',
            'customer_id' => $customer?->id,
        ...
        foreach ($userIds as $userId) {
            for ($d = 5; $d >= 1; $d--) {
                DB::table('timesheets')->insert([
                    ...
                    'date' => Carbon::now()->subDays($d)->format('Y-m-d'),
                    'hours' => rand(6, 8),
                    'description' => 'Pengerjaan modul sistem',
    ...
    private function seedEcommerce(): void
    {
        ...
        foreach (
            ...
            $chId = DB::table('ecommerce_channels')->insertGetId(array_merge($ch, [
                'tenant_id' => $this->tenantId,
                'api_key' => 'demo_key_' . Str::random(16),
                'created_at' => now(),
                'updated_at' => now(),
            ]));
    ...
    private function seedPrinting(): void
    {
        ...
        if (in_array('printer_destination', $columns)) {
            ...
            DB::table('print_jobs')->insert([
                'tenant_id' => $this->tenantId,
                'customer_id' => $customer?->id,
                'job_number' => 'PJ/MBI/2026/001',
                'job_name' => 'Brosur Produk Q2 2026',
                'description' => 'Cetak brosur promosi produk elektronik',
    ...
    private function seedFarmPlots(): void
    {
        ...
        $cycleId = DB::table('crop_cycles')->insertGetId([
            ...
            'crop_name' => 'Padi IR64',
            'variety' => 'IR64',
            'planting_date' => Carbon::now()->subMonths(2)->format('Y-m-d'),
        ...
        DB::table('harvest_logs')->insert([
            ...
            'user_id' => $this->adminId,
            'number' => 'HRV-A1-' . Carbon::now()->subMonths(5)->format('Ymd') . '-01',
    ...
    private function seedLivestockEnhancement(): void
    {
        ...
        $herdId = DB::table('livestock_herds')->insertGetId([
            ...
            'status' => 'active',
            'target_harvest_date' => Carbon::now()->addMonths(6)->format('Y-m-d'),
            'target_weight_kg' => 400,
            'notes' => 'Sapi perah produktif',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        // Dairy milk records
        ...
        if (Schema::hasTable('breeding_records')) {
            DB::table('breeding_records')->insert([
                ...
                'genetics_line' => 'Friesian Holstein',
                'expected_due_date' => Carbon::now()->addMonths(7)->format('Y-m-d'),
                'status' => 'pregnant',
                'notes' => 'Inseminasi buatan berhasil',
                'recorded_by' => $this->adminId,
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        ...
        if (Schema::hasTable('livestock_health_records') && Schema::hasColumn('livestock_health_records', 'date')) {
            DB::table('livestock_health_records')->insert([
                ...
                'affected_count' => 25,
                'death_count' => 0,
                'symptoms' => 'Demam, lemas',
                'medication' => 'Vaksin PMK',
                'medication_cost' => 500000,
                'administered_by' => 'drh. Bambang Susilo',
                'severity' => 'medium',
                'status' => 'resolved',
        ...
        if (Schema::hasTable('waste_management_logs') && Schema::hasColumn('waste_management_logs', 'collection_date')) {
            DB::table('waste_management_logs')->insert([
                ...
                'storage_location' => 'Area Kompos',
                'end_product' => 'Pupuk Organik',
                'notes' => 'Diolah menjadi pupuk organik',
                'recorded_by' => $this->adminId,
                'created_at' => now(),
                'updated_at' => now(),
            ]);
    ...
    private function seedFisheries(): void
    {
        ...
        $vesselId = DB::table('fishing_vessels')->insertGetId([
            ...
            'registration_number' => 'GT-2026-001',
            'vessel_type' => 'purse_seiner',
            'gross_tonnage' => 25,
            'crew_capacity' => 50,
            'fuel_capacity' => 300,
            'home_port' => 'Pelabuhan Muara Baru, Jakarta',
            'is_active' => true,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        ...
        $pondId = DB::table('aquaculture_ponds')->insertGetId([
            'tenant_id' => $this->tenantId,
            'pond_name' => 'Kolam Udang A1',
            'pond_code' => 'KU-A1',
            'pond_type' => 'earthen',
            'surface_area' => 5000,
            'depth' => 1.5,
            'volume' => 7500,
            'water_source' => 'natural',
            'status' => 'stocked',
    ...
    private function seedTourTravel(): void
    {
        ...
        foreach (
            [
                ['day_number' => 1, 'title' => 'Arrival & Kuta Beach', 'activities' => json_encode(['Tiba di Bali', 'Check-in hotel', 'Sore hari ke Pantai Kuta'])],
                ['day_number' => 2, 'title' => 'Ubud Cultural Tour', 'activities' => json_encode(['Pagi ke Tegalalang Rice Terrace', 'Siang ke Ubud Art Market', 'Sore ke Monkey Forest'])],
                ['day_number' => 3, 'title' => 'Nusa Dua & Water Sport', 'activities' => json_encode(['Pagi water sport di Nusa Dua', 'Siang relaksasi di pantai', 'Malam Kecak Dance'])],
                ['day_number' => 4, 'title' => 'Departure', 'activities' => json_encode(['Sarapan', 'Check-out', 'Airport transfer'])],
            ] as $it
        ) {
    ...
    private function seedCosmetic(): void
    {
        ...
        foreach (
            [
                ['common_name' => 'Aloe Vera Extract', 'percentage' => 15.0, 'function' => 'moisturizer', 'inci_name' => 'Aloe Barbadensis Leaf Juice', 'quantity' => 15.0, 'unit' => 'kg'],
                ['common_name' => 'Titanium Dioxide',  'percentage' => 5.0,  'function' => 'sunscreen',   'inci_name' => 'Titanium Dioxide',              'quantity' => 5.0,  'unit' => 'kg'],
                ['common_name' => 'Glycerin',           'percentage' => 3.0,  'function' => 'humectant',   'inci_name' => 'Glycerin',                      'quantity' => 3.0,  'unit' => 'kg'],
                ['common_name' => 'Aqua',               'percentage' => 70.0, 'function' => 'solvent',     'inci_name' => 'Aqua',                          'quantity' => 70.0, 'unit' => 'kg'],
            ] as $ing
        ) {
    ...
    private function seedSupplierScorecard(): void
    {
        ...
        foreach (array_slice($this->supplierIds, 0, 2) as $suppId) {
            DB::table('supplier_scorecards')->insert([
                ...
                'areas_for_improvement' => 'Supplier performa baik, perlu peningkatan ketepatan waktu pengiriman',
                'created_at' => now(),
                'updated_at' => now(),
            ]);
            DB::table('supplier_performances')->insert([
                ...
                'overall_score' => 85.75,
                'rating_grade' => 'B',
                'total_po_value' => 85000000,
                'actual_po_value' => 85000000,
                'evaluated_by' => $this->adminId,
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        }
    ...
    private function seedWorkflows(): void
    {
        ...
        $wfId = DB::table('workflows')->insertGetId([
            ...
            'trigger_type' => 'event',
            'trigger_config' => json_encode(['event' => 'invoice.due_soon', 'days_before' => 3]),
            'description' => 'Kirim notifikasi otomatis 3 hari sebelum invoice jatuh tempo',
            'is_active' => true,
            'created_by' => $this->adminId,
            'created_at' => now(),
            'updated_at' => now(),
        ...
        DB::table('workflow_actions')->insert([
            ...
            'order' => 1,
            'action_config' => json_encode(['channel' => 'email', 'template' => 'invoice_due_reminder']),
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    }
    ...
    private function seedHotelFbModule(): void
    {
        ...
        foreach ([1, 2, 3, 4, 5] as $t) {
            DB::table('restaurant_tables')->insertOrIgnore([
                'tenant_id' => $this->tenantId,
                'table_number' => 'T-' . str_pad($t, 2, '0', STR_PAD_LEFT),
                'capacity' => 4,
                'location' => 'indoor',
                'status' => 'available',
                'created_at' => now(),
    ...
    private function seedHotelHousekeeping(): void
    {
        ...
        foreach (
            ...
            DB::table('housekeeping_supplies')->insertOrIgnore([
                ...
                'item_code' => 'HK-' . str_pad($index + 1, 3, '0', STR_PAD_LEFT),
                'category' => 'amenities',
                'unit_of_measure' => $supply['unit'],
                'quantity_on_hand' => $supply['current_stock'],
                'reorder_point' => $supply['minimum_stock'],
    ...
    private function seedTelecom(): void
    {
        ...
        foreach (
            [
                ['name' => 'Router Utama MikroTik RB4011', 'device_type' => 'router', 'brand' => 'MikroTik', 'model' => 'RB4011', 'ip_address' => '192.168.1.1', 'mac_address' => 'AA:BB:CC:DD:EE:01', 'status' => 'online'],
                ['name' => 'Switch Core Cisco SG350', 'device_type' => 'switch', 'brand' => 'Cisco', 'model' => 'SG350-28', 'ip_address' => '192.168.1.2', 'mac_address' => 'AA:BB:CC:DD:EE:02', 'status' => 'online'],
                ['name' => 'Access Point Ubiquiti UAP-AC-Pro', 'device_type' => 'access_point', 'brand' => 'Ubiquiti', 'model' => 'UAP-AC-Pro', 'ip_address' => '192.168.1.10', 'mac_address' => 'AA:BB:CC:DD:EE:03', 'status' => 'online'],
            ] as $dev
        ) {
        ...
        foreach (
            [
                ['name' => 'Paket Basic 10 Mbps',    'code' => 'PKG-10M',  'download_speed_mbps' => 10,  'upload_speed_mbps' => 5,  'price' => 150000,  'quota_bytes' => 0],
                ['name' => 'Paket Standard 25 Mbps',  'code' => 'PKG-25M',  'download_speed_mbps' => 25,  'upload_speed_mbps' => 10, 'price' => 250000,  'quota_bytes' => 0],
                ['name' => 'Paket Premium 50 Mbps',   'code' => 'PKG-50M',  'download_speed_mbps' => 50,  'upload_speed_mbps' => 20, 'price' => 450000,  'quota_bytes' => 0],
                ['name' => 'Paket Unlimited 100 Mbps', 'code' => 'PKG-100M', 'download_speed_mbps' => 100, 'upload_speed_mbps' => 50, 'price' => 750000,  'quota_bytes' => 0],
                ['name' => 'Voucher Harian 5 Mbps',   'code' => 'VCH-5M',   'download_speed_mbps' => 5,   'upload_speed_mbps' => 2,  'price' => 10000,   'quota_bytes' => 1073741824],
            ] as $pkg
        ) {
        ...
        foreach (
            [
                ['username' => 'hotspot_user1', 'password' => 'pass123'],
                ['username' => 'hotspot_user2', 'password' => 'pass456'],
                ['username' => 'hotspot_guest1', 'password' => 'guest001'],
            ] as $hu
        ) {
        ...
        foreach (
            [
                ['device_id' => $deviceIds[0], 'alert_type' => 'high_cpu',       'severity' => 'medium', 'title' => 'High CPU Usage', 'message' => 'CPU usage mencapai 85%',              'status' => 'resolved'],
                ['device_id' => $deviceIds[1], 'alert_type' => 'port_down',       'severity' => 'critical', 'title' => 'Port Down', 'message' => 'Port GE0/1 down selama 5 menit',      'status' => 'new'],
                ['device_id' => $deviceIds[2], 'alert_type' => 'high_traffic',    'severity' => 'low',    'title' => 'High Traffic', 'message' => 'Traffic melebihi 80% kapasitas',       'status' => 'acknowledged'],
            ] as $alert
        ) {
        ...
        // Usage tracking
        $subscriptionIds = DB::table('telecom_subscriptions')->where('tenant_id', $this->tenantId)->pluck('id')->toArray();
        foreach (array_slice($subscriptionIds, 0, 2) as $subId) {
            for ($d = 6; $d >= 0; $d--) {
    ...
    private function seedHealthcare(): void
    {
        ...
        foreach (
            ...
            $doctorIds[] = DB::table('doctors')->insertGetId([
                'tenant_id' => $this->tenantId,
                'user_id' => $userId,
                'doctor_number' => 'DR-' . str_pad($i + 1, 5, '0', STR_PAD_LEFT),
        ...
        // Skip complex healthcare data seeding to avoid schema conflicts
        // Focus on core ERP functionality
    }
