...
<div class="flex h-full" id="pos-app">
    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <div class="flex items-center gap-2 px-3 sm:px-4 h-14 bg-gray-900 border-b border-gray-800 shrink-0">
            ...
            {{-- Offline indicator --}}
            <span id="offline-badge" class="hidden items-center gap-1 text-xs font-medium bg-amber-500/20 text-amber-400 border border-amber-500/30 px-2 py-0.5 rounded-full shrink-0 cursor-pointer" onclick="flushQueue()" title="Klik untuk sinkronisasi manual">
    ...
    <div id="cart-panel" class="fixed inset-y-0 right-0 z-30 w-80 xl:w-96 bg-gray-900 border-l border-gray-800 flex flex-col shrink-0 translate-x-full sm:translate-x-0 sm:relative sm:inset-auto transition-transform duration-300">
        ...
        <div class="px-4 py-2 border-b border-gray-800 shrink-0">
            <select id="customer-select" onchange="onCustomerChange(this.value)" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500">
            ...
            {{-- Loyalty Points Info --}}
            <div id="loyalty-info" class="hidden mt-2 bg-yellow-500/10 border border-yellow-500/30 rounded-xl px-3 py-2">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-1.5">
                        <span class="text-sm">⭐</span>
                        <span class="text-xs text-yellow-300 font-medium">Poin: <span id="loyalty-balance-display">0</span></span>
                    </div>
                ...
                {{-- Redeem section (hidden by default) --}}
                <div id="loyalty-redeem-section" class="hidden mt-2 pt-2 border-t border-yellow-500/20">
                    <div class="flex items-center gap-2">
                        <input type="number" id="loyalty-redeem-input" min="0" value="0" placeholder="Jumlah poin"
        ...
        <div class="px-4 py-3 border-t border-gray-800 space-y-2 shrink-0">
            ...
            <div id="loyalty-discount-row" class="hidden flex justify-between text-sm text-yellow-400 items-center">
        ...
        <div class="px-4 pb-2 shrink-0">
            ...
            <div id="card-section" class="mb-3 hidden">
                <div class="bg-gray-800 rounded-xl p-4 text-center">
                    ...
                    <p class="text-xs text-gray-500">Total: <span id="card-total-display" class="text-white font-semibold">Rp 0</span></p>
                </div>
            </div>
                ...
                <div class="bg-gray-800 rounded-xl p-4 text-center" id="qris-container">
                    ...
                    <div id="qris-display" class="hidden">
                        ...
                        <div class="bg-white rounded-xl p-2 mx-auto mb-3 inline-block">
                            <img id="qris-image" src="" alt="QRIS Code" class="w-40 h-40 object-contain">
                        </div>
                        <p class="text-xs text-gray-500 mb-1">Total: <span id="qris-total-display" class="text-white font-semibold">Rp 0</span></p>
                    ...
                    <div id="qris-error" class="hidden">
                        ...
                        <button onclick="generateQrisCode()" class="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-xl text-xs text-white transition">
                            Coba Lagi
                        </button>
                    </div>
                </div>
            ...
            <div id="transfer-section" class="mb-3 hidden">
                <div class="bg-gray-800 rounded-xl p-4 text-center">
                    ...
                    <p class="text-xs text-gray-500">Total: <span id="transfer-total-display" class="text-white font-semibold">Rp 0</span></p>
                </div>
            </div>
...
<div id="receipt-modal" class="fixed inset-0 bg-black/70 z-50 hidden items-center justify-center p-4">
    <div class="bg-white text-gray-900 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
        ...
        {{-- Receipt body --}}
        <div id="receipt-body" class="text-sm space-y-1 border-t border-b border-dashed border-gray-200 mx-6 py-3 mb-4"></div>
...
<div id="send-email-modal" class="fixed inset-0 bg-black/70 z-[60] hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-xs shadow-2xl p-6">
        ...
        <div class="flex gap-2">
            <button onclick="closeSendEmail()" class="flex-1 py-2 border border-gray-300 rounded-xl text-sm font-medium hover:bg-gray-50 transition">Batal</button>
...
<div id="send-wa-modal" class="fixed inset-0 bg-black/70 z-[60] hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-xs shadow-2xl p-6">
        ...
        <div class="flex gap-2">
            <button onclick="closeSendWa()" class="flex-1 py-2 border border-gray-300 rounded-xl text-sm font-medium hover:bg-gray-50 transition">Batal</button>
...
async function onCustomerChange(customerId) {
    ...
    try {
        const res = await fetch(`{{ url('/pos/loyalty-balance') }}/${customerId}`, {
        ...
        if (data.program_active) {
            ...
            document.getElementById('loyalty-balance-display').textContent = loyaltyBalance.toLocaleString('id-ID');
...
function applyLoyaltyRedeem() {
    const pts = parseInt(document.getElementById('loyalty-redeem-input').value) || 0;
    if (pts <= 0) { showToast('Masukkan jumlah poin yang ingin ditukarkan', 'warning'); return; }
    if (pts < loyaltyMinRedeem) { showToast(`Minimum penukaran ${loyaltyMinRedeem.toLocaleString('id-ID')} poin`, 'warning'); return; }
    if (pts > loyaltyBalance) { showToast(`Poin tidak mencukupi. Saldo: ${loyaltyBalance.toLocaleString('id-ID')} poin`, 'error'); return; }

    const discountVal = pts * loyaltyIdrPerPoint;
    const total = getTotal();
    if (discountVal > total) { showToast('Nilai penukaran melebihi total transaksi', 'error'); return; }
    ...
    recalculate();
    showToast(`${pts.toLocaleString('id-ID')} poin diterapkan sebagai diskon Rp ${discountVal.toLocaleString('id-ID')}`, 'success');
}
...
function setPayment(method) {
    ...
    document.getElementById('transfer-section').style.display  = (method === 'transfer' || method === 'bank_transfer') ? 'block' : 'none';
...
async function generateQrisCode() {
    ...
    try {
        ...
        const orderRes = await fetch('{{ route("pos.initiate-payment") }}', {
            method: 'POST',
            headers: {
        ...
        qrisExpiryTime = qrisData.expiry_time ? new Date(qrisData.expiry_time * 1000) : new Date(Date.now() + 15 * 60 * 1000);
        ...
        if (qrisData.qr_image_url) {
            ...
            qrisImg.src = `https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(qrisData.qr_string)}`;
        }
...
function startQrisPolling() {
    ...
    qrisPollingInterval = setInterval(async () => {
        ...
        try {
            const res = await fetch(`/api/payment/status?transaction_number=${encodeURIComponent(qrisTransactionNumber)}`, {
                headers: { 'X-Requested-With': 'XMLHttpRequest' },
            });
            const data = await res.json();
...
async function completeQrisTransaction(paymentData) {
    ...
    try {
        const res = await fetch(`{{ url('/pos/complete-payment') }}/${qrisPendingOrderId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
        ...
        if (data.success) {
            ...
            showReceipt({ ...data, total: getTotal(), paid_amount: getTotal(), change: 0, payment_method: 'qris' });
        } else {
...
const SPLIT_METHODS = {
    cash: 'Tunai', card: 'Kartu', qris: 'QRIS', transfer: 'Transfer', bank_transfer: 'Bank'
};

function addSplitRow() {
    const id = Date.now();
    splitRows.push({ id, method: 'cash', amount: 0 });
    renderSplitRows();
}
...
function showReceipt(data) {
    ...
    if (data.payment_method === 'cash' || paymentMethod === 'cash') {
        ...
        const methodLabel = { card: 'Kartu Debit/Kredit', qris: 'QRIS', transfer: 'Transfer', bank_transfer: 'Transfer Bank' };
        paymentInfo = `<div class="flex justify-between text-xs text-gray-500 mt-1"><span>Metode</span><span>${methodLabel[data.payment_method] || data.payment_method}</span></div>`;
    }
    ...
    if (data.earned_points > 0) {
        body.innerHTML += `<div class="mt-2 text-center text-xs text-yellow-600 font-medium">⭐ +${data.earned_points.toLocaleString('id-ID')} poin diperoleh</div>`;
    }
    if (data.loyalty_discount > 0) {
        body.innerHTML += `<div class="text-center text-xs text-yellow-600">⭐ ${loyaltyPointsToRedeem.toLocaleString('id-ID')} poin ditukarkan</div>`;
    }
...
async function doSendEmail() {
    ...
    try {
        const res = await fetch('{{ route("pos.send-receipt") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
            },
            body: JSON.stringify({ order_id: lastReceipt.order_id, email }),
        });
        const data = await res.json();
...
async function doSendWhatsApp() {
    ...
    try {
        const res = await fetch('{{ route("pos.send-receipt-whatsapp") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
            },
            body: JSON.stringify({ order_id: lastReceipt.order_id, phone }),
        });
        const data = await res.json();
...
function getSearchDropdown() {
    if (!searchDropdown) {
        ...
        searchDropdown.className = 'absolute left-0 right-0 top-full mt-1 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl z-50 max-h-72 overflow-y-auto hidden';
...
function addProductFromSearch(p) {
    ...
    if (existing) {
        if (existing.qty >= p.stock) { showToast('Stok tidak cukup', 'warning'); return; }
        existing.qty++;
    } else {
...
async function doServerSearch(q) {
    if (!q || q.length < 1) { hideSearchDropdown(); return; }
    try {
...
document.addEventListener('click', function(e) {
    if (!barcodeInput.contains(e.target) && !document.getElementById('search-dropdown')?.contains(e.target)) {
        hideSearchDropdown();
    }
});
...
async function flushQueue() {
    ...
    try {
        const res = await fetch('{{ route("pos.sync-offline") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
            ...
            for (const item of items) {
                try {
                    ...
                    if (r.ok) {
                        ...
                        const delTx = delDb.transaction('pos_queue', 'readwrite');
                        delTx.objectStore('pos_queue').delete(item.id);
                        await new Promise(r2 => { delTx.oncomplete = r2; delTx.onerror = r2; });
                        synced++;
                    } else {
                        failed++;
                    }
...
async function processCheckout() {
    ...
    if (paymentMethod === 'split') {
        ...
        if (Math.abs(splitTotal - total) > 1) {
            showToast(`Total split (${formatRp(splitTotal)}) harus sama dengan total (${formatRp(total)})`, 'error');
            return;
        }
        if (splitRows.length === 0) {
    ...
    if (paymentMethod === 'split') {
        payload.split_payments = splitRows.map(r => ({ method: r.method, amount: parseFloat(r.amount) || 0 }));
    }
    ...
    try {
        ...
        if (data.status === 'success') {
            lastReceipt = { ...data, order_id: data.order_id ?? null, items: [...cart], discount, tax, paid: payload.paid_amount, paid_amount: payload.paid_amount };
            showReceipt(data);
        } else {
...
function buildReceiptText(data) {
    ...
    if (data.change !== undefined && paymentMethod === 'cash') {
        const paid = data.paid_amount || data.paid || (parseFloat(document.getElementById('paid-input').value.replace(/\./g, '')) || 0);
