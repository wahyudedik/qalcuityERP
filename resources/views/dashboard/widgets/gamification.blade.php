...
<div class="bg-white rounded-2xl border border-gray-200 p-5 h-full flex flex-col">
    ...
    <div class="flex items-start justify-between mb-4">
        <div>
            ...
            <p class="text-2xl font-bold text-gray-900 mt-0.5">Lv. {{ $level }}</p>
        </div>
    ...
    <div class="flex items-center gap-3 mb-4">
        <div class="flex-1 bg-gray-50 rounded-xl px-3 py-2 text-center">
            ...
            <p class="text-base font-bold text-gray-900">{{ number_format($totalPoints) }}</p>
        </div>
        <div class="flex-1 bg-gray-50 rounded-xl px-3 py-2 text-center">
            <p class="text-xs text-gray-500">Peringkat</p>
            <p class="text-base font-bold text-gray-900">#{{ $rank }}</p>
        </div>
    </div>
    ...
    <div class="mb-4">
        <div class="flex items-center justify-between mb-1.5">
            <span class="text-xs text-gray-500">Level {{ $level }} →
                {{ $level + 1 }}</span>
        ...
        <p class="text-[10px] text-gray-400 mt-1">{{ $progressTo }}/{{ $pointsNeeded }} poin ke
            level berikutnya</p>
    </div>
