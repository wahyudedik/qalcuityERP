<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto space-y-5">
        ...
        <form method="POST" action="{{ route('journals.store') }}" id="journal-form" class="space-y-5">
            @csrf
            ...
            <div class="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="description" id="journal-description" value="{{ old('description') }}" required placeholder="Keterangan jurnal..."
                            class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="currency_code" value="{{ old('currency_code', 'IDR') }}" maxlength="3"
                            class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500">
                    </div>
                    <div>
                        <label class="text-xs text-gray-400 mb-1 block">Kurs</label>
                        <input type="number" name="currency_rate" value="{{ old('currency_rate', 1) }}" step="0.000001" min="0"
                            class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500">
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('journals.index') }}" class="bg-white/10 hover:bg-white/20 text-white px-6 py-2 rounded-lg text-sm">Batal</a>
            </div>
        </form>
    </div>
    ...
    function fetchAccountSuggestion(description) {
        ...
        const url = `{{ route('accounting.ai.suggest-accounts') }}?description=${encodeURIComponent(description)}&amount=${totalDebit}`;
    ...
    document.getElementById('btn-check-anomaly').addEventListener('click', () => {
        ...
        fetch('{{ route("accounting.ai.check-journal") }}', {
            method: 'POST',
