...
<button
    ...
    data-rgb="{{ $gc['rgb'] }}"
    title="{{ $label }}"
    style="--group-color:{{ $gc['color'] }};--group-rgb:{{ $gc['rgb'] }}"
    class="rail-btn {{ $isActive ? 'rail-active' : '' }}">
    <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $svgPath !!}</svg>
    <span class="rail-tip">{{ $label }}</span>
</button>
