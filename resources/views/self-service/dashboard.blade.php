<x-app-layout>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            ...
            <p class="text-xs text-gray-400 mt-1">dari {{ $leaveQuota }} hari</p>
            @if($leavePending > 0)
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            ...
            @if($latestPayslip)
            <p class="text-lg font-black text-gray-900">Rp {{ number_format($latestPayslip->net_salary, 0, ',', '.') }}</p>
            <p class="text-xs text-gray-400 mt-1">{{ $latestPayslip->payrollRun?->period ?? '-' }}</p>
            @else
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <p class="text-xs text-gray-400 mb-5">{{ today()->translatedFormat('l, d F Y') }}</p>
            ...
            <div class="text-xs text-gray-500 mb-3">
                Masuk pukul <span class="font-semibold text-gray-900">{{ $todayAttendance->check_in }}</span>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <p class="text-sm font-medium text-gray-900">{{ $latestReview->overallLabel() }}</p>
                    <p class="text-xs text-gray-400">{{ $latestReview->period }} · {{ ucfirst($latestReview->period_type) }}</p>
                </div>
            </div>
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs font-medium text-gray-700 w-4">{{ $latestReview->$field }}</span>
                </div>
                @endforeach
            </div>
            @if($latestReview->recommendation)
            <p class="text-xs text-gray-400 mt-3">Rekomendasi: <span class="font-medium text-gray-700">{{ $latestReview->recommendationLabel() }}</span></p>
            @endif
            @else
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="font-semibold text-gray-900">Slip Gaji Terakhir — {{ $latestPayslip->payrollRun?->period }}</h3>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div>
                ...
                <p class="text-sm font-semibold text-gray-900 mt-0.5">Rp {{ number_format($latestPayslip->base_salary, 0, ',', '.') }}</p>
            </div>
            <div>
