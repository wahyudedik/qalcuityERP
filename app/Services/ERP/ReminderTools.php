...
class ReminderTools
{
    ...
    private function parseRemindAt(string $input): Carbon
    {
        ...
        if (str_contains($input, 'besok')) {
            return now()->addDay()->setTime(9, 0);
        }
        if (str_contains($input, 'lusa')) {
            return now()->addDays(2)->setTime(9, 0);
        }
        if (preg_match('/(\d+)\s*hari\s*lagi/', $input, $m)) {
            return now()->addDays((int) $m[1])->setTime(9, 0);
        }
        if (preg_match('/(\d+)\s*minggu\s*lagi/', $input, $m)) {
            return now()->addWeeks((int) $m[1])->setTime(9, 0);
        }
        if (preg_match('/(\d+)\s*jam\s*lagi/', $input, $m)) {
