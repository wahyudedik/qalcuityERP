...
@section('content')
                    ...
                    @forelse($reports as $report)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-mono font-medium text-gray-900">{{ $report->report_number }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-xs text-gray-500">to {{ $report->end_date->format('d M Y') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $report->total_batches_monitored }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-red-600 font-medium">{{ $report->batches_expired }}</div>
                                <div class="text-xs text-gray-500">{{ $report->expiry_rate }}%</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-orange-600 font-medium">{{ $report->batches_recalled }}</div>
                                <div class="text-xs text-gray-500">{{ $report->recall_rate }}%</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">Rp
                                    {{ number_format($report->total_loss_value, 0, ',', '.') }}</div>
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    <div id="generate-report-modal"
        ...
        <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-lg bg-white">
            ...
            <form method="POST" action="{{ route('cosmetic.expiry.reports.generate') }}" class="space-y-4">
                @csrf
                <div>
