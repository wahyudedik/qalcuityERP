...
class ForecastTools
{
    ...
    private function forecastStockDepletion(array $args): array
    {
        ...
        if (!$product) return ['status' => 'error', 'message' => "Produk '{$productName}' tidak ditemukan."];

        $currentStock = ProductStock::where('product_id', $product->id)->sum('quantity');

        $soldQty = SalesOrderItem::whereHas('salesOrder', fn($q) =>
    ...
    private function forecastRestockNeed(array $args): array
    {
        ...
        if (empty($needs)) return ['status' => 'success', 'message' => "Stok semua produk cukup untuk {$aheadDays} hari ke depan.", 'data' => []];
    ...
    private function compareSales($startA, $endA, $startB, $endB, $labelA, $labelB): array
    {
        ...
        $revA = $qA->sum('total'); $ordA = $qA->count();
        $revB = $qB->sum('total'); $ordB = $qB->count();
    ...
    {
        $getProducts = fn($start, $end) => SalesOrderItem::whereHas('salesOrder', fn($q) =>
            $q->where('tenant_id', $this->tenantId)->whereNotIn('status', ['cancelled'])->whereBetween('date', [$start, $end])
