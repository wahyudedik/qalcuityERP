        ...
        <div
            ...
            <div class="bg-gray-50 px-6 py-4 border-b border-gray-200">
                <div class="flex justify-between items-center">
                    <div>
                        <p class="text-sm text-gray-600">Order #{{ $order->order_number ?? 'ORD-001' }}</p>
                        <p class="text-xs text-gray-500">
                            {{ $order->created_at->format('d M Y, H:i') ?? now()->format('d M Y, H:i') }}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-2xl font-bold text-gray-900">Rp
                            {{ number_format($order->grand_total ?? 0, 0, ',', '.') }}</p>
                        <p class="text-xs text-gray-500">{{ $order->items_count ?? 0 }} items</p>
                    </div>
                </div>
            </div>
        ...
        return {
            ...
            get canProceed() {
                ...
                if (this.selectedMethod === 'cash') {
                    return this.cashReceived >= {{ $order->grand_total ?? 0 }};
                }
            ...
            async processQrisPayment() {
                ...
                const response = await fetch(`/api/payment/qris/{{ $order->id }}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + localStorage.getItem('api_token'),
