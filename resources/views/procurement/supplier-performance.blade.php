<x-app-layout>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            ...
            <p class="text-2xl font-bold text-purple-600">{{ number_format($avgScore ?? 0, 1) }}</p>
            <p class="text-xs text-gray-400 mt-1">Performa keseluruhan</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">Terbaik</p>
            <p class="text-lg font-bold text-green-600 truncate">{{ $topGrade?->name ?? '-' }}</p>
            <p class="text-xs text-gray-400 mt-1">Grade: {{ $topGrade?->performance['current_grade'] ?? 'N/A' }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">Periode Evaluasi</p>
            <p class="text-2xl font-bold text-amber-600">{{ $period }} Hari</p>
                    ...
                    @forelse($rankings as $index => $ranking)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3">
                                ...
                                @else
                                    <span class="text-xs text-gray-400 font-medium pl-1">#{{ $index + 1 }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-right font-bold text-gray-900">{{ number_format($ranking['avg_score'], 1) }}</td>
                            <td class="px-4 py-3 text-right hidden md:table-cell text-gray-500">{{ number_format($ranking['avg_delivery'], 1) }}</td>
                            <td class="px-4 py-3 text-right hidden md:table-cell text-gray-500">{{ number_format($ranking['avg_quality'], 1) }}</td>
                            <td class="px-4 py-3 text-right hidden lg:table-cell text-gray-500">{{ number_format($ranking['avg_cost'], 1) }}</td>
                            <td class="px-4 py-3 text-right hidden lg:table-cell">
                                <span class="{{ $ranking['on_time_rate'] >= 90 ? 'text-green-600' : 'text-red-500' }} font-medium">
                                    {{ $ranking['on_time_rate'] }}%
                                </span>
                            </td>
                            <td class="px-4 py-3 text-right hidden lg:table-cell text-gray-500">{{ $ranking['evaluation_count'] }}</td>
                            <td class="px-4 py-3 text-center">
                                <a href="{{ route('supplier-performance.detail', $ranking['supplier_id']) }}"
            ...
            @forelse($suppliers as $supplier)
                ...
                <div class="border border-gray-200 rounded-xl p-4 hover:border-blue-300 transition-colors">
                    <div class="flex items-start justify-between gap-2 mb-3">
                        <div class="min-w-0">
                            <h4 class="font-semibold text-gray-900 text-sm truncate">{{ $supplier->name }}</h4>
                            <p class="text-xs text-gray-400 truncate">{{ $supplier->company ?? '-' }}</p>
                        </div>
                    ...
                    @if($hasData)
                        <div class="space-y-1.5 text-xs">
                            <div class="flex justify-between">
                                ...
                                <span class="font-semibold text-gray-900">{{ number_format($supplier->performance['avg_overall_score'], 1) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Tepat Waktu</span>
                                <span class="{{ $supplier->performance['on_time_delivery_rate'] >= 90 ? 'text-green-600' : 'text-red-500' }} font-medium">
                                    {{ $supplier->performance['on_time_delivery_rate'] }}%
                                </span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Kualitas</span>
                                <span class="text-gray-700">{{ number_format($supplier->performance['avg_quality_rate'], 1) }}%</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Total PO</span>
                                <span class="text-gray-700">{{ $supplier->performance['total_pos'] }}</span>
                            </div>
                            <div class="flex justify-between items-center pt-1.5 border-t border-gray-100">
                                ...
                                <span class="inline-flex items-center gap-1 text-xs font-medium
                                    {{ $trend === 'improving' ? 'text-green-600' : ($trend === 'declining' ? 'text-red-500' : 'text-gray-500') }}">
                                    @if($trend === 'improving')
