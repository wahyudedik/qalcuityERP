<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            Laporan HPP (COGS)
            <a href="{{ route('inventory.costing.valuation') }}" class="text-xs text-gray-500 dark:text-slate-400 hover:text-blue-500">← Valuasi Stok</a>
        </div>
    </x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
                ...
                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ count($report['rows']) }}</p>
            </div>
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            ...
                            <td class="px-4 py-3 text-right text-gray-900 dark:text-white">{{ number_format($row['qty_sold'], 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-right text-gray-700 dark:text-slate-300">Rp {{ number_format($row['avg_unit_cost'], 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-right font-semibold text-red-600 dark:text-red-400">Rp {{ number_format($row['total_cogs'], 0, ',', '.') }}</td>
                            <td class="px-4 py-3 hidden md:table-cell">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs text-gray-500 dark:text-slate-400 w-10 text-right">{{ number_format($pct, 1) }}%</span>
                                </div>
                            </td>
                        </tr>
                        @empty
                    ...
                    <tfoot class="bg-gray-50 dark:bg-white/5 font-semibold text-sm">
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right text-red-600 dark:text-red-400">Rp {{ number_format($report['total_cogs'], 0, ',', '.') }}</td>
