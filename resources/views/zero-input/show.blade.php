    ...
    <div class="py-6 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <div class="flex items-center justify-between mb-3">
                <div>
                    <p class="text-sm text-gray-500">Channel: <span
                            class="font-medium capitalize text-gray-900">{{ $log->channel }}</span></p>
                    <p class="text-sm text-gray-500">Modul: <span
                            class="font-medium capitalize text-gray-900">{{ str_replace('_', ' ', $log->mapped_module ?? '-') }}</span>
                    </p>
                </div>
                <div class="flex items-center gap-2">
                    @if ($log->confidence_score)
                        ...
                        @endphp
                        <span
                            class="px-2.5 py-1 rounded-full text-xs font-medium bg-{{ $confColor }}-100 text-{{ $confColor }}-700 $confColor }}-500/20 $confColor }}-400">
                            AI {{ $log->confidence_score }}%
                        </span>
                    @endif
                    <span
                        class="px-3 py-1 rounded-full text-sm font-medium
                        {{ in_array($log->status, ['mapped', 'created'])
            ...
            @if ($log->file_path)
                <img src="{{ Storage::url($log->file_path) }}" alt="Nota"
                    class="max-h-48 rounded-xl object-contain border border-gray-200">
            @endif
                    ...
                    @if ($log->status !== 'created' && $log->status !== 'rejected')
                        <div class="flex gap-3 pt-3">
                            <form method="POST" action="{{ route('zero-input.reject', $log) }}" class="contents">
                                @csrf
                                <button type="submit" data-confirm="Tolak hasil OCR ini? Feedback akan disimpan."
                        ...
                        <p class="text-xs text-gray-400 mt-2">
                            💡 Edit field di atas jika ada yang salah. Koreksi Anda akan disimpan untuk meningkatkan
                            akurasi AI.
                        </p>
