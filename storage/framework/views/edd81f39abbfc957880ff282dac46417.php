...
<?php $component->withAttributes([]); ?>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $trips; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 font-mono text-xs font-medium text-gray-900">
                                <?php echo e($t->trip_number); ?></td>
                            <td class="px-4 py-3 text-gray-700">
                                <?php echo e($t->vehicle?->plate_number ?? '-'); ?></td>
                            <td class="px-4 py-3 hidden sm:table-cell text-gray-700">
                                <?php echo e($t->driver?->name ?? '-'); ?></td>
                            <td class="px-4 py-3 text-gray-900"><?php echo e($t->purpose); ?>

                                <?php if($t->destination): ?>
                                    <span class="text-xs text-gray-400">→ <?php echo e($t->destination); ?></span>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs bg-<?php echo e($tc); ?>-100 text-<?php echo e($tc); ?>-700 $tc }}-500/20 $tc }}-400"><?php echo e($tl); ?></span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <?php if($t->status === 'in_progress'): ?>
                                    <?php if(auth()->check() && app(\App\Services\PermissionService::class)->check(auth()->user(), 'fleet', 'edit')): ?>
                                    <button
                                        onclick="openComplete(<?php echo e($t->id); ?>, '<?php echo e($t->trip_number); ?>', <?php echo e($t->odometer_start ?? 0); ?>)"
        ...
        <?php if($trips->hasPages()): ?>
            <div class="px-4 py-3 border-t border-gray-100"><?php echo e($trips->links()); ?></div>
        <?php endif; ?>
    </div>
    ...
    <div id="modal-add-trip" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <h3 class="font-semibold text-gray-900">Buat Trip</h3>
                <button onclick="document.getElementById('modal-add-trip').classList.add('hidden')"
            ...
            <form method="POST" action="<?php echo e(route('fleet.trips.store')); ?>" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Kendaraan
                        ...
                        <select name="vehicle_id" required class="<?php echo e($cls); ?>">
                            ...
                            <?php $__currentLoopData = $vehicles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($v->id); ?>"><?php echo e($v->plate_number); ?> — <?php echo e($v->name); ?>
                    ...
                    <div class="sm:col-span-2"><label
                            class="block text-xs font-medium text-gray-600 mb-1">Tujuan
                            *</label><input type="text" name="purpose" required
                            placeholder="Kirim barang ke customer" class="<?php echo e($cls); ?>"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Asal</label><input
                            type="text" name="origin" placeholder="Gudang Pusat" class="<?php echo e($cls); ?>"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Tujuan</label><input
                            type="text" name="destination" placeholder="Jakarta Selatan"
                            class="<?php echo e($cls); ?>"></div>
                    <div class="sm:col-span-2"><label
                            class="block text-xs font-medium text-gray-600 mb-1">Waktu
                            Berangkat</label><input type="datetime-local" name="departed_at"
                            class="<?php echo e($cls); ?>"></div>
                </div>
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-add-trip').classList.add('hidden')"
                        class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
                    <button type="submit"
                        class="px-4 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Buat Trip</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modal-complete" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-sm shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <h3 class="font-semibold text-gray-900">Selesaikan Trip</h3>
            ...
            <form id="form-complete" method="POST" class="p-6 space-y-4">
                ...
                <p class="text-sm text-gray-600">Trip: <span id="c-trip"
                        class="font-mono font-semibold text-gray-900"></span></p>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Odometer Akhir (km)
                        *</label>
                    <input type="number" name="odometer_end" id="c-odo" required min="0"
...
<?php endif; ?>
<?php /**PATH E:\PROJEKU\qalcuityERP\resources\views\fleet\trips.blade.php ENDPATH**/ ?>