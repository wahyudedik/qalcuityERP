<x-app-layout>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
            ...
            <p class="text-lg font-bold text-gray-900 dark:text-white">Rp {{ number_format($stats['total_sales'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3 text-gray-900 dark:text-white">{{ $c->user->name ?? '-' }}</td>
                        <td class="px-4 py-3 text-right text-gray-500 dark:text-slate-400">{{ $target ? 'Rp ' . number_format($target->target_amount, 0, ',', '.') : '-' }}</td>
                        <td class="px-4 py-3 text-right text-gray-900 dark:text-white">Rp {{ number_format($c->total_sales, 0, ',', '.') }}</td>
                        ...
                        <td class="px-4 py-3 text-right text-gray-900 dark:text-white">Rp {{ number_format($c->commission_amount, 0, ',', '.') }}</td>
                        ...
                        <td class="px-4 py-3 text-right font-semibold text-gray-900 dark:text-white">Rp {{ number_format($c->total_payout, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-center"><span class="px-2 py-0.5 rounded-full text-xs bg-{{ $cc }}-100 text-{{ $cc }}-700 dark:bg-{{ $cc }}-500/20 dark:text-{{ $cc }}-400">{{ $cl }}</span></td>
