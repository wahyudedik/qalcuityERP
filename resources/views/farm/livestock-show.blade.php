            ...
            @if ($totalFeed > 0)
                <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div
                            class="p-3 rounded-xl {{ $fcr && $fcr <= 1.8 ? 'bg-green-50 dark:bg-green-500/10' : ($fcr && $fcr <= 2.2 ? 'bg-amber-50 dark:bg-amber-500/10' : 'bg-gray-50 dark:bg-white/5') }}">
                            <p class="text-xs text-gray-500 dark:text-slate-400">FCR</p>
                            <p
                                class="text-2xl font-black {{ $fcr && $fcr <= 1.8 ? 'text-green-600' : ($fcr && $fcr <= 2.2 ? 'text-amber-600' : 'text-gray-900 dark:text-white') }}">
                                {{ $fcr ?? '-' }}</p>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                                @foreach ($livestockHerd->feedLogs->take(14) as $fl)
                                    <tr>
                                        ...
                                        <td class="px-4 py-2 text-gray-700 dark:text-slate-300">{{ $fl->feed_type }}
                                        </td>
                    ...
                    @foreach ($livestockHerd->healthRecords->take(20) as $hr)
                        ...
                        <div class="px-5 py-3">
                            <div class="flex items-center gap-2 mb-1">
                                <span
                                    class="text-xs px-2 py-0.5 rounded-full bg-{{ $sc }}-100 text-{{ $sc }}-700 dark:bg-{{ $sc }}-500/20 dark:text-{{ $sc }}-400">{{ ucfirst($hr->severity) }}</span>
                                <span
                                    class="text-sm font-medium text-gray-900 dark:text-white">{{ $hr->condition }}</span>
    ...
    <div id="movementModal"
        ...
        <div
            ...
            <p class="text-xs text-gray-400 mb-4">Populasi saat ini: <span
                    class="font-bold text-gray-900 dark:text-white">{{ $livestockHerd->current_count }} ekor</span>
            </p>
