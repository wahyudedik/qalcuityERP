{{--
    Task 4: Module Sidebar — Tampilan, Navigasi, Collapse
    Sidebar 260px yang muncul di sisi kiri setelah pengguna memilih modul dari Launcher.
    Desktop (>=1024px): push layout — aside is relative shrink-0, participates in flex row
    Tablet/Mobile (<1024px): overlay drawer — aside is fixed z-40, backdrop behind it

    IMPORTANT: This partial renders TWO sibling elements as direct flex children of #main-wrap:
      1. <aside> — the sidebar itself (participates in flex row on desktop)
      2. <div>   — the mobile backdrop (fixed overlay, hidden on desktop)
--}}

{{-- Module Sidebar — direct flex child of #main-wrap for push layout on desktop --}}
<aside id="module-sidebar" x-data x-show="$store.navSystem.sidebarVisible"
    ...
    {{-- Inner content wrapper — fixed 260px width, hidden when collapsed --}}
    <div class="w-[260px] h-full flex flex-col bg-white border-r border-gray-200"
        ...
        {{-- Nav Items with Collapsible Sections --}}
        <nav class="flex-1 overflow-y-auto py-2 px-2 scrollbar-thin" aria-label="Menu navigasi" x-data="{
            ...
            <template x-for="(item, index) in $store.navSystem.filteredNavItems" :key="`nav-item-${index}`">
                <div>
                    {{-- Section label (clickable accordion header) --}}
...
{{-- Mobile backdrop — shown behind sidebar on tablet/mobile (<1024px), hidden on desktop --}}
{{-- Clicking backdrop closes the sidebar drawer --}}
