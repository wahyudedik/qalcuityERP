<x-app-layout>
    <x-slot name="header">Create Group Booking</x-slot>

    <div class="max-w-3xl mx-auto">
        <form action="{{ route('hotel.group-bookings.store') }}" method="POST" class="space-y-6">
            @csrf

            {{-- Organizer Info --}}
            <div class="bg-white rounded-2xl border border-gray-200 p-6">
                <h3 class="text-sm font-semibold text-gray-900 mb-4">Organizer Information</h3>

                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Organizer Guest
                            *</label>
                        <select name="organizer_guest_id" required
                            ...
                            @foreach ($guests as $guest)
                                <option value="{{ $guest->id }}">{{ $guest->name }}
                                    ({{ $guest->email ?? $guest->phone }})</option>
                            @endforeach
                        </select>
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Group Name
                            *</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Group Type
                            *</label>
            ...
            {{-- Dates & Size --}}
            <div class="bg-white rounded-2xl border border-gray-200 p-6">
                <h3 class="text-sm font-semibold text-gray-900 mb-4">Stay Details</h3>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Start Date
                            *</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">End Date
                            *</label>
