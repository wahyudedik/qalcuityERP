...
<?php $component->withAttributes([]); ?>
    ...
    <?php if(session('success')): ?>
        <div class="mb-4 px-4 py-3 bg-green-50 dark:bg-green-500/20 border border-green-200 dark:border-green-500/30 text-green-700 dark:text-green-400 text-sm rounded-xl"><?php echo e(session('success')); ?></div>
    <?php endif; ?>
    <?php if(session('error')): ?>
        <div class="mb-4 px-4 py-3 bg-red-50 dark:bg-red-500/20 border border-red-200 dark:border-red-500/30 text-red-700 dark:text-red-400 text-sm rounded-xl"><?php echo e(session('error')); ?></div>
    <?php endif; ?>

    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        <?php $__empty_1 = true; $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6 flex flex-col <?php echo e(!$plan->is_active ? 'opacity-60' : ''); ?>">
            <div class="flex items-start justify-between mb-4">
                <div>
                    <h3 class="font-bold text-gray-900 dark:text-white text-lg"><?php echo e($plan->name); ?></h3>
                    <p class="text-xs text-gray-400 dark:text-slate-500 font-mono"><?php echo e($plan->slug); ?></p>
                </div>
                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium
                    <?php echo e($plan->is_active ? 'bg-green-50 dark:bg-green-500/20 text-green-700 dark:text-green-400' : 'bg-gray-100 dark:bg-white/10 text-gray-500 dark:text-slate-500'); ?>">
                    <span class="w-1.5 h-1.5 rounded-full <?php echo e($plan->is_active ? 'bg-green-500' : 'bg-gray-400 dark:bg-slate-500'); ?>"></span>
            ...
            <div class="space-y-2 text-sm text-gray-600 dark:text-slate-400 mb-5 flex-1">
                ...
                <?php if($plan->features): ?>
                    ...
                    <p class="text-xs text-gray-400 dark:text-slate-500 pl-6">+<?php echo e(count($plan->features) - 5); ?> fitur lainnya</p>
                    <?php endif; ?>
                <?php endif; ?>
            </div>

            <div class="flex items-center gap-2 pt-4 border-t border-gray-200 dark:border-white/10">
                <span class="text-xs text-gray-400 dark:text-slate-500"><?php echo e($plan->tenants_count ?? $plan->tenants()->count()); ?> tenant</span>
