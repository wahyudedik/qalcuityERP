    ...
    <a href="?tab=<?php echo e($t['tab']); ?>"
       class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition
        ...
        <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="<?php echo e($t['icon']); ?>"/>
        </svg>
        <?php echo e($t['label']); ?>

        <?php if($t['tab'] === 'errors' && $errorStats['unresolved'] > 0): ?>
            <span class="text-[10px] bg-red-500 text-white px-1.5 py-0.5 rounded-full font-bold"><?php echo e($errorStats['unresolved']); ?></span>
        <?php endif; ?>
        <?php if($t['tab'] === 'anomaly' && $anomalyStats['critical'] > 0): ?>
            <span class="text-[10px] bg-red-500 text-white px-1.5 py-0.5 rounded-full font-bold"><?php echo e($anomalyStats['critical']); ?></span>
        <?php endif; ?>
    </a>
...
<div class="flex flex-wrap items-center gap-3 mb-4">
    <form method="GET" action="" class="flex gap-2 flex-wrap flex-1">
        ...
        <select name="level" class="px-3 py-2 rounded-xl border border-white/10 bg-white/5 text-sm text-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <option value="<?php echo e($lv); ?>" <?php if(request('level')===$lv): echo 'selected'; endif; ?>><?php echo e(ucfirst($lv)); ?></option>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </select>
        <label class="flex items-center gap-2 text-sm text-slate-400 cursor-pointer">
            <input type="checkbox" name="unresolved" value="1" <?php if(request('unresolved')): echo 'checked'; endif; ?> class="rounded">
            Belum resolved
        </label>
    ...
    <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-white/5">
            ...
            <tr class="hover:bg-white/5 transition cursor-pointer <?php echo e($err->is_resolved ? 'opacity-60' : ''); ?>"
                ...
                <td class="px-4 py-3">
                    <span class="text-[11px] font-semibold px-2 py-0.5 rounded-full <?php echo e($err->levelColor()); ?>"><?php echo e(strtoupper($err->level)); ?></span>
                </td>
                <td class="px-4 py-3 max-w-xs">
                    <p class="text-slate-200 text-xs font-medium truncate"><?php echo e($err->message); ?></p>
                    <?php if($err->file): ?><p class="text-slate-500 text-[10px] truncate mt-0.5"><?php echo e(basename($err->file)); ?>:<?php echo e($err->line); ?></p><?php endif; ?>
                </td>
                <td class="px-4 py-3 hidden md:table-cell">
                    <p class="text-slate-400 text-xs truncate max-w-[180px]"><?php echo e($err->method); ?> <?php echo e($err->url); ?></p>
                    <?php if($err->ip): ?><p class="text-slate-500 text-[10px]"><?php echo e($err->ip); ?></p><?php endif; ?>
                </td>
                <td class="px-4 py-3 hidden lg:table-cell">
                    <p class="text-slate-400 text-xs"><?php echo e($err->created_at->format('d M Y')); ?></p>
                    <p class="text-slate-500 text-[10px]"><?php echo e($err->created_at->format('H:i:s')); ?></p>
                </td>
                <td class="px-4 py-3">
                ...
                <td class="px-4 py-3" @click.stop>
                    <div class="flex items-center justify-end gap-1">
                        ...
                        <?php if(!$err->is_resolved): ?>
                        <form method="POST" action="<?php echo e(route('super-admin.monitoring.resolve-error', $err)); ?>" @click.stop>
                            <?php echo csrf_field(); ?>
                        ...
                        <?php endif; ?>
                        <form method="POST" action="<?php echo e(route('super-admin.monitoring.delete-error', $err)); ?>" onsubmit="return confirm('Hapus error log ini?')" @click.stop>
...
<div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 mb-5">
    ...
    <div class="flex items-end gap-2 h-24">
        ...
        <div class="flex-1 flex flex-col items-center gap-1">
            ...
            <div class="w-full rounded-t-lg" style="height:<?php echo e(max(4, $pct * 0.8)); ?>px; background: linear-gradient(to top, #3b82f6, #6366f1);"></div>
            <span class="text-[10px] text-slate-500"><?php echo e(\Carbon\Carbon::createFromFormat('Y-m', $m->month)->format('M')); ?></span>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
</div>
<div class="bg-[#1e293b] border border-white/10 rounded-2xl overflow-hidden">
    <div class="px-5 py-4 border-b border-white/10">
        <p class="text-sm font-semibold text-slate-200">Penggunaan per Tenant — <?php echo e(now()->format('F Y')); ?></p>
    </div>
    <div class="overflow-x-auto">
    <table class="min-w-full text-sm">
        <thead>
        ...
        <tbody class="divide-y divide-white/5">
            ...
            <tr class="hover:bg-white/5 transition cursor-pointer"
                ...
                <td class="px-5 py-3">
                    <p class="text-slate-200 font-medium text-xs"><?php echo e($u->tenant?->name ?? 'Tenant #'.$u->tenant_id); ?></p>
                </td>
                <td class="px-5 py-3 text-right text-slate-300 text-xs font-semibold"><?php echo e(number_format($u->total_messages)); ?></td>
                <td class="px-5 py-3 text-right text-slate-400 text-xs"><?php echo e(number_format($u->total_tokens)); ?></td>
                <td class="px-5 py-3 hidden md:table-cell">
                    <?php $pct = round($u->total_messages / $maxUsage * 100); ?>
                    <div class="flex items-center gap-2">
                        <div class="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                            <div class="h-full bg-blue-500 rounded-full" style="width:<?php echo e($pct); ?>%"></div>
                        </div>
                        <span class="text-[10px] text-slate-500 w-8 text-right"><?php echo e($pct); ?>%</span>
                    </div>
                </td>
...
<div class="flex flex-wrap gap-3 mb-4">
    <form method="GET" action="" class="flex gap-2 flex-wrap flex-1">
        ...
        <input type="text" name="action" value="<?php echo e(request('action')); ?>" placeholder="Filter aksi (login, created, ...)"
    ...
    <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-white/5">
            ...
            <tr class="hover:bg-white/5 transition cursor-pointer"
                ...
                <script type="application/json" class="row-data"><?php echo json_encode($actData); ?></script>
                <td class="px-4 py-3">
                    <span class="text-[11px] font-medium px-2 py-0.5 rounded-full <?php echo e($actionColor); ?>"><?php echo e($act->action); ?></span>
                </td>
                <td class="px-4 py-3 max-w-xs">
                    <p class="text-slate-300 text-xs truncate"><?php echo e($act->description); ?></p>
                    <?php if($act->model_type): ?>
                    <p class="text-slate-500 text-[10px]"><?php echo e(class_basename($act->model_type)); ?> #<?php echo e($act->model_id); ?></p>
                    <?php endif; ?>
                </td>
                <td class="px-4 py-3 hidden md:table-cell">
                ...
                <td class="px-4 py-3">
                    ...
                    <p class="text-slate-500 text-[10px]"><?php echo e($act->created_at->format('H:i:s')); ?></p>
                </td>
...
<div class="space-y-3">
    ...
    <div class="border rounded-2xl p-4 <?php echo e($sevColor); ?> cursor-pointer hover:brightness-110 transition"
        ...
        <script type="application/json" class="row-data"><?php echo json_encode($anomalyData); ?></script>
        <div class="flex items-start justify-between gap-3">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1 flex-wrap">
                    <span class="text-[11px] font-semibold px-2 py-0.5 rounded-full <?php echo e($sevBadge); ?>"><?php echo e(strtoupper($a->severity)); ?></span>
                    <span class="text-[11px] text-slate-500 bg-white/5 px-2 py-0.5 rounded-full"><?php echo e($a->type); ?></span>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
    ...
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
        ...
        <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-[11px] px-2 py-0.5 rounded-full <?php echo e($health['db_ok'] ? 'text-green-400 bg-green-500/15' : 'text-red-400 bg-red-500/15'); ?>">
                <?php echo e($health['db_ok'] ? 'Online' : 'Error'); ?>

            </span>
        </div>
        <p class="text-2xl font-bold text-slate-100"><?php echo e($health['db_latency_ms']); ?> <span class="text-sm font-normal text-slate-400">ms</span></p>
    ...
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
        ...
        <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-[11px] px-2 py-0.5 rounded-full <?php echo e($health['disk_used_pct'] > 85 ? 'text-red-400 bg-red-500/15' : 'text-green-400 bg-green-500/15'); ?>">
                <?php echo e($health['disk_used_pct']); ?>% terpakai
            </span>
        </div>
        <div class="h-2 bg-white/10 rounded-full overflow-hidden mb-2">
            <div class="h-full rounded-full <?php echo e($health['disk_used_pct'] > 85 ? 'bg-red-500' : 'bg-blue-500'); ?>" style="width:<?php echo e($health['disk_used_pct']); ?>%"></div>
        </div>
        <p class="text-xs text-slate-500"><?php echo e($health['disk_free_gb']); ?> GB bebas dari <?php echo e($health['disk_total_gb']); ?> GB</p>
    </div>
    ...
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
        ...
        <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-[11px] px-2 py-0.5 rounded-full <?php echo e($health['log_size_mb'] > 100 ? 'text-amber-400 bg-amber-500/15' : 'text-green-400 bg-green-500/15'); ?>">
                <?php echo e($health['log_size_mb'] > 100 ? 'Besar' : 'Normal'); ?>

            </span>
        </div>
        <p class="text-2xl font-bold text-slate-100"><?php echo e($health['log_size_mb']); ?> <span class="text-sm font-normal text-slate-400">MB</span></p>
    ...
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
        ...
        <div class="space-y-2">
            <div class="flex justify-between text-xs"><span class="text-slate-400">Total</span><span class="text-slate-200 font-semibold"><?php echo e($health['total_tenants']); ?></span></div>
            <div class="flex justify-between text-xs"><span class="text-slate-400">Aktif</span><span class="text-green-400 font-semibold"><?php echo e($health['active_tenants']); ?></span></div>
            <div class="flex justify-between text-xs"><span class="text-slate-400">Total User</span><span class="text-slate-200 font-semibold"><?php echo e($health['total_users']); ?></span></div>
        </div>
    </div>
    ...
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
        ...
        <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-[11px] px-2 py-0.5 rounded-full <?php echo e($health['queue_failed'] > 0 ? 'text-red-400 bg-red-500/15' : 'text-green-400 bg-green-500/15'); ?>">
                <?php echo e($health['queue_failed'] > 0 ? $health['queue_failed'].' failed' : 'OK'); ?>

            </span>
        </div>
        <p class="text-2xl font-bold <?php echo e($health['queue_failed'] > 0 ? 'text-red-400' : 'text-slate-100'); ?>"><?php echo e($health['queue_failed']); ?></p>
    ...
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
         @click="show('health', rowData($el))">
        <script type="application/json" class="row-data"><?php echo json_encode(['component'=>'Server','status'=>'Info','ok'=>true,'created_at'=>now()->format('d M Y H:i:s'),'details'=>[['label'=>'PHP','value'=>$health['php_version']],['label'=>'Laravel','value'=>$health['laravel_version']],['label'=>'Uptime','value'=>$health['uptime']],['label'=>'Error Hari Ini','value'=>$health['errors_today']],['label'=>'Environment','value'=>app()->environment()],['label'=>'Debug Mode','value'=>config('app.debug')?'ON':'OFF']]]); ?></script>
        <p class="text-sm font-semibold text-slate-200 mb-3">Server</p>
        <div class="space-y-2">
            <div class="flex justify-between text-xs"><span class="text-slate-400">PHP</span><span class="text-slate-200 font-mono"><?php echo e($health['php_version']); ?></span></div>
            <div class="flex justify-between text-xs"><span class="text-slate-400">Laravel</span><span class="text-slate-200 font-mono"><?php echo e($health['laravel_version']); ?></span></div>
            <div class="flex justify-between text-xs"><span class="text-slate-400">Uptime</span><span class="text-slate-200 font-mono"><?php echo e($health['uptime']); ?></span></div>
            <div class="flex justify-between text-xs"><span class="text-slate-400">Error Hari Ini</span><span class="<?php echo e($health['errors_today'] > 0 ? 'text-red-400' : 'text-green-400'); ?> font-semibold"><?php echo e($health['errors_today']); ?></span></div>
        </div>
    </div>
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
    ...
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-4">
        ...
        <p class="text-2xl font-black <?php echo e($moduleHealth['total_alerts'] > 0 ? 'text-amber-400' : 'text-green-400'); ?>"><?php echo e($moduleHealth['total_alerts']); ?></p>
    </div>
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-4">
        <p class="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Critical</p>
        <p class="text-2xl font-black <?php echo e($moduleHealth['critical_alerts'] > 0 ? 'text-red-400' : 'text-green-400'); ?>"><?php echo e($moduleHealth['critical_alerts']); ?></p>
    </div>
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-4">
        <p class="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Status</p>
        <p class="text-2xl font-black <?php echo e($moduleHealth['critical_alerts'] > 0 ? 'text-red-400' : ($moduleHealth['total_alerts'] > 0 ? 'text-amber-400' : 'text-green-400')); ?>">
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
    ...
    <div class="<?php echo e($bgColor); ?> border <?php echo e($borderColor); ?> rounded-2xl p-4">
        ...
        <p class="text-xs text-slate-500 mb-2">Total records: <span class="text-slate-300 font-medium"><?php echo e(number_format($mod['total'])); ?></span></p>
        <?php if($hasAlerts): ?>
        <div class="space-y-1">
            ...
            ?>
            <p class="text-[11px] <?php echo e($ac); ?> px-2 py-1 rounded-lg"><?php echo e($alert['msg']); ?></p>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        <?php endif; ?>
    </div>
