<x-app-layout>
    <x-slot name="header">Printing Job Dashboard</x-slot>
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p class="text-2xl font-bold text-blue-600 mt-1">{{ $stats['active_jobs'] }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500">Completed Today</p>
            <p class="text-2xl font-bold text-green-600 mt-1">{{ $stats['completed_today'] }}</p>
        </div>
        <div class="bg-white rounded-xl border border-red-200 p-4">
            <p class="text-xs text-gray-500">Overdue Jobs</p>
            <p class="text-2xl font-bold text-red-600 mt-1">{{ $stats['overdue_jobs'] }}</p>
        </div>
        <div class="bg-white rounded-xl border border-orange-200 p-4">
            <p class="text-xs text-gray-500">Urgent Jobs</p>
            <p class="text-2xl font-bold text-orange-600 mt-1">{{ $stats['urgent_jobs'] }}</p>
        </div>
    </div>
            ...
            <div class="space-y-2">
                @foreach ($overdue as $job)
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-red-700">{{ $job->job_number }} -
                            {{ $job->job_name }}</span>
                        <span class="text-red-600">Due: {{ $job->due_date->format('d M Y') }}</span>
                    </div>
                @endforeach
            </div>
        </div>
                                ...
                                <td class="px-6 py-4">
                                    @if ($job->due_date)
                                        @if ($job->due_date->isPast())
                                            <span
                                                class="text-red-600 font-medium">{{ $job->due_date->format('d M Y') }}</span>
                                        @else
                                            <span
                                                class="text-gray-700">{{ $job->due_date->format('d M Y') }}</span>
                                        @endif
                                    @else
