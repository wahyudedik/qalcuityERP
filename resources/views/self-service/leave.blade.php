    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
            ...
            <div class="flex items-end gap-2">
                <p class="text-3xl font-black text-gray-900 dark:text-white">{{ $quota - $usedDays }}</p>
                <p class="text-sm text-gray-400 dark:text-slate-500 mb-1">/ {{ $quota }} hari tersisa</p>
            </div>
            ...
            <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">{{ $usedDays }} hari terpakai tahun {{ now()->year }}</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
    ...
    <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="flex-1">
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                    ...
                    <div class="px-5 py-4 flex items-start gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $leave->typeLabel() }}</p>
                            ...
                            @if($leave->reason)
                            <p class="text-xs text-gray-400 dark:text-slate-500 mt-0.5 italic">{{ $leave->reason }}</p>
                            @endif
                            @if($leave->status === 'rejected' && $leave->rejection_reason)
                            <p class="text-xs text-red-500 dark:text-red-400 mt-1">Alasan penolakan: {{ $leave->rejection_reason }}</p>
                            @endif
                            @if($leave->status === 'approved' && $leave->approver)
                            <p class="text-xs text-gray-400 dark:text-slate-500 mt-0.5">Disetujui oleh: {{ $leave->approver->name }}</p>
                            @endif
                        </div>
                        <div class="shrink-0 text-right">
                            <p class="text-xs text-gray-400 dark:text-slate-500">{{ $leave->created_at->format('d M Y') }}</p>
                            @if($leave->status === 'pending')
                            <form method="POST" action="{{ route('self-service.leave.cancel', $leave) }}"
                ...
                @if($leaves->hasPages())
                <div class="px-5 py-3 border-t border-gray-100 dark:border-white/5">{{ $leaves->links() }}</div>
                @endif
                @endif
            </div>
        </div>
