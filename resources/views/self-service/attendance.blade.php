<x-app-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="lg:col-span-1 bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6 flex flex-col items-center text-center">
            <p class="text-xs text-gray-400 dark:text-slate-500 uppercase tracking-wide mb-2">{{ now()->translatedFormat('l, d F Y') }}</p>

            {{-- Live clock --}}
            <p id="live-clock" class="text-4xl font-black text-gray-900 dark:text-white tabular-nums mb-4">{{ now()->format('H:i:s') }}</p>

            @if($today)
                <div class="w-full space-y-2 mb-5">
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500 dark:text-slate-400">Clock In</span>
                        <span class="font-semibold text-green-600 dark:text-green-400">{{ $today->check_in ?? '—' }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500 dark:text-slate-400">Clock Out</span>
                        <span class="font-semibold {{ $today->check_out ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-slate-500' }}">
                            {{ $today->check_out ?? '—' }}
                        </span>
                    </div>
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $h }}j {{ $m }}m</span>
                    </div>
                    @endif
                    <div class="flex justify-between text-sm">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        ...
                        <td class="px-4 py-3 text-center text-gray-700 dark:text-slate-300">{{ $att->check_in ?? '—' }}</td>
                        <td class="px-4 py-3 text-center text-gray-700 dark:text-slate-300">{{ $att->check_out ?? '—' }}</td>
                        <td class="px-4 py-3 text-center text-gray-500 dark:text-slate-400">{{ $dur ?: '—' }}</td>
