<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <span>⚓ Trip Detail - {{ $trip->trip_number }}</span>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6 mb-6">
        <div class="flex items-start justify-between mb-4">
            <div>
                <div class="flex items-center gap-3 mb-2">
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $trip->trip_number }}</h2>
                    @php
                        $statusColors = [
                            'planned' => 'gray',
                            'departed' => 'blue',
                    ...
                    @endphp
                    <span
                        class="px-3 py-1 text-sm rounded-full bg-{{ $color }}-100 text-{{ $color }}-700 dark:bg-{{ $color }}-500/20 dark:text-{{ $color }}-400">
                        {{ $label }}
                    </span>
                </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-100 dark:border-white/5">
            <div>
                <p class="text-xs text-gray-500 dark:text-slate-400">Waktu Berangkat</p>
            ...
            @if ($trip->actual_return)
                <div>
                    <p class="text-xs text-gray-500 dark:text-slate-400">Waktu Kembali</p>
                    <p class="text-sm font-medium text-gray-900 dark:text-white">
            ...
            @elseif($trip->expected_return)
                <div>
                    <p class="text-xs text-gray-500 dark:text-slate-400">Kembali (Rencana)</p>
                    <p class="text-sm font-medium text-gray-900 dark:text-white">
            ...
            @if ($trip->fishing_zone)
                <div>
                    <p class="text-xs text-gray-500 dark:text-slate-400">Zona Penangkapan</p>
                    <p class="text-sm font-medium text-gray-900 dark:text-white">
        ...
        @if ($trip->notes)
            <div class="mt-4 pt-4 border-t border-gray-100 dark:border-white/5">
                <p class="text-sm text-gray-600 dark:text-slate-400">{{ $trip->notes }}</p>
            </div>
        @endif
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-white/10 p-4">
            <p class="text-xs text-gray-500 dark:text-slate-400">Jumlah Entry</p>
            <p class="text-2xl font-bold text-blue-600">{{ $catches->total() }}</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-white/10 p-4">
            <p class="text-xs text-gray-500 dark:text-slate-400">Spesies Berbeda</p>
            <p class="text-2xl font-bold text-purple-600">{{ $catches->pluck('species_id')->unique()->count() }}</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-white/10 p-4">
            <p class="text-xs text-gray-500 dark:text-slate-400">Rata-rata Berat/Entry</p>
    ...
    <div
        ...
        <div class="px-6 py-4 border-b border-gray-200 dark:border-white/10 flex items-center justify-between">
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">🐟 Detail Tangkapan</h3>
        ...
        @if ($catches->isEmpty())
            <div class="p-12 text-center">
                ...
                <p class="text-sm text-gray-500 dark:text-slate-400">Belum ada tangkapan tercatat untuk trip ini.</p>
                @if (in_array($trip->status, ['departed', 'fishing']))
                    <button onclick="document.getElementById('addCatchModal').classList.remove('hidden')"
                        ...
                        @foreach ($catches as $catch)
                            <tr class="hover:bg-gray-50 dark:hover:bg-[#0f172a] transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if ($catch->grade)
                                        <span
                                            class="px-2 py-1 text-xs rounded-full bg-{{ $catch->grade->color ?? 'purple' }}-100 text-{{ $catch->grade->color ?? 'purple' }}-700 dark:bg-{{ $catch->grade->color ?? 'purple' }}-500/20 dark:text-{{ $catch->grade->color ?? 'purple' }}-400">
    ...
    @if ($trip->crew && count($trip->crew) > 0)
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6 mb-6">
            <h3 class="text-base font-semibold text-gray-900 dark:text-white mb-4">👥 Awak Kapal
                ({{ count($trip->crew) }} orang)</h3>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                @foreach ($trip->crew as $member)
                    <div class="px-3 py-2 bg-gray-50 dark:bg-[#0f172a] rounded-lg">
                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $member->name }}</p>
                        <p class="text-xs text-gray-500 dark:text-slate-400">{{ ucfirst($member->role ?? 'crew') }}</p>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <div id="addCatchModal"
        ...
        <div
            ...
            <div class="flex items-center justify-between mb-5">
                <h3 class="text-base font-semibold text-gray-900 dark:text-white">🐟 Catat Tangkapan</h3>
            ...
            <form method="POST" action="{{ route('fisheries.operations.record-catch', $trip->id) }}"
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Spesies *</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Jumlah (ekor)
                            *</label>
                        <input type="number" name="quantity" required step="1" min="0"
                            placeholder="100" class="{{ $cls }}">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Berat Total
                            (kg) *</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Grade
                            Kualitas</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Skor Kesegaran
                            (0-10)</label>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Lokasi GPS
                        (opsional)</label>
