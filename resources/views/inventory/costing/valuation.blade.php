<x-app-layout>
    <x-slot name="header">Valuasi Inventori</x-slot>

    {{-- Toolbar --}}
    <div class="flex flex-wrap items-center justify-end gap-2 mb-4">
        <a href="{{ route('inventory.costing.cogs') }}" class="text-xs text-gray-500 hover:text-blue-500">Laporan COGS →</a>
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                <p class="text-2xl font-bold text-blue-600 mt-1">{{ count($report['rows']) }}</p>
            </div>
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-gray-500 text-xs hidden md:table-cell">{{ $row['warehouse_name'] }}</td>
                            <td class="px-4 py-3 text-right text-gray-900">{{ number_format($row['quantity'], 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-right text-gray-500">Rp {{ number_format($row['price_buy'], 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-right font-semibold text-gray-900">Rp {{ number_format($row['unit_cost'], 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-right font-semibold text-blue-600">Rp {{ number_format($row['total_value'], 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-center hidden lg:table-cell">
                                @if(abs($diffPct) < 0.1)
                                <span class="text-xs text-gray-400">—</span>
                                @elseif($diff > 0)
                                <span class="text-xs text-amber-600">+{{ number_format($diffPct, 1) }}%</span>
                                @else
                                <span class="text-xs text-green-600">{{ number_format($diffPct, 1) }}%</span>
                                @endif
                            </td>
                        </tr>
                        @empty
                    ...
                    <tfoot class="bg-gray-50 font-semibold text-sm">
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right text-blue-600">Rp {{ number_format($report['total'], 0, ',', '.') }}</td>
