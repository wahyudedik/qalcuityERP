{{--
    Task 3: Launcher Overlay — Grid Modul + Search + Recently Visited
...
<div x-data id="launcher-overlay" x-show="$store.navSystem.launcherOpen" role="dialog" aria-modal="true"
    ...
    <div x-show="$store.navSystem.launcherOpen" x-transition:enter="transition ease-out duration-200"
        ...
        class="absolute bg-white shadow-2xl border border-gray-200 overflow-hidden
                   top-14 left-0 rounded-2xl w-[480px]
                   max-lg:w-full max-lg:right-0 max-lg:rounded-none max-lg:bottom-0 max-lg:overflow-y-auto"
        style="transform-origin: top left;" @click.stop>
        {{-- Header: Search + Close button (mobile) --}}
        ...
        <div class="p-4">
            {{-- Empty state --}}
            <p x-show="$store.navSystem.filteredModules.length === 0" class="text-sm text-gray-400 text-center py-6">
                    ...
                    <button role="listitem" :aria-pressed="module.key === $store.navSystem.activeModule"
                        ...
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
                            ...
                            <template x-if="module.icon === 'home'">
                                <svg class="w-6 h-6" :style="`color: var(--module-color-${module.key})`" fill="none"
                            ...
                            {{-- Fallback icon --}}
                            <template
                                x-if="!['home','sparkle','tag','cube','cog','currency','gear','building'].includes(module.icon)">
                                <svg class="w-6 h-6" :style="`color: var(--module-color-${module.key})`"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
                        ...
                        {{-- Module name --}}
                        <span class="text-xs font-medium text-gray-700 leading-tight" x-text="module.label"></span>
                    </button>
                </template>
            </div>
        </div>
        ...
        <div x-show="$store.navSystem.recentlyVisited.length > 0" class="border-t border-gray-100 px-4 py-3">
