    ...
    <div class="flex flex-col lg:flex-row gap-5">
        ...
        <div class="lg:w-72 shrink-0 space-y-4">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5 space-y-2 text-sm">
                <p class="font-semibold text-gray-900 dark:text-white">{{ $session->program->name }}</p>
                @if($session->program->category)
                <span class="px-2 py-0.5 rounded-full text-xs bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400">{{ $session->program->category }}</span>
                @endif
                <div class="space-y-1 text-xs text-gray-500 dark:text-slate-400 pt-1">
                    ...
                    @if($session->trainer)<p>👤 {{ $session->trainer }}</p>@endif
                    <p>👥 {{ $session->participants->count() }}{{ $session->max_participants > 0 ? '/'.$session->max_participants : '' }} peserta</p>
                </div>
                {{-- Update status --}}
                <form method="POST" action="{{ route('hrm.training.sessions.status', $session) }}" class="pt-2">
                    ...
                    <div class="flex gap-2">
                        <select name="status"
                            ...
                            <option value="{{ $v }}" @selected($session->status===$v)>{{ $l }}</option>
                            @endforeach
                        </select>
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
                ...
                <form method="POST" action="{{ route('hrm.training.sessions.participants.add', $session) }}" class="space-y-2">
                    @csrf
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                <td class="px-4 py-3">
                                    <p class="font-medium text-gray-900 dark:text-white">{{ $p->employee->name ?? '-' }}</p>
                                    <p class="text-xs text-gray-400 dark:text-slate-500">{{ $p->employee->department ?? $p->employee->position ?? '' }}</p>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <form method="POST" action="{{ route('hrm.training.participants.update', $p) }}" class="inline-flex items-center gap-1">
                                        @csrf @method('PATCH')
                                ...
                                <td class="px-4 py-3 text-center">
                                    <form method="POST" action="{{ route('hrm.training.participants.update', $p) }}" class="inline-flex items-center gap-1">
                                        ...
                                        <input type="number" name="score" value="{{ $p->score }}" min="0" max="100" placeholder="—"
                                ...
                                <td class="px-4 py-3 text-center">
                                    <form method="POST" action="{{ route('hrm.training.participants.remove', $p) }}">
                                        @csrf @method('DELETE')
