...
<?php $component->withAttributes([]); ?>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            ...
            <p class="text-xl font-bold text-blue-500 mt-1">Rp <?php echo e(number_format($summary['total_pay'], 0, ',', '.')); ?></p>
        </div>
    </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                <td class="px-4 py-3">
                                    <p class="font-medium text-gray-900 dark:text-white"><?php echo e($ot->employee->name ?? '-'); ?></p>
                                    <p class="text-xs text-gray-400 dark:text-slate-500"><?php echo e($ot->employee->department ?? $ot->employee->position ?? ''); ?></p>
                                </td>
                                <td class="px-4 py-3 text-gray-700 dark:text-slate-300 whitespace-nowrap">
                                    <?php echo e($ot->date->format('d M Y')); ?>

                                </td>
                                <td class="px-4 py-3 text-center text-gray-600 dark:text-slate-400 whitespace-nowrap text-xs">
                                    <?php echo e($ot->start_time); ?> – <?php echo e($ot->end_time); ?>

                                </td>
                                <td class="px-4 py-3 text-center">
                                    <span class="text-sm font-medium text-gray-900 dark:text-white"><?php echo e($ot->durationLabel()); ?></span>
                                </td>
                                <td class="px-4 py-3 text-right hidden sm:table-cell">
                                    <?php if($ot->status === 'approved'): ?>
                                    <span class="text-green-600 dark:text-green-400 font-medium">Rp <?php echo e(number_format($ot->overtime_pay, 0, ',', '.')); ?></span>
                                    <?php else: ?>
                                ...
                                <td class="px-4 py-3 text-center">
                                    ...
                                    <span class="px-2 py-0.5 rounded-full text-xs bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400">Menunggu</span>
                                    <?php elseif($ot->status === 'approved'): ?>
                                    <div>
                                        <span class="px-2 py-0.5 rounded-full text-xs bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400">Disetujui</span>
                                        <?php if($ot->included_in_payroll): ?>
                                        <p class="text-xs text-gray-400 dark:text-slate-500 mt-0.5">Payroll <?php echo e($ot->payroll_period); ?></p>
                                        <?php else: ?>
                                    ...
                                    <?php else: ?>
                                    <div>
                                        <span class="px-2 py-0.5 rounded-full text-xs bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400">Ditolak</span>
                                        <?php if($ot->rejection_reason): ?>
                                        <p class="text-xs text-gray-400 dark:text-slate-500 mt-0.5 max-w-[120px] truncate" title="<?php echo e($ot->rejection_reason); ?>"><?php echo e($ot->rejection_reason); ?></p>
                                        <?php endif; ?>
                                    </div>
                                    <?php endif; ?>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    ...
                                    <div class="flex items-center justify-center gap-1">
                                        <form method="POST" action="<?php echo e(route('hrm.overtime.approve', $ot)); ?>">
                                            ...
                                            <button type="submit" class="px-2.5 py-1 text-xs bg-green-600 text-white rounded-lg hover:bg-green-700">✓ Setuju</button>
                                        </form>
                                        <button onclick="openReject(<?php echo e($ot->id); ?>)"
                                            class="px-2.5 py-1 text-xs border border-red-500/30 text-red-400 rounded-lg hover:bg-red-500/10">✕ Tolak</button>
                                    </div>
                            ...
                            <tr class="bg-gray-50/50 dark:bg-white/[0.02]">
                                <td colspan="7" class="px-4 py-1.5 text-xs text-gray-400 dark:text-slate-500 italic">
    ...
    <div id="modal-reject" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 w-full max-w-sm shadow-2xl">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/10">
                ...
                <button onclick="document.getElementById('modal-reject').classList.add('hidden')" class="text-gray-400 hover:text-white">✕</button>
            </div>
            <form id="form-reject" method="POST" class="p-5 space-y-4">
                <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Alasan Penolakan (opsional)</label>
