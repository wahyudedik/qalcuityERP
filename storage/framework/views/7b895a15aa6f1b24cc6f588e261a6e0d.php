    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
                ...
                <p class="text-lg font-bold <?php echo e($kpi['color']); ?> mt-1"><?php echo e($kpi['value']); ?></p>
            </div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <?php $exp = $report['expenses'][$tid]['amount'] ?? 0; $profit = $rev['amount'] - $exp; ?>
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3 font-medium text-gray-900 dark:text-white"><?php echo e($rev['name']); ?></td>
                        <td class="px-4 py-3 text-right text-green-500">Rp <?php echo e(number_format($rev['amount'], 0, ',', '.')); ?></td>
                        <td class="px-4 py-3 text-right text-red-500">Rp <?php echo e(number_format($exp, 0, ',', '.')); ?></td>
                        <td class="px-4 py-3 text-right font-medium <?php echo e($profit >= 0 ? 'text-blue-500' : 'text-red-500'); ?>">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
                ...
                <div class="space-y-2 mb-4">
                    ...
                    <div class="flex items-center justify-between py-2 border-b border-gray-100 dark:border-white/5">
                        <div>
                            ...
                            <p class="text-xs text-gray-400 dark:text-slate-500"><?php echo e(ucfirst($member->pivot->role)); ?> · ID #<?php echo e($member->id); ?></p>
                        </div>
                ...
                <form method="POST" action="<?php echo e(route('company-groups.members.add', $companyGroup)); ?>" class="flex gap-2">
                    <?php echo csrf_field(); ?>
                ...
                <form method="POST" action="<?php echo e(route('company-groups.transactions.store', $companyGroup)); ?>" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        ...
                        <div>
                            ...
                            <input type="date" name="date" value="<?php echo e(today()->toDateString()); ?>" required
                                class="w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-sm text-gray-900 dark:text-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            <td class="px-4 py-3 text-xs text-gray-500 dark:text-slate-400 whitespace-nowrap"><?php echo e($tx->date->format('d M Y')); ?></td>
                            <td class="px-4 py-3">
                                <span class="text-gray-900 dark:text-white font-medium"><?php echo e($tx->fromTenant?->name ?? '#'.$tx->from_tenant_id); ?></span>
                                <span class="text-gray-400 mx-1">→</span>
                                <span class="text-gray-900 dark:text-white font-medium"><?php echo e($tx->toTenant?->name ?? '#'.$tx->to_tenant_id); ?></span>
                            </td>
                            <td class="px-4 py-3 hidden sm:table-cell text-xs text-gray-600 dark:text-slate-400"><?php echo e($typeLabels[$tx->type] ?? $tx->type); ?></td>
                            <td class="px-4 py-3 hidden md:table-cell text-xs text-gray-500 dark:text-slate-400 max-w-xs truncate"><?php echo e($tx->description ?? '-'); ?></td>
                            <td class="px-4 py-3 text-right font-medium text-gray-900 dark:text-white">Rp <?php echo e(number_format($tx->amount, 0, ',', '.')); ?></td>
                            <td class="px-4 py-3 text-center">
                                <span class="px-2 py-0.5 rounded-full text-xs <?php echo e($tx->status === 'posted' ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' : ($tx->status === 'voided' ? 'bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-slate-500' : 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400')); ?>">
                            ...
                            <td class="px-4 py-3 text-center">
                                ...
                                <div class="flex items-center justify-center gap-1">
                                    <form method="POST" action="<?php echo e(route('company-groups.transactions.post', $tx)); ?>">
                                        <?php echo csrf_field(); ?>
                                        <button class="text-xs px-2 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700">Post</button>
                                    </form>
                                    <form method="POST" action="<?php echo e(route('company-groups.transactions.void', $tx)); ?>" onsubmit="return confirm('Void transaksi ini?')">
                                        <?php echo csrf_field(); ?>
