...
class SmartQueryTools
{
    ...
    private function overdueCustomers(int $days, int $limit): array
    {
        ...
        if ($rows->isEmpty()) return ['status' => 'success', 'message' => "Tidak ada customer dengan piutang jatuh tempo lebih dari {$days} hari.", 'data' => []];

        return ['status' => 'success', 'data' => $rows->map(fn($i) => [
    ...
    {
        $soldIds = SalesOrderItem::whereHas('salesOrder', fn($q) =>
        ...
        if ($rows->isEmpty()) return ['status' => 'success', 'message' => "Semua produk aktif terjual dalam {$days} hari terakhir.", 'data' => []];

        return ['status' => 'success', 'data' => $rows->map(fn($p) => [
            'produk' => $p->name,
    ...
    private function absentEmployees(int $days, int $limit): array
    {
        ...
        if ($rows->isEmpty()) return ['status' => 'success', 'message' => "Tidak ada data absensi dalam {$days} hari terakhir.", 'data' => []];

        return ['status' => 'success', 'data' => $rows->map(fn($r) => [
    ...
    private function topCustomers(int $days, int $limit): array
    {
        ...
        if ($rows->isEmpty()) return ['status' => 'success', 'message' => 'Belum ada data penjualan.', 'data' => []];
    ...
    private function topSuppliers(int $days, int $limit): array
    {
        ...
        if ($rows->isEmpty()) return ['status' => 'success', 'message' => 'Belum ada data pembelian.', 'data' => []];
    ...
    private function lowStockAlert(int $limit): array
    {
        ...
        if ($rows->isEmpty()) return ['status' => 'success', 'message' => 'Semua stok dalam kondisi aman.', 'data' => []];

        return ['status' => 'success', 'data' => $rows->map(fn($s) => [
    ...
    private function inactiveCustomers(int $days, int $limit): array
    {
        ...
        if ($rows->isEmpty()) return ['status' => 'success', 'message' => "Semua customer aktif dalam {$days} hari terakhir.", 'data' => []];

        return ['status' => 'success', 'data' => $rows->map(fn($c) => [
            'customer'       => $c->name,
            'telepon'        => $c->phone ?? '-',
    ...
    private function overduePayables(int $limit): array
    {
        ...
        if ($rows->isEmpty()) return ['status' => 'success', 'message' => 'Tidak ada hutang yang jatuh tempo.', 'data' => []];
    ...
    {
        $rows = SalesOrderItem::whereHas('salesOrder', fn($q) =>
            $q->where('tenant_id', $this->tenantId)
              ->whereNotIn('status', ['cancelled'])
              ->where('date', '>=', now()->subDays($days))
