<x-app-layout>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border {{ $profit >= 0 ? 'border-blue-200' : 'border-red-200' }} p-4">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 font-medium text-gray-900">{{ $rev['name'] }}</td>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            ...
            <div class="space-y-3">
                ...
                <div class="p-3 rounded-xl bg-gray-50">
                    <p class="text-xs font-medium text-gray-700">{{ $cf['name'] }}</p>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-gray-100">
            ...
            <div class="p-5">
                <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">{{ $data['label'] }}</p>
                <p class="text-xl font-black text-gray-900 mb-3">Rp {{ number_format($data['total'], 0, ',', '.') }}</p>
                <div class="space-y-1.5">
                    @foreach($data['per_member'] as $m)
                    <div class="flex justify-between text-xs">
                        <span class="text-gray-500">{{ $m['name'] }}</span>
                        <span class="font-mono text-gray-700">Rp {{ number_format($m['amount'], 0, ',', '.') }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-5 mb-6">
        ...
        <div class="space-y-2">
            ...
            <div class="flex items-center gap-3">
                ...
                <span class="text-xs font-mono text-gray-600 w-28 text-right">Rp {{ number_format($t['revenue'], 0, ',', '.') }}</span>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-right font-mono font-medium text-gray-900">Rp {{ number_format($tx->amount, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-xs {{ $tx->status === 'posted' ? 'bg-green-100 text-green-700' : ($tx->status === 'voided' ? 'bg-gray-100 text-gray-500' : 'bg-amber-100 text-amber-700') }}">
                                {{ ucfirst($tx->status) }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-center">
