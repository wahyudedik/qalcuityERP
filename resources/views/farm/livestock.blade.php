<x-app-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <a href="{{ route('farm.livestock.show', $herd) }}" class="block bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden hover:border-emerald-300 dark:hover:border-emerald-500/30 transition">
            <div class="px-5 py-4">
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center gap-2">
                        <span class="text-lg">{{ explode(' ', \App\Models\LivestockHerd::ANIMAL_TYPES[$herd->animal_type] ?? '🐾')[0] }}</span>
                        <span class="font-bold text-gray-900 dark:text-white">{{ $herd->code }}</span>
                        <span class="text-xs px-2 py-0.5 rounded-full {{ $herd->status === 'active' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' : 'bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-slate-400' }}">{{ ucfirst($herd->status) }}</span>
                    </div>
                    <span class="text-2xl font-black text-gray-900 dark:text-white">{{ number_format($herd->current_count) }}</span>
                </div>
                <p class="text-sm text-gray-500 dark:text-slate-400">{{ $herd->name }}</p>
                <div class="grid grid-cols-3 gap-2 mt-3 text-xs">
                    <div><span class="text-gray-400">Awal:</span> <span class="text-gray-700 dark:text-slate-300">{{ $herd->initial_count }}</span></div>
                    <div><span class="text-gray-400">Mati:</span> <span class="text-red-500">{{ abs($herd->mortalityCount()) }} ({{ $herd->mortalityRate() }}%)</span></div>
                    <div><span class="text-gray-400">Umur:</span> <span class="text-gray-700 dark:text-slate-300">{{ $herd->ageDays() ?? '-' }} hari</span></div>
                </div>
                @if($herd->plot)
