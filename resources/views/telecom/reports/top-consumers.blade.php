    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">{{ __('Konsumen Teratas') }}</h1>
                    <p class="mt-1 text-sm text-gray-600">{{ __('Identifikasi konsumen bandwidth tertinggi') }}</p>
                </div>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-4 mb-6">
                <form method="GET" action="{{ route('telecom.reports.top-consumers') }}" class="flex flex-wrap gap-4 items-end">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Metrik') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Jumlah') }}</label>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h2 class="text-lg font-semibold text-gray-900">{{ __('Peringkat Konsumen Bandwidth') }}</h2>
                </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($report['consumers'] ?? [] as $index => $consumer)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $consumer['customer_name'] ?? '-' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $consumer['package_name'] ?? '-' }}</td>
                                    <td class="px-6 py-4 text-sm text-blue-600">{{ number_format(($consumer['total_download'] ?? 0) / 1073741824, 2) }} GB</td>
                                    <td class="px-6 py-4 text-sm text-green-600">{{ number_format(($consumer['total_upload'] ?? 0) / 1073741824, 2) }} GB</td>
                                    <td class="px-6 py-4 text-sm font-bold text-gray-900">{{ number_format((($consumer['total_download'] ?? 0) + ($consumer['total_upload'] ?? 0)) / 1073741824, 2) }} GB</td>
                                </tr>
                            @empty
                                <tr>
