...
<?php $component->withAttributes([]); ?>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 mb-4">
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4">
            ...
            <div class="flex gap-2">
                <a href="<?php echo e(route('inventory.index')); ?>" class="px-3 py-2 text-sm border border-gray-200 dark:border-white/10 rounded-xl text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-white/5">Inventori</a>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <?php $totalStk = $product->productStocks->sum('quantity'); $isLow = $totalStk <= $product->stock_min; ?>
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                <?php if($product->image): ?>
                                <img src="<?php echo e($product->image); ?>" alt="<?php echo e($product->name); ?>" class="w-9 h-9 rounded-xl object-cover shrink-0 border border-gray-200 dark:border-white/10">
                                <?php else: ?>
                                <div class="w-9 h-9 rounded-xl bg-gray-100 dark:bg-white/5 flex items-center justify-center shrink-0">
                        ...
                        <td class="px-4 py-3 text-right text-gray-500 dark:text-slate-400">Rp <?php echo e(number_format($product->price_buy,0,',','.')); ?></td>
                        <td class="px-4 py-3 text-right hidden lg:table-cell">
                            <span class="font-semibold <?php echo e($isLow ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'); ?>"><?php echo e($totalStk); ?></span>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button onclick="openEditProduct(<?php echo e($product->id); ?>, <?php echo \Illuminate\Support\Js::from($product->name)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($product->sku ?? '')->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($product->category ?? '')->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($product->unit)->toHtml() ?>, <?php echo e($product->price_sell); ?>, <?php echo e($product->price_buy); ?>, <?php echo e($product->stock_min); ?>, <?php echo e($product->is_active ? 'true' : 'false'); ?>, <?php echo \Illuminate\Support\Js::from($product->image ?? '')->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($product->description ?? '')->toHtml() ?>)"
                                ...
                                <form method="POST" action="<?php echo e(route('products.toggle', $product)); ?>">
                                    ...
                                    <button type="submit" class="p-1.5 rounded-lg <?php echo e($product->is_active ? 'text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-500/10' : 'text-green-500 hover:bg-green-50 dark:hover:bg-green-500/10'); ?>" title="<?php echo e($product->is_active ? 'Nonaktifkan' : 'Aktifkan'); ?>">
                                ...
                                <form method="POST" action="<?php echo e(route('products.destroy', $product)); ?>" onsubmit="return confirm('Hapus produk ini?')">
                    ...
                    <tr><td colspan="8" class="px-4 py-12 text-center text-gray-400 dark:text-slate-500">Belum ada produk. Klik "+ Produk" untuk menambahkan.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div id="modal-add-product" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" action="<?php echo e(route('products.store')); ?>" enctype="multipart/form-data" class="p-6 space-y-4">
                <?php echo csrf_field(); ?>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="category" list="cat-list-add" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <datalist id="cat-list-add"><?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($c); ?>"><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></datalist>
                    </div>
                    <div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form id="form-edit-product" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">SKU</label>
                        <input type="text" id="edit-sku" name="sku" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Kategori</label>
                        <input type="text" id="edit-category" name="category" list="cat-list-edit" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <datalist id="cat-list-edit"><?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($c); ?>"><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></datalist>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Satuan *</label>
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Deskripsi</label>
                        <textarea id="edit-description" name="description" rows="2" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"></textarea>
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-2">
    ...
    function openEditProduct(id, name, sku, category, unit, priceSell, priceBuy, stockMin, isActive, image, description) {
