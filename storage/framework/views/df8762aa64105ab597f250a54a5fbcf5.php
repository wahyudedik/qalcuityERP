        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    ...
                    ?>
                    <span class="mt-2 inline-block px-3 py-1 rounded-full text-xs <?php echo e($statusColors[$salesOrder->status] ?? ''); ?>">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <?php endif; ?>
                    <?php if(!$salesOrder->invoice()->where('status', '!=', 'cancelled')->exists()): ?>
                    ...
                    <?php else: ?>
                        <?php $inv = $salesOrder->invoice()->where('status', '!=', 'cancelled')->first(); ?>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-100 dark:border-white/10">
                <div>
                    ...
                    <p class="text-sm font-medium text-gray-900 dark:text-white mt-0.5"><?php echo e($salesOrder->customer->name ?? '-'); ?></p>
                </div>
                <div>
                    <p class="text-xs text-gray-500 dark:text-slate-400">Tanggal</p>
                    <p class="text-sm font-medium text-gray-900 dark:text-white mt-0.5"><?php echo e($salesOrder->date->format('d/m/Y')); ?></p>
                </div>
                <div>
                    <p class="text-xs text-gray-500 dark:text-slate-400">Pengiriman</p>
                    <p class="text-sm font-medium text-gray-900 dark:text-white mt-0.5"><?php echo e($salesOrder->delivery_date?->format('d/m/Y') ?? '-'); ?></p>
                </div>
                <div>
                    ...
                    <p class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">
                        ...
                        <?php if($salesOrder->due_date): ?> — <?php echo e($salesOrder->due_date->format('d/m/Y')); ?> <?php endif; ?>
                    </p>
                </div>
                ...
                <?php if($salesOrder->currency_code && $salesOrder->currency_code !== 'IDR'): ?>
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">
                            ...
                            <span class="text-xs text-gray-400 dark:text-slate-500">(Kurs: Rp <?php echo e(number_format($salesOrder->currency_rate, 0, ',', '.')); ?>)</span>
                        </p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-slate-400">Ekuivalen IDR</p>
                        <p class="text-sm font-medium text-green-600 dark:text-green-400 mt-0.5">Rp <?php echo e(number_format($salesOrder->total * $salesOrder->currency_rate, 0, ',', '.')); ?></p>
                    </div>
                <?php endif; ?>
                <?php if($salesOrder->shipping_address): ?>
                    <div class="col-span-2">
                        <p class="text-xs text-gray-500 dark:text-slate-400">Alamat Pengiriman</p>
                        <p class="text-sm text-gray-700 dark:text-slate-300 mt-0.5"><?php echo e($salesOrder->shipping_address); ?></p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                    <?php $__currentLoopData = $salesOrder->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr>
                            <td class="px-4 py-3 text-gray-700 dark:text-slate-300"><?php echo e($item->product->name ?? '-'); ?></td>
                            <td class="px-4 py-3 text-right"><?php echo e($item->quantity); ?> <?php echo e($item->product->unit ?? ''); ?></td>
                            <td class="px-4 py-3 text-right">Rp <?php echo e(number_format($item->price, 0, ',', '.')); ?></td>
                            <td class="px-4 py-3 text-right text-red-400"><?php echo e($item->discount > 0 ? '-Rp ' . number_format($item->discount, 0, ',', '.') : '-'); ?></td>
                ...
                <tfoot class="border-t border-gray-100 dark:border-white/10 text-sm">
                    <tr>
                        ...
                        <td class="px-4 py-2 text-right font-medium text-gray-900 dark:text-white">Rp <?php echo e(number_format($salesOrder->subtotal, 0, ',', '.')); ?></td>
                    </tr>
                    <?php if($salesOrder->discount > 0): ?>
                        <tr>
                            <td colspan="4" class="px-4 py-2 text-right text-red-400">Diskon</td>
                            <td class="px-4 py-2 text-right text-red-400">-Rp <?php echo e(number_format($salesOrder->discount, 0, ',', '.')); ?></td>
                        </tr>
                    <?php endif; ?>
                    <?php if($salesOrder->tax_amount > 0): ?>
                        <tr>
                            <td colspan="4" class="px-4 py-2 text-right text-gray-500 dark:text-slate-400">Pajak</td>
                            <td class="px-4 py-2 text-right text-gray-700 dark:text-slate-300">Rp <?php echo e(number_format($salesOrder->tax_amount, 0, ',', '.')); ?></td>
                        </tr>
                    <?php endif; ?>
                    <tr class="font-bold">
                        <td colspan="4" class="px-4 py-3 text-right text-gray-900 dark:text-white">Total</td>
                        <td class="px-4 py-3 text-right text-blue-400 text-base">Rp <?php echo e(number_format($salesOrder->total, 0, ',', '.')); ?></td>
                    </tr>
                </tfoot>
            </table>
        </div>
            ...
            <div class="flex flex-wrap gap-4">
                ...
                <div class="flex items-center gap-3 bg-gray-50 dark:bg-white/5 rounded-xl p-3 border border-gray-200 dark:border-white/10">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400 dark:text-slate-500"><?php echo e($sig->signed_at?->format('d M Y H:i')); ?></p>
                    </div>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
