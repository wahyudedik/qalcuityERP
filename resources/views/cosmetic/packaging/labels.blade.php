...
@section('title', 'Label Versions')

@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Header -->
        <div class="mb-6">
            <a href="{{ route('cosmetic.packaging.index') }}" class="text-blue-600 hover:text-blue-900 mb-2 inline-block">
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $stats['total_labels'] }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-green-600">{{ $stats['active_labels'] }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-orange-600">{{ $stats['in_review'] }}</p>
                    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($labels as $label)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $label->version_number }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @if ($compliance['total'] > 0)
                                    <div class="text-sm">
                                        <span
                                            class="{{ $compliance['percentage'] == 100 ? 'text-green-600' : 'text-orange-600' }} font-medium">
                                            {{ $compliance['percentage'] }}%
                                        </span>
                                        <span
                                            class="text-gray-500">({{ $compliance['compliant'] }}/{{ $compliance['total'] }})</span>
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex gap-2">
                                    <a href="{{ route('cosmetic.packaging.labels.show', $label->id) }}"
                                        class="text-blue-600 hover:text-blue-900">
                                        View
                                    </a>
                                    @if ($label->status == 'draft')
                                        <form method="POST"
                                            action="{{ route('cosmetic.packaging.labels.submit', $label->id) }}"
                                            class="inline">
                                            @csrf
                                            <button type="submit"
                                    ...
                                    @if ($label->status == 'approved')
                                        <form method="POST"
                                            action="{{ route('cosmetic.packaging.labels.activate', $label->id) }}"
                                            class="inline">
                                            @csrf
                                            <button type="submit"
    ...
    <div id="add-label-modal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-full max-w-3xl shadow-lg rounded-lg bg-white">
            ...
            <form method="POST" action="{{ route('cosmetic.packaging.labels.store') }}" class="space-y-4">
                @csrf
