...
<?php $component->withAttributes([]); ?>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-6">
        <div class="bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-white/10 p-4">
            ...
            <p class="text-xl font-bold text-gray-900 dark:text-white"><?php echo e($stats['total']); ?></p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-white/10 p-4">
    ...
    <div class="flex items-center justify-between mb-4">
        <form class="flex items-center gap-2">
            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari lahan..." class="px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0f172a] text-gray-900 dark:text-white w-48">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <a href="<?php echo e(route('farm.plots.show', $plot)); ?>" class="block bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden hover:border-emerald-300 dark:hover:border-emerald-500/30 transition group">
            <div class="px-5 py-4 flex items-start justify-between">
                <div>
                    <div class="flex items-center gap-2">
                        <span class="text-lg font-bold text-gray-900 dark:text-white"><?php echo e($plot->code); ?></span>
                        <span class="text-xs px-2 py-0.5 rounded-full bg-<?php echo e($sc); ?>-100 text-<?php echo e($sc); ?>-700 dark:bg-<?php echo e($sc); ?>-500/20 dark:text-<?php echo e($sc); ?>-400"><?php echo e($plot->statusLabel()); ?></span>
                    </div>
                    <p class="text-sm text-gray-500 dark:text-slate-400 mt-0.5"><?php echo e($plot->name); ?></p>
                </div>
                <span class="text-xs text-gray-400 dark:text-slate-500"><?php echo e(number_format($plot->area_size, 1)); ?> <?php echo e($plot->area_unit); ?></span>
            </div>
            <div class="px-5 pb-4 grid grid-cols-2 gap-2 text-xs">
                <?php if($plot->current_crop): ?>
                <div><span class="text-gray-400">Tanaman:</span> <span class="text-gray-700 dark:text-slate-300"><?php echo e($plot->current_crop); ?></span></div>
                <?php endif; ?>
                <?php if($plot->planted_at): ?>
                <div><span class="text-gray-400">Tanam:</span> <span class="text-gray-700 dark:text-slate-300"><?php echo e($plot->planted_at->format('d M Y')); ?></span></div>
                <?php endif; ?>
                <?php if($plot->expected_harvest): ?>
                <div>
                    <span class="text-gray-400">Panen:</span>
                    <span class="<?php echo e($plot->isHarvestOverdue() ? 'text-red-500 font-medium' : 'text-gray-700 dark:text-slate-300'); ?>">
                ...
                <div><span class="text-gray-400">Aktivitas:</span> <span class="text-gray-700 dark:text-slate-300"><?php echo e($plot->activities_count); ?></span></div>
                <?php endif; ?>
            </div>
        </a>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
