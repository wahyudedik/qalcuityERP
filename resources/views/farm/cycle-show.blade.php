<x-app-layout>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-5 mb-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                <span class="text-lg font-bold text-gray-900">{{ $cropCycle->crop_name }}</span>
                @if($cropCycle->crop_variety) <span class="text-sm text-gray-400 ml-2">var. {{ $cropCycle->crop_variety }}</span> @endif
                @if($cropCycle->season) <span class="text-xs ml-2 px-2 py-0.5 rounded bg-blue-100 text-blue-600">{{ $cropCycle->season }}</span> @endif
            </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div class="bg-white rounded-xl border border-gray-200 p-4">
                    ...
                    <p class="text-lg font-bold text-gray-900">{{ $cropCycle->durationDays() ?? 0 }} hari</p>
                </div>
                <div class="bg-white rounded-xl border border-gray-200 p-4">
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-4">
                    ...
                    <p class="text-lg font-bold {{ $cropCycle->budgetUsedPercent() > 100 ? 'text-red-500' : 'text-gray-900' }}">Rp {{ number_format($cropCycle->actual_cost, 0, ',', '.') }}</p>
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-4">
                    <p class="text-xs text-gray-500">HPP / {{ $cropCycle->target_yield_unit }}</p>
                    <p class="text-lg font-bold text-gray-900">{{ $cropCycle->costPerUnit() ? 'Rp '.number_format($cropCycle->costPerUnit(), 0, ',', '.') : '-' }}</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                    <h3 class="font-semibold text-gray-900">Aktivitas ({{ $cropCycle->activities->count() }})</h3>
                    @if(!in_array($cropCycle->phase, ['completed', 'cancelled']))
                ...
                <div class="divide-y divide-gray-100">
                    ...
                    <div class="px-5 py-3 flex items-start gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm text-gray-900">{{ $act->description }}</p>
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between"><span class="text-gray-500">Renc. Tanam</span><span>{{ $cropCycle->plan_plant_date->format('d M Y') }}</span></div>
                    @endif
