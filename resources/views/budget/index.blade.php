    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
        ...
        <div class="flex gap-3 flex-wrap">
            <div class="bg-white rounded-xl px-4 py-2 border border-gray-200 text-sm">
                ...
                <span class="font-semibold text-gray-900">Rp {{ number_format($totalBudget,0,',','.') }}</span>
            </div>
            <div class="bg-white rounded-xl px-4 py-2 border border-gray-200 text-sm">
                <span class="text-gray-500">Realisasi: </span>
                <span class="font-semibold {{ $totalRealized > $totalBudget ? 'text-red-500' : 'text-green-600' }}">Rp {{ number_format($totalRealized,0,',','.') }}</span>
            </div>
            @if($overCount > 0)
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            ...
                            @if($budget->category)<p class="text-xs text-gray-500">{{ $budget->category }}</p>@endif
                        </td>
                        <td class="px-4 py-3 hidden sm:table-cell text-gray-500">{{ $budget->department ?? '-' }}</td>
                        <td class="px-4 py-3 text-right font-medium text-gray-900">Rp {{ number_format($budget->amount,0,',','.') }}</td>
                        <td class="px-4 py-3 text-right font-medium {{ $over ? 'text-red-500' : 'text-gray-900' }}">Rp {{ number_format($budget->realized,0,',','.') }}</td>
                        <td class="px-4 py-3 text-right hidden md:table-cell {{ $over ? 'text-red-500' : 'text-gray-500' }}">
                    ...
                    @empty
                    <tr><td colspan="8" class="px-4 py-12 text-center text-gray-400">Belum ada anggaran untuk periode {{ $period }}.</td></tr>
                    @endforelse
                </tbody>
                @if($budgets->count() > 0)
                <tfoot class="bg-gray-50 text-sm font-semibold">
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right text-gray-900">Rp {{ number_format($totalBudget,0,',','.') }}</td>
                        <td class="px-4 py-3 text-right {{ $totalRealized > $totalBudget ? 'text-red-500' : 'text-gray-900' }}">Rp {{ number_format($totalRealized,0,',','.') }}</td>
                        <td class="px-4 py-3 text-right hidden md:table-cell {{ $totalRealized > $totalBudget ? 'text-red-500' : 'text-gray-500' }}">
            ...
            <form method="POST" action="{{ route('budget.store') }}" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="month" name="period" value="{{ $period }}" required class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
