<x-app-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                    <div>
                        ...
                        @if($invoice->customer?->company)<p class="text-gray-500 text-xs">{{ $invoice->customer->company }}</p>@endif
                        @if($invoice->customer?->email)<p class="text-gray-500 text-xs">{{ $invoice->customer->email }}</p>@endif
                        @if($invoice->customer?->phone)<p class="text-gray-500 text-xs">{{ $invoice->customer->phone }}</p>@endif
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="font-medium text-green-600">Rp {{ number_format($invoice->total_amount * $invoice->currency_rate, 0, ',', '.') }}</p>
                    </div>
                    @endif
                </div>
            </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3 text-center text-gray-600 hidden sm:table-cell">{{ $item->quantity }}</td>
                                <td class="px-4 py-3 text-right text-gray-600 hidden sm:table-cell">Rp {{ number_format($item->price, 0, ',', '.') }}</td>
        ...
        <div class="space-y-5">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 space-y-3">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between border-t border-gray-100 pt-2">
                        ...
                        <span class="font-bold text-lg {{ $invoice->remaining_amount > 0 ? 'text-red-600' : 'text-green-600' }}">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-4">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-900">{{ $sig->user?->name }}</p>
                            <p class="text-xs text-gray-400">{{ $sig->signed_at?->format('d M Y H:i') }} · {{ $sig->ip_address }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center justify-between p-3 rounded-xl bg-green-50 text-sm">
                        <div>
                            <p class="font-medium text-green-700">Rp {{ number_format($pay->amount, 0, ',', '.') }}</p>
                            <p class="text-xs text-gray-500">{{ $pay->payment_date?->format('d M Y') }} · {{ strtoupper($pay->payment_method ?? '-') }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            <a href="{{ route('invoices.index') }}" class="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 transition">
