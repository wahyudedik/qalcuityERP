<x-app-layout>
            ...
            <div
                ...
                <div class="flex gap-2 text-xs">
                    <span
                        ...
                        matched</span>
                    <span
                        class="px-2 py-1 bg-amber-500/20 text-amber-400 rounded-full">{{ $statements->where('status', 'unmatched')->count() }}
                        unmatched</span>
                </div>
            </div>
                        ...
                        @forelse($statements as $stmt)
                            <tr id="row-{{ $stmt->id }}"
                                ...
                                <td class="px-4 py-3 hidden sm:table-cell">
                                    <span
                                        class="px-2 py-0.5 rounded-full text-xs font-medium {{ $stmt->type === 'credit' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400' }}">
                                        {{ $stmt->type === 'credit' ? 'Kredit' : 'Debit' }}
                                    </span>
                                </td>
                                <td
                                    class="px-4 py-3 text-right font-medium {{ $stmt->type === 'credit' ? 'text-green-400' : 'text-red-400' }}">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <span id="status-{{ $stmt->id }}"
                                        ...
                                        @if ($stmt->status === 'matched')
                                            Matched
                                        @elseif($stmt->status === 'journalized')
                                            Journalized
                                        @else
                                            Unmatched
                                        @endif
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex flex-col items-center gap-1">
                                        @if ($stmt->status === 'unmatched' || $stmt->status === 'matched')
                                            ...
                                            <button
                                                onclick="openManualMatch({{ $stmt->id }}, '{{ addslashes($stmt->description) }}', {{ $stmt->amount }})"
                ...
                <div class="space-y-2 max-h-60 overflow-y-auto" id="manual-erp-list">
                    @forelse($unmatchedErp ?? collect() as $erp)
                        <button onclick="applyManualMatch({{ $erp['id'] }})"
                            ...
                            <span
                                class="text-sm font-medium shrink-0 ml-3 {{ $erp['type'] === 'debit' ? 'text-green-500' : 'text-red-500' }}">
    ...
    @push('scripts')
        ...
        {{-- Journal Generation & Bulk Actions --}}
        <script>
            ...
            async function openJournalPreview(stmtId) {
                ...
                try {
                    const res = await fetch(`{{ url('bank/ai/preview-journal') }}/${stmtId}`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
            ...
            async function approveAndPostJournal() {
                ...
                try {
                    const res = await fetch(`{{ url('bank/ai/approve-and-post') }}/${currentJournalStmtId}`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
            ...
            async function generateSelectedJournals() {
                ...
                for (let i = 0; i < statementIds.length; i++) {
                    try {
                        const res = await fetch(`{{ url('bank/ai/generate-journal') }}/${statementIds[i]}`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': csrfToken,
            ...
            async function approveSelectedJournals() {
                ...
                try {
                    const res = await fetch(`{{ url('bank/ai/approve-and-post/bulk') }}`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
            ...
            async function autoGenerateAllJournals(autoPost = false) {
                ...
                try {
                    const res = await fetch(`{{ url('bank/ai/auto-generate-all') }}`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
            ...
            function startProgressPolling(jobId) {
                ...
                progressPolling = setInterval(async () => {
                    try {
                        const res = await fetch(`{{ url('bank/ai/job-progress') }}/${jobId}`);
                        const data = await res.json();
            ...
            async function viewJobResults() {
                ...
                try {
                    const res = await fetch(`{{ url('bank/ai/job-results') }}/${currentJobId}`);
                    const data = await res.json();
