...
<?php $component->withAttributes([]); ?>
        ...
        <div class="lg:w-72 shrink-0 space-y-4">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
                ...
                <form method="POST" action="<?php echo e(route('hrm.disciplinary.store')); ?>" id="form-sp" class="space-y-3">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Berlaku Hingga</label>
                        <input type="date" name="valid_until"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Jenis Pelanggaran *</label>
                        <input type="text" name="violation_type" id="sp-vtype" required
                            placeholder="cth: Pelanggaran Kehadiran"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Uraian Pelanggaran *</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Tindakan Perbaikan *</label>
                        <textarea name="corrective_action" id="sp-corrective" required rows="2"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"></textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Konsekuensi</label>
                    ...
                    <button type="submit" class="w-full py-2 text-sm bg-red-600 text-white rounded-xl hover:bg-red-700">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-red-500/20 p-4">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="px-1.5 py-0.5 rounded text-xs font-bold <?php echo e($sp->levelColor()); ?>"><?php echo e($sp->levelLabel()); ?></span>
                        <span class="text-xs text-gray-700 dark:text-slate-300 truncate"><?php echo e($sp->employee->name ?? '-'); ?></span>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                <td class="px-4 py-3">
                                    <p class="font-medium text-gray-900 dark:text-white"><?php echo e($sp->employee->name ?? '-'); ?></p>
                                    <p class="text-xs text-gray-400 dark:text-slate-500"><?php echo e($sp->letter_number); ?></p>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <span class="px-2 py-0.5 rounded-full text-xs font-semibold <?php echo e($sp->levelColor()); ?>"><?php echo e($sp->levelLabel()); ?></span>
                                    <?php if($sp->source === 'ai_anomaly'): ?>
                                    <p class="text-xs text-purple-400 mt-0.5">AI</p>
                                    <?php endif; ?>
                                </td>
                                <td class="px-4 py-3 hidden sm:table-cell">
                                    <p class="text-xs font-medium text-gray-700 dark:text-slate-300"><?php echo e($sp->violation_type); ?></p>
                                    <p class="text-xs text-gray-400 dark:text-slate-500 truncate max-w-[200px]"><?php echo e($sp->violation_description); ?></p>
                                </td>
                                <td class="px-4 py-3 hidden md:table-cell text-center text-xs text-gray-500 dark:text-slate-400">
                                    <?php echo e($sp->issued_date->format('d M Y')); ?>

                                    <?php if($sp->valid_until): ?>
                                    <p class="text-xs <?php echo e($sp->valid_until->isPast() ? 'text-red-400' : 'text-gray-400 dark:text-slate-500'); ?>">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <form method="POST" action="<?php echo e(route('hrm.disciplinary.acknowledge', $sp)); ?>">
                                            <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                            <button type="submit" class="px-2.5 py-1 text-xs bg-green-600/80 text-white rounded-lg hover:bg-green-600">✓ Konfirmasi</button>
                                        </form>
                                        <?php endif; ?>
                                    </div>
                                </td>
                            ...
                            <tr><td colspan="6" class="px-4 py-12 text-center text-gray-400 dark:text-slate-500">Tidak ada surat peringatan.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
    ...
    <div id="modal-ai-draft" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 w-full max-w-lg shadow-2xl">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/10">
                ...
                <button onclick="document.getElementById('modal-ai-draft').classList.add('hidden')" class="text-gray-400 hover:text-white">✕</button>
            </div>
            <div class="p-5 space-y-4">
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Pilih Karyawan</label>
                ...
                <div id="ai-draft-result" class="hidden space-y-3">
                    <div class="p-3 rounded-xl bg-purple-500/10 border border-purple-500/20 text-xs text-purple-300 space-y-1">
    ...
    async function loadAiDraft() {
        ...
        try {
            ...
            const draftRes = await fetch(AI_DRAFT_URL, {
                ...
                body: JSON.stringify({ employee_id: empId, anomalies: empAnomalies[0].anomalies }),
            });
