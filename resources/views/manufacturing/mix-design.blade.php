    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden">
            ...
            <div class="px-5 py-4 border-b border-gray-100 dark:border-white/10 flex items-center justify-between">
                ...
                <span class="text-xs text-gray-400 dark:text-slate-500">{{ $d->target_strength }} {{ $d->strength_unit === 'K' ? 'kg/cm²' : 'MPa' }}</span>
            </div>
            ...
            <div class="px-5 py-3 space-y-2">
                ...
                <div class="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                    <div class="flex justify-between"><span class="text-gray-500 dark:text-slate-400">Semen</span><span class="font-mono text-gray-700 dark:text-slate-300">{{ number_format($d->cement_kg, 0) }} kg</span></div>
                    <div class="flex justify-between"><span class="text-gray-500 dark:text-slate-400">Air</span><span class="font-mono text-gray-700 dark:text-slate-300">{{ number_format($d->water_liter, 0) }} L</span></div>
                    <div class="flex justify-between"><span class="text-gray-500 dark:text-slate-400">Pasir</span><span class="font-mono text-gray-700 dark:text-slate-300">{{ number_format($d->fine_agg_kg, 0) }} kg</span></div>
                    <div class="flex justify-between"><span class="text-gray-500 dark:text-slate-400">Kerikil</span><span class="font-mono text-gray-700 dark:text-slate-300">{{ number_format($d->coarse_agg_kg, 0) }} kg</span></div>
                    @if($d->admixture_liter > 0)
                    <div class="flex justify-between col-span-2"><span class="text-gray-500 dark:text-slate-400">Admixture</span><span class="font-mono text-gray-700 dark:text-slate-300">{{ $d->admixture_liter }} L</span></div>
                    @endif
                </div>
            ...
            <div class="px-5 py-3 border-t border-gray-100 dark:border-white/10 flex items-center gap-2">
                <button onclick="openCalcModal({{ $d->id }}, '{{ $d->grade }}')" class="text-xs text-blue-500 hover:text-blue-600 transition">🧮 Hitung</button>
                @if(!$d->bom_id)
                <form method="POST" action="{{ route('manufacturing.mix-design.generate-bom', $d) }}" class="inline">
                    @csrf
                ...
                @endif
                <form method="POST" action="{{ route('manufacturing.mix-design.destroy', $d) }}" onsubmit="return confirm('Hapus mix design ini?')" class="inline ml-auto">
                    @csrf @method('DELETE')
