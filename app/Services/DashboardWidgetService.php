...
{
    /**
     * Registry of all built-in dashboard widgets.
     *
    ...
    /**
     * Full registry including tenant-specific custom widgets.
     * Merges built-in registry with custom widgets for a given tenant + role.
     */
    public static function registryForTenant(int $tenantId, string $role): array
    {
        ...
        foreach ($customWidgets as $cw) {
            ...
            $registry[$cw->registryKey()] = [
                ...
                'icon_bg' => $cw->icon_bg,
                'icon_color' => $cw->icon_color,
                'cols' => $cw->cols,
                'data_group' => 'custom',
                'roles' => $cw->visible_to_roles ?? ['admin', 'manager', 'staff', 'kasir', 'gudang'],
                'custom_id' => $cw->id,
                'is_custom' => true,
            ];
        }
     ...
     * Includes cols_override = null for future per-widget size overrides.
     */
    public static function defaultsForRole(string $role): array
    {
        $keys = match ($role) {
            'admin' => ['ai_insights', 'anomaly_alerts', 'kpi_revenue', 'kpi_orders', 'ecommerce_orders', 'kpi_low_stock', 'kpi_attendance', 'chart_sales', 'chart_finance', 'low_stock_list', 'quick_stats', 'gamification'],
            'manager' => ['ai_insights', 'kpi_revenue', 'kpi_orders', 'ecommerce_orders', 'kpi_low_stock', 'kpi_attendance', 'chart_sales', 'chart_finance', 'quick_stats', 'gamification'],
            'kasir' => ['pos_today', 'kpi_orders', 'chart_sales', 'gamification'],
            'gudang' => ['kpi_low_stock', 'low_stock_list', 'kpi_attendance', 'gamification'],
            'staff' => ['kpi_revenue', 'kpi_orders', 'kpi_low_stock', 'chart_sales', 'quick_stats', 'gamification'],
        ...
        foreach ($keys as $i => $key) {
            if (isset($registry[$key])) {
                $widgets[] = ['key' => $key, 'order' => $i, 'visible' => true, 'cols_override' => null];
            }
        }
    ...
    /**
     * All built-in widgets available for a given role.
     */
    ...
    /**
     * All widgets (built-in + custom) available for a given role/tenant.
     */
