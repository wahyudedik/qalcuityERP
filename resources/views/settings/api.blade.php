...
<div class="p-6 space-y-8 max-w-4xl">
    ...
    <div class="p-4 rounded-xl bg-amber-50 border border-amber-300">
        ...
        <div class="flex items-center gap-2">
            <code class="flex-1 text-xs bg-white border border-amber-200 rounded-lg px-3 py-2 font-mono text-amber-900 break-all">{{ session('new_token') }}</code>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
        ...
        <code class="text-xs text-blue-700 font-mono">{{ url('/api/v1') }}</code>
        ...
        <div class="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs text-blue-700">
            ...
            <code class="bg-blue-100 px-2 py-1 rounded">{{ $ep }}</code>
            @endforeach
        </div>
    </div>
        ...
        <div class="divide-y divide-gray-100">
            ...
            <div class="px-5 py-4">
                <div class="flex items-start justify-between gap-4">
                    <div class="min-w-0">
                        <div class="flex items-center gap-2">
                            <p class="text-sm font-medium text-gray-900">{{ $wh->name }}</p>
                            <span class="text-xs px-2 py-0.5 rounded-full {{ $wh->is_active ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-100 text-gray-500' }}">
                                {{ $wh->is_active ? 'Aktif' : 'Nonaktif' }}
                            </span>
                        </div>
                        <p class="text-xs text-gray-400 mt-0.5 truncate max-w-xs">{{ $wh->url }}</p>
                        <p class="text-xs text-gray-400 mt-0.5">Events: {{ implode(', ', $wh->events ?? []) }}</p>
                        <div class="flex items-center gap-3 mt-0.5">
                            @if($wh->last_triggered_at)
                            <p class="text-xs text-gray-400">Terakhir: {{ $wh->last_triggered_at->diffForHumans() }}</p>
                            @endif
                            @if($wh->retry_count > 0)
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">
                            Secret: <code class="bg-gray-100 px-1 rounded text-[10px]">{{ Str::mask($wh->secret, '*', 4) }}</code>
                        </p>
                    </div>
                    <div class="flex items-center gap-2 shrink-0">
                        ...
                        <div class="space-y-1">
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="events[]" value="{{ $ev }}"
                                       class="rounded border-gray-300 text-blue-600">
                                <span class="text-sm text-gray-700 font-mono text-xs">{{ $ev }}</span>
                            </label>
                            @endforeach
                        </div>
                    </div>
