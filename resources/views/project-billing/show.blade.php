    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-6 gap-4">
            <div class="bg-white rounded-2xl border border-gray-200 p-4">
                ...
                <p class="font-semibold text-gray-900">{{ ['time_material'=>'T&M','milestone'=>'Milestone','retainer'=>'Retainer','fixed_price'=>'Fixed','termin'=>'Termin'][$config->billing_type ?? ''] ?? 'Belum diset' }}</p>
            </div>
            <div class="bg-white rounded-2xl border border-gray-200 p-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <form method="POST" action="{{ route('project-billing.time-material', $project) }}" class="space-y-3 mb-4">
                    @csrf
                    @php $cls = 'w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900'; @endphp
                    <p class="text-sm text-gray-500">Time & Material — {{ number_format($unbilledHours, 1) }}h unbilled</p>
                    <div class="grid grid-cols-2 gap-3">
                        <div><label class="block text-xs text-gray-600 mb-1">Dari</label><input type="date" name="period_start" required class="{{ $cls }}"></div>
                        <div><label class="block text-xs text-gray-600 mb-1">Sampai</label><input type="date" name="period_end" required value="{{ date('Y-m-d') }}" class="{{ $cls }}"></div>
                    </div>
                ...
                <form method="POST" action="{{ route('project-billing.retainer', $project) }}">
                    @csrf
                    <p class="text-sm text-gray-500 mb-2">Retainer: Rp {{ number_format($config->retainer_amount ?? 0, 0, ',', '.') }} / {{ $config->retainer_cycle ?? 'monthly' }}</p>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                <div class="p-3 rounded-xl bg-gray-50">
                    ...
                    <p class="text-sm font-bold text-gray-900">Rp {{ number_format($contractVal, 0, ',', '.') }}</p>
                </div>
                <div class="p-3 rounded-xl bg-gray-50">
                ...
                <div class="p-3 rounded-xl bg-gray-50">
                    ...
                    <p class="text-sm font-bold text-gray-900">{{ $terminCount + 1 }}</p>
                </div>
            </div>
            ...
            <div class="divide-y divide-gray-100">
                ...
                <div class="px-6 py-4 flex items-center justify-between">
                    ...
                    <form method="POST" action="{{ route('project-billing.release-retention', $ri) }}" class="flex items-center gap-2">
                        @csrf
                        <input type="number" name="amount" step="1" min="1" max="{{ $ri->retentionOutstanding() }}" value="{{ $ri->retentionOutstanding() }}" class="w-32 px-2 py-1.5 text-xs rounded-lg border border-gray-200 bg-gray-50 text-gray-900">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right font-medium text-gray-900">Rp {{ number_format($ts->laborCost(), 0, ',', '.') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    <div id="modal-ms" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="{{ route('project-billing.milestones.store', $project) }}" class="p-6 space-y-4">
                @csrf
                @php $cls = 'w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900'; @endphp
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Nama *</label><input type="text" name="name" required class="{{ $cls }}"></div>
                <div class="grid grid-cols-2 gap-3">
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Nilai (Rp) *</label><input type="number" name="amount" required min="0" step="1000" class="{{ $cls }}"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Due Date</label><input type="date" name="due_date" class="{{ $cls }}"></div>
                </div>
