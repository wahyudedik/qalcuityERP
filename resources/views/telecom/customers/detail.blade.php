            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Subscription Aktif') }}</h2>
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">{{ __('Paket') }}</p>
                                <p class="mt-1 font-medium text-gray-900 dark:text-white">{{ $subscription->package?->name ?? '-' }}</p>
                            </div>
                            <div>
                                ...
                                <span class="mt-1 px-2 py-0.5 inline-flex text-xs font-semibold rounded-full
                                    {{ $subscription->status === 'active' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400' }}">
                                    {{ ucfirst($subscription->status) }}
                                </span>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">{{ __('Kecepatan') }}</p>
                            ...
                            <div>
                                ...
                                <p class="mt-1 font-medium text-gray-900 dark:text-white">{{ $subscription->device?->name ?? '-' }}</p>
                            </div>
                            @if ($subscription->hotspot_username)
                                <div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">{{ __('Username') }}</p>
                                    <p class="mt-1 font-mono text-gray-900 dark:text-white">{{ $subscription->hotspot_username }}</p>
                                </div>
                            @endif
                            <div>
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Ringkasan Penggunaan') }}</h2>
                            <div class="flex gap-2">
                                @foreach (['daily' => __('Harian'), 'weekly' => __('Mingguan'), 'monthly' => __('Bulanan')] as $p => $label)
                                    <a href="{{ route('telecom.customers.show-usage', $customer) }}?period={{ $p }}"
                                        class="px-3 py-1 text-xs rounded-full {{ $period === $p ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                                        {{ $label }}
                                    </a>
                                @endforeach
                            </div>
                        ...
                        @if ($quotaBytes > 0)
                            <div class="mb-6">
                                <div class="flex justify-between text-sm mb-2">
                                    <span class="text-gray-600 dark:text-gray-400">{{ round($usedBytes / 1073741824, 2) }} GB {{ __('terpakai') }}</span>
                                    <span class="text-gray-600 dark:text-gray-400">{{ round($quotaBytes / 1073741824, 2) }} GB {{ __('total') }}</span>
                                </div>
                                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4">
                                    <div class="bg-{{ $color }}-500 h-4 rounded-full" style="width: {{ $percentage }}%"></div>
                                </div>
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $percentage }}% {{ __('terpakai') }}</p>
                            </div>
                        @else
                            <p class="text-sm text-blue-600 dark:text-blue-400 font-semibold mb-4">∞ {{ __('Kuota Unlimited') }}</p>
                        @endif

                        @if (isset($usageSummary))
                            <div class="grid grid-cols-3 gap-4">
                                <div class="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">{{ __('Download') }}</p>
                                    <p class="text-xl font-bold text-blue-600 dark:text-blue-400">
                                        {{ round(($usageSummary['total_download'] ?? 0) / 1073741824, 2) }} GB
                                    </p>
                                </div>
                                <div class="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">{{ __('Upload') }}</p>
                                    <p class="text-xl font-bold text-green-600 dark:text-green-400">
                                        {{ round(($usageSummary['total_upload'] ?? 0) / 1073741824, 2) }} GB
                                    </p>
                                </div>
                                <div class="text-center p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">{{ __('Total') }}</p>
                    ...
                    @if (isset($chartData) && count($chartData['labels']) > 0)
                        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Tren Penggunaan (7 Hari Terakhir)') }}</h2>
                    ...
                    @if (isset($usageHistory) && $usageHistory->count() > 0)
                        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg overflow-hidden">
                            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Riwayat Penggunaan') }}</h2>
                            </div>
                ...
                <div class="space-y-4">
                    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Aksi') }}</h2>
                        <div class="space-y-3">
                    ...
                    <!-- Customer Info -->
                    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Info Pelanggan') }}</h2>
                        <div class="space-y-3 text-sm">
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">{{ __('Nama') }}</p>
                                <p class="mt-1 font-medium text-gray-900 dark:text-white">{{ $customer->name }}</p>
                            </div>
                            @if ($customer->email)
                                <div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">{{ __('Email') }}</p>
                                    <p class="mt-1 text-gray-900 dark:text-white">{{ $customer->email }}</p>
                                </div>
                            @endif
                            @if ($customer->phone)
                                <div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">{{ __('Telepon') }}</p>
