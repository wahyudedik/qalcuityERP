...
<?php $component->withAttributes([]); ?>
    ...
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="hover:bg-white/5">
                        ...
                        <td class="px-4 py-3 text-right text-amber-400 font-bold">Rp <?php echo e(number_format($p->amount, 0, ',', '.')); ?></td>
                        <td class="px-4 py-3 text-xs text-slate-400">
                            <?php echo e($aff->bank_name ?? '-'); ?> <?php echo e($aff->bank_account ?? ''); ?><br>
                            a/n <?php echo e($aff->bank_holder ?? '-'); ?>

                        </td>
                        <td class="px-4 py-3 text-center text-xs text-slate-400"><?php echo e($p->requested_at?->format('d/m/Y H:i') ?? $p->created_at->format('d/m/Y')); ?></td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-xs bg-<?php echo e($sc); ?>-500/20 text-<?php echo e($sc); ?>-400"><?php echo e(ucfirst($p->status)); ?></span>
                            <?php if($p->reject_reason): ?><p class="text-xs text-red-400 mt-1"><?php echo e($p->reject_reason); ?></p><?php endif; ?>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <?php if($p->status === 'pending'): ?>
                            <div class="flex items-center justify-center gap-1">
                                <form method="POST" action="<?php echo e(route('super-admin.affiliates.payouts.approve', $p)); ?>">
                                ...
                                <form method="POST" action="<?php echo e(route('super-admin.affiliates.payouts.reject', $p)); ?>" onsubmit="const r=prompt('Alasan reject:'); if(!r) return false; this.querySelector('[name=reason]').value=r;">
                                    <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
        ...
        <?php if($payouts->hasPages()): ?><div class="px-4 py-3 border-t border-white/5"><?php echo e($payouts->links()); ?></div><?php endif; ?>
    </div>
