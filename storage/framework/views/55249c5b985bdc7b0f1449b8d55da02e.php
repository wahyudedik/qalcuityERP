    ...
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-slate-400">
                        ...
                        <?php if($landedCost->purchaseOrder?->supplier): ?> · <?php echo e($landedCost->purchaseOrder->supplier->name); ?> <?php endif; ?>
                    </p>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <form method="POST" action="<?php echo e(route('landed-cost.post', $landedCost)); ?>" onsubmit="return confirm('Posting akan update HPP produk. Lanjutkan?')">
                        <?php echo csrf_field(); ?>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                <div><p class="text-xs text-gray-500 dark:text-slate-400">Tanggal</p><p class="text-gray-900 dark:text-white"><?php echo e($landedCost->date->format('d/m/Y')); ?></p></div>
                <div><p class="text-xs text-gray-500 dark:text-slate-400">Metode Alokasi</p><p class="text-gray-900 dark:text-white"><?php echo e(['by_value'=>'Berdasarkan Nilai','by_quantity'=>'Berdasarkan Qty','by_weight'=>'Berdasarkan Berat','equal'=>'Rata (Equal)'][$landedCost->allocation_method] ?? $landedCost->allocation_method); ?></p></div>
                <div><p class="text-xs text-gray-500 dark:text-slate-400">Total Biaya Tambahan</p><p class="font-semibold text-gray-900 dark:text-white">Rp <?php echo e(number_format($landedCost->total_additional_cost, 0, ',', '.')); ?></p></div>
                <div><p class="text-xs text-gray-500 dark:text-slate-400">Jurnal</p>
                    <?php if($landedCost->journalEntry): ?>
                    <a href="<?php echo e(url('accounting/journals')); ?>/<?php echo e($landedCost->journalEntry->id); ?>" class="text-blue-500 hover:underline"><?php echo e($landedCost->journalEntry->number); ?></a>
                    <?php else: ?> <p class="text-gray-400">—</p> <?php endif; ?>
                </div>
            </div>
        </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right font-medium text-gray-900 dark:text-white">Rp <?php echo e(number_format($comp->amount, 0, ',', '.')); ?></td>
                            <td class="px-4 py-3 hidden sm:table-cell text-gray-500 dark:text-slate-400"><?php echo e($comp->vendor ?? '-'); ?></td>
                            <td class="px-4 py-3 hidden sm:table-cell text-gray-500 dark:text-slate-400 text-xs"><?php echo e($comp->reference ?? '-'); ?></td>
                        </tr>
                        ...
                        <tr class="bg-gray-50 dark:bg-white/5 font-semibold">
                            ...
                            <td class="px-4 py-3 text-right text-gray-900 dark:text-white">Rp <?php echo e(number_format($landedCost->components->sum('amount'), 0, ',', '.')); ?></td>
                            <td colspan="2"></td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr>
                            <td class="px-4 py-3 text-gray-900 dark:text-white"><?php echo e($alloc->product->name ?? '-'); ?></td>
                            <td class="px-4 py-3 text-right text-gray-700 dark:text-slate-300"><?php echo e(number_format($alloc->quantity, 0, ',', '.')); ?></td>
                            <td class="px-4 py-3 text-right text-gray-700 dark:text-slate-300">Rp <?php echo e(number_format($alloc->original_cost, 0, ',', '.')); ?></td>
                            <td class="px-4 py-3 text-right font-medium <?php echo e($alloc->allocated_cost > 0 ? 'text-amber-500' : 'text-gray-400'); ?>">
                            ...
                            <td class="px-4 py-3 text-right">
                                ...
                                <form method="POST" action="<?php echo e(route('landed-cost.weight', $alloc)); ?>" class="flex items-center justify-end gap-1">
                                    <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                    <input type="number" name="weight" value="<?php echo e($alloc->weight); ?>" min="0.001" step="0.001"
