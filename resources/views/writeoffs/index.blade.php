<x-app-layout>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3">
                            <p class="font-medium text-gray-900 dark:text-white">{{ $wo->number }}</p>
                            <p class="text-xs text-gray-400 dark:text-slate-500">{{ $wo->created_at->format('d M Y') }}</p>
                        </td>
                        <td class="px-4 py-3 hidden sm:table-cell">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium {{ $wo->type === 'receivable' ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400' : 'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400' }}">
                                {{ $wo->typeLabel() }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-gray-700 dark:text-slate-300">{{ $wo->reference_number }}</td>
                        <td class="px-4 py-3 text-right font-semibold text-red-600 dark:text-red-400">Rp {{ number_format($wo->writeoff_amount,0,',','.') }}</td>
                        <td class="px-4 py-3 hidden md:table-cell text-gray-500 dark:text-slate-400 max-w-xs truncate">{{ $wo->reason }}</td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                @endif
                                @if($wo->journalEntry)
                                <a href="{{ route('journals.show', $wo->journalEntry) }}" class="px-2 py-1 text-xs border border-gray-200 dark:border-white/10 text-gray-600 dark:text-slate-300 rounded-lg hover:bg-gray-50 dark:hover:bg-white/5">Jurnal</a>
                                @endif
                            </div>
                        </td>
                    </tr>
        ...
        <div class="px-4 py-3 border-t border-gray-100 dark:border-white/5">{{ $writeoffs->links() }}</div>
        @endif
    </div>
