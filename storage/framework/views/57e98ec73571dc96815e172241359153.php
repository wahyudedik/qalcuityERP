...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> Scan Asset untuk Maintenance <?php $__env->endSlot(); ?>

    <div class="max-w-3xl mx-auto space-y-4">

        
        <div class="bg-blue-50 dark:bg-blue-900/20 rounded-2xl border border-blue-200 dark:border-blue-800 p-5">
            <p class="text-sm font-semibold text-blue-900 dark:text-blue-200 mb-3">Scan Barcode Asset</p>
        ...
        <div id="asset-details-card"
            class="hidden bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
            <div class="flex items-start justify-between mb-4">
                <div>
                    <h2 id="asset-name" class="text-lg font-bold text-gray-900 dark:text-white">-</h2>
                    <p id="asset-code" class="text-xs font-mono text-gray-500 dark:text-slate-400 mt-0.5">-</p>
                </div>
            ...
            <div class="grid grid-cols-2 gap-3 text-sm mb-5">
                <div>
                    <span class="text-gray-500 dark:text-slate-400 text-xs block">Kategori</span>
                    <span id="asset-category" class="font-medium text-gray-900 dark:text-white">-</span>
                </div>
                <div>
                    <span class="text-gray-500 dark:text-slate-400 text-xs block">Lokasi</span>
                    <span id="asset-location" class="font-medium text-gray-900 dark:text-white">-</span>
                </div>
                <?php if(auth()->user()->can('edit', 'assets')): ?>
                    <div>
                        <span class="text-gray-500 dark:text-slate-400 text-xs block">Brand</span>
                        <span id="asset-brand" class="font-medium text-gray-900 dark:text-white">-</span>
                    </div>
                    <div>
                        <span class="text-gray-500 dark:text-slate-400 text-xs block">Model</span>
                        <span id="asset-model" class="font-medium text-gray-900 dark:text-white">-</span>
                    </div>
                <?php endif; ?>
            </div>

            
            <form method="POST" action="<?php echo e(route('assets.maintenance.store')); ?>"
                class="border-t border-gray-100 dark:border-white/10 pt-4">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="asset_id" id="form-asset-id">

                <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">Buat Maintenance Record</h3>
                ...
                <div class="grid grid-cols-2 gap-3 mb-3">
                    ...
                    <div>
                        <label class="block text-xs text-gray-500 dark:text-slate-400 mb-1">Tanggal Jadwal</label>
                        <input type="date" name="scheduled_date" required value="<?php echo e(date('Y-m-d')); ?>"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#1e293b] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>

                <div class="mb-3">
                    <label class="block text-xs text-gray-500 dark:text-slate-400 mb-1">Deskripsi Pekerjaan</label>
                ...
                <div class="grid grid-cols-2 gap-3 mb-4">
                    <div>
                        <label class="block text-xs text-gray-500 dark:text-slate-400 mb-1">Estimasi Biaya (Rp)</label>
                        <input type="number" name="cost" step="0.01"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#1e293b] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                            placeholder="0">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-500 dark:text-slate-400 mb-1">Vendor/Teknisi</label>
        ...
        <div id="maintenance-history"
            class="hidden bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">Riwayat Maintenance Terakhir</h3>
    ...
    <div id="barcode-scanner-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/10">
                <h3 class="font-semibold text-gray-900 dark:text-white text-sm">Scan Barcode Asset</h3>
            ...
            <div class="p-4">
                ...
                <p class="text-xs text-center text-gray-500 dark:text-slate-400 mt-3">Arahkan kamera ke barcode asset
                </p>
    ...
    <?php $__env->startPush('scripts'); ?>
        ...
        <script>
            ...
            function processBarcode(barcode) {
                ...
                feedback.className = 'text-xs mt-2 text-blue-600 dark:text-blue-300 min-h-[16px]';

                fetch(`<?php echo e(route('assets.lookup-barcode')); ?>?barcode=${encodeURIComponent(barcode)}`)
                    .then(r => r.json())
                    .then(data => {
                        if (data.success) {
            ...
            function displayAsset(asset) {
                ...
                badge.className =
                    `px-2 py-0.5 rounded-full text-xs bg-${color}-100 text-${color}-700 dark:bg-${color}-500/20 dark:text-${color}-400`;
                ...
                fetch(`/api/assets/${assetId}/maintenances`)
                    ...
                    .then(history => {
                        ...
                        <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-white/5 rounded-xl">
                            <div class="flex-1">
                                <p class="text-xs font-medium text-gray-900 dark:text-white">${ucfirst(m.type)} Maintenance</p>
                                <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">${new Date(m.scheduled_date).toLocaleDateString('id-ID')}</p>
                            </div>
...
<?php endif; ?>
<?php /**PATH E:\PROJEKU\qalcuityERP\resources\views\assets\scan-maintenance.blade.php ENDPATH**/ ?>