<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <form method="GET" action="{{ route('healthcare.financial-reports.index') }}"
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">Rp
                                {{ number_format($statistics['total_paid'], 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">Rp
                                {{ number_format($statistics['total_outstanding'], 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">{{ $statistics['insurance_claims'] }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">Rp
                                {{ number_format($statistics['insurance_approved'], 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
            </div>

            @if (count($revenueByDepartment) > 0)
                                ...
                                @foreach ($revenueByDepartment as $dept)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                            {{ ucfirst($dept->department) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">Rp
                                            {{ number_format($dept->total, 0, ',', '.') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="w-32 bg-gray-200 rounded-full h-2.5 mr-3">
                                                    <div class="bg-blue-600 h-2.5 rounded-full"
                                                        style="width: {{ $statistics['total_revenue'] > 0 ? ($dept->total / $statistics['total_revenue']) * 100 : 0 }}%">
                                                    </div>
                                                </div>
                                                <span
                                                    class="text-sm font-semibold text-gray-900">{{ $statistics['total_revenue'] > 0 ? number_format(($dept->total / $statistics['total_revenue']) * 100, 1) : 0 }}%</span>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($revenueByPaymentMethod as $method)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                            {{ ucfirst($method->payment_method) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">Rp
                                            {{ number_format($method->total, 0, ',', '.') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="w-32 bg-gray-200 rounded-full h-2.5 mr-3">
                                                    <div class="bg-green-600 h-2.5 rounded-full"
                                                        style="width: {{ $statistics['total_revenue'] > 0 ? ($method->total / $statistics['total_revenue']) * 100 : 0 }}%">
                                                    </div>
                                                </div>
                                                <span
                                                    class="text-sm font-semibold text-gray-900">{{ $statistics['total_revenue'] > 0 ? number_format(($method->total / $statistics['total_revenue']) * 100, 1) : 0 }}%</span>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
    ...
    <script>
        function exportReport() {
            const url = '{{ route('healthcare.financial-reports.export') }}' +
                '?date_from={{ $dateFrom instanceof \Carbon\Carbon ? $dateFrom->format('Y-m-d') : $dateFrom }}' +
                '&date_to={{ $dateTo instanceof \Carbon\Carbon ? $dateTo->format('Y-m-d') : $dateTo }}';
