...
class SalesTools
{
    ...
    public function createSalesOrder(array $args): array
    {
        ...
        $itemSummary = collect($itemsData)->zip(collect($args['items'])->pluck('product_name'))->map(fn ($pair) => "- {$pair[1]}: {$pair[0]['quantity']} × Rp ".number_format($pair[0]['price'], 0, ',', '.').
    ...
    public function createQuotation(array $args): array
    {
        ...
        $itemSummary = collect($itemsData)->map(fn ($i) => "- {$i['description']}: {$i['quantity']} × Rp ".number_format($i['price'], 0, ',', '.').
