<x-app-layout>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ $totalAccounts }}</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            <p class="text-xs text-gray-500 dark:text-slate-400">Rekening Aktif</p>
            <p class="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">{{ $activeAccounts }}</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10 col-span-2 sm:col-span-1">
            ...
            <p class="text-xl font-bold text-blue-600 dark:text-blue-400 mt-1">Rp {{ number_format($totalBalance, 0, ',', '.') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 mb-4">
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4">
            ...
            <div class="flex gap-2">
                <a href="{{ route('bank.reconciliation') }}" class="px-3 py-2 text-sm border border-gray-200 dark:border-white/10 rounded-xl text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-white/5">Rekonsiliasi</a>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-slate-400 sm:hidden">{{ $acc->account_number }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-3 hidden sm:table-cell font-mono text-xs text-gray-600 dark:text-slate-300">{{ $acc->account_number }}</td>
                        <td class="px-4 py-3 hidden md:table-cell text-gray-600 dark:text-slate-300">{{ $acc->account_name }}</td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <form method="POST" action="{{ route('bank-accounts.toggle', $acc) }}">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="p-1.5 rounded-lg {{ $acc->is_active ? 'text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-500/10' : 'text-green-500 hover:bg-green-50 dark:hover:bg-green-500/10' }}" title="{{ $acc->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/></svg>
                                    </button>
                                </form>
                                @endcanmodule
                                @canmodule('bank', 'delete')
                                <form method="POST" action="{{ route('bank-accounts.destroy', $acc) }}" onsubmit="return confirm('Hapus rekening ini?')">
                                    @csrf @method('DELETE')
