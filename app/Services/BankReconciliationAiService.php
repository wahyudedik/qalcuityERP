...
class BankReconciliationAiService
{
    ...
    private function scoreCandidate(BankStatement $stmt, Transaction $tx): array
    {
        ...
        if ($diffPct === 0.0) {
            $score += 40;
            $reasons[] = 'Jumlah sama persis';
        } elseif ($diffPct <= 0.001) {
            $score += 35;
            $reasons[] = 'Jumlah hampir sama (< 0.1%)';
        } elseif ($diffPct <= self::AMOUNT_TOLERANCE_PCT) {
            $score += 25;
            $reasons[] = 'Jumlah mendekati (< 1%)';
        }
        ...
        if ($daysDiff === 0) {
            $score += 30;
            $reasons[] = 'Tanggal sama';
        } elseif ($daysDiff <= 1) {
            $score += 25;
            $reasons[] = 'Selisih 1 hari';
        } elseif ($daysDiff <= 3) {
            $score += 15;
            $reasons[] = "Selisih {$daysDiff} hari";
        ...
        if ($descScore >= 0.8) {
            $score += 20;
            $reasons[] = 'Deskripsi sangat mirip';
        } elseif ($descScore >= 0.5) {
            $score += 12;
            $reasons[] = 'Deskripsi cukup mirip';
        } elseif ($descScore >= 0.3) {
            $score += 6;
            $reasons[] = 'Deskripsi ada kesamaan kata';
        }
        ...
        if ($stmt->reference && $tx->reference) {
            ...
            if ($stmtRef === $txRef) {
                $score += 10;
                $reasons[] = 'Nomor referensi sama';
            } elseif (str_contains($stmtRef, $txRef) || str_contains($txRef, $stmtRef)) {
                $score += 6;
                $reasons[] = 'Referensi sebagian cocok';
            }
        }
        ...
        if ($txIsCredit === $stmtCredit) {
            $score += 5;
            $reasons[] = 'Tipe transaksi sesuai';
        } else {
            $score -= 10;
            $reasons[] = 'Tipe transaksi berbeda';
        }
