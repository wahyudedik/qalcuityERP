<x-app-layout>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
            ...
            <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $dashboard['total_suppliers'] }}</p>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ number_format($dashboard['average_score'], 1) }}<span class="text-sm font-normal text-gray-400">/100</span></p>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
            ...
            <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $dashboard['top_performers'] }}</p>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
            ...
            <p class="text-2xl font-bold text-red-500 dark:text-red-400">{{ $dashboard['at_risk'] }}</p>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden mb-4">
        ...
        <div class="p-4 grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div class="bg-gray-50 dark:bg-[#0f172a] rounded-xl p-3">
                <p class="text-xs text-gray-500 dark:text-slate-400 mb-1 truncate">{{ $category }}</p>
                <p class="text-xl font-bold text-gray-900 dark:text-white">{{ $stats['avg_score'] }}</p>
                <p class="text-xs text-gray-400 dark:text-slate-500 mt-0.5">{{ $stats['count'] }} supplier</p>
            </div>
            @endforeach
        </div>
    </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            <td class="px-4 py-3">
                                <p class="font-medium text-gray-900 dark:text-white">{{ $scorecard->supplier->name }}</p>
                                @if($scorecard->supplier->company)
                                    <p class="text-xs text-gray-400 dark:text-slate-500">{{ $scorecard->supplier->company }}</p>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <span class="font-bold text-gray-900 dark:text-white text-sm">{{ number_format($scorecard->overall_score, 1) }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-right hidden md:table-cell text-gray-500 dark:text-slate-400">{{ number_format($scorecard->quality_score, 1) }}</td>
                            <td class="px-4 py-3 text-right hidden md:table-cell text-gray-500 dark:text-slate-400">{{ number_format($scorecard->delivery_score, 1) }}</td>
                            <td class="px-4 py-3 text-right hidden lg:table-cell text-gray-500 dark:text-slate-400">{{ number_format($scorecard->cost_score, 1) }}</td>
                            <td class="px-4 py-3 text-right hidden lg:table-cell text-gray-500 dark:text-slate-400">{{ number_format($scorecard->service_score, 1) }}</td>
                            <td class="px-4 py-3 text-center">
