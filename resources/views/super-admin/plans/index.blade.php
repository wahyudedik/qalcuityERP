<x-app-layout>
    ...
    <x-slot name="topbarActions">
        ...
        <a href="{{ route('super-admin.plans.create') }}"
           class="flex items-center gap-2 text-sm bg-blue-600 hover:bg-blue-700 text-gray-900 dark:text-white px-4 py-2 rounded-xl transition font-medium">
    ...
    @endif
    @if(session('error'))
        <div class="mb-4 px-4 py-3 bg-red-500/20 border border-red-500/30 text-red-400 text-sm rounded-xl">{{ session('error') }}</div>
    @endif

    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        @forelse($plans as $plan)
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6 flex flex-col">
            <div class="flex items-start justify-between mb-4">
                ...
                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium
                    {{ $plan->is_active ? 'bg-green-500/20 text-green-400' : 'bg-[#f8f8f8] dark:bg-white/10 text-gray-400 dark:text-slate-500' }}">
                    <span class="w-1.5 h-1.5 rounded-full {{ $plan->is_active ? 'bg-green-500' : 'bg-slate-500' }}"></span>
            ...
            <div class="space-y-2 text-sm text-gray-500 dark:text-slate-400 mb-5 flex-1">
                ...
                <div class="flex items-center gap-2">
                    <svg class="w-4 h-4 text-purple-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17H3a2 2 0 01-2-2V5a2 2 0 012-2h14a2 2 0 012 2v10a2 2 0 01-2 2h-2"/></svg>
            ...
            <div class="flex items-center gap-2 pt-4 border-t border-gray-200 dark:border-white/10">
                ...
                <a href="{{ route('super-admin.plans.edit', $plan) }}"
                   class="text-xs text-blue-400 hover:text-blue-300 font-medium px-3 py-1.5 rounded-lg hover:bg-blue-500/10 transition">
                    Edit
                </a>
                <form method="POST" action="{{ route('super-admin.plans.destroy', $plan) }}"
                    ...
                    @csrf @method('DELETE')
                    <button type="submit"
                        class="text-xs text-red-400 hover:text-red-300 font-medium px-3 py-1.5 rounded-lg hover:bg-red-500/10 transition">
                        Hapus
                    </button>
                </form>
            </div>
        ...
        @empty
        <div class="col-span-3 bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-12 text-center">
            <p class="text-gray-400 dark:text-slate-500 text-sm mb-4">Belum ada paket langganan.</p>
            <form method="POST" action="{{ route('super-admin.plans.seed') }}" class="inline">
                @csrf
                <button type="submit" class="text-sm bg-blue-600 text-gray-900 dark:text-white px-4 py-2 rounded-xl hover:bg-blue-700 transition">
