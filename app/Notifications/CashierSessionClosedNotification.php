...
class CashierSessionClosedNotification extends Notification implements ShouldQueue
{
    ...
    public function via(object $notifiable): array
    {
        ...
        if (NotificationPreference::isEnabled($notifiable->id, 'cashier_session_closed', 'in_app')) {
            $channels[] = 'database';
        }
        if (NotificationPreference::isEnabled($notifiable->id, 'cashier_session_closed', 'email')) {
            $channels[] = 'mail';
        }
        if (NotificationPreference::isEnabled($notifiable->id, 'cashier_session_closed', 'push')) {
            $channels[] = 'broadcast';
        }
