<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <span>🐠 Pond Detail - {{ $pond->code }}</span>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6 mb-6">
        <div class="flex items-start justify-between mb-4">
            <div>
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $pond->code }}</h2>
                <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">{{ $pond->name }}</p>
                @if ($pond->location)
                    <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">📍 {{ $pond->location }}</p>
                @endif
            </div>
            @php
                $statusColors = [
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-100 dark:border-white/5">
            <div>
                ...
                <p class="text-lg font-medium text-gray-900 dark:text-white">{{ number_format($pond->area_size, 1) }}
                    m²</p>
            </div>
            @if ($pond->depth)
                <div>
                    <p class="text-xs text-gray-500 dark:text-slate-400">Kedalaman</p>
                    <p class="text-lg font-medium text-gray-900 dark:text-white">{{ number_format($pond->depth, 1) }} m
                    </p>
                </div>
            @endif
            @if ($pond->pond_type)
            ...
            @if ($pond->water_source)
                <div>
                    ...
                    <p class="text-lg font-medium text-gray-900 dark:text-white">{{ ucfirst($pond->water_source) }}</p>
                </div>
            @endif
        </div>
        ...
        @if ($pond->current_stock_species)
            <div class="mt-4 pt-4 border-t border-gray-100 dark:border-white/5">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-base font-bold text-gray-900 dark:text-white">{{ $pond->current_stock_species }}
                        </p>
    ...
    @if ($pond->latestWaterQuality)
        <div
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <span
                    class="text-xs text-gray-500 dark:text-slate-400">{{ $pond->latestWaterQuality->logged_at->format('d M Y, H:i') }}</span>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-white/5">
                        @foreach ($waterQualityLogs as $log)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-[#0f172a] transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="px-2 py-1 text-xs rounded-full {{ $isGood ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400' }}">
            ...
            <div class="divide-y divide-gray-200 dark:divide-white/5">
                @foreach ($feedings as $feeding)
                    <div class="px-6 py-4 hover:bg-gray-50 dark:hover:bg-[#0f172a] transition">
                        ...
                        @if ($feeding->notes)
                            <p class="text-xs text-gray-500 dark:text-slate-400 mt-2">{{ $feeding->notes }}</p>
                        @endif
                    </div>
                @endforeach
            </div>
