                ...
                <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                    ...
                    @else
                                ...
                                <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                                    @foreach($transfers as $t)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition">
                                            ...
                                            <td class="px-4 py-3 text-gray-700 dark:text-slate-300">{{ $t->product->name ?? '-' }}</td>
                                            <td class="px-4 py-3 text-xs text-gray-500 dark:text-slate-400">{{ $t->warehouse->name ?? '-' }}</td>
                                            <td class="px-4 py-3 text-xs text-gray-500 dark:text-slate-400">{{ $t->toWarehouse->name ?? '-' }}</td>
                                            <td class="px-4 py-3 text-right font-medium text-gray-900 dark:text-white">{{ $t->quantity }}</td>
                                            <td class="px-4 py-3 text-xs text-gray-500 dark:text-slate-400">{{ $t->user->name ?? '-' }}</td>
                                            <td class="px-4 py-3 text-xs text-gray-500 dark:text-slate-400">{{ $t->created_at->format('d/m/Y H:i') }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        @if($transfers->hasPages())
                            <div class="px-6 py-4 border-t border-gray-100 dark:border-white/5">{{ $transfers->links() }}</div>
                        @endif
                    @endif
                </div>
            </div>
