<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
        ...
        <a href="{{ route('consignment.index') }}" class="px-3 py-2 text-sm text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-white">← Konsinyasi</a>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        ...
                        <td class="px-4 py-3 hidden sm:table-cell text-gray-500 dark:text-slate-400 text-xs">{{ $p->contact_person ?? '-' }} · {{ $p->phone ?? '-' }}</td>
                        <td class="px-4 py-3 text-right text-gray-900 dark:text-white">{{ $p->commission_pct }}%</td>
                        <td class="px-4 py-3 text-center text-gray-700 dark:text-slate-300">{{ $p->shipments_count }}</td>
        ...
        @if($partners->hasPages())<div class="px-4 py-3 border-t border-gray-100 dark:border-white/5">{{ $partners->links() }}</div>@endif
    </div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="{{ route('consignment.partners.store') }}" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="col-span-2"><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Nama Toko/Outlet *</label><input type="text" name="name" required class="{{ $cls }}"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Kontak</label><input type="text" name="contact_person" class="{{ $cls }}"></div>
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Email</label><input type="email" name="email" class="{{ $cls }}"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Komisi (%)</label><input type="number" name="commission_pct" min="0" max="100" step="0.01" value="10" class="{{ $cls }}"></div>
                    <div class="col-span-2"><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Alamat</label><input type="text" name="address" class="{{ $cls }}"></div>
                </div>
