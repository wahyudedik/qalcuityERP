<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Add Medical Supply') }}</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('healthcare.medical-supplies.store') }}">
                    @csrf
                    <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="category" required
                                    ...
                                    <option value="consumables"
                                        {{ old('category') === 'consumables' ? 'selected' : '' }}>Consumables</option>
                                    <option value="instruments"
                                        {{ old('category') === 'instruments' ? 'selected' : '' }}>Instruments</option>
                                    <option value="medications"
                                        {{ old('category') === 'medications' ? 'selected' : '' }}>Medications</option>
                                    <option value="surgical" {{ old('category') === 'surgical' ? 'selected' : '' }}>
                                        Surgical Supplies</option>
                                    <option value="diagnostic" {{ old('category') === 'diagnostic' ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="quantity" required value="{{ old('quantity', 0) }}"
                                    min="0"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            </div>
                            <div>
                                <label for="unit" class="block text-sm font-medium text-gray-700">Unit *</label>
                                <input type="text" name="unit" required value="{{ old('unit') }}"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500"
