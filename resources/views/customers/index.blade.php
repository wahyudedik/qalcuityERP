            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5 {{ !$c->is_active ? 'opacity-60' : '' }}">
                        ...
                        <td class="px-4 py-3 hidden sm:table-cell text-gray-500 dark:text-slate-400">{{ $c->company ?? '-' }}</td>
                        <td class="px-4 py-3 hidden md:table-cell text-gray-500 dark:text-slate-400">{{ $c->phone ?? '-' }}</td>
                        <td class="px-4 py-3 hidden lg:table-cell text-gray-500 dark:text-slate-400">{{ $c->email ?? '-' }}</td>
                        ...
                        <td class="px-4 py-3 text-center hidden sm:table-cell">
                            <span class="px-2 py-0.5 rounded-full text-xs {{ $c->is_active ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' : 'bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-slate-400' }}">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button onclick="openEdit({{ $c->id }}, {{ json_encode($c->name) }}, {{ json_encode($c->company ?? '') }}, {{ json_encode($c->phone ?? '') }}, {{ json_encode($c->email ?? '') }}, {{ json_encode($c->address ?? '') }}, {{ json_encode($c->npwp ?? '') }}, {{ $c->credit_limit ?? 0 }}, {{ $c->is_active ? 'true' : 'false' }})"
                                ...
                                <form method="POST" action="{{ route('customers.toggle', $c) }}">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="p-1.5 rounded-lg text-gray-500 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-white/10" title="{{ $c->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                                        @if($c->is_active)
            ...
            <form method="POST" action="{{ route('customers.store') }}" class="p-6 space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="text" name="npwp" value="{{ old('npwp') }}" placeholder="00.000.000.0-000.000"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                    ...
                    <div class="col-span-2">
                        ...
                        <input type="number" name="credit_limit" value="{{ old('credit_limit', 0) }}" min="0" step="1000000"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
