                    ...
                    @forelse($users as $user)
                        <tr class="hover:bg-gray-50 transition">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <div class="flex items-center gap-1.5 mt-1 sm:hidden">
                                            ...
                                            @endphp
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium {{ $roleStyle[$user->role] ?? '' }}">{{ $roleLabel[$user->role] ?? $user->role }}</span>
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-medium {{ $user->is_active ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400' }}">
                            ...
                            <td class="px-4 sm:px-6 py-4 hidden sm:table-cell">
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium {{ $roleStyle[$user->role] ?? 'bg-[#f8f8f8] text-slate-300' }}">
                                    {{ $roleLabel[$user->role] ?? $user->role }}
                                </span>
                            </td>
                            <td class="px-4 sm:px-6 py-4 hidden sm:table-cell">
                                <span
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium {{ $user->is_active ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400' }}">
                                    <span
                                ...
                                <div class="flex items-center justify-end gap-1">
                                    @unless ($user->isAdmin())
                                        <a href="{{ route('tenant.users.permissions', $user) }}"
                                            class="p-2 rounded-lg text-gray-500 hover:text-green-400 hover:bg-green-500/10 transition"
                                            title="Izin Akses">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <form method="POST" action="{{ route('tenant.users.destroy', $user) }}"
                                            class="inline" data-confirm="Hapus pengguna {{ $user->name }}?"
