                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1">
                            ...
                            @else
                                <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-lg bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">{{ ucfirst($admission->status ?? '-') }}</span>
                            @endif
                        </p>
                    </div>
                    ...
                    @if($admission->treatment_plan)
                        <div class="md:col-span-2">
                            ...
                            <p class="text-sm text-gray-700 dark:text-slate-300 mt-1">{{ $admission->treatment_plan }}</p>
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    <div class="divide-y divide-gray-100 dark:divide-white/5">
                        @foreach($admission->wardRounds->take(5) as $round)
                            <div class="p-4">
                                ...
                                <p class="text-sm text-gray-600 dark:text-slate-400">{{ $round->assessment ?? '-' }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $admission->bed?->ward?->name ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-slate-400">No. Bed</p>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $admission->bed?->bed_number ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-slate-400">Kelas</p>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $admission->bed?->bed_class ?? '-' }}</p>
                    </div>
                </div>
            </div>
