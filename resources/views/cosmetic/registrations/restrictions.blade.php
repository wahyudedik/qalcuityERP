...
@section('title', 'Ingredient Restrictions')
...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($restrictions as $restriction)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $restriction->ingredient_name }}</div>
                            </td>
                            <td class="px-6 py-4">
                                @if ($restriction->cas_number)
                                    <span class="text-sm text-gray-900 font-mono">{{ $restriction->cas_number }}</span>
                                @else
                            ...
                            @if ($restriction->restriction_type == 'banned') bg-red-100 text-red-800
                            @elseif($restriction->restriction_type == 'restricted') bg-orange-100 text-orange-800
                            @else bg-yellow-100 text-yellow-800 @endif">
                                    {{ $restriction->type_label }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                @if ($restriction->max_limit)
                                    <span class="text-sm font-medium text-gray-900">{{ $restriction->max_limit }}%</span>
                                @else
                                    <span class="text-sm text-gray-400">N/A</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                @if ($restriction->regulation_reference)
                                    <div class="text-sm text-gray-900">
                                        {{ Str::limit($restriction->regulation_reference, 30) }}</div>
                                @else
                                    <span class="text-sm text-gray-400">-</span>
                                @endif
                            </td>
        ...
        @if ($restrictions->hasPages())
            <div class="mt-4">{{ $restrictions->links() }}</div>
        @endif
    </div>
    ...
    <div id="add-restriction-modal"
        ...
        <div class="relative top-20 mx-auto p-5 border w-[600px] shadow-lg rounded-md bg-white">
            ...
            <form method="POST" action="{{ route('cosmetic.registrations.restrictions.store') }}">
                @csrf
                <div class="space-y-3">
                    <div>
