    ...
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
                <div>
                    <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ $quotation->number }}</h2>
                    <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">
                        Customer: <span class="font-medium text-gray-700 dark:text-slate-300">{{ $quotation->customer->name }}</span>
                    </p>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr>
                            <td class="px-4 py-3 text-gray-700 dark:text-slate-300">
                                {{ $item->description }}
                                @if($item->product) <span class="text-xs text-gray-400 dark:text-slate-500">({{ $item->product->name }})</span> @endif
                            </td>
                            <td class="px-4 py-3 text-right text-gray-900 dark:text-white">{{ $item->quantity }}</td>
                            <td class="px-4 py-3 text-right text-gray-900 dark:text-white">Rp {{ number_format($item->price, 0, ',', '.') }}</td>
                    ...
                    <tfoot class="border-t border-gray-200 dark:border-white/10">
                        <tr>
                            ...
                            <td class="px-4 py-2 text-right text-gray-900 dark:text-white">Rp {{ number_format($quotation->subtotal, 0, ',', '.') }}</td>
                        </tr>
                        ...
                        <tr class="font-semibold">
                            ...
                            <td class="px-4 py-2 text-right text-lg text-gray-900 dark:text-white">Rp {{ number_format($quotation->total, 0, ',', '.') }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            @if($quotation->notes)
            <p class="mt-4 text-sm text-gray-500 dark:text-slate-400">Catatan: {{ $quotation->notes }}</p>
            @endif
        </div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center justify-between px-4 py-3 rounded-xl bg-gray-50 dark:bg-white/5">
                    <span class="font-mono text-sm text-gray-900 dark:text-white">{{ $so->number }}</span>
                    <span class="text-sm text-gray-500 dark:text-slate-400">{{ $so->date->format('d M Y') }}</span>
                    <span class="font-medium text-gray-900 dark:text-white">Rp {{ number_format($so->total, 0, ',', '.') }}</span>
