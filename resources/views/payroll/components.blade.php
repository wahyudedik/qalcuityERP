<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('payroll.index') }}" class="text-gray-400 hover:text-gray-600 dark:hover:text-white">
        ...
        <div class="flex-1 space-y-6">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                <div class="px-5 py-3 border-b border-gray-100 dark:border-white/10 flex items-center justify-between">
                    ...
                    <span class="text-xs text-gray-400 dark:text-slate-500">{{ $group->count() }} komponen</span>
                </div>
                @if($group->isEmpty())
                <div class="px-5 py-8 text-center text-sm text-gray-400 dark:text-slate-500">Belum ada komponen {{ strtolower($label) }}.</div>
                @else
                    ...
                    <div class="px-5 py-3 flex items-center gap-3">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                                ...
                                @if($comp->code)
                                <span class="text-xs text-gray-400 dark:text-slate-500 font-mono">{{ $comp->code }}</span>
                                @endif
                                @if(!$comp->is_active)
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            <button onclick="editComp({{ $comp->id }}, '{{ addslashes($comp->name) }}', '{{ $comp->code }}', '{{ $comp->type }}', '{{ $comp->calc_type }}', {{ $comp->default_amount }}, {{ $comp->taxable ? 1 : 0 }}, '{{ addslashes($comp->description ?? '') }}')"
                            ...
                            <form method="POST" action="{{ route('payroll.components.destroy', $comp) }}" onsubmit="return confirm('Hapus komponen ini?')">
                                @csrf @method('DELETE')
                ...
                <div class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <div class="px-5 py-3 flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-500/20 flex items-center justify-center shrink-0">
                            <span class="text-blue-600 dark:text-blue-400 text-xs font-bold">{{ strtoupper(substr($emp->name, 0, 1)) }}</span>
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $emp->name }}</p>
                            <p class="text-xs text-gray-500 dark:text-slate-400">{{ $emp->position ?? '-' }} · {{ $emp->department ?? '-' }}</p>
                        </div>
