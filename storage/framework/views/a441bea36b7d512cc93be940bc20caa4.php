...
<?php $component->withAttributes([]); ?>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4">
        ...
        <div class="flex gap-2">
            <a href="<?php echo e(route('assets.scan-maintenance')); ?>"
                class="px-3 py-2 text-sm bg-purple-600 text-white rounded-xl hover:bg-purple-700 inline-flex items-center gap-1.5">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    <?php $__empty_1 = true; $__currentLoopData = $maintenances; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            <td class="px-4 py-3 font-medium text-gray-900 dark:text-white"><?php echo e($m->asset->name ?? '-'); ?>

                            </td>
                            <td class="px-4 py-3 hidden sm:table-cell text-gray-500 dark:text-slate-400 capitalize">
                                <?php echo e($m->type); ?></td>
                            <td class="px-4 py-3 text-gray-700 dark:text-slate-300 text-xs">
                                <?php echo e(Str::limit($m->description, 50)); ?></td>
                            ...
                            <td class="px-4 py-3 text-right hidden md:table-cell text-gray-700 dark:text-slate-300">
                                <?php echo e($m->cost > 0 ? 'Rp ' . number_format($m->cost, 0, ',', '.') : '-'); ?></td>
                            <td class="px-4 py-3 text-center">
                                <?php $mc = ['pending'=>'amber','in_progress'=>'blue','completed'=>'green'][$m->status] ?? 'gray'; ?>
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs bg-<?php echo e($mc); ?>-100 text-<?php echo e($mc); ?>-700 dark:bg-<?php echo e($mc); ?>-500/20 dark:text-<?php echo e($mc); ?>-400">
                            ...
                            <td class="px-4 py-3 text-center">
                                <?php if($m->status !== 'completed'): ?>
                                    <form method="POST" action="<?php echo e(route('assets.maintenance.status', $m)); ?>"
                                        class="inline">
                                        <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                        <select name="status" onchange="this.form.submit()"
                                            class="text-xs px-2 py-1 rounded-lg border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white">
    ...
    <div id="modal-add-maintenance" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-md shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/10">
                ...
                <button onclick="document.getElementById('modal-add-maintenance').classList.add('hidden')"
            ...
            <form method="POST" action="<?php echo e(route('assets.maintenance.store')); ?>" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <select name="asset_id" required
                        ...
                        <?php $__currentLoopData = $assets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($a->id); ?>"><?php echo e($a->name); ?> (<?php echo e($a->asset_code); ?>)</option>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Tanggal
                            *</label>
                        <input type="date" name="scheduled_date" value="<?php echo e(today()->format('Y-m-d')); ?>" required
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Deskripsi Pekerjaan
                        *</label>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Estimasi
                            Biaya</label>
                        <input type="number" name="cost" min="0" step="10000"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label
                            class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Vendor/Bengkel</label>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button"
                        onclick="document.getElementById('modal-add-maintenance').classList.add('hidden')"
