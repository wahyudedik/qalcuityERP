    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                <td class="px-4 py-3">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-slate-400">{{ $lp->customer->phone ?? $lp->customer->email ?? '' }}</p>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-{{ $tc }}-100 text-{{ $tc }}-700 dark:bg-{{ $tc }}-500/20 dark:text-{{ $tc }}-400">
                                        {{ $lp->tier }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-right font-semibold text-gray-900 dark:text-white">{{ number_format($lp->total_points) }}</td>
                                <td class="px-4 py-3 text-right hidden md:table-cell text-gray-500 dark:text-slate-400">{{ number_format($lp->lifetime_points) }}</td>
                                @if($program)
                ...
                @if($points->hasPages())
                <div class="px-4 py-3 border-t border-gray-100 dark:border-white/5">{{ $points->links() }}</div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="{{ route('loyalty.add-points') }}" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    @if($program)<p class="text-xs text-gray-400 dark:text-slate-500 mt-1">Poin otomatis: Rp 100 = 1 poin</p>@endif
                </div>
                <div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="{{ route('loyalty.redeem') }}" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="number" name="points" min="{{ $program?->min_redeem_points ?? 100 }}" required class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @if($program)<p class="text-xs text-gray-400 dark:text-slate-500 mt-1">Min. {{ $program->min_redeem_points }} poin · 1 poin = Rp {{ number_format($program->idr_per_point,0,',','.') }}</p>@endif
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Referensi</label>
