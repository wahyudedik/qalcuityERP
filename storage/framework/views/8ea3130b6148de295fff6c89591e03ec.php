...
<?php $component->withAttributes([]); ?>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">Total Kendaraan</p>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">Sedang Dipakai</p>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">BBM Bulan Ini</p>
            <p class="text-lg font-bold text-gray-900">Rp
                <?php echo e(number_format($stats['fuel_month'], 0, ',', '.')); ?></p>
        </div>
    </div>
    ...
    <?php if(count($alerts) > 0 || $upcomingMaint->isNotEmpty()): ?>
        <div
            ...
            <ul class="text-xs text-amber-600 space-y-0.5">
                ...
                <?php $__currentLoopData = $upcomingMaint; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <li>Maintenance <?php echo e($m->vehicle?->plate_number ?? '-'); ?>: <?php echo e($m->description); ?>

                        (<?php echo e($m->scheduled_date?->format('d/m')); ?>)
                    </li>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </ul>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $vehicles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 font-mono text-xs font-medium text-gray-900">
                                <?php echo e($v->plate_number); ?></td>
                            <td class="px-4 py-3 text-gray-700">
                                <?php echo e($v->name); ?>

                                <?php if($v->brand): ?>
                                    <span class="text-xs text-gray-400">(<?php echo e($v->brand); ?>)</span>
                                <?php endif; ?>
                            </td>
                            <td
                            ...
                            <td class="px-4 py-3 text-right hidden md:table-cell text-gray-900">
                                <?php echo e(number_format($v->odometer, 0, ',', '.')); ?> km</td>
                            <td class="px-4 py-3 text-center">
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs bg-<?php echo e($sc); ?>-100 text-<?php echo e($sc); ?>-700 $sc }}-500/20 $sc }}-400"><?php echo e($sl); ?></span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <?php if(auth()->check() && app(\App\Services\PermissionService::class)->check(auth()->user(), 'fleet', 'edit')): ?>
                                    <button onclick='openEdit(<?php echo json_encode($v, 15, 512) ?>)'
                                    ...
                                    <form method="POST" action="<?php echo e(route('fleet.vehicles.destroy', $v)); ?>"
        ...
        <?php if($vehicles->hasPages()): ?>
            <div class="px-4 py-3 border-t border-gray-100"><?php echo e($vehicles->links()); ?></div>
        <?php endif; ?>
    </div>
        ...
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" action="<?php echo e(route('fleet.vehicles.store')); ?>" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">No. Plat
                            *</label><input type="text" name="plate_number" required placeholder="B 1234 XYZ"
                            class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Nama
                            *</label><input type="text" name="name" required placeholder="Toyota Avanza 2024"
                            class="<?php echo e($cls); ?>"></div>
                    ...
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Merek</label><input
                            type="text" name="brand" placeholder="Toyota" class="<?php echo e($cls); ?>"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Model</label><input
                            type="text" name="model" placeholder="Avanza" class="<?php echo e($cls); ?>"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Tahun</label><input
                            type="number" name="year" min="1990" max="2030"
                            class="<?php echo e($cls); ?>"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Warna</label><input
                            type="text" name="color" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Odometer
                            (km)</label><input type="number" name="odometer" min="0" value="0"
                            class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">STNK
                            Expired</label><input type="date" name="registration_expiry"
                            class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Asuransi
                            Expired</label><input type="date" name="insurance_expiry"
                            class="<?php echo e($cls); ?>"></div>
                    <?php if($assets->isNotEmpty()): ?>
                        <div class="sm:col-span-2"><label
                                class="block text-xs font-medium text-gray-600 mb-1">Link ke
                                Aset</label>
                            <select name="asset_id" class="<?php echo e($cls); ?>">
                                ...
                                <?php $__currentLoopData = $assets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($a->id); ?>"><?php echo e($a->asset_code); ?> — <?php echo e($a->name); ?>
                    ...
                    <?php endif; ?>
                    <div class="sm:col-span-2"><label
                            class="block text-xs font-medium text-gray-600 mb-1">Catatan</label><input
                            type="text" name="notes" class="<?php echo e($cls); ?>"></div>
                </div>
    ...
    <div id="modal-edit" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form id="form-edit" method="POST" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">No. Plat
                            *</label><input type="text" name="plate_number" id="e-plate" required
                            class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Nama
                            *</label><input type="text" name="name" id="e-name" required
                            class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Tipe *</label>
                    ...
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Merek</label><input
                            type="text" name="brand" id="e-brand" class="<?php echo e($cls); ?>"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Model</label><input
                            type="text" name="model" id="e-model" class="<?php echo e($cls); ?>"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Odometer</label><input
                            type="number" name="odometer" id="e-odo" min="0"
                            class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">STNK
                            Expired</label><input type="date" name="registration_expiry" id="e-reg"
                            class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Asuransi
                            Expired</label><input type="date" name="insurance_expiry" id="e-ins"
                            class="<?php echo e($cls); ?>"></div>
                </div>
                <div class="flex justify-end gap-3">
