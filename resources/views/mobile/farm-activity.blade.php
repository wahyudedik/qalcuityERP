...
<head>
    ...
    <title>Catat Aktivitas Lahan — {{ config('app.name') }}</title>
...
<body class="bg-gray-950 min-h-screen text-white">
    ...
    <div x-data="{
        ...
        {{-- ── FORM ── --}}
        <form method="POST" action="{{ route('mobile.farm-activity.store') }}" enctype="multipart/form-data"
            id="farmActivityForm">
            @csrf
            ...
            <div class="px-4 pt-5 space-y-5">
                ...
                <div class="bg-[#1e293b] rounded-2xl border border-white/10 p-4">
                    ...
                    <div x-show="targetType === 'plot'" x-transition>
                        @php $plots = $plots ?? collect(); @endphp
                        @if ($plots->isEmpty())
                            ...
                            <div class="space-y-2 max-h-64 overflow-y-auto pr-1">
                                @foreach ($plots as $plot)
                                    <div class="target-card flex items-center gap-3 p-3 rounded-xl border cursor-pointer min-h-[56px]"
                                        :class="selectedTarget === '{{ $plot->id }}'
                                            ?
                                            'border-blue-500/70 bg-blue-600/10 ring-2 ring-blue-500' :
                                            'border-white/10 bg-gray-900/40 hover:border-white/20'"
                                        @click="selectTarget('{{ $plot->id }}')">
                                        <div
                                            class="w-10 h-10 rounded-lg bg-emerald-600/20 flex items-center justify-center flex-shrink-0">
                                            <span class="text-lg">🌿</span>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <div class="font-semibold text-sm text-white">{{ $plot->code }} —
                                                {{ $plot->name }}</div>
                                            <div class="text-xs text-slate-400 flex items-center gap-2 mt-0.5">
                                                <span>{{ number_format($plot->area_size, 1) }}
                                                    {{ $plot->area_unit ?? 'ha' }}</span>
                                                @if ($plot->current_crop)
                                                    <span class="text-slate-500">•</span>
                                                    <span>{{ $plot->current_crop }}</span>
                                                @endif
                                            </div>
                                        </div>
                                        <div x-show="selectedTarget === '{{ $plot->id }}'" class="flex-shrink-0">
                                            <div
                    ...
                    <div x-show="targetType === 'livestock'" x-transition>
                        @php $livestockGroups = $livestockGroups ?? collect(); @endphp
                        ...
                        @else
                            <div class="space-y-2 max-h-64 overflow-y-auto pr-1">
                                @foreach ($livestockGroups as $herd)
                                    @php
                                        $animalEmoji = explode(
                                            ' ',
                                    ...
                                    @endphp
                                    <div class="target-card flex items-center gap-3 p-3 rounded-xl border cursor-pointer min-h-[56px]"
                                        :class="selectedTarget === 'livestock_{{ $herd->id }}'
                                            ?
                                            'border-blue-500/70 bg-blue-600/10 ring-2 ring-blue-500' :
                                            'border-white/10 bg-gray-900/40 hover:border-white/20'"
                                        @click="selectTarget('livestock_{{ $herd->id }}')">
                                        <div
                                            class="w-10 h-10 rounded-lg bg-amber-600/20 flex items-center justify-center flex-shrink-0">
                                            <span class="text-lg">{{ $animalEmoji }}</span>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <div class="font-semibold text-sm text-white">{{ $herd->code }} —
                                                {{ $herd->name }}</div>
                                            <div class="text-xs text-slate-400 flex items-center gap-2 mt-0.5">
                                                <span>{{ number_format($herd->current_count) }} ekor</span>
                                                <span class="text-slate-500">•</span>
                                                <span>{{ $herd->animal_type }}</span>
                                            </div>
                                        </div>
                                        <div x-show="selectedTarget === 'livestock_{{ $herd->id }}'"
                                            class="flex-shrink-0">
                                            <div
                                                class="w-5 h-5 rounded-full bg-blue-500 flex items-center justify-center">
                                                <svg class="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24"
                ...
                <div class="bg-[#1e293b] rounded-2xl border border-white/10 p-4 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="activity_date" value="{{ now()->format('Y-m-d\TH:i') }}"
                ...
                <div class="bg-[#1e293b] rounded-2xl border border-white/10 p-4">
                    ...
                    {{-- Additional photo slots (2nd & 3rd) --}}
