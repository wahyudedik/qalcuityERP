<x-app-layout>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-center text-xs text-gray-500">{{ ['service'=>'Jasa','lease'=>'Sewa','supply'=>'Supply','maintenance'=>'Maintenance','subscription'=>'Langganan'][$t->category] ?? $t->category }}</td>
                        <td class="px-4 py-3 text-center">
                            @if($t->is_active)<span class="px-2 py-0.5 rounded-full text-xs bg-green-100 text-green-700">Aktif</span>
        ...
        @if($templates->hasPages())<div class="px-4 py-3 border-t border-gray-100">{{ $templates->links() }}</div>@endif
    </div>
    ...
    <div id="modal-add-tpl" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-xl">
            ...
            <form method="POST" action="{{ route('contracts.templates.store') }}" class="p-6 space-y-4">
                ...
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Nama *</label><input type="text" name="name" required placeholder="Template Kontrak Sewa" class="{{ $cls }}"></div>
                ...
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Body Template</label><textarea name="body_template" rows="4" placeholder="Isi template kontrak... Gunakan {customer_name}, {start_date}, {end_date}, {value}" class="{{ $cls }}"></textarea></div>
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Default Terms</label><textarea name="default_terms" rows="3" class="{{ $cls }}"></textarea></div>
