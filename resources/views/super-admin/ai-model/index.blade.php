<x-app-layout>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="sm:col-span-2 bg-[#1e293b] border border-white/10 rounded-2xl p-5 flex items-center gap-5">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-xl font-bold text-slate-100 font-mono truncate">{{ $activeModel }}</p>
            </div>
            <div class="shrink-0">
                <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border {{ $statusColor }}">
    ...
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl overflow-hidden mb-6">
        <div class="px-5 py-4 border-b border-white/10 flex items-center justify-between">
            ...
            <span class="text-xs text-slate-500">{{ count($modelAvailability) }} model dalam fallback chain</span>
        </div>
                        ...
                        <tr class="hover:bg-white/5 transition">
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-2">
                                    <span class="font-mono text-slate-200 text-xs">{{ $item['model'] }}</span>
                            ...
                            <td class="px-5 py-3.5">
                                <span class="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full {{ $rowStatusClass }}">
                            ...
                            <td class="px-5 py-3.5 hidden md:table-cell">
                                @if ($item['reason'])
                                    <span class="text-xs text-slate-400 font-mono">{{ $item['reason'] }}</span>
                                @else
                            ...
                            <td class="px-5 py-3.5 hidden lg:table-cell">
                                @if ($item['recovers_at'])
                                    <div>
                                        <p class="text-xs text-slate-300">{{ $item['recovers_at']->format('d M Y H:i:s') }}</p>
                                        <p class="text-[10px] text-slate-500 mt-0.5">{{ $item['recovers_at']->diffForHumans() }}</p>
                                    </div>
                                @else
                                    <span class="text-xs text-slate-600">—</span>
                                @endif
    ...
    <div class="bg-[#1e293b] border border-white/10 rounded-2xl overflow-hidden">
        <div class="px-5 py-4 border-b border-white/10 flex items-center justify-between">
            ...
            <span class="text-xs text-slate-500">{{ $switchLogs->total() }} total event</span>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead>
                ...
                <tbody class="divide-y divide-white/5">
                    @forelse($switchLogs as $log)
                        ...
                        <tr class="hover:bg-white/5 transition">
                            <td class="px-5 py-3.5">
                                <span class="font-mono text-slate-300 text-xs">{{ $log->from_model }}</span>
                            </td>
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <span class="font-mono text-slate-200 text-xs">{{ $log->to_model }}</span>
                                </div>
                            </td>
                            <td class="px-5 py-3.5 hidden md:table-cell">
                            ...
                            <td class="px-5 py-3.5 hidden lg:table-cell">
                                <p class="text-xs text-slate-300">{{ $log->switched_at->format('d M Y') }}</p>
                                <p class="text-[10px] text-slate-500 mt-0.5">{{ $log->switched_at->format('H:i:s') }}</p>
                            </td>
                            <td class="px-5 py-3.5 hidden xl:table-cell">
                                @if ($log->triggered_by_tenant_id)
                                    <span class="text-xs font-mono text-slate-400">{{ $log->triggered_by_tenant_id }}</span>
                                @else
                                    <span class="text-xs text-slate-600">—</span>
                                @endif
                            </td>
