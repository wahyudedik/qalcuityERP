...
{{-- ── Trigger Button ──────────────────────────────────────────────────────── --}}
<div class="barcode-scanner-wrapper" data-scanner-id="{{ $componentId }}">

    <button type="button" onclick="openBarcodeScanner_{{ $componentId }}()"
        class="inline-flex items-center gap-2 min-h-[44px] px-4 py-2.5 bg-blue-600 hover:bg-blue-500 active:scale-95 text-white text-sm font-medium rounded-xl transition select-none touch-manipulation {{ $buttonClass }}">
    ...
    @if ($showManualInput)
        <div class="flex gap-2 mt-2">
            <input type="text" id="manualBarcode_{{ $componentId }}" placeholder="{{ $inputPlaceholder }}"
                autocomplete="off"
                ...
                onkeydown="if(event.key==='Enter'){event.preventDefault();submitManualBarcode_{{ $componentId }}();}">
            <button type="button" onclick="submitManualBarcode_{{ $componentId }}()"
...
{{-- ── Camera Modal ────────────────────────────────────────────────────────── --}}
<div id="scannerModal_{{ $componentId }}"
    class="scanner-modal-{{ $componentId }} fixed inset-0 bg-gray-900/95 z-[9999] hidden items-center justify-center p-4"
    onclick="(function(e){if(e.target===e.currentTarget)closeBarcodeScanner_{{ $componentId }}();})(event)">
    ...
    <div class="bg-gray-900 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl border border-gray-800">
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-gray-800">
            ...
            <button type="button" onclick="closeBarcodeScanner_{{ $componentId }}()"
        ...
        <div class="relative bg-black" style="aspect-ratio:4/3">
            <video id="scannerVideo_{{ $componentId }}" autoplay playsinline muted class="w-full h-full object-cover">
            </video>
            ...
            <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div class="relative w-56 h-36">
                    ...
                    {{-- Animated scan line --}}
                    <div class="scanner-line-{{ $componentId }} absolute left-1 right-1 h-0.5 bg-blue-400/80 rounded shadow-[0_0_6px_#60a5fa]"
                        style="top:10%"></div>
                </div>
            </div>

            {{-- Status text --}}
            <div id="scannerStatus_{{ $componentId }}" class="absolute bottom-3 left-0 right-0 text-center">
        ...
        <div class="px-4 pt-1.5 pb-4 border-t border-gray-800 mt-2">
            ...
            <div class="flex gap-2">
                <input type="text" id="modalManualBarcode_{{ $componentId }}" placeholder="Masukkan barcode..."
                    autocomplete="off"
                    ...
                    onkeydown="if(event.key==='Enter'){event.preventDefault();submitModalManualBarcode_{{ $componentId }}();}">
                <button type="button" onclick="submitModalManualBarcode_{{ $componentId }}()"
...
{{-- ── Styles ──────────────────────────────────────────────────────────────── --}}
<style>
    @keyframes scanLine_{{ $componentId }} {
        0% {
            top: 10%;
        }
    ...
    .scanner-line-{{ $componentId }} {
        animation: scanLine_{{ $componentId }} 1.8s ease-in-out infinite;
    }

    #scannerModal_{{ $componentId }}.flex {
        display: flex !important;
    }

    /* Prevent body scroll when modal is open */
    body.scanner-open-{{ $componentId }} {
        overflow: hidden;
    }

    /* Responsive: fullscreen on small screens */
    @media (max-width: 480px) {
        #scannerModal_{{ $componentId }} .bg-gray-900.rounded-2xl {
            border-radius: 0;
            max-width: 100%;
            height: 100dvh;
            display: flex;
        ...
        #scannerModal_{{ $componentId }} .relative.bg-black {
            flex: 1;
            aspect-ratio: unset;
        }

        #scannerModal_{{ $componentId }} video {
            height: 100%;
        }
    }
</style>
