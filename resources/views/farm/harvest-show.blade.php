        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <p class="font-bold text-gray-900 dark:text-white">{{ $harvestLog->crop_name }} — Lahan {{ $harvestLog->plot?->code }}</p>
                        <p class="text-xs text-gray-500 dark:text-slate-400">{{ $harvestLog->harvest_date->format('d M Y') }} · oleh {{ $harvestLog->user?->name }}</p>
                    </div>
                </div>
                <div class="grid grid-cols-2 sm:grid-cols-5 gap-4">
                    <div><p class="text-xs text-gray-500 dark:text-slate-400">Total</p><p class="text-lg font-bold text-emerald-600">{{ number_format($harvestLog->total_qty, 0) }} {{ $harvestLog->unit }}</p></div>
                    <div><p class="text-xs text-gray-500 dark:text-slate-400">Bersih</p><p class="text-lg font-bold text-gray-900 dark:text-white">{{ number_format($harvestLog->netQty(), 0) }} {{ $harvestLog->unit }}</p></div>
                    <div><p class="text-xs text-gray-500 dark:text-slate-400">Reject</p><p class="text-lg font-bold {{ $harvestLog->reject_qty > 0 ? 'text-red-500' : 'text-gray-400' }}">{{ number_format($harvestLog->reject_qty, 0) }} ({{ $harvestLog->rejectPercent() }}%)</p></div>
                    <div><p class="text-xs text-gray-500 dark:text-slate-400">Biaya</p><p class="text-lg font-bold text-gray-900 dark:text-white">Rp {{ number_format($harvestLog->totalCost(), 0, ',', '.') }}</p></div>
                    <div><p class="text-xs text-gray-500 dark:text-slate-400">HPP/{{ $harvestLog->unit }}</p><p class="text-lg font-bold text-gray-900 dark:text-white">{{ $harvestLog->costPerUnit() ? 'Rp '.number_format($harvestLog->costPerUnit(), 0, ',', '.') : '-' }}</p></div>
                </div>
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr>
                            <td class="px-4 py-2 font-medium text-gray-900 dark:text-white">{{ $g->grade }}</td>
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100 dark:border-white/10">
                    <h3 class="font-semibold text-gray-900 dark:text-white">Pekerja Panen ({{ $harvestLog->workers->count() }})</h3>
                </div>
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 dark:bg-white/5 text-xs text-gray-500 uppercase">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr>
                            <td class="px-4 py-2 text-gray-900 dark:text-white">{{ $w->worker_name }}</td>
        ...
        <div>
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    @if($harvestLog->notes)<div class="pt-2 border-t border-gray-100 dark:border-white/10"><p class="text-xs text-gray-500">{{ $harvestLog->notes }}</p></div>@endif
                </div>
            </div>
        </div>
    </div>
