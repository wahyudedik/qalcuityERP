<x-app-layout>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            ...
            <p class="text-xl font-bold text-gray-900">Rp
                {{ number_format($monthlySummary->total_cost ?? 0, 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">Total Liter</p>
            <p class="text-xl font-bold text-blue-500">
                {{ number_format($monthlySummary->total_liters ?? 0, 1, ',', '.') }} L</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        @if ($fuelLogs->hasPages())
            <div class="px-4 py-3 border-t border-gray-100">{{ $fuelLogs->links() }}</div>
        @endif
    </div>
            ...
            <form method="POST" action="{{ route('fleet.fuel-logs.store') }}" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Tanggal
                            *</label><input type="date" name="date" required value="{{ date('Y-m-d') }}"
                            class="{{ $cls }}"></div>
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Harga/Liter
                            *</label><input type="number" name="price_per_liter" required min="0" step="100"
                            value="13900" class="{{ $cls }}"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">SPBU</label><input
                            type="text" name="station" placeholder="SPBU 34.xxx" class="{{ $cls }}">
                    </div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">No.
                            Struk</label><input type="text" name="receipt_number" class="{{ $cls }}">
                    </div>
                </div>
