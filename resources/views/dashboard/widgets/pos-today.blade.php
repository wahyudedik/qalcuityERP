<div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5 h-full">
    ...
    <p class="text-2xl font-bold text-gray-900 dark:text-white">Rp {{ number_format($data['revenue'] ?? 0, 0, ',', '.') }}</p>
    <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">{{ $data['count'] ?? 0 }} transaksi · Avg Rp {{ number_format($data['avg_ticket'] ?? 0, 0, ',', '.') }}</p>
</div>
