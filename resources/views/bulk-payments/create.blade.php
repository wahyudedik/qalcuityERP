...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
        <a href="{{ route('bulk-payments.index') }}" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </a>
        <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Buat Bulk Payment</h1>
    </div>
    ...
    <form method="POST" action="{{ route('bulk-payments.store') }}" id="bpForm">
        @csrf
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 space-y-5">
            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                    ...
                    <select name="customer_id" id="customerSelect" required onchange="loadInvoices()"
                        ...
                        <option value="{{ $c->id }}" {{ old('customer_id', $selectedCustomer?->id) == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Tanggal Pembayaran <span class="text-red-500">*</span></label>
                ...
                <div class="col-span-2">
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Total Pembayaran Diterima <span class="text-red-500">*</span></label>
                    <input type="number" name="total_amount" id="totalAmount" min="1" step="1" required
                           value="{{ old('total_amount') }}" onchange="calcOverpayment()"
                           class="w-full px-3 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
            ...
            {{-- Invoice List --}}
            <div>
                <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">Invoice yang Dibayar</h3>
                <div id="invoiceList" class="space-y-2">
                    @if($pendingInvoices->isNotEmpty())
                        ...
                        <div class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                            <input type="checkbox" name="invoices[{{ $loop->index }}][invoice_id]" value="{{ $inv->id }}"
                                   class="invoice-check w-4 h-4 text-blue-600 rounded" onchange="calcOverpayment()">
                            <input type="hidden" name="invoices[{{ $loop->index }}][invoice_id]" value="{{ $inv->id }}" disabled class="inv-id-hidden">
                            <div class="flex-1">
                                <p class="text-sm font-mono font-medium text-slate-800 dark:text-white">{{ $inv->number }}</p>
                                <p class="text-xs text-slate-500">Jatuh tempo: {{ $inv->due_date->format('d/m/Y') }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-xs text-slate-500">Sisa tagihan</p>
                                <p class="text-sm font-medium text-slate-800 dark:text-white">Rp {{ number_format($inv->remaining_amount, 0, ',', '.') }}</p>
                            </div>
                            <div class="w-36">
                                <input type="number" name="invoices[{{ $loop->index }}][amount]"
                                       class="inv-amount w-full px-2 py-1.5 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                                       min="0" step="1" value="{{ $inv->remaining_amount }}" max="{{ $inv->remaining_amount }}"
                    ...
                    @else
                        <p class="text-sm text-slate-400 dark:text-slate-500 text-center py-6">
                            {{ $selectedCustomer ? 'Tidak ada invoice outstanding untuk customer ini.' : 'Pilih customer untuk melihat invoice outstanding.' }}
                        </p>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-4 space-y-2">
                <div class="flex justify-between text-sm">
                    <span class="text-slate-600 dark:text-slate-400">Total Diterapkan ke Invoice</span>
                    <span id="totalApplied" class="font-medium text-slate-800 dark:text-white">Rp 0</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-slate-600 dark:text-slate-400">Total Pembayaran</span>
                    <span id="totalPaymentDisplay" class="font-medium text-slate-800 dark:text-white">Rp 0</span>
                </div>
                <div class="flex justify-between text-sm border-t border-slate-200 dark:border-slate-600 pt-2">
                    <span class="text-slate-600 dark:text-slate-400">Overpayment → Saldo Customer</span>
                    <span id="overpaymentDisplay" class="font-medium text-amber-600">Rp 0</span>
                </div>
            </div>

            <div class="flex justify-end gap-3 pt-2">
                <a href="{{ route('bulk-payments.index') }}"
...
function loadInvoices() {
    ...
    window.location.href = `{{ route('bulk-payments.create') }}?customer_id=${customerId}`;
}
