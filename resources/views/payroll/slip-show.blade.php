            ...
            <div id="slip-print"
                ...
                <div class="flex items-start justify-between border-b-2 border-gray-800 dark:border-white/20 pb-5 mb-6 print:border-gray-800">
                    <div>
                        ...
                        @if($profile?->address)
                        <p class="text-xs text-gray-500 dark:text-slate-400 print:text-gray-600 mt-0.5">{{ $profile->address }}</p>
                        @endif
                    </div>
                    <div class="text-right">
                        ...
                        @endphp
                        <p class="text-xl font-black text-blue-600 dark:text-blue-400 print:text-blue-700 capitalize">{{ $monthName }}</p>
                ...
                <div class="grid grid-cols-2 gap-x-8 gap-y-1.5 mb-6 text-sm">
                    ...
                    <div class="flex gap-2">
                        ...
                        <span class="text-gray-900 dark:text-white print:text-black font-medium">: {{ $value }}</span>
                    </div>
                    @endforeach
                </div>
                        ...
                        <table class="w-full text-sm">
                            <tbody class="divide-y divide-gray-100 dark:divide-white/5 print:divide-gray-200">
                                <tr>
                                    ...
                                    <td class="py-1.5 text-right font-medium text-gray-900 dark:text-white print:text-black">Rp {{ number_format($item->base_salary, 0, ',', '.') }}</td>
                                </tr>
                                @php $compAllowances = $item->components->where('type','allowance'); @endphp
                                @if($compAllowances->count())
                                    @foreach($compAllowances as $c)
                                    <tr>
                                        <td class="py-1.5 text-gray-600 dark:text-slate-300 print:text-gray-700">{{ $c->name }}</td>
                                        <td class="py-1.5 text-right font-medium text-gray-900 dark:text-white print:text-black">Rp {{ number_format($c->amount, 0, ',', '.') }}</td>
                                    </tr>
                                    @endforeach
                                @elseif(($item->allowances ?? 0) > 0)
                                <tr>
                                    <td class="py-1.5 text-gray-600 dark:text-slate-300 print:text-gray-700">Tunjangan</td>
                                    <td class="py-1.5 text-right font-medium text-gray-900 dark:text-white print:text-black">Rp {{ number_format($item->allowances, 0, ',', '.') }}</td>
                                </tr>
                                @endif
                                ...
                                <tr>
                                    ...
                                    <td class="py-1.5 text-right font-medium text-green-600 dark:text-green-400 print:text-green-700">+Rp {{ number_format($item->overtime_pay, 0, ',', '.') }}</td>
                                </tr>
                                @endif
                    ...
                    <div>
                        ...
                        <table class="w-full text-sm">
                            <tbody class="divide-y divide-gray-100 dark:divide-white/5 print:divide-gray-200">
                                ...
                                <tr>
                                    <td class="py-1.5 text-gray-600 dark:text-slate-300 print:text-gray-700">Potongan Absen ({{ $item->absent_days }}h)</td>
                                    <td class="py-1.5 text-right font-medium text-red-600 dark:text-red-400 print:text-red-700">-Rp {{ number_format($item->deduction_absent, 0, ',', '.') }}</td>
                                </tr>
                                @endif
                                @if(($item->deduction_late ?? 0) > 0)
                                <tr>
                                    <td class="py-1.5 text-gray-600 dark:text-slate-300 print:text-gray-700">Potongan Terlambat ({{ $item->late_days }}h)</td>
                                    <td class="py-1.5 text-right font-medium text-red-600 dark:text-red-400 print:text-red-700">-Rp {{ number_format($item->deduction_late, 0, ',', '.') }}</td>
                                </tr>
                                @endif
                                ...
                                <tr>
                                    ...
                                    <td class="py-1.5 text-right font-medium text-red-600 dark:text-red-400 print:text-red-700">-Rp {{ number_format($item->tax_pph21, 0, ',', '.') }}</td>
                                </tr>
                                @endif
                                ...
                                @if($compDeductions->count())
                                    ...
                                    <tr>
                                        <td class="py-1.5 text-gray-600 dark:text-slate-300 print:text-gray-700">{{ $c->name }}</td>
                                        <td class="py-1.5 text-right font-medium text-red-600 dark:text-red-400 print:text-red-700">-Rp {{ number_format($c->amount, 0, ',', '.') }}</td>
                                    </tr>
                                    @endforeach
                                @else
                                <tr>
                                    <td class="py-1.5 text-gray-600 dark:text-slate-300 print:text-gray-700">Potongan Lain</td>
                                    <td class="py-1.5 text-right font-medium text-red-600 dark:text-red-400 print:text-red-700">-Rp {{ number_format($item->deduction_other, 0, ',', '.') }}</td>
                                </tr>
                                @endif
                                @endif
                                ...
                                <tr class="border-t-2 border-gray-300 dark:border-white/20 print:border-gray-400">
                                    ...
                                    <td class="py-2 text-right font-bold text-red-600 dark:text-red-400 print:text-red-700">-Rp {{ number_format($totalDeduct, 0, ',', '.') }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
