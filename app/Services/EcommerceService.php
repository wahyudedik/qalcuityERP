    ...
    private function importShopeeOrders(EcommerceChannel $channel, array $orders): int
    {
        ...
        foreach ($orders as $order) {
            ...
            EcommerceOrder::create([
                ...
                'ordered_at' => isset($order['create_time']) ? Carbon::createFromTimestamp($order['create_time']) : now(),
                'synced_at' => now(),
            ]);
    ...
    private function importTokopediaOrders(EcommerceChannel $channel, array $orders): int
    {
        ...
        foreach ($orders as $order) {
            ...
            EcommerceOrder::create([
                ...
                'ordered_at' => isset($order['create_time']) ? Carbon::parse($order['create_time']) : now(),
                'synced_at' => now(),
            ]);
        ...
        try {
            ...
            foreach ($orders as $order) {
                ...
                EcommerceOrder::create([
                    ...
                    'ordered_at' => isset($order['created_at']) ? Carbon::parse($order['created_at']) : now(),
                    'synced_at' => now(),
                ]);
