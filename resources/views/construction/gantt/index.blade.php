    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-6">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">Project Number: {{ $project->number }}</p>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ route('construction.gantt.export', $project->id) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
                ...
                <div class="text-2xl font-bold text-blue-700">{{ $ganttData['timeline']['total_days'] }} days</div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
                <div class="text-sm text-gray-600">Elapsed</div>
                <div class="text-2xl font-bold text-green-700">{{ $ganttData['timeline']['elapsed_days'] }} days</div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-yellow-500">
                <div class="text-sm text-gray-600">Remaining</div>
                <div class="text-2xl font-bold text-yellow-700">{{ $ganttData['timeline']['remaining_days'] }} days</div>
            </div>
            <div
                class="bg-white rounded-lg shadow p-4 border-l-4 {{ $ganttData['timeline']['is_overdue'] ? 'border-red-500' : 'border-purple-500' }}">
                <div class="text-sm text-gray-600">Status</div>
                <div
                    class="text-2xl font-bold {{ $ganttData['timeline']['is_overdue'] ? 'text-red-700' : 'text-purple-700' }}">
                    {{ $ganttData['timeline']['is_overdue'] ? 'OVERDUE' : 'On Track' }}
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            ...
            <div class="p-6">
                ...
                <div class="space-y-4">
                    @forelse($ganttData['tasks'] as $task)
                        <div class="relative">
                            <div class="flex items-center mb-2">
                                <div class="w-1/3 pr-4">
                                    ...
                                    <div class="text-xs text-gray-500 ml-7">
                                        {{ $task['assigned_to'] }} • Weight: {{ $task['weight'] }}%
                                    </div>
                                </div>
                                <div class="w-2/3">
                                    ...
                                    <div class="flex justify-between text-xs text-gray-500 mt-1">
                                        ...
                                        <span>{{ $task['end'] ?? 'No deadline' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        @if (!empty($ganttData['critical_path']))
            <div class="mt-6 bg-white rounded-lg shadow overflow-hidden">
                ...
                <div class="p-6">
                    <div class="space-y-3">
                        @foreach ($ganttData['critical_path'] as $task)
                            <div
                                ...
                                <div>
                                    ...
                                    <div class="text-sm text-gray-600">Weight: {{ $task['weight'] }}% • Due:
                                        {{ $task['due_date'] ?? 'N/A' }}</div>
                                </div>
                                <div class="text-right">
                                    <div class="text-2xl font-bold text-yellow-700">{{ $task['progress'] }}%</div>
