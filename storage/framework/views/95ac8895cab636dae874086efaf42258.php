...
<?php endif; ?>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1"><?php echo e(number_format($statistics['total_items'])); ?></p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            <p class="text-xs text-gray-500 dark:text-slate-400">Stok Menipis</p>
            <p class="text-2xl font-bold text-amber-600 dark:text-amber-400 mt-1"><?php echo e($statistics['low_stock']); ?></p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            <p class="text-xs text-gray-500 dark:text-slate-400">Resep Pending</p>
            <p class="text-2xl font-bold text-blue-600 dark:text-blue-400 mt-1"><?php echo e($statistics['pending_prescriptions']); ?></p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            <p class="text-xs text-gray-500 dark:text-slate-400">Selesai Hari Ini</p>
            <p class="text-2xl font-bold text-green-600 dark:text-green-400 mt-1"><?php echo e($statistics['today_dispensed']); ?></p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            <p class="text-xs text-gray-500 dark:text-slate-400">Terverifikasi</p>
            <p class="text-2xl font-bold text-purple-600 dark:text-purple-400 mt-1"><?php echo e($statistics['verified_prescriptions']); ?></p>
        </div>
    </div>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $lowStockItemsList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800">
                        <div class="flex items-center justify-between mb-2">
                            <div>
                                <p class="font-bold text-gray-900 dark:text-white"><?php echo e($item->name); ?></p>
                                <p class="text-xs text-gray-500 dark:text-slate-400"><?php echo e($item->generic_name ?? '-'); ?> |
                        ...
                        <?php if($item->reorder_level > 0): ?>
                            ...
                            <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">Minimum: <?php echo e($item->reorder_level); ?>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            ...
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-lg
                                    <?php if(($activity->type ?? '') === 'dispensed'): ?> bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400
                                    <?php elseif(($activity->type ?? '') === 'received'): ?> bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400
                                    <?php elseif(($activity->type ?? '') === 'returned'): ?> bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400
