...
class LeaveApprovedNotification extends Notification implements ShouldQueue
{
    ...
    public function via(object $notifiable): array
    {
        ...
        if (NotificationPreference::isEnabled($notifiable->id, 'leave_approved', 'in_app')) {
            $channels[] = 'database';
        }
        if (NotificationPreference::isEnabled($notifiable->id, 'leave_approved', 'email')) {
            $channels[] = 'mail';
        }
        if (NotificationPreference::isEnabled($notifiable->id, 'leave_approved', 'push')) {
            $channels[] = 'broadcast';
        }
