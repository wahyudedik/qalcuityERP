    ...
    <div x-data="detailModal()" x-cloak>
        ...
        ['tab' => 'modules', 'label' => 'Module Health', 'icon' => 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10'],
              ...
              {{ $tab === $t['tab'] ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}">
                    ...
                    @if ($t['tab'] === 'errors' && $errorStats['unresolved'] > 0)
                        <span
                            class="text-[10px] bg-red-500 text-white px-1.5 py-0.5 rounded-full font-bold">{{ $errorStats['unresolved'] }}</span>
                    @endif
                    @if ($t['tab'] === 'anomaly' && $anomalyStats['critical'] > 0)
                        <span
                            class="text-[10px] bg-red-500 text-white px-1.5 py-0.5 rounded-full font-bold">{{ $anomalyStats['critical'] }}</span>
                    @endif
                </a>
            @endforeach
        </div>
                ...
                <div class="overflow-x-auto">
                    ...
                    'tenant_id'=>$err->tenant_id,'user_id'=>$err->user_id,
                                    ...
                                    <td class="px-4 py-3 hidden md:table-cell">
                                        <p class="text-slate-400 text-xs truncate max-w-[180px]">{{ $err->method }}
                                            {{ $err->url }}</p>
                                        @if ($err->ip)
                                            <p class="text-slate-500 text-[10px]">{{ $err->ip }}</p>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 hidden lg:table-cell">
                                        <p class="text-slate-400 text-xs">{{ $err->created_at->format('d M Y') }}</p>
                                        <p class="text-slate-500 text-[10px]">{{ $err->created_at->format('H:i:s') }}
                                        </p>
                                    </td>
                                    <td class="px-4 py-3">
                                        @if ($err->is_resolved)
        ...
        @if ($tab === 'ai')
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
                @foreach ([['label' => 'Pesan Bulan Ini', 'value' => number_format($aiStats['total_this_month']), 'color' => 'text-blue-400'], ['label' => 'Total Token', 'value' => number_format($aiStats['total_tokens']), 'color' => 'text-purple-400'], ['label' => 'Tenant Aktif AI', 'value' => $aiStats['active_tenants'], 'color' => 'text-green-400']] as $s)
                ...
                <div class="flex items-end gap-2 h-24">
                    ...
                    @foreach ($aiMonthly->reverse() as $m)
                        ...
                        <div class="flex-1 flex flex-col items-center gap-1">
                            ...
                            <span
                                class="text-[10px] text-slate-500">{{ \Carbon\Carbon::createFromFormat('Y-m', $m->month)->format('M') }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
            <div class="bg-[#1e293b] border border-white/10 rounded-2xl overflow-hidden">
                <div class="px-5 py-4 border-b border-white/10">
                    <p class="text-sm font-semibold text-slate-200">Penggunaan per Tenant — {{ now()->format('F Y') }}
                    </p>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                    ...
                    'breakdown' => $breakdown,
                                    ...
                                    <td class="px-5 py-3 hidden md:table-cell">
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span
                                                class="text-[10px] text-slate-500 w-8 text-right">{{ $pct }}%</span>
                                        </div>
                                    </td>
                            ...
                            @forelse($activities as $act)
                                ...
                                <tr class="hover:bg-white/5 transition cursor-pointer"
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        <p class="text-slate-500 text-[10px]">{{ $act->created_at->format('H:i:s') }}
                                        </p>
                                    </td>
        ...
        @if ($tab === 'anomaly')
            <div class="grid grid-cols-3 gap-3 mb-5">
                @foreach ([['label' => 'Open', 'value' => $anomalyStats['open'], 'color' => 'text-slate-300', 'bg' => 'bg-white/5'], ['label' => 'Critical', 'value' => $anomalyStats['critical'], 'color' => 'text-red-400', 'bg' => 'bg-red-500/10'], ['label' => 'Warning', 'value' => $anomalyStats['warning'], 'color' => 'text-amber-400', 'bg' => 'bg-amber-500/10']] as $s)
                    <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-4 text-center {{ $s['bg'] }}">
                        <p class="text-2xl font-bold {{ $s['color'] }}">{{ $s['value'] }}</p>
                        <p class="text-xs text-slate-500 mt-1">{{ $s['label'] }}</p>
                    </div>
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-1 flex-wrap">
                                    ...
                                    <span
                                        class="text-[11px] text-slate-500 bg-white/5 px-2 py-0.5 rounded-full">{{ $a->type }}</span>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
                ...
                <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full {{ $health['db_ok'] ? 'text-green-400 bg-green-500/15' : 'text-red-400 bg-red-500/15' }}">
                            {{ $health['db_ok'] ? 'Online' : 'Error' }}
                        </span>
                    </div>
                    <p class="text-2xl font-bold text-slate-100">{{ $health['db_latency_ms'] }} <span
                ...
                <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full {{ $health['disk_used_pct'] > 85 ? 'text-red-400 bg-red-500/15' : 'text-green-400 bg-green-500/15' }}">
                    ...
                    <p class="text-xs text-slate-500">{{ $health['disk_free_gb'] }} GB bebas dari
                ...
                <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full {{ $health['log_size_mb'] > 100 ? 'text-amber-400 bg-amber-500/15' : 'text-green-400 bg-green-500/15' }}">
                            {{ $health['log_size_mb'] > 100 ? 'Besar' : 'Normal' }}
                        </span>
                    </div>
                    <p class="text-2xl font-bold text-slate-100">{{ $health['log_size_mb'] }} <span
                ...
                <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs"><span class="text-slate-400">Total</span><span
                                class="text-slate-200 font-semibold">{{ $health['total_tenants'] }}</span></div>
                        <div class="flex justify-between text-xs"><span class="text-slate-400">Aktif</span><span
                                class="text-green-400 font-semibold">{{ $health['active_tenants'] }}</span></div>
                        <div class="flex justify-between text-xs"><span class="text-slate-400">Total User</span><span
                                class="text-slate-200 font-semibold">{{ $health['total_users'] }}</span></div>
                    </div>
                </div>
                ...
                <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full {{ $health['queue_failed'] > 0 ? 'text-red-400 bg-red-500/15' : 'text-green-400 bg-green-500/15' }}">
                            {{ $health['queue_failed'] > 0 ? $health['queue_failed'] . ' failed' : 'OK' }}
                        </span>
                    </div>
                    <p
                        class="text-2xl font-bold {{ $health['queue_failed'] > 0 ? 'text-red-400' : 'text-slate-100' }}">
                ...
                <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-5 cursor-pointer hover:border-white/20 transition"
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs"><span class="text-slate-400">PHP</span><span
                                class="text-slate-200 font-mono">{{ $health['php_version'] }}</span></div>
                        <div class="flex justify-between text-xs"><span class="text-slate-400">Laravel</span><span
                                class="text-slate-200 font-mono">{{ $health['laravel_version'] }}</span></div>
                        <div class="flex justify-between text-xs"><span class="text-slate-400">Uptime</span><span
                                class="text-slate-200 font-mono">{{ $health['uptime'] }}</span></div>
                        <div class="flex justify-between text-xs"><span class="text-slate-400">Error Hari
                                Ini</span><span
                                class="{{ $health['errors_today'] > 0 ? 'text-red-400' : 'text-green-400' }} font-semibold">{{ $health['errors_today'] }}</span>
                        </div>
                    </div>
                </div>
        ...
        @if ($tab === 'modules')
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                ...
                <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-4">
                    ...
                    <p
                        class="text-2xl font-black {{ $moduleHealth['total_alerts'] > 0 ? 'text-amber-400' : 'text-green-400' }}">
                        {{ $moduleHealth['total_alerts'] }}</p>
                </div>
                <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-4">
                    <p class="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Critical</p>
                    <p
                        class="text-2xl font-black {{ $moduleHealth['critical_alerts'] > 0 ? 'text-red-400' : 'text-green-400' }}">
                        {{ $moduleHealth['critical_alerts'] }}</p>
                </div>
                <div class="bg-[#1e293b] border border-white/10 rounded-2xl p-4">
                    <p class="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Status</p>
                    <p
                        class="text-2xl font-black {{ $moduleHealth['critical_alerts'] > 0 ? 'text-red-400' : ($moduleHealth['total_alerts'] > 0 ? 'text-amber-400' : 'text-green-400') }}">
                ...
                @foreach ($moduleHealth['modules'] as $key => $mod)
                    ...
                    <div class="{{ $bgColor }} border {{ $borderColor }} rounded-2xl p-4">
                        ...
                        <p class="text-xs text-slate-500 mb-2">Total records: <span
                                class="text-slate-300 font-medium">{{ number_format($mod['total']) }}</span></p>
                        @if ($hasAlerts)
                            <div class="space-y-1">
