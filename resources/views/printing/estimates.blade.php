<x-app-layout>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        @if ($estimates->count() === 0)
            <x-empty-state icon="document" title="Belum ada estimasi"
                message="Belum ada estimasi biaya cetak. Buat estimasi pertama Anda." actionText="Buat Estimasi" />
        @else
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($estimates as $estimate)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 text-gray-700">
                                    {{ ucfirst(str_replace('_', ' ', $estimate->product_type)) }}
                                </td>
                                <td class="px-6 py-4 text-gray-700">
                                    {{ number_format($estimate->quantity) }}
                                </td>
                                <td class="px-6 py-4 text-gray-700">
                                    Rp {{ number_format($estimate->total_cost ?? 0, 0, ',', '.') }}
                                </td>
                                <td class="px-6 py-4 font-semibold text-gray-900">
                                    Rp {{ number_format($estimate->quoted_price ?? 0, 0, ',', '.') }}
                                </td>
                                <td class="px-6 py-4">
                                    @php
                                        $statusColors = [
                                ...
                                <td class="px-6 py-4 text-gray-700">
                                    @if ($estimate->valid_until)
                                        @if ($estimate->is_expired)
                                            <span
                                                class="text-red-600">{{ $estimate->valid_until->format('d M Y') }}</span>
                                        @else
    ...
    <div id="estimateModal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div
                ...
                <form action="{{ route('printing.estimate.create') }}" method="POST">
                    @csrf
