    ...
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                <div>
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white"><?php echo e($ship->number); ?></h2>
                    <p class="text-sm text-gray-500 dark:text-slate-400">🏪 <?php echo e($ship->partner->name ?? '-'); ?> · <?php echo e($ship->warehouse->name ?? '-'); ?></p>
                </div>
                <?php
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                <div><p class="text-xs text-gray-500 dark:text-slate-400">Tanggal Kirim</p><p class="text-gray-900 dark:text-white"><?php echo e($ship->ship_date->format('d/m/Y')); ?></p></div>
                <div><p class="text-xs text-gray-500 dark:text-slate-400">Nilai HPP</p><p class="text-gray-900 dark:text-white">Rp <?php echo e(number_format($ship->total_cost, 0, ',', '.')); ?></p></div>
                <div><p class="text-xs text-gray-500 dark:text-slate-400">Nilai Retail</p><p class="font-semibold text-gray-900 dark:text-white">Rp <?php echo e(number_format($ship->total_retail, 0, ',', '.')); ?></p></div>
                <div><p class="text-xs text-gray-500 dark:text-slate-400">Komisi Partner</p><p class="text-gray-900 dark:text-white"><?php echo e($ship->partner->commission_pct ?? 0); ?>%</p></div>
            </div>
        </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right text-gray-700 dark:text-slate-300"><?php echo e(number_format($item->quantity_sent, 0, ',', '.')); ?></td>
                            ...
                            <td class="px-4 py-3 text-right text-gray-500 dark:text-slate-400">Rp <?php echo e(number_format($item->cost_price, 0, ',', '.')); ?></td>
                            <td class="px-4 py-3 text-right text-gray-900 dark:text-white">Rp <?php echo e(number_format($item->retail_price, 0, ',', '.')); ?></td>
                        </tr>
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
                ...
                <form method="POST" action="<?php echo e(route('consignment.sales-report.store', $ship)); ?>" class="space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Periode Mulai</label><input type="date" name="period_start" required class="<?php echo e($cls); ?>"></div>
                        <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Periode Akhir</label><input type="date" name="period_end" required value="<?php echo e(date('Y-m-d')); ?>" class="<?php echo e($cls); ?>"></div>
                    </div>
                    <?php $__currentLoopData = $ship->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if($item->remainingQty() > 0): ?>
                    <div class="flex items-center gap-3 text-sm">
                        <input type="hidden" name="items[<?php echo e($loop->index); ?>][item_id]" value="<?php echo e($item->id); ?>">
                        <span class="flex-1 text-gray-700 dark:text-slate-300"><?php echo e($item->product->name ?? '-'); ?> <span class="text-xs text-gray-400">(sisa <?php echo e(number_format($item->remainingQty(), 0)); ?>)</span></span>
            ...
            <div class="divide-y divide-gray-100 dark:divide-white/5">
                ...
                <div class="p-4">
                    <div class="flex items-center justify-between mb-2">
                        <div>
                            <span class="font-mono text-xs font-medium text-gray-900 dark:text-white"><?php echo e($rpt->number); ?></span>
                            <span class="text-xs text-gray-500 dark:text-slate-400 ml-2"><?php echo e($rpt->period_start->format('d/m')); ?> — <?php echo e($rpt->period_end->format('d/m/Y')); ?></span>
                        </div>
                        <?php $rc = ['draft'=>'gray','confirmed'=>'blue','settled'=>'green'][$rpt->status] ?? 'gray'; ?>
                        <span class="px-2 py-0.5 rounded-full text-xs bg-<?php echo e($rc); ?>-100 text-<?php echo e($rc); ?>-700 dark:bg-<?php echo e($rc); ?>-500/20 dark:text-<?php echo e($rc); ?>-400"><?php echo e(ucfirst($rpt->status)); ?></span>
                    </div>
                    <div class="grid grid-cols-4 gap-3 text-xs mb-2">
                        <div><span class="text-gray-500 dark:text-slate-400">Penjualan:</span> <span class="text-gray-900 dark:text-white">Rp <?php echo e(number_format($rpt->total_sales, 0, ',', '.')); ?></span></div>
                        <div><span class="text-gray-500 dark:text-slate-400">Komisi (<?php echo e($rpt->commission_pct); ?>%):</span> <span class="text-red-500">Rp <?php echo e(number_format($rpt->commission_amount, 0, ',', '.')); ?></span></div>
                        <div><span class="text-gray-500 dark:text-slate-400">Net:</span> <span class="font-semibold text-gray-900 dark:text-white">Rp <?php echo e(number_format($rpt->net_receivable, 0, ',', '.')); ?></span></div>
                        <div><span class="text-gray-500 dark:text-slate-400">Sisa:</span> <span class="<?php echo e($rpt->remainingBalance() > 0 ? 'text-amber-500' : 'text-green-500'); ?>">Rp <?php echo e(number_format($rpt->remainingBalance(), 0, ',', '.')); ?></span></div>
                    </div>
                    ...
                    <form method="POST" action="<?php echo e(route('consignment.settlement.store', $rpt)); ?>" class="flex items-end gap-2 mt-2">
                        <?php echo csrf_field(); ?>
                        <input type="date" name="settlement_date" required value="<?php echo e(date('Y-m-d')); ?>" class="px-2 py-1 text-xs rounded-lg border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white">
