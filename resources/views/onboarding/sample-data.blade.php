    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8"
        ...
        <div class="max-w-4xl w-full">
            ...
            <div x-show="!loading && !done" class="bg-white rounded-2xl shadow-xl p-8">
                <div class="mb-6">
                    <span
                        class="inline-block bg-blue-100 text-blue-700 text-sm font-semibold px-3 py-1 rounded-full capitalize">
                ...
                @if (count($templates) > 0)
                    ...
                    <p class="text-gray-600 mb-8">Select a pre-built dataset to populate your workspace, or skip to start
                        fresh.</p>
                                ...
                                @if (!empty($template['modules_included']))
                                    <div class="flex flex-wrap gap-2">
                                        @foreach ($template['modules_included'] as $module)
                                            <span
                                                class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">{{ $module }}</span>
                                        @endforeach
                                    </div>
                                @endif
                            </button>
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex gap-3">
                        <button @click="skip()"
                            class="px-6 py-3 rounded-lg border border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 transition-colors">
                            Skip for now
                        </button>
                        @if (count($templates) > 0)
            ...
            <div x-show="done" class="bg-white rounded-2xl shadow-xl p-12 text-center">
                ...
                <a href="{{ route('dashboard') }}"
                    class="inline-block px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-colors">
                    Go to Dashboard →
                </a>
            </div>
        </div>
                ...
                async generate() {
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            Dialog.warning(result.message || 'Failed to generate sample data.');
                            this.loading = false;
                        }
                    } catch (e) {
                        console.error(e);
