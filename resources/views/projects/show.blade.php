<x-app-layout>
    ...
    <div class="mb-4 flex items-center gap-2">
        <a href="{{ route('projects.rab', $project) }}"
        ...
        <a href="{{ route('project-billing.show', $project) }}"
            class="inline-flex items-center gap-2 px-4 py-2 text-sm bg-green-600 text-white rounded-xl hover:bg-green-700">
            💰 Project Billing
        </a>
        @endcanmodule
    </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-semibold text-gray-900">
                            {{ $project->budget > 0 ? 'Rp ' . number_format($project->budget, 0, ',', '.') : '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500">Realisasi</p>
                        <p
                            class="text-sm font-semibold {{ $project->budget > 0 && $project->actual_cost > $project->budget ? 'text-red-500' : 'text-gray-900' }}">
                    ...
                    <div>
                        ...
                        <p
                            class="text-sm font-semibold {{ $project->end_date && $project->end_date->isPast() && !in_array($project->status, ['completed', 'cancelled']) ? 'text-red-500' : 'text-gray-900' }}">
                            {{ $project->end_date?->format('d M Y') ?? '-' }}
                        </p>
                    </div>
                </div>
                <div class="w-full bg-gray-100 rounded-full h-3">
                    <div class="h-3 rounded-full {{ $project->progress >= 100 ? 'bg-green-500' : 'bg-blue-500' }} transition-all"
                        style="width:{{ min(100, $project->progress) }}%"></div>
                </div>
                @if ($project->budget > 0)
                    <div class="mt-2 w-full bg-gray-100 rounded-full h-1.5">
                        @php $budgetPct = min(100, $project->budgetUsedPercent()); @endphp
                        <div class="h-1.5 rounded-full {{ $budgetPct >= 100 ? 'bg-red-500' : ($budgetPct >= 80 ? 'bg-amber-500' : 'bg-emerald-500') }} transition-all"
                        ...
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-gray-50 group">
                            <select onchange="updateTaskStatus({{ $task->id }}, this.value)"
                                ...
                                @foreach ($taskStatuses ?? [] as $v => $l)
                                    <option value="{{ $v }}" @selected($task->status === $v)>
                                        {{ $l }}</option>
                                @endforeach
                            </select>
                            <div class="flex-1 min-w-0">
                                <p
                                    class="text-sm font-medium text-gray-900 {{ $task->status === 'done' ? 'line-through opacity-60' : '' }}">
                                    {{ $task->name }}</p>
                                ...
                                @if ($task->isVolumeTracked())
                                    <div class="mt-1.5">
                                        <div class="flex items-center gap-2 text-xs">
                                            ...
                                            <span
                                                class="font-mono font-medium {{ $task->volumeProgress() >= 100 ? 'text-green-600' : 'text-blue-600' }}">
                                                {{ number_format($task->actual_volume, $task->actual_volume == (int) $task->actual_volume ? 0 : 1) }}
                                                /
                                                {{ number_format($task->target_volume, $task->target_volume == (int) $task->target_volume ? 0 : 1) }}
                                                {{ $task->volume_unit }}
                                            </span>
                                            <span class="text-gray-400">({{ $task->volumeProgress() }}%)</span>
                                        </div>
                                        <div class="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                                            <div class="h-1.5 rounded-full {{ $task->volumeProgress() >= 100 ? 'bg-green-500' : 'bg-blue-500' }} transition-all"
                                                style="width:{{ min(100, $task->volumeProgress()) }}%"></div>
                                        </div>
                                        @if ($task->status !== 'done' && $task->status !== 'cancelled')
                                            <form method="POST" action="{{ route('projects.tasks.volume', $task) }}"
                                                class="flex items-center gap-2 mt-1.5">
                                                @csrf
                                                <input type="number" name="volume" step="0.001" min="0.001"
                                                    required placeholder="+vol"
                                                    class="w-20 px-2 py-1 text-xs rounded-lg border border-gray-200 bg-white text-gray-900">
                                                <input type="text" name="description" placeholder="Keterangan"
                                                    class="flex-1 px-2 py-1 text-xs rounded-lg border border-gray-200 bg-white text-gray-900">
                                                <button type="submit"
                                                    class="px-2 py-1 text-xs bg-blue-600 text-white rounded-lg hover:bg-blue-700">+</button>
                                            </form>
                                        @endif
                                    </div>
                                @endif
                            </div>
                            <form method="POST" action="{{ route('projects.tasks.destroy', $task) }}"
                                data-confirm="Hapus task?" data-confirm-type="danger">
                                @csrf @method('DELETE')
                                <button type="submit"
                    ...
                    @forelse($project->expenses->sortByDesc('date') as $exp)
                        <div class="flex items-center justify-between p-3 rounded-xl bg-gray-50 text-sm">
                            <div>
                                ...
                                <p class="text-xs text-gray-500">{{ $exp->category }} ·
                                    {{ $exp->date->format('d M Y') }}</p>
                            </div>
                            <span class="font-semibold text-gray-900">Rp
                                {{ number_format($exp->amount, 0, ',', '.') }}</span>
                        </div>
                    @empty
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                <form method="POST" action="{{ route('projects.update', $project) }}" class="space-y-3">
                    ...
                    <div>
                        ...
                        <select name="status"
                            ...
                            @foreach ($statusLabels ?? [] as $v => $l)
                                <option value="{{ $v }}" @selected($project->status === $v)>{{ $l }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="budget" value="{{ $project->budget }}" min="0"
                            step="100000"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        ...
                        <textarea name="notes" rows="2"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $project->notes }}</textarea>
                    </div>
                    <button type="submit"
                        class="w-full py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Simpan</button>
                </form>
                <form method="POST" action="{{ route('projects.destroy', $project) }}"
                    data-confirm="Hapus proyek ini?" data-confirm-type="danger" class="mt-3">
                    @csrf @method('DELETE')
                    <button type="submit"
                        class="w-full py-2 text-sm border border-red-200 text-red-500 rounded-xl hover:bg-red-50">Hapus
                        Proyek</button>
                </form>
            </div>
            ...
            @if ($project->timesheets->count() > 0)
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    ...
                    <p class="text-2xl font-bold text-gray-900">
                        {{ number_format($project->timesheets->sum('hours'), 1) }} jam</p>
    ...
    <div id="modal-add-task" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-add-task').classList.add('hidden')"
            ...
            <form method="POST" action="{{ route('projects.tasks.store', $project) }}" class="p-6 space-y-4">
                ...
                <div class="border-t border-gray-100 pt-3">
                    <label class="flex items-center gap-2 cursor-pointer mb-2">
                        <input type="checkbox" id="toggle-volume"
                            onchange="document.getElementById('volume-fields').classList.toggle('hidden')"
                            class="rounded border-gray-300 text-blue-600">
                        <span class="text-xs font-medium text-gray-600">📐 Track progress berdasarkan volume
                            fisik</span>
                    </label>
                    <div id="volume-fields" class="hidden grid grid-cols-3 gap-3">
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="number" name="target_volume" step="0.001" placeholder="120"
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="text" name="volume_unit" placeholder="m³, m², kg"
                                class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900">
                        </div>
                        <div class="col-span-1 flex items-end">
                            <p class="text-[10px] text-gray-400 pb-2">Progress otomatis dihitung dari volume aktual vs
                                target</p>
                        </div>
                    </div>
                </div>
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit"
                        class="px-4 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Tambah</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <form method="POST" action="{{ route('projects.expenses.store', $project) }}" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" name="category" placeholder="Material, Jasa..."
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
