<x-app-layout>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-5 mb-6">
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div>
                ...
                <p class="font-mono text-sm font-medium text-gray-900 mt-0.5">{{ $asset->asset_code }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-500">Harga Perolehan</p>
                <p class="text-sm font-medium text-gray-900 mt-0.5">Rp {{ number_format($asset->purchase_price, 0, ',', '.') }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-500">Nilai Buku Saat Ini</p>
                <p class="text-sm font-medium text-blue-600 mt-0.5">Rp {{ number_format($asset->current_value, 0, ',', '.') }}</p>
            </div>
            <div>
            ...
            <div>
                ...
                <p class="text-sm font-medium text-gray-900 mt-0.5">{{ $asset->purchase_date->format('d M Y') }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-500">Umur Ekonomis</p>
                <p class="text-sm font-medium text-gray-900 mt-0.5">{{ $asset->useful_life_years }} tahun</p>
            </div>
            <div>
                <p class="text-xs text-gray-500">Depresiasi/Bulan</p>
                <p class="text-sm font-medium text-gray-900 mt-0.5">Rp {{ number_format($asset->monthlyDepreciation(), 0, ',', '.') }}</p>
            </div>
            <div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {{-- Riwayat Depresiasi --}}
        <div>
            <h2 class="text-sm font-semibold text-gray-900 mb-3">Riwayat Depresiasi ({{ $depreciations->count() }} periode)</h2>
    ...
    <div class="mt-4">
        <a href="{{ route('assets.index') }}" class="text-sm text-gray-500 hover:text-blue-500">← Kembali ke Daftar Aset</a>
    </div>
