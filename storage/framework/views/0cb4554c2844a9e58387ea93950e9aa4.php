...
<?php $component->withAttributes([]); ?>
    ...
    <div class="flex flex-col lg:flex-row gap-5">
        ...
        <div class="lg:w-72 shrink-0 space-y-4">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5 space-y-2 text-sm">
                <p class="font-semibold text-gray-900 dark:text-white"><?php echo e($session->program->name); ?></p>
                <?php if($session->program->category): ?>
                <span class="px-2 py-0.5 rounded-full text-xs bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400"><?php echo e($session->program->category); ?></span>
                <?php endif; ?>
                <div class="space-y-1 text-xs text-gray-500 dark:text-slate-400 pt-1">
                    ...
                    <?php if($session->location): ?><p>📍 <?php echo e($session->location); ?></p><?php endif; ?>
                    <?php if($session->trainer): ?><p>👤 <?php echo e($session->trainer); ?></p><?php endif; ?>
                    <p>👥 <?php echo e($session->participants->count()); ?><?php echo e($session->max_participants > 0 ? '/'.$session->max_participants : ''); ?> peserta</p>
                </div>
                ...
                <form method="POST" action="<?php echo e(route('hrm.training.sessions.status', $session)); ?>" class="pt-2">
                    ...
                    <div class="flex gap-2">
                        <select name="status"
                            ...
                            <option value="<?php echo e($v); ?>" <?php if($session->status===$v): echo 'selected'; endif; ?>><?php echo e($l); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                        <button type="submit" class="px-3 py-1.5 text-xs bg-blue-600 text-white rounded-lg hover:bg-blue-700">Simpan</button>
                    </div>
                </form>
            </div>
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
                ...
                <form method="POST" action="<?php echo e(route('hrm.training.sessions.participants.add', $session)); ?>" class="space-y-2">
                    <?php echo csrf_field(); ?>
                    ...
                    <button type="submit" class="w-full py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Daftarkan</button>
                </form>
            </div>
            <?php else: ?>
            <div class="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 text-xs text-amber-300">Sesi sudah penuh.</div>
            <?php endif; ?>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                <td class="px-4 py-3">
                                    <p class="font-medium text-gray-900 dark:text-white"><?php echo e($p->employee->name ?? '-'); ?></p>
                                    <p class="text-xs text-gray-400 dark:text-slate-500"><?php echo e($p->employee->department ?? $p->employee->position ?? ''); ?></p>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <form method="POST" action="<?php echo e(route('hrm.training.participants.update', $p)); ?>" class="inline-flex items-center gap-1">
                                        <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                        <select name="status" onchange="this.form.submit()"
                                            ...
                                            <option value="<?php echo e($v); ?>" <?php if($p->status===$v): echo 'selected'; endif; ?>><?php echo e($l); ?></option>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        </select>
                                    </form>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <form method="POST" action="<?php echo e(route('hrm.training.participants.update', $p)); ?>" class="inline-flex items-center gap-1">
                                        ...
                                        <input type="number" name="score" value="<?php echo e($p->score); ?>" min="0" max="100" placeholder="—"
                                        ...
                                        <button type="submit" class="text-xs text-blue-400 hover:text-blue-300">✓</button>
                                    </form>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <form method="POST" action="<?php echo e(route('hrm.training.participants.remove', $p)); ?>">
                            ...
                            <tr><td colspan="4" class="px-4 py-10 text-center text-gray-400 dark:text-slate-500">Belum ada peserta terdaftar.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
