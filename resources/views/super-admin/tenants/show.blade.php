<x-app-layout>
    ...
    <div class="max-w-3xl space-y-4">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            <div class="flex items-start justify-between mb-5 flex-wrap gap-3">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @endphp
                    <span
                        class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium {{ $planColors[$tenant->plan] ?? 'bg-[#f8f8f8] text-gray-500' }}">
                        {{ ucfirst($tenant->plan) }}
                    </span>
                    <span
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                ...
                <div>
                    ...
                    <dd class="text-gray-900">{{ $tenant->phone ?? '—' }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium text-gray-400 uppercase tracking-wide mb-0.5">Trial Berakhir</dt>
                    <dd class="{{ $tenant->isTrialExpired() ? 'text-red-500 font-medium' : 'text-gray-900' }}">
                ...
                <div>
                    ...
                    <dd class="{{ $tenant->isPlanExpired() ? 'text-red-500 font-medium' : 'text-gray-900' }}">
                ...
                <div>
                    ...
                    <dd class="text-gray-900">{{ $tenant->subscriptionPlan?->name ?? ucfirst($tenant->plan) }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium text-gray-400 uppercase tracking-wide mb-0.5">Terdaftar</dt>
                    <dd class="text-gray-900">{{ $tenant->created_at->format('d M Y, H:i') }}</dd>
                </div>
            </dl>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            ...
            <form method="POST" action="{{ route('super-admin.tenants.update-plan', $tenant) }}"
                class="p-6 space-y-4">
                @csrf @method('PATCH')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="subscription_plan_id"
                            ...
                            @foreach ($plans ?? [] as $p)
                                <option value="{{ $p->id }}"
                                    {{ $tenant->subscription_plan_id == $p->id ? 'selected' : '' }}>
                ...
                <div class="flex flex-wrap gap-2 pt-1">
                    ...
                    @foreach ([['label' => '+1 Bulan', 'days' => 30], ['label' => '+3 Bulan', 'days' => 90], ['label' => '+6 Bulan', 'days' => 180], ['label' => '+1 Tahun', 'days' => 365]] as $opt)
                        <button type="button" onclick="setExpiry({{ $opt['days'] }})"
        ...
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                ...
                <span
                    class="text-xs bg-[#f8f8f8] text-gray-500 font-medium px-2.5 py-1 rounded-lg">{{ $tenant->users->count() }}</span>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($tenant->users as $user)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-3.5 text-sm text-gray-500 hidden sm:table-cell">{{ $user->email }}
                                </td>
                                <td class="px-6 py-3.5">
                                    <span
                                        class="text-xs font-medium text-gray-500 capitalize">{{ $user->role }}</span>
                                </td>
                                <td class="px-6 py-3.5">
                                    <span
                                        class="inline-flex items-center gap-1.5 text-xs font-medium {{ $user->is_active ? 'text-green-400' : 'text-red-400' }}">
                                        <span
