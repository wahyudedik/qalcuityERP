...
<?php $component->withAttributes([]); ?>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
        ...
        <div class="flex gap-3 flex-wrap">
            <div class="bg-white rounded-xl px-4 py-2 border border-gray-200 text-sm">
                <span class="text-gray-500">Total Anggaran: </span>
                <span class="font-semibold text-gray-900">Rp <?php echo e(number_format($totalBudget,0,',','.')); ?></span>
            </div>
            <div class="bg-white rounded-xl px-4 py-2 border border-gray-200 text-sm">
                <span class="text-gray-500">Realisasi: </span>
                <span class="font-semibold <?php echo e($totalRealized > $totalBudget ? 'text-red-500' : 'text-green-600'); ?>">Rp <?php echo e(number_format($totalRealized,0,',','.')); ?></span>
            </div>
            <?php if($overCount > 0): ?>
    ...
    <div id="ai-suggest-panel" class="hidden mb-4 bg-white border border-purple-500/30 rounded-2xl overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <?php
                        ...
                        $statusColor = $over ? 'text-red-500 bg-red-50' : ($warn ? 'text-amber-600 bg-amber-50' : 'text-green-600 bg-green-50');
                    ?>
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            <p class="font-medium text-gray-900"><?php echo e($budget->name); ?></p>
                            <?php if($budget->category): ?><p class="text-xs text-gray-500"><?php echo e($budget->category); ?></p><?php endif; ?>
                        </td>
                        <td class="px-4 py-3 hidden sm:table-cell text-gray-500"><?php echo e($budget->department ?? '-'); ?></td>
                        <td class="px-4 py-3 text-right font-medium text-gray-900">Rp <?php echo e(number_format($budget->amount,0,',','.')); ?></td>
                        <td class="px-4 py-3 text-right font-medium <?php echo e($over ? 'text-red-500' : 'text-gray-900'); ?>">Rp <?php echo e(number_format($budget->realized,0,',','.')); ?></td>
                        <td class="px-4 py-3 text-right hidden md:table-cell <?php echo e($over ? 'text-red-500' : 'text-gray-500'); ?>">
                    ...
                    <tr><td colspan="8" class="px-4 py-12 text-center text-gray-400">Belum ada anggaran untuk periode <?php echo e($period); ?>.</td></tr>
                    <?php endif; ?>
                </tbody>
                ...
                <tfoot class="bg-gray-50 text-sm font-semibold">
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right text-gray-900">Rp <?php echo e(number_format($totalBudget,0,',','.')); ?></td>
                        <td class="px-4 py-3 text-right <?php echo e($totalRealized > $totalBudget ? 'text-red-500' : 'text-gray-900'); ?>">Rp <?php echo e(number_format($totalRealized,0,',','.')); ?></td>
                        <td class="px-4 py-3 text-right hidden md:table-cell <?php echo e($totalRealized > $totalBudget ? 'text-red-500' : 'text-gray-500'); ?>">
            ...
            <form method="POST" action="<?php echo e(route('budget.store')); ?>" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" name="department" placeholder="Marketing, Ops..." class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Kategori</label>
                        <input type="text" name="category" placeholder="Gaji, Material..." class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Jumlah Anggaran (Rp) *</label>
                    <input type="number" name="amount" min="0" step="100000" required class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Periode *</label>
                        <input type="month" name="period" value="<?php echo e($period); ?>" required class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Tipe</label>
                        <select name="period_type" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" onclick="document.getElementById('modal-add-budget').classList.add('hidden')" class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
        ...
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form id="form-edit-budget" method="POST" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Departemen</label>
                        <input type="text" id="edit-dept" name="department" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Kategori</label>
                        <input type="text" id="edit-cat" name="category" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Anggaran (Rp) *</label>
                        <input type="number" id="edit-amount" name="amount" min="0" step="100000" required class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Realisasi (Rp)</label>
                        <input type="number" id="edit-realized" name="realized" min="0" step="1000" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" onclick="document.getElementById('modal-edit-budget').classList.add('hidden')" class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
                    <button type="submit" class="px-4 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Simpan</button>
                </div>
            </form>
        </div>
