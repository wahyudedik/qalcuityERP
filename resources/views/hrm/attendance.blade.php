<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6">
        ...
        <div class="flex gap-2 ml-auto">
            ...
            <a href="{{ route('hrm.index') }}" class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50">← Karyawan</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
        ...
        <div class="bg-white rounded-2xl p-3 border border-gray-200 text-center">
            ...
            <p class="text-xl font-bold text-{{ $color }}-600 $color }}-400 mt-1">{{ $summary[$key] ?? 0 }}</p>
        </div>
        @endforeach
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100">
            <h3 class="font-semibold text-gray-900">Absensi {{ $date->format('d F Y') }}</h3>
        </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 hidden sm:table-cell text-gray-500">{{ $emp->position ?? '-' }}</td>
                            <td class="px-4 py-3 hidden lg:table-cell">
                                <span class="shift-badge-{{ $emp->id }} text-xs text-gray-400">—</span>
                            </td>
                            <td class="px-4 py-3">
                                <select name="records[{{ $i }}][status]" class="w-full px-2 py-1.5 text-sm rounded-lg border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
