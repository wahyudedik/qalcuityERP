<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
        ...
        <a href="{{ route('helpdesk.index') }}" class="px-3 py-2 text-sm text-gray-500 dark:text-slate-400">← Helpdesk</a>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
            <div class="flex items-start justify-between mb-2">
                <span class="px-2 py-0.5 rounded-full text-xs bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400">{{ ucfirst($a->category) }}</span>
            ...
            <h3 class="font-semibold text-gray-900 dark:text-white mb-2">{{ $a->title }}</h3>
            <p class="text-xs text-gray-500 dark:text-slate-400 line-clamp-3">{{ Str::limit(strip_tags($a->body), 150) }}</p>
    ...
    <div id="modal-add-kb" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-lg shadow-xl">
            ...
            <form method="POST" action="{{ route('helpdesk.kb.store') }}" class="p-6 space-y-4">
                ...
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Judul *</label><input type="text" name="title" required class="{{ $cls }}"></div>
                ...
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Isi Artikel *</label><textarea name="body" required rows="6" class="{{ $cls }}"></textarea></div>
