...
<?php $component->withAttributes([]); ?>
    ...
    <div class="flex justify-between items-center mb-4">
        <a href="<?php echo e(route('production.index')); ?>" class="text-sm text-gray-500 hover:text-gray-700">← Work Order</a>
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <div class="flex items-start justify-between mb-3">
                <div>
                    <h3 class="font-semibold text-gray-900"><?php echo e($recipe->name); ?></h3>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <div class="px-3 py-1.5 text-xs rounded-xl bg-gray-100 text-gray-700">
                    ...
                    <span class="text-gray-500 ml-1"><?php echo e($ing->quantity_per_batch); ?> <?php echo e($ing->unit); ?></span>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
            <?php if($recipe->notes): ?>
            <p class="mt-2 text-xs text-gray-400"><?php echo e($recipe->notes); ?></p>
            <?php endif; ?>
        </div>
    ...
    <div id="modal-create-recipe" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-2xl shadow-xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
                <h3 class="font-semibold text-gray-900">Tambah Resep / BOM</h3>
                <button onclick="document.getElementById('modal-create-recipe').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form method="POST" action="<?php echo e(route('production.recipes.store')); ?>" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Nama Resep *</label>
                        <input type="text" name="name" required placeholder="Resep Standar A"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Ukuran Batch *</label>
                        <input type="number" name="batch_size" required min="0.001" step="0.001" placeholder="100"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Satuan Batch *</label>
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <button type="button" onclick="addIngredient()" class="text-xs text-blue-600 hover:underline">+ Tambah Bahan</button>
                    </div>
                    <div id="ingredients" class="space-y-2">
                        <div class="ingredient grid grid-cols-12 gap-2 items-center">
                            ...
                            <div class="col-span-3">
                                <input type="number" name="ingredients[0][quantity_per_batch]" placeholder="Qty" min="0.001" step="0.001" required class="w-full px-2 py-1.5 text-sm rounded-lg border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div class="col-span-3">
                                <input type="text" name="ingredients[0][unit]" placeholder="Satuan" required class="w-full px-2 py-1.5 text-sm rounded-lg border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-create-recipe').classList.add('hidden')" class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
    ...
    function addIngredient() {
        ...
        div.innerHTML = `
            <div class="col-span-5">
                <select name="ingredients[${i}][product_id]" required class="w-full px-2 py-1.5 text-sm rounded-lg border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="">-- Bahan --</option>${productOptions}
                </select>
            </div>
            <div class="col-span-3">
                <input type="number" name="ingredients[${i}][quantity_per_batch]" placeholder="Qty" min="0.001" step="0.001" required class="w-full px-2 py-1.5 text-sm rounded-lg border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div class="col-span-3">
                <input type="text" name="ingredients[${i}][unit]" placeholder="Satuan" required class="w-full px-2 py-1.5 text-sm rounded-lg border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div class="col-span-1 text-center">
                <button type="button" onclick="this.closest('.ingredient').remove()" class="text-red-500 hover:text-red-700 text-lg leading-none">×</button>
            </div>`;
