...
<?php $component->withAttributes([]); ?>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
            ...
            <p class="text-xl font-bold text-gray-900 dark:text-white">Rp <?php echo e(number_format($monthlySummary->total_cost ?? 0, 0, ',', '.')); ?></p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
            <p class="text-xs text-gray-500 dark:text-slate-400 mb-1">Total Liter</p>
            <p class="text-xl font-bold text-blue-500"><?php echo e(number_format($monthlySummary->total_liters ?? 0, 1, ',', '.')); ?> L</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3 text-xs text-gray-500 dark:text-slate-400"><?php echo e($f->date->format('d/m/Y')); ?></td>
                        <td class="px-4 py-3 text-gray-900 dark:text-white"><?php echo e($f->vehicle->plate_number ?? '-'); ?></td>
                        <td class="px-4 py-3 hidden sm:table-cell text-gray-700 dark:text-slate-300"><?php echo e(ucfirst($f->fuel_type)); ?></td>
                        <td class="px-4 py-3 text-right text-gray-900 dark:text-white"><?php echo e(number_format($f->liters, 1, ',', '.')); ?></td>
                        <td class="px-4 py-3 text-right text-gray-500 dark:text-slate-400">Rp <?php echo e(number_format($f->price_per_liter, 0, ',', '.')); ?></td>
                        <td class="px-4 py-3 text-right font-medium text-gray-900 dark:text-white">Rp <?php echo e(number_format($f->total_cost, 0, ',', '.')); ?></td>
                        <td class="px-4 py-3 text-right hidden md:table-cell text-gray-500 dark:text-slate-400"><?php echo e(number_format($f->odometer, 0, ',', '.')); ?> km</td>
                    </tr>
        ...
        <?php if($fuelLogs->hasPages()): ?><div class="px-4 py-3 border-t border-gray-100 dark:border-white/5"><?php echo e($fuelLogs->links()); ?></div><?php endif; ?>
    </div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="<?php echo e(route('fleet.fuel-logs.store')); ?>" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Tanggal *</label><input type="date" name="date" required value="<?php echo e(date('Y-m-d')); ?>" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Odometer (km) *</label><input type="number" name="odometer" required min="0" class="<?php echo e($cls); ?>"></div>
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Liter *</label><input type="number" name="liters" required min="0.01" step="0.01" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Harga/Liter *</label><input type="number" name="price_per_liter" required min="0" step="100" value="13900" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">SPBU</label><input type="text" name="station" placeholder="SPBU 34.xxx" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">No. Struk</label><input type="text" name="receipt_number" class="<?php echo e($cls); ?>"></div>
                </div>
