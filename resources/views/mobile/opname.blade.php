<x-app-layout>
    @section('title', 'Stock Opname — Mode Lapangan')
    ...
    <div class="min-h-screen bg-gray-950 pb-28" x-data>
        ...
        <div class="px-4 pt-4 space-y-3">
            ...
            @forelse($opnameSessions as $s)
                ...
                <a href="{{ route('mobile.opname.show', $s) }}"
                    ...
                    <div>
                        <div class="flex justify-between text-xs mb-1">
                            ...
                            <span
                                class="{{ $progressPct === 100 ? 'text-emerald-400' : 'text-slate-300' }} font-medium">
                                {{ $countedItems }}/{{ $totalItems }} ({{ $progressPct }}%)
                            </span>
                        </div>
                        <div class="w-full h-2 bg-white/5 rounded-full overflow-hidden">
                            <div class="h-full rounded-full transition-all duration-500
                        {{ $progressPct === 100 ? 'bg-emerald-500' : ($progressPct > 0 ? 'bg-blue-500' : 'bg-gray-600') }}"
            ...
            {{-- ── Pagination ────────────────────────────────────────────── --}}
            @if ($opnameSessions instanceof \Illuminate\Pagination\LengthAwarePaginator && $opnameSessions->hasPages())
                <div class="py-2">
                    {{ $opnameSessions->links() }}
                </div>
            @endif
