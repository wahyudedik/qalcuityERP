    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                <div>
                    <h2 class="text-lg font-semibold text-gray-900"><?php echo e($contract->title); ?></h2>
                    <p class="text-sm text-gray-500"><?php echo e($contract->contract_number); ?> · <?php echo e($contract->party_type === 'customer' ? '👤' : '🏢'); ?> <?php echo e($contract->partyName()); ?></p>
                </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                <div><p class="text-xs text-gray-500">Periode</p><p class="text-gray-900"><?php echo e($contract->start_date->format('d/m/Y')); ?> — <?php echo e($contract->end_date->format('d/m/Y')); ?></p></div>
                <div><p class="text-xs text-gray-500">Sisa Hari</p><p class="font-semibold <?php echo e($contract->daysRemaining() < 30 ? 'text-amber-500' : 'text-gray-900'); ?>"><?php echo e($contract->daysRemaining()); ?> hari</p></div>
                <div><p class="text-xs text-gray-500">Nilai Kontrak</p><p class="font-semibold text-gray-900">Rp <?php echo e(number_format($contract->value, 0, ',', '.')); ?></p></div>
                <div><p class="text-xs text-gray-500">Billing</p><p class="text-gray-900">Rp <?php echo e(number_format($contract->billing_amount, 0, ',', '.')); ?> / <?php echo e(['monthly'=>'bulan','quarterly'=>'triwulan','semi_annual'=>'semester','annual'=>'tahun','one_time'=>'sekali'][$contract->billing_cycle] ?? $contract->billing_cycle); ?></p></div>
                <div><p class="text-xs text-gray-500">Next Billing</p><p class="text-gray-900"><?php echo e($contract->next_billing_date?->format('d/m/Y') ?? '-'); ?></p></div>
                <div><p class="text-xs text-gray-500">Auto Renew</p><p class="text-gray-900"><?php echo e($contract->auto_renew ? '✅ Ya' : '❌ Tidak'); ?></p></div>
                <div><p class="text-xs text-gray-500">Kategori</p><p class="text-gray-900"><?php echo e(['service'=>'Jasa','lease'=>'Sewa','supply'=>'Supply','maintenance'=>'Maintenance','subscription'=>'Langganan'][$contract->category] ?? $contract->category); ?></p></div>
                <div><p class="text-xs text-gray-500">Dibuat oleh</p><p class="text-gray-900"><?php echo e($contract->user->name ?? '-'); ?></p></div>
            </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-100">
                ...
                <div class="flex flex-wrap gap-4 text-sm">
                    <?php if($contract->sla_response_hours): ?><span class="text-gray-700">Response: <span class="font-semibold text-gray-900"><?php echo e($contract->sla_response_hours); ?>h</span></span><?php endif; ?>
                    <?php if($contract->sla_resolution_hours): ?><span class="text-gray-700">Resolution: <span class="font-semibold text-gray-900"><?php echo e($contract->sla_resolution_hours); ?>h</span></span><?php endif; ?>
                    <?php if($contract->sla_uptime_pct): ?><span class="text-gray-700">Uptime: <span class="font-semibold text-gray-900"><?php echo e($contract->sla_uptime_pct); ?>%</span></span><?php endif; ?>
                    <?php $compliance = $contract->slaComplianceRate(); ?>
                    <?php if($compliance !== null): ?><span class="text-gray-700">Compliance: <span class="font-semibold <?php echo e($compliance >= 90 ? 'text-green-500' : 'text-red-500'); ?>"><?php echo e($compliance); ?>%</span></span><?php endif; ?>
                </div>
            </div>
            <?php endif; ?>
        </div>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-2xl p-4 flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-blue-700">Billing berikutnya: <?php echo e($contract->next_billing_date->format('d/m/Y')); ?></p>
                <p class="text-xs text-blue-600">Rp <?php echo e(number_format($contract->billing_amount, 0, ',', '.')); ?></p>
            </div>
            <?php if(auth()->check() && app(\App\Services\PermissionService::class)->check(auth()->user(), 'contracts', 'create')): ?>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <h3 class="font-semibold text-gray-900">Riwayat Billing</h3>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr>
                            <td class="px-4 py-3 text-xs text-gray-500"><?php echo e($b->billing_date->format('d/m/Y')); ?></td>
                            <td class="px-4 py-3 text-xs text-gray-700"><?php echo e($b->period_start->format('d/m')); ?> — <?php echo e($b->period_end->format('d/m/Y')); ?></td>
                            <td class="px-4 py-3 text-right text-gray-900">Rp <?php echo e(number_format($b->amount, 0, ',', '.')); ?></td>
                            <td class="px-4 py-3 text-center"><span class="px-2 py-0.5 rounded-full text-xs bg-<?php echo e($bc); ?>-100 text-<?php echo e($bc); ?>-700 $bc }}-500/20 $bc }}-400"><?php echo e(ucfirst($b->status)); ?></span></td>
                            <td class="px-4 py-3 text-xs text-gray-500"><?php echo e($b->invoice->invoice_number ?? '-'); ?></td>
                        </tr>
            ...
            <?php else: ?>
            <div class="px-6 py-8 text-center text-gray-400 text-sm">Belum ada billing.</div>
            <?php endif; ?>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 class="font-semibold text-gray-900">SLA Incidents</h3>
                <?php if(auth()->check() && app(\App\Services\PermissionService::class)->check(auth()->user(), 'contracts', 'create')): ?>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr>
                            <td class="px-4 py-3 text-xs text-gray-700"><?php echo e(['support'=>'Support','downtime'=>'Downtime','delivery_delay'=>'Keterlambatan'][$log->incident_type] ?? $log->incident_type); ?></td>
                            <td class="px-4 py-3 text-gray-900"><?php echo e(Str::limit($log->description, 40)); ?></td>
                            <td class="px-4 py-3 text-center text-xs text-gray-500"><?php echo e($log->reported_at->format('d/m H:i')); ?></td>
            ...
            <?php else: ?>
            <div class="px-6 py-8 text-center text-gray-400 text-sm">Belum ada insiden SLA.</div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div id="modal-sla" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="<?php echo e(route('contracts.sla.store', $contract)); ?>" class="p-6 space-y-4">
                ...
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Deskripsi *</label><input type="text" name="description" required class="<?php echo e($cls); ?>"></div>
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Waktu Dilaporkan *</label><input type="datetime-local" name="reported_at" required value="<?php echo e(now()->format('Y-m-d\TH:i')); ?>" class="<?php echo e($cls); ?>"></div>
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-sla').classList.add('hidden')" class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
    ...
    <div id="modal-resolve" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-sm shadow-xl">
            ...
            <form id="form-resolve" method="POST" class="p-6 space-y-4">
                ...
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Waktu Response</label><input type="datetime-local" name="responded_at" class="<?php echo e($cls); ?>"></div>
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Waktu Resolved *</label><input type="datetime-local" name="resolved_at" required value="<?php echo e(now()->format('Y-m-d\TH:i')); ?>" class="<?php echo e($cls); ?>"></div>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-resolve').classList.add('hidden')" class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
