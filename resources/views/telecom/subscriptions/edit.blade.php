    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __('Edit Subscription') }}</h1>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                <form action="{{ route('telecom.subscriptions.update', $subscription) }}" method="POST" class="p-6 space-y-6">
                    ...
                    <!-- Customer (read-only) -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Pelanggan') }}</label>
                        <p class="mt-1 text-sm text-gray-900 dark:text-white font-medium">{{ $subscription->customer?->name ?? '-' }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('status') border-red-300 @enderror">
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="3"
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('notes', $subscription->notes) }}</textarea>
                    </div>
