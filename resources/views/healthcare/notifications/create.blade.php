<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('New Notification Rule') }}</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('healthcare.notifications.store') }}">
                    @csrf
                    <div class="space-y-6">
                        <div>
                            ...
                            <div>
                                ...
                                <select name="trigger_event" required
                                    ...
                                    <option value="lab_result_critical"
                                        {{ old('trigger_event') === 'lab_result_critical' ? 'selected' : '' }}>Critical
                                        Lab Result</option>
                                    <option value="appointment_reminder"
                                        {{ old('trigger_event') === 'appointment_reminder' ? 'selected' : '' }}>
                                        Appointment Reminder</option>
                                    <option value="prescription_ready"
                                        {{ old('trigger_event') === 'prescription_ready' ? 'selected' : '' }}>
                                        Prescription Ready</option>
                                    <option value="admission_confirmed"
                                        {{ old('trigger_event') === 'admission_confirmed' ? 'selected' : '' }}>Admission
                                        Confirmed</option>
                                    <option value="discharge_summary"
                                        {{ old('trigger_event') === 'discharge_summary' ? 'selected' : '' }}>Discharge
                                        Summary</option>
                                    <option value="billing_overdue"
                                        {{ old('trigger_event') === 'billing_overdue' ? 'selected' : '' }}>Billing
                                        Overdue</option>
                                    <option value="equipment_maintenance"
                                        {{ old('trigger_event') === 'equipment_maintenance' ? 'selected' : '' }}>
                                        Equipment Maintenance</option>
                                    <option value="low_stock_alert"
                                        {{ old('trigger_event') === 'low_stock_alert' ? 'selected' : '' }}>Low Stock
                                        Alert</option>
                                </select>
                            </div>
                            <div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="channels[]" value="in_app"
                                        {{ in_array('in_app', old('channels', [])) ? 'checked' : '' }}
                                        class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                                    <span class="ml-2 text-sm text-gray-700"><i
                        ...
                        <div>
                            ...
                            <textarea name="message_template" required rows="6"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                placeholder="e.g., Dear {patient_name}, your lab result for {test_name} is ready...">{{ old('message_template') }}</textarea>
