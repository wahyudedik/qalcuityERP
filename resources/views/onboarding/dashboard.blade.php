    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8 mb-8">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <span class="inline-block bg-blue-100 text-blue-700 text-sm font-semibold px-3 py-1 rounded-full capitalize mb-2">
                        {{ $profile->industry_label ?? ucfirst($profile->industry) }}
                    </span>
                    <h2 class="text-xl font-bold text-gray-900">Your Setup Progress</h2>
                </div>
                <div class="text-right">
                    <div class="text-3xl font-bold text-blue-600" id="pct-display">{{ $progress['completion_percentage'] }}%</div>
            ...
            <div class="mt-4 p-4 bg-green-50 border border-green-200 rounded-xl text-center">
                ...
                <a href="{{ route('dashboard') }}" class="ml-4 inline-block px-5 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-lg transition-colors">
                    Go to Dashboard →
                </a>
            </div>
            @endif
            ...
            <div class="mb-6 last:mb-0">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-xl border transition-all
                        {{ $isCompleted ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50 hover:border-blue-300 hover:bg-blue-50' }}"
                        id="step-row-{{ $step->step_key }}">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="font-semibold text-gray-900 {{ $isCompleted ? 'line-through text-gray-400' : '' }}">
                                {{ $step->step_name }}
                            </div>
                        ...
                        @if(!$isCompleted)
                            ...
                            @elseif($cfg['url'] && $cfg['url'] !== '#')
                                <a href="{{ $cfg['url'] }}"
                                    onclick="markCompleteAndNavigate(event, '{{ $step->step_key }}', '{{ $cfg['url'] }}')"
                                    class="flex-shrink-0 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition-colors">
                                    {{ $cfg['label'] }} →
                                </a>
                            @endif
        ...
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('dashboard') }}" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-colors">
                Go to Dashboard →
            </a>
        </div>
