            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                <form method="POST" action="{{ route('payroll.process') }}" class="space-y-3">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="include_bpjs" id="include_bpjs" value="1" checked
                            class="rounded">
        ...
        <div class="flex-1">
            @if ($run)
                ...
                <div class="mb-4 space-y-2">
                    {{-- Jurnal 1: Beban Gaji --}}
                    <div
                        class="px-4 py-3 rounded-xl border flex items-center justify-between gap-3
                        ...
                        <div class="flex items-center gap-2.5 min-w-0">
                            @if ($run->journal_entry_id)
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-xs text-green-600 truncate">{{ $run->journalEntry?->number }} · Dr
                                        Beban Gaji Rp {{ number_format($run->total_gross, 0, ',', '.') }} · Cr Hutang Gaji
                                        + PPh21 + BPJS</p>
                                </div>
                            @else
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-amber-600">Dr Beban Gaji / Cr Hutang Gaji + PPh21 + BPJS
                                        belum diposting ke GL.</p>
                                </div>
                            @endif
                        </div>
                        <div class="shrink-0">
                            @if ($run->journal_entry_id)
                                <a href="{{ route('journals.show', $run->journalEntry) }}"
                                    class="px-3 py-1.5 text-xs border border-green-300 text-green-700 rounded-xl hover:bg-green-100">Lihat</a>
                            @else
                                <form method="POST" action="{{ route('payroll.gl-journal', $run) }}">
                                    @csrf
                                    <button type="submit"
                                        class="px-3 py-1.5 text-xs bg-amber-600 text-white rounded-xl hover:bg-amber-700">Buat
                                        Jurnal</button>
                                </form>
                            @endif
                        </div>
                    ...
                    {{ $run->payment_journal_entry_id ? 'bg-green-50 border-green-200' : 'bg-amber-50 border-amber-200' }}">
                                ...
                                @if ($run->payment_journal_entry_id)
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-xs text-green-600 truncate">
                                            {{ $run->paymentJournalEntry?->number }} · Dr Hutang Gaji Rp
                                            {{ number_format($run->total_net, 0, ',', '.') }} · Cr Bank</p>
                                    </div>
                                @else
                                    <svg class="w-4 h-4 text-amber-600 shrink-0" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                            ...
                            <div class="shrink-0">
                                @if ($run->payment_journal_entry_id)
                                    <a href="{{ route('journals.show', $run->paymentJournalEntry) }}"
                                        class="px-3 py-1.5 text-xs border border-green-300 text-green-700 rounded-xl hover:bg-green-100">Lihat</a>
                                @else
                ...
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route('payroll.components.index') }}"
                            class="px-3 py-2 text-sm border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 flex items-center gap-1.5">
                        ...
                        @if ($run->status === 'processed')
                            <form method="POST" action="{{ route('payroll.paid', $run) }}"
                                data-confirm="Tandai semua gaji periode ini sebagai sudah dibayar?">
                                @csrf @method('PATCH')
                                <button type="submit"
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse($items as $item)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-4 py-3">
                                            <p class="font-medium text-gray-900">{{ $item->employee?->name ?? '-' }}
                                            </p>
                                            <p class="text-xs text-gray-500">{{ $item->employee?->position ?? '' }}
                                            </p>
                                        </td>
                                        <td class="px-4 py-3 text-right hidden sm:table-cell text-gray-700">Rp
                                            {{ number_format($item->base_salary, 0, ',', '.') }}</td>
                                        <td class="px-4 py-3 text-center hidden md:table-cell text-gray-700">
                                            {{ $item->present_days }}h</td>
                                        <td class="px-4 py-3 text-center hidden md:table-cell text-red-600">
                                            {{ $item->absent_days }}h</td>
                                        ...
                                        <td class="px-4 py-3 text-right hidden sm:table-cell text-red-600">
                                            Rp
                                            {{ number_format($item->bpjs_employee + $item->tax_pph21 + $item->deduction_absent + $item->deduction_late, 0, ',', '.') }}
                                        </td>
                                        <td class="px-4 py-3 text-right font-semibold text-gray-900">Rp
                                            {{ number_format($item->net_salary, 0, ',', '.') }}</td>
                                        <td class="px-4 py-3 text-center">
                                            <span
                                                class="px-2 py-0.5 rounded-full text-xs {{ $item->status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700' }}">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="px-4 py-10 text-center text-gray-400">Tidak ada
                                            data.</td>
                                    </tr>
                                @endforelse
                            </tbody>
            ...
            @else
                <div class="bg-white rounded-2xl border border-gray-200 p-12 text-center">
                    <p class="text-gray-400 text-sm">Belum ada data penggajian untuk periode <span
                            class="font-medium text-gray-700">{{ $period }}</span>.</p>
                    <p class="text-gray-400 text-xs mt-1">Gunakan form di sebelah kiri untuk menghitung gaji.</p>
                    <p class="text-gray-400 text-xs mt-1">Total karyawan aktif: <span
                            class="font-medium text-gray-700">{{ $totalEmployees }}</span></p>
                    <a href="{{ route('payroll.components.index') }}"
                        class="inline-flex items-center gap-1.5 mt-4 px-4 py-2 text-sm border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 4a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4" />
                        </svg>
