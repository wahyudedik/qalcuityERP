...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <dd class="mt-1 text-2xl font-semibold text-gray-900">Rp
                    {{ number_format($quickStats['today_revenue'], 0, ',', '.') }}</dd>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <dt class="text-sm font-medium text-gray-500">MTD Revenue</dt>
                <dd class="mt-1 text-2xl font-semibold text-gray-900">Rp
                    {{ number_format($quickStats['mtd_revenue'], 0, ',', '.') }}</dd>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <dt class="text-sm font-medium text-gray-500">Total Customers</dt>
                <dd class="mt-1 text-2xl font-semibold text-gray-900">{{ number_format($quickStats['total_customers']) }}
                </dd>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <dt class="text-sm font-medium text-gray-500">Active Products</dt>
                <dd class="mt-1 text-2xl font-semibold text-gray-900">{{ number_format($quickStats['active_products']) }}
                </dd>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <dt class="text-sm font-medium text-gray-500">Outstanding Invoices</dt>
                <dd class="mt-1 text-2xl font-semibold text-red-600">Rp
                    {{ number_format($quickStats['outstanding_invoices'], 0, ',', '.') }}</dd>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl shadow-lg p-8 mb-8 text-white">
            <div class="flex items-center justify-between">
                ...
                <div class="text-right">
                    <div class="text-6xl font-bold">{{ $healthScore['overall_score'] }}</div>
                    <div class="text-3xl font-semibold mt-2">Grade: {{ $healthScore['grade'] }}</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mt-6 pt-6 border-t border-blue-400">
                @foreach ($healthScore['components'] as $key => $component)
                    <div>
                        <div class="text-xs text-blue-200 uppercase">{{ str_replace('_', ' ', $key) }}</div>
                        <div class="text-xl font-semibold">{{ $component['score'] }}/100</div>
                        <div class="text-xs text-blue-200">Weight: {{ $component['weight'] }}%</div>
                    </div>
                @endforeach
            </div>
            ...
            @if (!empty($healthScore['recommendations']))
                <div class="mt-6 pt-6 border-t border-blue-400">
                    ...
                    <ul class="space-y-1">
                        @foreach ($healthScore['recommendations'] as $rec)
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            ...
            <a href="{{ route('analytics.customer-segmentation') }}"
            ...
            <a href="{{ route('analytics.product-profitability') }}"
                class="bg-white rounded-lg shadow hover:shadow-lg transition p-6 block">
                <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('analytics.employee-performance') }}"
                class="bg-white rounded-lg shadow hover:shadow-lg transition p-6 block">
                <div class="flex items-center justify-between mb-4">
            ...
            <!-- Cashflow Forecast -->
            <a href="{{ route('analytics.cashflow-forecast') }}"
                class="bg-white rounded-lg shadow hover:shadow-lg transition p-6 block">
                <div class="flex items-center justify-between mb-4">
            ...
            <!-- Churn Risk -->
            <a href="{{ route('analytics.churn-risk') }}"
                class="bg-white rounded-lg shadow hover:shadow-lg transition p-6 block">
                <div class="flex items-center justify-between mb-4">
            ...
            <!-- Seasonal Trends -->
            <a href="{{ route('analytics.seasonal-trends') }}"
                class="bg-white rounded-lg shadow hover:shadow-lg transition p-6 block">
                <div class="flex items-center justify-between mb-4">
