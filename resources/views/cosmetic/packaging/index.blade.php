...
@section('title', 'Packaging Materials')
...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            <div class="flex justify-between items-center">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('cosmetic.packaging.labels') }}"
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-green-600">{{ $stats['primary_packaging'] }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-orange-600">{{ $stats['secondary_packaging'] }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-indigo-600">{{ $stats['recyclable'] }}</p>
                    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 mb-6">
            <form method="GET" action="{{ route('cosmetic.packaging.index') }}"
                class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Search</label>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Material name..."
                ...
                <div>
                    ...
                    <select name="category"
                        ...
                        <option value="bottle" {{ request('category') == 'bottle' ? 'selected' : '' }}>Bottle</option>
                        <option value="tube" {{ request('category') == 'tube' ? 'selected' : '' }}>Tube</option>
                        <option value="jar" {{ request('category') == 'jar' ? 'selected' : '' }}>Jar</option>
                        <option value="box" {{ request('category') == 'box' ? 'selected' : '' }}>Box</option>
                        <option value="carton" {{ request('category') == 'carton' ? 'selected' : '' }}>Carton</option>
                        <option value="label" {{ request('category') == 'label' ? 'selected' : '' }}>Label</option>
                        <option value="cap" {{ request('category') == 'cap' ? 'selected' : '' }}>Cap</option>
                        <option value="pump" {{ request('category') == 'pump' ? 'selected' : '' }}>Pump</option>
                    </select>
                </div>
                    ...
                    @forelse($materials as $material)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-mono font-medium text-gray-900">{{ $material->sku }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $material->material_name }}</div>
                                @if ($material->dimensions)
                                    <div class="text-xs text-gray-500">{{ $material->dimensions }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-1 text-xs font-medium rounded-full 
                            @if ($material->material_type == 'primary') bg-green-100 text-green-800
                            @elseif($material->material_type == 'secondary') bg-orange-100 text-orange-800
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if ($material->unit_cost)
                                    <div class="text-sm font-medium text-gray-900">Rp
                                        {{ number_format($material->unit_cost, 0, ',', '.') }}</div>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <form method="POST"
                                    action="{{ route('cosmetic.packaging.materials.destroy', $material->id) }}"
    ...
    <div id="add-material-modal"
        ...
        <div class="relative top-20 mx-auto p-5 border w-full max-w-3xl shadow-lg rounded-lg bg-white dark:bg-gray-800">
            ...
            <form method="POST" action="{{ route('cosmetic.packaging.materials.store') }}" class="space-y-4">
                @csrf
