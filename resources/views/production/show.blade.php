        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @endphp
                    <span
                        class="px-3 py-1 rounded-full text-sm font-medium bg-{{ $c }}-100 text-{{ $c }}-700 $c }}-500/20 $c }}-400">
                    ...
                    @if ($workOrder->status === 'pending')
                        <form method="POST" action="{{ route('production.status', $workOrder) }}">
                            ...
                            <button type="submit"
                                class="px-3 py-1 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Mulai
                                Produksi</button>
                        </form>
                    @elseif($workOrder->status === 'in_progress')
                        <form method="POST" action="{{ route('production.status', $workOrder) }}">
                            ...
                            <button type="submit"
                                class="px-3 py-1 text-sm bg-green-600 text-white rounded-xl hover:bg-green-700">Selesaikan</button>
                        </form>
                    @endif
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div>
                    ...
                    <p class="font-semibold text-gray-900">{{ number_format($workOrder->target_quantity, 0, ',', '.') }}
                        {{ $workOrder->unit }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500">Output Bagus</p>
                    <p class="font-semibold text-green-500">{{ $workOrder->totalGoodQty() }} {{ $workOrder->unit }}
                    </p>
                </div>
                <div>
                    <p class="text-xs text-gray-500">Reject</p>
                    <p class="font-semibold text-red-500">{{ $workOrder->totalRejectQty() }} {{ $workOrder->unit }}
                    </p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="font-semibold text-gray-900">Rp
                        {{ number_format($workOrder->material_cost, 0, ',', '.') }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500">Biaya Tenaga Kerja</p>
                    <p class="font-semibold text-gray-900">Rp {{ number_format($workOrder->labor_cost, 0, ',', '.') }}
                    </p>
                </div>
                <div>
                    <p class="text-xs text-gray-500">Overhead</p>
                    <p class="font-semibold text-gray-900">Rp
                        {{ number_format($workOrder->overhead_cost, 0, ',', '.') }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500">Total Biaya</p>
                    <p class="font-semibold text-gray-900">Rp {{ number_format($workOrder->total_cost, 0, ',', '.') }}
                    </p>
                </div>
            </div>
            ...
            @if ($workOrder->recipe)
                <div class="mt-4 pt-4 border-t border-gray-100">
                    <p class="text-xs font-medium text-gray-500 mb-2">Resep: {{ $workOrder->recipe?->name }} (batch
                        {{ $workOrder->recipe?->batch_size }} {{ $workOrder->recipe?->batch_unit }})</p>
                ...
                <div class="mt-4 pt-4 border-t border-gray-100">
                    <div class="flex items-center justify-between mb-2">
                        <p class="text-xs font-medium text-gray-500">
                            BOM: {{ $workOrder->bom?->name }} (batch {{ $workOrder->bom?->batch_size }}
                            {{ $workOrder->bom?->batch_unit }})
                        </p>
                        <div class="flex items-center gap-2">
                            @if ($workOrder->materials_consumed)
                                <span class="px-2 py-0.5 rounded-full text-xs bg-green-100 text-green-700">Material
                                    Dikonsumsi</span>
                            @elseif($workOrder->status === 'in_progress')
                                <form method="POST" action="{{ url('manufacturing') }}/{{ $workOrder->id }}/consume"
                                    data-confirm="Konsumsi material dari stok sesuai BOM?">
                                    @csrf
                                    <button type="submit"
                                        class="px-3 py-1 text-xs bg-amber-600 text-white rounded-xl hover:bg-amber-700">Konsumsi
                                        Material</button>
                                </form>
                            @else
                                <span class="px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-500">Belum
                                    Dikonsumsi</span>
                            @endif
                        </div>
                    </div>
                    <div class="flex flex-wrap gap-2">
                        @foreach ($workOrder->bom?->lines as $line)
                            <span class="px-2 py-1 text-xs rounded-lg bg-gray-100 text-gray-700">
                                ...
                                @if ($line->childBom)
                                    <span class="text-purple-500">(sub-BOM)</span>
                                @endif
                            </span>
                        @endforeach
                    </div>
            ...
            @if ($workOrder->journalEntry)
                <div class="mt-4 pt-4 border-t border-gray-100">
                    <p class="text-xs text-gray-500">
                        Jurnal Material: <a
                            href="{{ url('accounting/journals') }}/{{ $workOrder->journalEntry?->id }}"
                            class="text-blue-500 hover:underline">{{ $workOrder->journalEntry?->number }}</a>
                        <span
                            class="ml-2 px-1.5 py-0.5 rounded text-xs {{ $workOrder->journalEntry?->status === 'posted' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">{{ $workOrder->journalEntry?->status }}</span>
                    </p>
                </div>
            @endif
        </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <form method="POST" action="{{ route('production.output', $workOrder) }}" class="space-y-4">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <button type="submit"
                            class="px-4 py-2 text-sm bg-green-600 text-white rounded-xl hover:bg-green-700">Simpan
                            Output</button>
                    </div>
                </form>
            </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($workOrder->operations as $op)
                                ...
                                @endphp
                                <tr>
                                    <td class="px-4 py-3 text-center font-mono text-xs text-gray-900">
                                        {{ $op->sequence }}</td>
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <span
                                            class="px-2 py-0.5 rounded-full text-xs bg-{{ $oc }}-100 text-{{ $oc }}-700 $oc }}-500/20 $oc }}-400">
                            ...
                            @foreach ($workOrder->outputs as $out)
                                <tr>
                                    <td class="px-4 py-3 text-xs text-gray-500">
                                        {{ $out->created_at->format('d M Y H:i') }}</td>
                                    <td class="px-4 py-3 text-right text-green-500 font-medium">
                                        {{ $out->good_qty + 0 }}</td>
