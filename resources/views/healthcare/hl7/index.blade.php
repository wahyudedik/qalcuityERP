<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('HL7 Messages') }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">{{ $statistics['errors'] }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <form method="GET" action="{{ route('healthcare.hl7.index') }}"
                        ...
                        <div>
                            ...
                            <select name="message_type" class="w-full border-gray-300 rounded-md shadow-sm">
                                ...
                                <option value="ADT" {{ request('message_type') === 'ADT' ? 'selected' : '' }}>ADT
                                    (Admission)</option>
                                <option value="ORM" {{ request('message_type') === 'ORM' ? 'selected' : '' }}>ORM
                                    (Order)</option>
                                <option value="ORU" {{ request('message_type') === 'ORU' ? 'selected' : '' }}>ORU
                                    (Result)</option>
                                <option value="SIU" {{ request('message_type') === 'SIU' ? 'selected' : '' }}>SIU
                                    (Scheduling)</option>
                                <option value="DFT" {{ request('message_type') === 'DFT' ? 'selected' : '' }}>DFT
                                    (Billing)</option>
                            </select>
                        </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($messages as $message)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span
                                                class="px-2 py-1 text-xs font-semibold rounded-full bg-indigo-100 text-indigo-800">{{ $message->message_type }}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                            {{ $message->receiving_app ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                            {{ $message->retry_count }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('healthcare.hl7.show', $message) }}"
