<x-app-layout>
    ...
    {{-- New Task Modal --}}
    <div x-show="showNewTaskModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50"
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-md shadow-xl" @click.stop>
            ...
            <form method="POST" action="{{ route('hotel.housekeeping.store') }}" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-2">Task Type *</label>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-2">Priority *</label>
                    <select name="priority" required
                        class="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                        @foreach ($priorities as $priority)
                            <option value="{{ $priority }}" @selected($priority === 'normal')>{{ ucfirst($priority) }}
                            </option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-2">Assigned To</label>
    ...
    {{-- Assign Modal --}}
    <div x-show="showAssignModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50"
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-sm shadow-xl" @click.stop>
            ...
            <form :action="assignUrl" method="POST" class="p-6 space-y-4">
                ...
                @method('PATCH')
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-2">Assign to *</label>
