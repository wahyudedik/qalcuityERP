...
<?php $component->withAttributes([]); ?>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-4 mb-6">
        <div class="bg-white rounded-2xl p-4 border border-gray-200">
            ...
            <p class="text-2xl font-bold text-gray-900 mt-1"><?php echo e(number_format($statistics['total_invoices'])); ?></p>
        </div>
        <div class="bg-white rounded-2xl p-4 border border-gray-200">
            <p class="text-xs text-gray-500">Belum Bayar</p>
            <p class="text-2xl font-bold text-red-600 mt-1"><?php echo e($statistics['unpaid_invoices']); ?></p>
        </div>
        <div class="bg-white rounded-2xl p-4 border border-gray-200">
            <p class="text-xs text-gray-500">Sebagian</p>
            <p class="text-2xl font-bold text-amber-600 mt-1"><?php echo e($statistics['partial_invoices']); ?></p>
        </div>
        <div class="bg-white rounded-2xl p-4 border border-gray-200">
            <p class="text-xs text-gray-500">Lunas Hari Ini</p>
            <p class="text-2xl font-bold text-green-600 mt-1"><?php echo e($statistics['paid_today']); ?></p>
        </div>
        <div class="bg-white rounded-2xl p-4 border border-gray-200">
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php $__empty_1 = true; $__currentLoopData = $invoices ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $invoice): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3">
                                <span
                                    class="font-mono text-sm font-bold text-blue-600"><?php echo e($invoice->invoice_number ?? '-'); ?></span>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-center">
                                <?php if($invoice->status === 'paid'): ?>
                                    <span
                                        class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-lg bg-green-100 text-green-700">Paid</span>
                                ...
                                <?php elseif($invoice->status === 'overdue'): ?>
                                    <span
                                        class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-lg bg-red-100 text-red-700">Overdue</span>
                                <?php else: ?>
                                    <span
                                        class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-lg bg-gray-100 text-gray-700">Unpaid</span>
                                <?php endif; ?>
                            </td>
                            <td class="px-4 py-3 text-center">
