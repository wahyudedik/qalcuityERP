...
<head>
    ...
    <title>Kwitansi Check-out — {{ $receipt->receipt_number }}</title>
    <style>
...
<body>
    ...
    <div class="section">
        ...
        <div class="row">
            ...
            <span class="value">{{ $reservation->room?->number ?? '-' }}</span>
        </div>
        <div class="row">
    ...
    <div class="section">
        ...
        <div class="row">
            ...
            <span class="value">{{ $reservation->nights }} malam</span>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div class="row">
            <span class="label">Biaya Kamar ({{ $charges['nights'] ?? $reservation->nights }} malam × Rp {{ number_format($charges['rate_per_night'] ?? $reservation->rate_per_night, 0, ',', '.') }})</span>
            <span class="value">Rp {{ number_format($charges['room_charge'] ?? $reservation->total_amount, 0, ',', '.') }}</span>
        </div>
        @if (!empty($charges['minibar_charges']) && $charges['minibar_charges'] > 0)
        <div class="row">
            <span class="label">Minibar</span>
            <span class="value">Rp {{ number_format($charges['minibar_charges'], 0, ',', '.') }}</span>
        </div>
        @endif
        @if (!empty($charges['additional_charges']) && $charges['additional_charges'] > 0)
        <div class="row">
            <span class="label">Biaya Tambahan</span>
            <span class="value">Rp {{ number_format($charges['additional_charges'], 0, ',', '.') }}</span>
        </div>
        @endif
        @if (!empty($charges['discount']) && $charges['discount'] > 0)
        <div class="row">
            <span class="label">Diskon</span>
            <span class="value" style="color: #16a34a;">- Rp {{ number_format($charges['discount'], 0, ',', '.') }}</span>
        </div>
        @endif
        <div class="row">
            <span class="label">Pajak ({{ $charges['tax_rate'] ?? 11 }}%)</span>
            <span class="value">Rp {{ number_format($charges['tax_amount'] ?? $reservation->tax, 0, ',', '.') }}</span>
        </div>
        @if (!empty($charges['deposit_paid']) && $charges['deposit_paid'] > 0)
        <div class="row">
            <span class="label">Deposit Dibayar</span>
            <span class="value" style="color: #16a34a;">- Rp {{ number_format($charges['deposit_paid'], 0, ',', '.') }}</span>
        </div>
        @endif
        <div class="total-row">
            <span>TOTAL</span>
    ...
    <div class="section">
        ...
        <div class="row">
            ...
            <span class="value">{{ strtoupper(str_replace('_', ' ', $receipt->payment_method)) }}</span>
        </div>
        <div class="row">
            <span class="label">Jumlah Dibayar</span>
            <span class="value">Rp {{ number_format($receipt->amount_paid, 0, ',', '.') }}</span>
        </div>
        @if ($receipt->change_amount > 0)
        <div class="row">
            <span class="label">Kembalian</span>
            <span class="value">Rp {{ number_format($receipt->change_amount, 0, ',', '.') }}</span>
        </div>
        @endif
        <div class="row">
            <span class="label">Status</span>
            <span class="value">
                <span class="badge {{ $receipt->payment_status === 'paid' ? 'badge-paid' : 'badge-partial' }}">
                    {{ $receipt->payment_status === 'paid' ? 'LUNAS' : 'SEBAGIAN' }}
                </span>
            </span>
        </div>
        ...
        <div class="row">
            ...
            <span class="value">{{ $receipt->transaction_reference }}</span>
        </div>
        @endif
    </div>
    ...
    <div class="footer">
        ...
        <p>{{ $receipt->receipt_number }} | {{ now()->format('d/m/Y H:i') }}</p>
    </div>
</body>
</html>
