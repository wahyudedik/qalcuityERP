<x-app-layout>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                            ...
                            <div>
                                ...
                                <div class="font-bold">{{ ucfirst($forecast['confidence'] ?? 'medium') }}</div>
                            </div>
                            <div>
                                <div class="text-sm opacity-80">Products Analyzed</div>
                                <div class="font-bold">{{ $insights['total_products_analyzed'] ?? 0 }}</div>
                            </div>
                            <div>
                                <div class="text-sm opacity-80">Generated At</div>
                                <div class="font-bold text-sm">
                                    {{ \Carbon\Carbon::parse($forecast['generated_at'] ?? now())->format('d M Y H:i') }}
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div
                            ...
                            <div class="flex justify-between items-start mb-4">
                                ...
                                <span class="px-2 py-1 rounded-full text-xs font-bold {{ $urgencyColor }}">
                                    {{ strtoupper($product['reorder_urgency']) }}
                                </span>
                            </div>
                            ...
                            <div class="mb-4 p-3 bg-gray-50 dark:bg-white/5 rounded-lg">
                                ...
                                <div class="flex justify-between text-sm mb-1">
                                    ...
                                    <span
                                        class="font-bold">{{ number_format($product['total_forecasted_demand'], 0) }}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-500">Stock Status:</span>
                                    <span
                                        class="font-bold {{ $product['stock_status'] === 'sufficient' ? 'text-green-600' : 'text-red-600' }}">
                            ...
                            <div
                                ...
                                <div class="text-sm text-gray-700 dark:text-slate-300">
                                    ...
                                    units by
                                    <strong>{{ \Carbon\Carbon::parse($product['recommended_order_date'])->format('d M Y') }}</strong>
                                </div>
                            </div>
            ...
            @else
                <div
                    ...
                    <p class="text-sm text-gray-400">
                        Message: {{ $forecast['message'] ?? 'Click "Refresh Forecast" to try again' }}
                    </p>
                </div>
            @endif
    ...
    @if (!empty($forecast['forecast']['products']))
        ...
        <script>
            @foreach ($forecast['forecast']['products'] as $product)
                new Chart(document.getElementById('chart-{{ $product['product_id'] }}'), {
                    type: 'bar',
                    data: {
                        labels: [
                            @foreach ($product['forecasted_demand'] as $month => $demand)
                                'Month {{ str_replace('month_', '', $month) }}',
                            @endforeach
                        ],
                        datasets: [{
                            label: 'Forecasted Demand',
                            data: [{{ implode(',', array_values($product['forecasted_demand'])) }}],
