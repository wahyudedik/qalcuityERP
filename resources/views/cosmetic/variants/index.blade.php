...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            <div class="flex justify-between items-center">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('cosmetic.variants.attributes') }}"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $stats['total_variants'] }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-green-600">{{ $stats['active_variants'] }}</p>
                    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 mb-6">
            <form method="GET" action="{{ route('cosmetic.variants.index') }}"
                class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Search</label>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="SKU or name..."
                    ...
                    @forelse($variants as $variant)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-mono font-medium text-gray-900">{{ $variant->sku }}</div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @if ($variant->compare_price && $variant->compare_price > $variant->price)
                                    <div class="text-xs text-gray-500 line-through">Rp
                                        {{ number_format($variant->compare_price, 0, ',', '.') }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex gap-2">
                                    <a href="{{ route('cosmetic.variants.show', $variant->id) }}"
    ...
    <div id="add-variant-modal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-full max-w-4xl shadow-lg rounded-lg bg-white dark:bg-gray-800">
            ...
            <form method="POST" action="{{ route('cosmetic.variants.store') }}" class="space-y-4">
                @csrf
