                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right text-gray-500">Rp {{ number_format($item->cost_price, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-right text-gray-900">Rp {{ number_format($item->retail_price, 0, ',', '.') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <form method="POST" action="{{ route('consignment.sales-report.store', $ship) }}" class="space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div><label class="block text-xs font-medium text-gray-600 mb-1">Periode Mulai</label><input type="date" name="period_start" required class="{{ $cls }}"></div>
                        <div><label class="block text-xs font-medium text-gray-600 mb-1">Periode Akhir</label><input type="date" name="period_end" required value="{{ date('Y-m-d') }}" class="{{ $cls }}"></div>
                    </div>
                    @foreach($ship->items as $item)
            ...
            <div class="divide-y divide-gray-100">
                ...
                <div class="p-4">
                    <div class="flex items-center justify-between mb-2">
                        <div>
                            <span class="font-mono text-xs font-medium text-gray-900">{{ $rpt->number }}</span>
                    ...
                    <div class="grid grid-cols-4 gap-3 text-xs mb-2">
                        ...
                        <div><span class="text-gray-500">Komisi ({{ $rpt->commission_pct }}%):</span> <span class="text-red-500">Rp {{ number_format($rpt->commission_amount, 0, ',', '.') }}</span></div>
                        <div><span class="text-gray-500">Net:</span> <span class="font-semibold text-gray-900">Rp {{ number_format($rpt->net_receivable, 0, ',', '.') }}</span></div>
                        <div><span class="text-gray-500">Sisa:</span> <span class="{{ $rpt->remainingBalance() > 0 ? 'text-amber-500' : 'text-green-500' }}">Rp {{ number_format($rpt->remainingBalance(), 0, ',', '.') }}</span></div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('consignment.settlement.store', $rpt) }}" class="flex items-end gap-2 mt-2">
                        @csrf
                        <input type="date" name="settlement_date" required value="{{ date('Y-m-d') }}" class="px-2 py-1 text-xs rounded-lg border border-gray-200 bg-gray-50 text-gray-900">
