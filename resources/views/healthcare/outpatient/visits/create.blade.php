    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('healthcare.outpatient.visits.store') }}" method="POST" class="space-y-6">
            ...
            <div
                class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Detail Kunjungan</h3>
                </div>
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="visit_date"
                                ...
                                class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 @error('visit_date') border-red-500 @enderror">
                            @error('visit_date')
                                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="purpose" rows="3" required placeholder="Jelaskan keperluan atau keluhan pasien..."
                                class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0f172a] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 @error('purpose') border-red-500 @enderror">{{ old('purpose') }}</textarea>
                            @error('purpose')
                                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-700 dark:text-slate-300">Buat nomor antrian
                                    otomatis</span>
                            </label>
                        </div>
                    </div>
