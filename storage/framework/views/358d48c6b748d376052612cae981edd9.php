...
<?php $component->withAttributes([]); ?>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1"><?php echo e($summary['total_programs']); ?></p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
    ...
    <div class="flex flex-col lg:flex-row gap-5">
        ...
        <div class="flex-1 min-w-0">
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                <td class="px-4 py-3">
                                    <p class="font-medium text-gray-900 dark:text-white"><?php echo e($s->program->name ?? '-'); ?></p>
                                    <p class="text-xs text-gray-400 dark:text-slate-500"><?php echo e($s->program->category ?? ''); ?> <?php echo e($s->location ? '· '.$s->location : ''); ?></p>
                                </td>
                                ...
                                <td class="px-4 py-3 hidden md:table-cell text-gray-500 dark:text-slate-400 text-xs"><?php echo e($s->trainer ?? '—'); ?></td>
                                <td class="px-4 py-3 text-center">
                                    <span class="text-sm font-medium text-gray-900 dark:text-white"><?php echo e($s->participants_count); ?></span>
                                    <?php if($s->max_participants > 0): ?>
                                    <span class="text-xs text-gray-400 dark:text-slate-500">/<?php echo e($s->max_participants); ?></span>
                                    <?php endif; ?>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <?php $sc = ['scheduled'=>'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400','ongoing'=>'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400','completed'=>'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400','cancelled'=>'bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-slate-400'][$s->status] ?? ''; ?>
                ...
                <div class="px-4 py-3 border-t border-gray-100 dark:border-white/10"><?php echo e($sessions->links()); ?></div>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col lg:flex-row gap-5">
        ...
        <div class="flex-1 min-w-0 space-y-3">
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5 <?php echo e($days !== null && $days <= 30 ? 'bg-red-50/30 dark:bg-red-500/5' : ''); ?>">
                                <td class="px-4 py-3">
                                    <p class="font-medium text-gray-900 dark:text-white text-xs"><?php echo e($cert->employee->name ?? '-'); ?></p>
                                    <p class="text-xs text-gray-400 dark:text-slate-500"><?php echo e($cert->employee->department ?? $cert->employee->position ?? ''); ?></p>
                                </td>
                                <td class="px-4 py-3">
                                    <p class="font-medium text-gray-900 dark:text-white text-xs"><?php echo e($cert->name); ?></p>
                                    <?php if($cert->certificate_number): ?>
                                    <p class="text-xs text-gray-400 dark:text-slate-500"><?php echo e($cert->certificate_number); ?></p>
                                    <?php endif; ?>
                                </td>
                                <td class="px-4 py-3 hidden sm:table-cell text-gray-500 dark:text-slate-400 text-xs"><?php echo e($cert->issuer ?? '—'); ?></td>
                                <td class="px-4 py-3 hidden md:table-cell text-center text-xs text-gray-500 dark:text-slate-400"><?php echo e($cert->issued_date->format('d M Y')); ?></td>
                                <td class="px-4 py-3 text-center">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-400 dark:text-slate-500 mt-0.5"><?php echo e($days); ?>h lagi</p>
                ...
                <div class="px-4 py-3 border-t border-gray-100 dark:border-white/10"><?php echo e($certifications->links()); ?></div>
                <?php endif; ?>
            </div>
        </div>
    </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                <td class="px-4 py-3">
                                    <p class="font-medium text-gray-900 dark:text-white"><?php echo e($p->name); ?></p>
                                    <?php if($p->description): ?>
                                    <p class="text-xs text-gray-400 dark:text-slate-500 truncate max-w-[200px]"><?php echo e($p->description); ?></p>
                                    <?php endif; ?>
                                </td>
                                <td class="px-4 py-3 hidden sm:table-cell">
                                    <?php if($p->category): ?>
                                    <span class="px-2 py-0.5 rounded-full text-xs bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400"><?php echo e($p->category); ?></span>
                                    <?php else: ?>
                                    <span class="text-gray-400 dark:text-slate-500">—</span>
                                    <?php endif; ?>
                                </td>
                                <td class="px-4 py-3 hidden md:table-cell text-gray-500 dark:text-slate-400 text-xs"><?php echo e($p->provider ?? '—'); ?></td>
                                <td class="px-4 py-3 hidden sm:table-cell text-center text-gray-600 dark:text-slate-300 text-xs"><?php echo e($p->duration_hours); ?>j</td>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3 font-medium text-gray-900 dark:text-white sticky left-0 bg-white dark:bg-[#1e293b]"><?php echo e($dept); ?></td>
                        <?php $__currentLoopData = $matrix['categories']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $count = $cats[$cat] ?? 0; ?>
                        <td class="px-4 py-3 text-center">
                            <?php if($count > 0): ?>
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-full text-xs font-bold
                                <?php echo e($count >= 5 ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' :
                        ...
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <td class="px-4 py-3 text-center font-semibold text-gray-900 dark:text-white"><?php echo e($rowTotal); ?></td>
                    </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </tbody>
            </table>
...
<?php endif; ?>
<?php /**PATH E:\PROJEKU\qalcuityERP\resources\views/hrm/training.blade.php ENDPATH**/ ?>