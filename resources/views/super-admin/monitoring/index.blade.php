<x-app-layout>
    ...
    <div x-data="detailModal()" x-cloak>
        ...
        ['tab' => 'modules', 'label' => 'Module Health', 'icon' => 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10'],
              ...
              {{ $tab === $t['tab'] ? 'bg-blue-600 text-gray-900' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100' }}">
                    ...
                    @if ($t['tab'] === 'errors' && $errorStats['unresolved'] > 0)
                        <span
                            class="text-[10px] bg-red-500 text-gray-900 px-1.5 py-0.5 rounded-full font-bold">{{ $errorStats['unresolved'] }}</span>
                    @endif
                    @if ($t['tab'] === 'anomaly' && $anomalyStats['critical'] > 0)
                        <span
                            class="text-[10px] bg-red-500 text-gray-900 px-1.5 py-0.5 rounded-full font-bold">{{ $anomalyStats['critical'] }}</span>
                    @endif
                </a>
            @endforeach
        </div>
        ...
        @if ($tab === 'errors')
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                @foreach ([['label' => 'Total Error', 'value' => $errorStats['total'], 'color' => 'text-gray-700', 'bg' => 'bg-gray-50'], ['label' => 'Belum Resolved', 'value' => $errorStats['unresolved'], 'color' => 'text-red-600', 'bg' => 'bg-red-50'], ['label' => 'Hari Ini', 'value' => $errorStats['today'], 'color' => 'text-orange-600', 'bg' => 'bg-orange-50'], ['label' => 'Critical', 'value' => $errorStats['critical'], 'color' => 'text-red-600', 'bg' => 'bg-red-50']] as $s)
                    <div class="bg-white border border-gray-200 rounded-2xl p-4 text-center {{ $s['bg'] }}">
                ...
                <form method="GET" action="" class="flex gap-2 flex-wrap flex-1">
                    ...
                    <select name="level"
                        ...
                        @foreach (['critical', 'error', 'warning', 'info'] as $lv)
                            <option value="{{ $lv }}" @selected(request('level') === $lv)>{{ ucfirst($lv) }}
                            </option>
                        @endforeach
                    </select>
                    <label class="flex items-center gap-2 text-sm text-gray-500 cursor-pointer">
                        <input type="checkbox" name="unresolved" value="1" @checked(request('unresolved'))
                            class="rounded">
                        Belum resolved
                    </label>
                    <button type="submit"
            ...
            <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($errors as $err)
                                <tr class="hover:bg-gray-50 transition cursor-pointer {{ $err->is_resolved ? 'opacity-60' : '' }}"
                    ...
                    'tenant_id'=>$err->tenant_id,'user_id'=>$err->user_id,
                                    ...
                                    <td class="px-4 py-3 max-w-xs">
                                        <p class="text-gray-800 text-xs font-medium truncate">{{ $err->message }}</p>
                                        @if ($err->file)
                                            <p class="text-gray-400 text-[10px] truncate mt-0.5">
                                                {{ basename($err->file) }}:{{ $err->line }}</p>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 hidden md:table-cell">
                                        <p class="text-gray-500 text-xs truncate max-w-[180px]">{{ $err->method }}
                                            {{ $err->url }}</p>
                                        @if ($err->ip)
                                            <p class="text-gray-400 text-[10px]">{{ $err->ip }}</p>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 hidden lg:table-cell">
                                        <p class="text-gray-500 text-xs">{{ $err->created_at->format('d M Y') }}</p>
                                        <p class="text-gray-400 text-[10px]">{{ $err->created_at->format('H:i:s') }}
                                        </p>
                                    </td>
                                    <td class="px-4 py-3">
                                        @if ($err->is_resolved)
                                    ...
                                    <td class="px-4 py-3" @click.stop>
                                        <div class="flex items-center justify-end gap-1">
                                            ...
                                            @if (!$err->is_resolved)
                                                <form method="POST"
                                                    action="{{ route('super-admin.monitoring.resolve-error', $err) }}"
                                                    @click.stop>
                                                    @csrf
                                                    <button
                ...
                @if ($errors->hasPages())
                    <div class="px-6 py-4 border-t border-gray-200">{{ $errors->links() }}</div>
                @endif
            </div>
        @endif
        ...
        @if ($tab === 'ai')
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
                @foreach ([['label' => 'Pesan Bulan Ini', 'value' => number_format($aiStats['total_this_month']), 'color' => 'text-blue-600'], ['label' => 'Total Token', 'value' => number_format($aiStats['total_tokens']), 'color' => 'text-purple-600'], ['label' => 'Tenant Aktif AI', 'value' => $aiStats['active_tenants'], 'color' => 'text-green-600']] as $s)
                ...
                <div class="flex items-end gap-2 h-24">
                    ...
                    @foreach ($aiMonthly->reverse() as $m)
                        ...
                        <div class="flex-1 flex flex-col items-center gap-1">
                            <span class="text-[10px] text-gray-500">{{ number_format($m->total) }}</span>
                            <div class="w-full rounded-t-lg"
                                style="height:{{ max(4, $pct * 0.8) }}px; background: linear-gradient(to top, #3b82f6, #6366f1);">
                            </div>
                            <span
                                class="text-[10px] text-gray-400">{{ \Carbon\Carbon::createFromFormat('Y-m', $m->month)->format('M') }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
            <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-200">
                    <p class="text-sm font-semibold text-gray-800">Penggunaan per Tenant — {{ now()->format('F Y') }}
                    </p>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                    ...
                    'breakdown' => $breakdown,
                                    ...
                                    <td class="px-5 py-3 text-right text-gray-700 text-xs font-semibold">
                                        {{ number_format($u->total_messages) }}</td>
                                    <td class="px-5 py-3 text-right text-gray-500 text-xs">
                                        {{ number_format($u->total_tokens) }}</td>
                                    <td class="px-5 py-3 hidden md:table-cell">
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span
                                                class="text-[10px] text-gray-400 w-8 text-right">{{ $pct }}%</span>
                                        </div>
                                    </td>
                ...
                <form method="GET" action="" class="flex gap-2 flex-wrap flex-1">
                    ...
                    <select name="tenant_id"
                        ...
                        @foreach ($tenants as $t)
                            <option value="{{ $t->id }}" @selected(request('tenant_id') == $t->id)>{{ $t->name }}
                            </option>
                        @endforeach
                    </select>
                    <button type="submit"
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($activities as $act)
                                ...
                                <tr class="hover:bg-gray-50 transition cursor-pointer"
                                    ...
                                    <td class="px-4 py-3 max-w-xs">
                                        <p class="text-gray-700 text-xs truncate">{{ $act->description }}</p>
                                        @if ($act->model_type)
                                            <p class="text-gray-400 text-[10px]">
                                                {{ class_basename($act->model_type) }} #{{ $act->model_id }}</p>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 hidden md:table-cell">
                                        <p class="text-gray-700 text-xs">{{ $act->user?->name ?? 'System' }}</p>
                                        <p class="text-gray-400 text-[10px]">{{ $act->user?->email }}</p>
                                    </td>
                                    <td class="px-4 py-3 hidden lg:table-cell">
                                        <p class="text-gray-500 text-xs font-mono">{{ $act->ip_address ?? '-' }}</p>
                                    </td>
                                    <td class="px-4 py-3">
                                        <p class="text-gray-500 text-xs">{{ $act->created_at->format('d M Y') }}</p>
                                        <p class="text-gray-400 text-[10px]">{{ $act->created_at->format('H:i:s') }}
                                        </p>
                                    </td>
        ...
        @if ($tab === 'anomaly')
            <div class="grid grid-cols-3 gap-3 mb-5">
                @foreach ([['label' => 'Open', 'value' => $anomalyStats['open'], 'color' => 'text-gray-700', 'bg' => 'bg-gray-50'], ['label' => 'Critical', 'value' => $anomalyStats['critical'], 'color' => 'text-red-600', 'bg' => 'bg-red-50'], ['label' => 'Warning', 'value' => $anomalyStats['warning'], 'color' => 'text-amber-600', 'bg' => 'bg-amber-50']] as $s)
                    <div class="bg-white border border-gray-200 rounded-2xl p-4 text-center {{ $s['bg'] }}">
                        <p class="text-2xl font-bold {{ $s['color'] }}">{{ $s['value'] }}</p>
                        <p class="text-xs text-gray-400 mt-1">{{ $s['label'] }}</p>
                    </div>
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-1 flex-wrap">
                                    <span
                                        class="text-[11px] font-semibold px-2 py-0.5 rounded-full {{ $sevBadge }}">{{ strtoupper($a->severity) }}</span>
                                    <span
                                        class="text-[11px] text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full">{{ $a->type }}</span>
                                    <span class="text-[11px] text-gray-400">{{ $a->tenant?->name }}</span>
                                </div>
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                <p class="text-[10px] text-gray-400">{{ $a->created_at->diffForHumans() }}</p>
                                <button
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full {{ $health['db_ok'] ? 'text-green-600 bg-green-100' : 'text-red-600 bg-red-100' }}">
                            {{ $health['db_ok'] ? 'Online' : 'Error' }}
                        </span>
                    </div>
                    <p class="text-2xl font-bold text-gray-900">{{ $health['db_latency_ms'] }} <span
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full {{ $health['disk_used_pct'] > 85 ? 'text-red-600 bg-red-100' : 'text-green-600 bg-green-100' }}">
                            {{ $health['disk_used_pct'] }}% terpakai
                        </span>
                    </div>
                    <div class="h-2 bg-gray-100 rounded-full overflow-hidden mb-2">
                        <div class="h-full rounded-full {{ $health['disk_used_pct'] > 85 ? 'bg-red-500' : 'bg-blue-500' }}"
                            style="width:{{ $health['disk_used_pct'] }}%"></div>
                    </div>
                    <p class="text-xs text-gray-400">{{ $health['disk_free_gb'] }} GB bebas dari
                        {{ $health['disk_total_gb'] }} GB</p>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full {{ $health['log_size_mb'] > 100 ? 'text-amber-600 bg-amber-100' : 'text-green-600 bg-green-100' }}">
                            {{ $health['log_size_mb'] > 100 ? 'Besar' : 'Normal' }}
                        </span>
                    </div>
                    <p class="text-2xl font-bold text-gray-900">{{ $health['log_size_mb'] }} <span
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Total</span><span
                                class="text-gray-800 font-semibold">{{ $health['total_tenants'] }}</span></div>
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Aktif</span><span
                                class="text-green-600 font-semibold">{{ $health['active_tenants'] }}</span></div>
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Total User</span><span
                                class="text-gray-800 font-semibold">{{ $health['total_users'] }}</span></div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full {{ $health['queue_failed'] > 0 ? 'text-red-600 bg-red-100' : 'text-green-600 bg-green-100' }}">
                            {{ $health['queue_failed'] > 0 ? $health['queue_failed'] . ' failed' : 'OK' }}
                        </span>
                    </div>
                    <p
                        class="text-2xl font-bold {{ $health['queue_failed'] > 0 ? 'text-red-600' : 'text-gray-900' }}">
                        {{ $health['queue_failed'] }}</p>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs"><span class="text-gray-500">PHP</span><span
                                class="text-gray-800 font-mono">{{ $health['php_version'] }}</span></div>
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Laravel</span><span
                                class="text-gray-800 font-mono">{{ $health['laravel_version'] }}</span></div>
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Uptime</span><span
                                class="text-gray-800 font-mono">{{ $health['uptime'] }}</span></div>
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Error Hari
                                Ini</span><span
                                class="{{ $health['errors_today'] > 0 ? 'text-red-600' : 'text-green-600' }} font-semibold">{{ $health['errors_today'] }}</span>
                        </div>
                    </div>
                </div>
        ...
        @if ($tab === 'modules')
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                <div class="bg-white border border-gray-200 rounded-2xl p-4">
                    ...
                    <p class="text-2xl font-black text-gray-900">{{ count($moduleHealth['modules']) }}</p>
                </div>
                <div class="bg-white border border-gray-200 rounded-2xl p-4">
                    <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Total Alert</p>
                    <p
                        class="text-2xl font-black {{ $moduleHealth['total_alerts'] > 0 ? 'text-amber-600' : 'text-green-600' }}">
                        {{ $moduleHealth['total_alerts'] }}</p>
                </div>
                <div class="bg-white border border-gray-200 rounded-2xl p-4">
                    <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Critical</p>
                    <p
                        class="text-2xl font-black {{ $moduleHealth['critical_alerts'] > 0 ? 'text-red-600' : 'text-green-600' }}">
                        {{ $moduleHealth['critical_alerts'] }}</p>
                </div>
                <div class="bg-white border border-gray-200 rounded-2xl p-4">
                    <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Status</p>
                    <p
                        class="text-2xl font-black {{ $moduleHealth['critical_alerts'] > 0 ? 'text-red-600' : ($moduleHealth['total_alerts'] > 0 ? 'text-amber-600' : 'text-green-600') }}">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                @foreach ($moduleHealth['modules'] as $key => $mod)
                    ...
                    <div class="{{ $bgColor }} border {{ $borderColor }} rounded-2xl p-4">
                        <div class="flex items-center justify-between mb-2">
                            <h4 class="text-sm font-semibold text-gray-900">{{ $mod['label'] }}</h4>
                            @if ($hasCritical)
                                <span
                        ...
                        <p class="text-xs text-gray-400 mb-2">Total records: <span
                                class="text-gray-700 font-medium">{{ number_format($mod['total']) }}</span></p>
                        @if ($hasAlerts)
                            <div class="space-y-1">
                                @foreach ($mod['alerts'] as $alert)
                                    ...
                                    @endphp
                                    <p class="text-[11px] {{ $ac }} px-2 py-1 rounded-lg">
                                        {{ $alert['msg'] }}</p>
                                @endforeach
                            </div>
                        @endif
