    ...
    <div class="flex flex-col lg:flex-row gap-5">
        ...
        <div class="lg:w-64 shrink-0 space-y-4">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-4">
                ...
                <div class="space-y-1.5" id="shift-palette">
                    @forelse($shifts as $shift)
                        ...
                        <div draggable="true" data-shift-id="{{ $shift->id }}" data-shift-name="{{ $shift->name }}"
                            data-shift-color="{{ $shift->color }}" data-shift-time="{{ $shift->timeLabel() }}"
                            ...
                            <button
                                onclick="openEditShift({{ $shift->id }}, {{ json_encode($shift->only(['name', 'color', 'start_time', 'end_time', 'break_minutes', 'crosses_midnight', 'description'])) }})"
            ...
            <form method="POST" action="{{ route('hrm.shifts.copy-week') }}">
                ...
                <button type="submit" data-confirm="Salin jadwal minggu ini ke minggu depan?"
        ...
        <div class="flex-1 min-w-0 space-y-4">
            ...
            <div id="conflict-panel" class="hidden bg-white rounded-2xl border border-orange-200 p-4">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <button onclick="document.getElementById('conflict-panel').classList.add('hidden')"
                        class="text-gray-400 hover:text-gray-600 text-xs">✕ Tutup</button>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm border-collapse" id="scheduler-table">
                        <thead>
                            <tr class="bg-gray-50">
                                <th
                                    class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase w-40 border-b border-gray-100 sticky left-0 bg-gray-50 z-10">
                                    Karyawan</th>
                                @foreach ($weekDays ?? [] as $i => $day)
                                    <th
                                        class="px-2 py-3 text-center text-xs font-semibold border-b border-gray-100 min-w-[90px]
                                    {{ $day->isToday() ? 'text-blue-600' : 'text-gray-500' }}">
                                        <p>{{ $days[$i] }}</p>
                                        <p
                                            class="text-base font-bold {{ $day->isToday() ? 'text-blue-600' : 'text-gray-900' }}">
                                            {{ $day->format('d') }}
                                        </p>
                                    </th>
                                @endforeach
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($employees as $emp)
                                <tr class="hover:bg-gray-50/50 group" data-emp-id="{{ $emp->id }}">
                                    <td
                                        class="px-4 py-2 border-r border-gray-100 sticky left-0 bg-white group-hover:bg-gray-50/50 z-10">
                                        <p class="font-medium text-gray-900 text-xs truncate max-w-[140px]">
                                            {{ $emp->name }}</p>
                                        <p class="text-xs text-gray-400 truncate">
                                            {{ $emp->department ?? ($emp->position ?? '-') }}</p>
                                    </td>
                                    @foreach ($weekDays ?? [] as $day)
                                        ...
                                        <td class="px-1 py-1 text-center {{ $isWeekend ? 'bg-gray-50/50' : '' }}"
                                            ...
                                            {{ $schedule ? 'cursor-grab active:cursor-grabbing' : ($isWeekend ? 'cursor-default' : 'hover:bg-gray-100 cursor-pointer') }}"
                                                @if ($schedule) style="background-color: {{ $schedule->shift?->color }}22; border: 1px solid {{ $schedule->shift?->color }}55; color: {{ $schedule->shift?->color }}" @endif>
                                                @if ($schedule)
                                                    <span
                                                        class="font-semibold leading-tight">{{ $schedule->shift?->name }}</span>
                                                    <span
                                                        class="opacity-75 leading-tight text-[10px]">{{ $schedule->shift?->timeLabel() }}</span>
                                                @else
                                                    <span
                                                        class="text-lg leading-none text-gray-300">{{ $isWeekend ? '—' : '+' }}</span>
                                                @endif
                                            </button>
                                        </td>
                                    @endforeach
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-4 py-12 text-center text-gray-400">Belum ada karyawan
                                        aktif.</td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <div id="modal-shift-picker"
        ...
        <div class="bg-white rounded-2xl border border-gray-200 w-full max-w-sm shadow-2xl">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-shift-picker').classList.add('hidden')"
                ...
                @forelse($shifts as $shift)
                    <button onclick="assignShift(pickerEmpId, pickerDate, {{ $shift->id }})"
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-400">{{ $shift->timeLabel() }} ·
                                {{ round($shift->workMinutes() / 60, 1) }} jam</p>
                        </div>
                    </button>
                @empty
    ...
    <div id="modal-add-shift"
        ...
        <div class="bg-white rounded-2xl border border-gray-200 w-full max-w-md shadow-2xl">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-add-shift').classList.add('hidden')"
                    class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form method="POST" action="{{ route('hrm.shifts.shift.store') }}" class="p-5 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="col-span-2 flex items-center gap-2">
                        <input type="checkbox" name="crosses_midnight" id="add-crosses-midnight" value="1"
                            class="rounded">
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button"
                        onclick="document.getElementById('modal-add-shift').classList.add('hidden')"
    ...
    <div id="modal-edit-shift"
        ...
        <div class="bg-white rounded-2xl border border-gray-200 w-full max-w-md shadow-2xl">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-edit-shift').classList.add('hidden')"
                    class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form id="form-edit-shift" method="POST" class="p-5 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="col-span-2 flex items-center gap-2">
                        <input type="checkbox" name="crosses_midnight" id="edit-crosses-midnight" value="1"
                            class="rounded">
                ...
                <div class="flex justify-between items-center pt-2">
                    ...
                    <div class="flex gap-2">
                        <button type="button"
                            onclick="document.getElementById('modal-edit-shift').classList.add('hidden')"
                            class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
                        <button type="submit"
                            class="px-4 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Simpan</button>
                    </div>
        ...
        <script>
            ...
            function doAssign(empId, date, shiftId) {
                fetch(ASSIGN_URL, {
                    ...
                    .catch(() => Dialog.warning('Gagal menyimpan jadwal.'));
            }
            ...
            function openEditShift(id, data) {
                ...
                document.getElementById('btn-delete-shift').onclick = async () => {
                    const confirmed = await Dialog.danger('Nonaktifkan shift ini?');
                    if (!confirmed) return;
            ...
            function runConflictDetection() {
                ...
                <div class="flex items-start gap-2 p-2 rounded-lg bg-orange-50 border border-orange-200">
                    ...
                    <p class="text-xs text-orange-700">${c}</p>
                            ...
                            content.innerHTML =
                                '<p class="text-xs text-green-600 text-center py-2">✓ Tidak ada konflik jadwal ditemukan.</p>';
                        }
                        if (data.summary) {
                    ...
                    .catch(() => {
                        ...
                        btn.disabled = false;
                        content.innerHTML =
                            '<p class="text-xs text-red-400 text-center py-2">Gagal menganalisis konflik.</p>';
                    });
            }
        </script>
    @endpush
