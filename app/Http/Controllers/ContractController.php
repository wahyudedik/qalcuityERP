...
class ContractController extends Controller
{
    ...
    {
        abort_if($contract->tenant_id !== $this->tid(), 403);
        if ($contract->status !== 'draft') return back()->with('error', 'Hanya kontrak draft yang bisa diaktifkan.');
    ...
    {
        abort_if($contract->tenant_id !== $this->tid(), 403);
        if (!in_array($contract->status, ['active', 'draft'])) return back()->with('error', 'Kontrak tidak bisa diterminasi.');
    ...
    {
        abort_if($contract->tenant_id !== $this->tid(), 403);
        if ($contract->status !== 'active') return back()->with('error', 'Kontrak harus aktif.');
        if (!$contract->next_billing_date) return back()->with('error', 'Tidak ada jadwal billing.');
    ...
    public function resolveSlaLog(Request $request, ContractSlaLog $contractSlaLog)
    {
        ...
        if ($data['responded_at'] && $contract->sla_response_hours) {
            ...
            if ($responseH > $contract->sla_response_hours) $slaMet = false;
        }
        if ($contract->sla_resolution_hours) {
            $resolveH = $contractSlaLog->reported_at->diffInMinutes($data['resolved_at']) / 60;
            if ($resolveH > $contract->sla_resolution_hours) $slaMet = false;
        }
    ...
    {
        abort_if($contract->tenant_id !== $this->tid(), 403);
        if ($contract->status === 'active') return back()->with('error', 'Kontrak aktif tidak bisa dihapus. Terminasi dulu.');
        $contract->delete();
