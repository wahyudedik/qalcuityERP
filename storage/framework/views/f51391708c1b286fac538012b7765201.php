...
<?php $component->withAttributes([]); ?>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
            ...
            <p class="text-lg font-bold text-gray-900 dark:text-white">Rp
                <?php echo e(number_format($stats['mrr'], 0, ',', '.')); ?></p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
        ...
        <div class="flex gap-2">
            <a href="<?php echo e(route('subscription-billing.plans')); ?>"
                class="px-3 py-2 text-sm border border-gray-200 dark:border-white/10 rounded-xl text-gray-600 dark:text-slate-300">Plans</a>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $subscriptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            <td class="px-4 py-3 font-mono text-xs font-medium text-gray-900 dark:text-white">
                                <a href="<?php echo e(route('subscription-billing.show', $s)); ?>"
                                    class="hover:text-blue-500"><?php echo e($s->subscription_number); ?></a>
                            </td>
                            <td class="px-4 py-3 text-gray-700 dark:text-slate-300"><?php echo e($s->customer->name ?? '-'); ?>

                            </td>
                            <td class="px-4 py-3 hidden sm:table-cell text-gray-500 dark:text-slate-400 text-xs">
                                <?php echo e($s->plan->name ?? '-'); ?> (<?php echo e($s->plan->cycleLabel() ?? ''); ?>)</td>
                            <td class="px-4 py-3 text-right hidden md:table-cell text-gray-900 dark:text-white">Rp
                                <?php echo e(number_format($s->effectivePrice(), 0, ',', '.')); ?></td>
                            <td
                                class="px-4 py-3 text-center hidden md:table-cell text-xs <?php echo e($s->next_billing_date->isPast() ? 'text-red-500 font-semibold' : 'text-gray-500 dark:text-slate-400'); ?>">
                                <?php echo e($s->next_billing_date->format('d/m/Y')); ?></td>
                            <td class="px-4 py-3 text-center"><span
                                    class="px-2 py-0.5 rounded-full text-xs bg-<?php echo e($sc); ?>-100 text-<?php echo e($sc); ?>-700 dark:bg-<?php echo e($sc); ?>-500/20 dark:text-<?php echo e($sc); ?>-400"><?php echo e($sl); ?></span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <a href="<?php echo e(route('subscription-billing.show', $s)); ?>"
                                        class="text-xs px-2 py-1 border border-gray-200 dark:border-white/10 rounded-lg text-gray-600 dark:text-slate-300">Detail</a>
                                    <?php if(in_array($s->status, ['active', 'trial']) && $s->next_billing_date->lte(today())): ?>
                                        <?php if(auth()->check() && app(\App\Services\PermissionService::class)->check(auth()->user(), 'subscription_billing', 'create')): ?>
                                        <form method="POST" action="<?php echo e(route('subscription-billing.generate', $s)); ?>"
                                            class="inline"><?php echo csrf_field(); ?>
                                            <button type="submit"
                                                class="text-xs px-2 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700">Invoice</button>
                                        </form>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                </div>
        ...
        <?php if($subscriptions->hasPages()): ?>
            <div class="px-4 py-3 border-t border-gray-100 dark:border-white/5"><?php echo e($subscriptions->links()); ?></div>
        <?php endif; ?>
    </div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="<?php echo e(route('subscription-billing.store')); ?>" class="p-6 space-y-4">
                ...
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Customer *</label>
                    <select name="customer_id" required class="<?php echo e($cls); ?>">
                ...
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Plan *</label>
                    <select name="plan_id" required class="<?php echo e($cls); ?>">
                        <option value="">-- Pilih --</option>
                        <?php $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($p->id); ?>"><?php echo e($p->name); ?> — Rp
                                <?php echo e(number_format($p->price, 0, ',', '.')); ?>/<?php echo e($p->cycleLabel()); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Mulai
                            *</label><input type="date" name="start_date" required value="<?php echo e(date('Y-m-d')); ?>"
                            class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Diskon
                            (%)</label><input type="number" name="discount_pct" min="0" max="100"
                            step="0.01" value="0" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Harga
                            Override</label><input type="number" name="price_override" min="0"
                            step="1000" placeholder="Kosong = pakai harga plan" class="<?php echo e($cls); ?>">
                    </div>
                    <div class="flex items-end"><label class="flex items-center gap-2 cursor-pointer"><input
                                type="checkbox" name="auto_renew" value="1" checked class="rounded"><span
                                class="text-sm text-gray-700 dark:text-slate-300">Auto Renew</span></label></div>
                </div>
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit"
                        class="px-4 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Buat</button>
                </div>
            </form>
        </div>
    </div>
