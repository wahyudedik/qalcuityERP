        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <form method="POST" action="{{ route('reimbursement.pay', $r) }}" class="inline" onsubmit="return confirm('Bayar reimbursement ini?')">
                                    @csrf
                                    <select name="payment_method" class="text-xs px-1 py-0.5 rounded border border-gray-200 bg-white text-gray-900">
                    ...
                    @empty
                    <tr><td colspan="7" class="px-4 py-12 text-center text-gray-400">Belum ada reimbursement.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($reimbursements->hasPages())<div class="px-4 py-3 border-t border-gray-100">{{ $reimbursements->links() }}</div>@endif
    </div>
        ...
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="{{ route('reimbursement.store') }}" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Tanggal *</label><input type="date" name="expense_date" required value="{{ date('Y-m-d') }}" class="{{ $cls }}"></div>
                </div>
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Deskripsi *</label><input type="text" name="description" required placeholder="Ongkos taksi meeting client" class="{{ $cls }}"></div>
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Jumlah (Rp) *</label><input type="number" name="amount" required min="1000" step="500" class="{{ $cls }}"></div>
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Foto Struk/Bukti</label><input type="file" name="receipt_image" accept="image/*" class="{{ $cls }}"></div>
