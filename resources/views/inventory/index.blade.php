        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4">
            ...
            <div class="flex gap-2">
                <a href="{{ route('inventory.movements') }}"
                    class="px-3 py-2 text-sm border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50">Riwayat
                    Stok</a>
                <a href="{{ route('inventory.warehouses') }}"
                    class="px-3 py-2 text-sm border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50">Gudang</a>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($products as $product)
                        @php
                            ...
                            $isLow = $totalStock <= $product->stock_min;
                        @endphp
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 hidden sm:table-cell text-gray-500 font-mono text-xs">
                                {{ $product->sku }}</td>
                            <td class="px-4 py-3 hidden md:table-cell text-gray-500">{{ $product->category ?? '-' }}
                            </td>
                            <td class="px-4 py-3 text-right font-medium text-gray-900">Rp
                                {{ number_format($product->price_sell, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-right">
                                <span
                                    class="font-semibold {{ $isLow ? 'text-red-600' : 'text-gray-900' }}">{{ $totalStock }}</span>
                                @if ($isLow)
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    @canmodule('inventory', 'create')
                                    <button
                                        onclick="openAddStock({{ $product->id }}, '{{ addslashes($product->name) }}', '{{ $product->unit }}')"
                                    ...
                                    @endcanmodule
                                    <button
                                        onclick="openAiDetail({{ $product->id }}, '{{ addslashes($product->name) }}', '{{ $product->unit }}')"
                                        class="p-1.5 rounded-lg text-purple-500 hover:bg-purple-50" title="Analisis AI">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.364.364A4.004 4.004 0 0112 16a4.004 4.004 0 01-2.772-1.1l-.364-.364z" />
                                        </svg>
                                    </button>
                                    @canmodule('inventory', 'edit')
                                    <button
                                        onclick="openEditProduct({{ $product->id }}, '{{ addslashes($product->name) }}', '{{ $product->sku }}', '{{ addslashes($product->category ?? '') }}', '{{ $product->unit }}', {{ $product->price_sell }}, {{ $product->price_buy }}, {{ $product->stock_min }}, {{ $product->is_active ? 'true' : 'false' }}, '{{ $product->image }}')"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center text-gray-400">Belum ada produk.
                                Tambahkan produk pertama Anda.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-xl">
            ...
            <form id="form-edit-product" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Kategori</label>
                        <input type="text" id="edit-category" name="category" list="cat-list"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Satuan *</label>
    ...
    <div id="modal-add-stock" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-sm shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-add-stock').classList.add('hidden')"
                    class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form id="form-add-stock" method="POST" class="p-6 space-y-4">
                ...
                <div class="flex justify-end gap-3">
                    <button type="button"
                        onclick="document.getElementById('modal-add-stock').classList.add('hidden')"
    ...
    <div id="modal-ai-inventory" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-ai-inventory').classList.add('hidden')"
                    class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
    ...
    @push('scripts')
        <script>
            ...
            async function loadBatchAnalysis() {
                try {
                    const res = await fetch(analyzeAllUrl, {
                        headers: {
            ...
            async function openAiDetail(productId, productName, unit) {
                ...
                try {
                    const [predRes, reorderRes] = await Promise.all([
                        fetch(stockoutBase + productId, {
                            headers: {
                                'X-Requested-With': 'XMLHttpRequest'
                            }
                        }),
                        fetch(reorderBase + productId, {
                            headers: {
                                'X-Requested-With': 'XMLHttpRequest'
                            }
                        }),
