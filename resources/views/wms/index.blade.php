    ...
    @if ($zones->isNotEmpty())
        <div class="flex flex-wrap gap-2 mb-4">
            <a href="?warehouse_id={{ $warehouseId }}"
                class="px-3 py-1.5 text-xs rounded-xl {{ !request('zone_id') ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600' }}">Semua</a>
            @foreach ($zones as $z)
                <a href="?warehouse_id={{ $warehouseId }}&zone_id={{ $z->id }}"
                    class="px-3 py-1.5 text-xs rounded-xl {{ request('zone_id') == $z->id ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600' }}">
