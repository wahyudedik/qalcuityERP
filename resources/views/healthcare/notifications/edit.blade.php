<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Notification Rule') }} -
            {{ $rule->name }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('healthcare.notifications.update', $rule) }}">
                    @csrf
                    @method('PUT')
                    <div class="space-y-6">
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700">Rule Name *</label>
                            <input type="text" name="name" required value="{{ old('name', $rule->name) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="trigger_event" required
                                    ...
                                    <option value="lab_result_critical"
                                        {{ old('trigger_event', $rule->trigger_event) === 'lab_result_critical' ? 'selected' : '' }}>
                                        Critical Lab Result</option>
                                    <option value="appointment_reminder"
                                        {{ old('trigger_event', $rule->trigger_event) === 'appointment_reminder' ? 'selected' : '' }}>
                                        Appointment Reminder</option>
                                    <option value="prescription_ready"
                                        {{ old('trigger_event', $rule->trigger_event) === 'prescription_ready' ? 'selected' : '' }}>
                                        Prescription Ready</option>
                                    <option value="admission_confirmed"
                                        {{ old('trigger_event', $rule->trigger_event) === 'admission_confirmed' ? 'selected' : '' }}>
                                        Admission Confirmed</option>
                                    <option value="discharge_summary"
                                        {{ old('trigger_event', $rule->trigger_event) === 'discharge_summary' ? 'selected' : '' }}>
                                        Discharge Summary</option>
                                    <option value="billing_overdue"
                                        {{ old('trigger_event', $rule->trigger_event) === 'billing_overdue' ? 'selected' : '' }}>
                                        Billing Overdue</option>
                                    <option value="equipment_maintenance"
                                        {{ old('trigger_event', $rule->trigger_event) === 'equipment_maintenance' ? 'selected' : '' }}>
                                        Equipment Maintenance</option>
                                    <option value="low_stock_alert"
                                        {{ old('trigger_event', $rule->trigger_event) === 'low_stock_alert' ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="priority" required
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    <option value="low"
                                        {{ old('priority', $rule->priority) === 'low' ? 'selected' : '' }}>Low</option>
                                    <option value="medium"
                                        {{ old('priority', $rule->priority) === 'medium' ? 'selected' : '' }}>Medium
                                    </option>
                                    <option value="high"
                                        {{ old('priority', $rule->priority) === 'high' ? 'selected' : '' }}>High
                                    </option>
                                    <option value="critical"
                                        {{ old('priority', $rule->priority) === 'critical' ? 'selected' : '' }}>
                                        Critical</option>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <label class="flex items-center">
                                    <input type="checkbox" name="channels[]" value="email"
                                        {{ in_array('email', $channels) ? 'checked' : '' }}
                        ...
                        <div>
                            ...
                            <input type="text" name="subject_template" required
                                value="{{ old('subject_template', $rule->subject_template) }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <textarea name="message_template" required rows="6"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('message_template', $rule->message_template) }}</textarea>
