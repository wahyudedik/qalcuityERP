        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    @php $totalStk = $product->productStocks->sum('quantity'); $isLow = $totalStk <= $product->stock_min; @endphp
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                @if($product->image)
                                <img src="{{ $product->image }}" alt="{{ $product->name }}" class="w-9 h-9 rounded-xl object-cover shrink-0 border border-gray-200 dark:border-white/10">
                                @else
                        ...
                        <td class="px-4 py-3 text-right text-gray-500 dark:text-slate-400">Rp {{ number_format($product->price_buy,0,',','.') }}</td>
                        <td class="px-4 py-3 text-right hidden lg:table-cell">
                            <span class="font-semibold {{ $isLow ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white' }}">{{ $totalStk }}</span>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button onclick="openEditProduct({{ $product->id }}, @js($product->name), @js($product->sku ?? ''), @js($product->category ?? ''), @js($product->unit), {{ $product->price_sell }}, {{ $product->price_buy }}, {{ $product->stock_min }}, {{ $product->is_active ? 'true' : 'false' }}, @js($product->image ?? ''), @js($product->description ?? ''))"
                                ...
                                <form method="POST" action="{{ route('products.toggle', $product) }}">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="p-1.5 rounded-lg {{ $product->is_active ? 'text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-500/10' : 'text-green-500 hover:bg-green-50 dark:hover:bg-green-500/10' }}" title="{{ $product->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
            ...
            <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <datalist id="cat-list-add">@foreach($categories as $c)<option value="{{ $c }}">@endforeach</datalist>
                    </div>
                    <div>
            ...
            <form id="form-edit-product" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <datalist id="cat-list-edit">@foreach($categories as $c)<option value="{{ $c }}">@endforeach</datalist>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Satuan *</label>
