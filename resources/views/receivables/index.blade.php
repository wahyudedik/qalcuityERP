<x-app-layout>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5 {{ $overdue ? 'bg-red-50/50 dark:bg-red-900/10' : '' }}">
                        ...
                        <td class="px-4 py-3 text-center">
                            ...
                            <div class="flex justify-center gap-1">
                                <button onclick="openPayModal('{{ $inv->id }}','{{ $inv->number }}','{{ $inv->remaining_amount }}')"
                                    class="text-xs px-2 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700">
                                    Bayar
                                </button>
                                <a href="{{ route('receivables.installments', $inv) }}"
    ...
    function openPayModal(id, number, remaining) {
        ...
        document.getElementById('form-pay').action = '{{ url("receivables") }}/' + id + '/payment';
