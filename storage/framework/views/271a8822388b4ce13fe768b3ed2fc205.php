...
<?php $component->withAttributes([]); ?>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        ...
                        <td class="px-4 py-3 text-gray-700 dark:text-slate-300"><?php echo e($bom->product->name ?? '-'); ?></td>
                        <td class="px-4 py-3 text-right text-gray-900 dark:text-white"><?php echo e(number_format($bom->batch_size, 0, ',', '.')); ?> <?php echo e($bom->batch_unit); ?></td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                <button onclick="openDetailModal(<?php echo e($bom->id); ?>)" class="text-xs px-2 py-1 border border-gray-200 dark:border-white/10 rounded-lg text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-white/5">Detail</button>
                                <?php if(auth()->check() && app(\App\Services\PermissionService::class)->check(auth()->user(), 'manufacturing', 'delete')): ?>
                                <form method="POST" action="<?php echo e(url('manufacturing/bom')); ?>/<?php echo e($bom->id); ?>" class="inline" onsubmit="return confirm('Hapus BOM ini?')">
    ...
    <div id="modal-detail-<?php echo e($bom->id); ?>" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-lg shadow-xl max-h-[80vh] overflow-y-auto">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/10">
                ...
                <button onclick="document.getElementById('modal-detail-<?php echo e($bom->id); ?>').classList.add('hidden')" class="text-gray-400 hover:text-gray-600 dark:hover:text-white">✕</button>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-2 gap-3 mb-4 text-sm">
                    <div><span class="text-gray-500 dark:text-slate-400">Produk:</span> <span class="text-gray-900 dark:text-white"><?php echo e($bom->product->name ?? '-'); ?></span></div>
                    <div><span class="text-gray-500 dark:text-slate-400">Batch:</span> <span class="text-gray-900 dark:text-white"><?php echo e($bom->batch_size); ?> <?php echo e($bom->batch_unit); ?></span></div>
                </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr>
                            <td class="py-1.5 text-gray-900 dark:text-white"><?php echo e($line->product->name ?? '-'); ?></td>
                            <td class="py-1.5 text-right text-gray-900 dark:text-white"><?php echo e(number_format($line->quantity_per_batch, 3)); ?></td>
                            <td class="py-1.5 text-gray-500 dark:text-slate-400"><?php echo e($line->unit); ?></td>
                            <td class="py-1.5">
                                <?php if($line->childBom): ?>
                                    <span class="px-1.5 py-0.5 rounded text-xs bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400"><?php echo e($line->childBom->name); ?></span>
                                <?php else: ?>
