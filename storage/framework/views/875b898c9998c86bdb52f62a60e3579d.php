...
<?php $__env->startSection('content'); ?>
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = $treatments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $treatment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    <?php echo e($treatment->date?->format('d M Y') ?? '-'); ?>

                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <?php echo e($treatment->herd?->name ?? 'N/A'); ?>

                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    <?php echo e($treatment->typeLabel()); ?>

                                </td>
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    <?php echo e(Str::limit($treatment->condition, 50)); ?>

                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <?php if($treatment->severity): ?>
                                        <span
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                            <?php if($treatment->severity === 'low'): ?> bg-gray-100 text-gray-800
                                            <?php elseif($treatment->severity === 'medium'): ?> bg-amber-100 text-amber-800
                                            <?php elseif($treatment->severity === 'high'): ?> bg-orange-100 text-orange-800
                                            <?php else: ?> bg-red-100 text-red-800 <?php endif; ?>">
                                            <?php echo e(ucfirst($treatment->severity)); ?>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                        <?php if($treatment->status === 'active'): ?> bg-yellow-100 text-yellow-800
                                        <?php elseif($treatment->status === 'resolved'): ?> bg-green-100 text-green-800
                                        <?php else: ?> bg-gray-100 text-gray-800 <?php endif; ?>">
                                        <?php echo e($treatment->status === 'active' ? 'Aktif' : ($treatment->status === 'resolved' ? 'Selesai' : ucfirst($treatment->status))); ?>
    ...
    <div id="addTreatmentModal"
        ...
        <div
            ...
            <form action="<?php echo e(route('livestock-enhancement.health.treatments.store')); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <div class="space-y-4">
                    <div>
