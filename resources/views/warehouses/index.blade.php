<x-app-layout>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-2xl p-4 border border-gray-200">
            ...
            <p class="text-2xl font-bold text-gray-900 mt-1">{{ $totalWarehouses }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 border border-gray-200">
            <p class="text-xs text-gray-500">Gudang Aktif</p>
            <p class="text-2xl font-bold text-green-600 mt-1">{{ $activeWarehouses }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 border border-gray-200">
            <p class="text-xs text-gray-500">Gudang Nonaktif</p>
            <p class="text-2xl font-bold text-gray-500 mt-1">{{ $totalWarehouses - $activeWarehouses }}</p>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($warehouses as $wh)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 hidden sm:table-cell font-mono text-xs text-gray-500">
                                {{ $wh->code }}</td>
                            <td class="px-4 py-3 hidden md:table-cell text-gray-500 text-xs max-w-xs truncate">
                                {{ $wh->address ?? '-' }}</td>
                            <td class="px-4 py-3 text-right hidden lg:table-cell text-gray-700">
                                {{ number_format($wh->product_stocks_count ?? 0) }}</td>
                            <td class="px-4 py-3 text-right font-semibold text-gray-900">
                                {{ number_format($wh->product_stocks_sum_quantity ?? 0) }}</td>
                            <td class="px-4 py-3 text-center hidden sm:table-cell">
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs {{ $wh->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <button
                                        onclick="openEditWarehouse({{ $wh->id }}, @js($wh->name), @js($wh->code ?? ''), @js($wh->address ?? ''))"
                                    ...
                                    <form method="POST" action="{{ route('warehouses.toggle', $wh) }}">
                                        @csrf @method('PATCH')
                                        <button type="submit"
                                            class="p-1.5 rounded-lg {{ $wh->is_active ? 'text-yellow-500 hover:bg-yellow-50' : 'text-green-500 hover:bg-green-50' }}"
                                    ...
                                    <form method="POST" action="{{ route('warehouses.destroy', $wh) }}"
                                        data-confirm="Hapus gudang ini?" data-confirm-type="danger">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center text-gray-400">Belum ada gudang. Klik "+
                                Gudang" untuk menambahkan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="modal-add-warehouse" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="{{ route('warehouses.store') }}" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Kode (opsional, auto-generate jika
                        kosong)</label>
