<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Telemedicine Consultation') }} -
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('healthcare.telemedicine.update', $telemedicine) }}">
                    @csrf
                    @method('PUT')
                    <div class="space-y-6">
                        <div>
                            ...
                            <select name="consultation_type" required
                                ...
                                <option value="video"
                                    {{ old('consultation_type', $telemedicine->consultation_type) === 'video' ? 'selected' : '' }}>
                                    Video Call</option>
                                <option value="voice"
                                    {{ old('consultation_type', $telemedicine->consultation_type) === 'voice' ? 'selected' : '' }}>
                                    Voice Call</option>
                                <option value="chat"
                                    {{ old('consultation_type', $telemedicine->consultation_type) === 'chat' ? 'selected' : '' }}>
                                    Chat</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status"
                                ...
                                <option value="in_progress"
                                    {{ old('status', $telemedicine->status) === 'in_progress' ? 'selected' : '' }}>In
                                    Progress</option>
                        ...
                        <div>
                            ...
                            <textarea name="prescription" rows="3"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                placeholder="Medications prescribed...">{{ old('prescription', $telemedicine->prescription) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="scheduled_at"
                                value="{{ old('scheduled_at', $telemedicine->scheduled_at ? $telemedicine->scheduled_at->format('Y-m-d\TH:i') : '') }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
