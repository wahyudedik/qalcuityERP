<x-app-layout>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ number_format($totalOpnames) }}</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            <p class="text-xs text-gray-500 dark:text-slate-400">Berlangsung</p>
            <p class="text-2xl font-bold text-blue-600 dark:text-blue-400 mt-1">{{ $inProgressOpnames }}</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            <p class="text-xs text-gray-500 dark:text-slate-400">Selesai</p>
            <p class="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">{{ $completedOpnames }}</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            <p class="text-xs text-gray-500 dark:text-slate-400">Total Discrepancy</p>
            <p class="text-2xl font-bold text-red-600 dark:text-red-400 mt-1">{{ number_format($totalDiscrepancies) }}
            </p>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    @forelse($opnames ?? [] as $opname)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            <td class="px-4 py-3">
                                <span
                                    class="font-mono text-sm font-bold text-blue-600 dark:text-blue-400">{{ $opname->opname_number ?? '-' }}</span>
                            </td>
                            <td class="px-4 py-3 hidden md:table-cell">
                                <p class="text-gray-900 dark:text-white">{{ $opname->location ?? '-' }}</p>
                            ...
                            <td class="px-4 py-3 text-center hidden sm:table-cell">
                                <span
                                    class="font-medium text-gray-900 dark:text-white">{{ $opname->items_checked ?? 0 }}</span>
                            </td>
                            <td class="px-4 py-3 text-center hidden lg:table-cell">
                                @if ($opname->discrepancy_count > 0)
                                    <span
                                        class="font-bold text-red-600 dark:text-red-400">{{ $opname->discrepancy_count }}</span>
                                @else
