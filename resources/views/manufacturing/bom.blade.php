                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    @forelse($boms as $bom)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            ...
                            <td class="px-4 py-3 text-gray-700 dark:text-slate-300">{{ $bom->product->name ?? '-' }}
                            </td>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-2 gap-3 mb-4 text-sm">
                        <div><span class="text-gray-500 dark:text-slate-400">Produk:</span> <span
                                class="text-gray-900 dark:text-white">{{ $bom->product->name ?? '-' }}</span></div>
                        <div><span class="text-gray-500 dark:text-slate-400">Batch:</span> <span
                                class="text-gray-900 dark:text-white">{{ $bom->batch_size }}
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            @foreach ($bom->lines as $line)
                                <tr>
                                    <td class="py-1.5 text-gray-900 dark:text-white">{{ $line->product->name ?? '-' }}
                                    </td>
                                    ...
                                    <td class="py-1.5">
                                        @if ($line->childBom)
                                            <span
                                                class="px-1.5 py-0.5 rounded text-xs bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400">{{ $line->childBom->name }}</span>
                                        @else
