            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 space-y-5">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">
                        ...
                        <x-help-icon topic="customer-selection" title="Bantuan memilih customer" />
                    </label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">
                        ...
                        <x-help-icon topic="sales-order-link" title="Bantuan link sales order" />
                    </label>
                    <select name="sales_order_id" id="sales_order_id"
                        class="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="">-- Tanpa Sales Order --</option>
                        @foreach ($orders as $o)
                            <option value="{{ $o->id }}" data-total="{{ $o->total }}"
                                {{ old('sales_order_id') == $o->id ? 'selected' : '' }}>
                                {{ $o->number }} — {{ $o->customer?->name ?? 'Walk-in' }} — Rp
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">
                        ...
                        <x-help-icon topic="invoice-total" title="Bantuan total tagihan" />
                    </label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">
                        ...
                        <x-help-icon topic="due-date-selection" title="Bantuan tanggal jatuh tempo" />
                    </label>
                        ...
                        <select name="currency_code"
                            ...
                            @foreach ($currencies as $cur)
                                <option value="{{ $cur->code }}"
                                    ...
                                    {{ $cur->code }} — {{ $cur->name }}
                                    {{ $cur->is_base ? '(Base)' : '(Kurs: ' . number_format($cur->rate_to_idr, 0, ',', '.') . ')' }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3" placeholder="Instruksi pembayaran, nomor rekening, dll."
                        class="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('notes') }}</textarea>
                </div>
            </div>
