<x-app-layout>
    <x-slot name="header">{{ __('Approval Workflow') }} - {{ $document->title }}</x-slot>
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-4">
                        @forelse($history['steps'] as $step)
                            <div
                                class="flex items-start space-x-4 p-4 border border-gray-200 rounded-lg @if ($step['status'] === 'approved') bg-green-50 @elseif($step['status'] === 'rejected') bg-red-50 @else bg-white @endif">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <p class="text-sm font-medium text-gray-900">Step
                                            {{ $step['step_number'] }}</p>
                                        <span
                                            class="px-2 py-1 text-xs font-semibold rounded-full @if ($step['status'] === 'approved') bg-green-100 text-green-800 @elseif($step['status'] === 'rejected') bg-red-100 text-red-800 @else bg-yellow-100 text-yellow-800 @endif">
                                            {{ ucfirst($step['status']) }}
                                        </span>
                                    </div>
