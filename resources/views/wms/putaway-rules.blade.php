<x-app-layout>
    ...
    <div class="flex justify-end mb-4">
        ...
        <button onclick="document.getElementById('modal-rule').classList.remove('hidden')"
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-xs text-gray-500 uppercase">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left">Produk/Kategori</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($rules as $r)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-gray-700">
                                {{ $r->product?->name ?? ($r->product_category ?? 'Semua') }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                ...
                                <form method="POST" action="{{ route('wms.putaway-rules.destroy', $r) }}"
                                    class="inline" data-confirm="Hapus?" data-confirm-type="danger">@csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center text-gray-400">Belum ada putaway rule.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="modal-rule" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-sm shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-rule').classList.add('hidden')"
            ...
            <form method="POST" action="{{ route('wms.putaway-rules.store') }}" class="p-6 space-y-3">
                ...
                <div><label class="block text-xs text-gray-600 mb-1">Produk (opsional)</label>
                    <select name="product_id" class="{{ $cls }}">
                ...
                <div><label class="block text-xs text-gray-600 mb-1">Kategori Produk (opsional)</label><input
                        type="text" name="product_category" placeholder="elektronik, makanan..."
                        class="{{ $cls }}"></div>
                <div><label class="block text-xs text-gray-600 mb-1">Prioritas</label><input type="number"
                        name="priority" min="0" value="0" class="{{ $cls }}"></div>
                <button type="submit"
