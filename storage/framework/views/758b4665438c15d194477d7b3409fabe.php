...
<?php $component->withAttributes([]); ?>
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500 dark:text-slate-400 mt-0.5"><?php echo e(now()->translatedFormat('l, d F Y')); ?> · Platform Overview</p>
    </div>

    
    <?php if($expiringIn7->isNotEmpty()): ?>
    <div class="mb-6 bg-red-500/10 border border-red-500/30 rounded-2xl p-4">
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-sm font-semibold text-red-400"><?php echo e($expiringIn7->count()); ?> tenant akan expired dalam 7 hari</p>
                <p class="text-xs text-red-300/80 mt-0.5">
                    <?php echo e($expiringIn7->pluck('name')->join(', ')); ?>

                </p>
            </div>
            <a href="<?php echo e(route('super-admin.tenants.index')); ?>?status=expired" class="text-xs text-red-400 hover:underline shrink-0">Lihat →</a>
        </div>
    </div>
    <?php endif; ?>
                ...
                <div class="space-y-2 flex-1 w-full">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background:<?php echo e($planColors[$plan] ?? '#94a3b8'); ?>"></span>
                            <span class="text-sm text-gray-700 dark:text-slate-300"><?php echo e($planLabels[$plan] ?? ucfirst($plan)); ?></span>
                        </div>
                        <span class="text-sm font-semibold text-gray-900 dark:text-white"><?php echo e($count); ?></span>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    <?php if(empty($planDist)): ?><p class="text-sm text-gray-400 dark:text-slate-500">Belum ada data</p><?php endif; ?>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5 mb-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs bg-amber-500/20 text-amber-400 font-medium px-2 py-0.5 rounded-full"><?php echo e($expiringIn30->count()); ?> tenant</span>
        </div>
            ...
            <div class="flex items-center justify-between py-2.5">
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-gray-400 dark:text-slate-500"><?php echo e($tenant->admins->first()?->email ?? $tenant->slug); ?></p>
                    </div>
                </div>
                <div class="text-right shrink-0 ml-3">
                    <p class="text-xs font-semibold <?php echo e($urgency); ?>"><?php echo e($expiryDate?->format('d M Y')); ?></p>
                    <p class="text-xs text-gray-400 dark:text-slate-500"><?php echo e($daysLeft !== null ? $daysLeft . ' hari lagi' : '—'); ?></p>
                </div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
            ...
            <div class="divide-y divide-gray-100 dark:divide-white/5">
                ...
                <div class="flex items-center justify-between py-2.5">
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            <p class="text-sm font-medium text-gray-900 dark:text-white truncate"><?php echo e($tenant->name); ?></p>
                            <p class="text-xs text-gray-400 dark:text-slate-500"><?php echo e($tenant->created_at->format('d M Y')); ?></p>
                        </div>
                    </div>
                    <span class="text-xs font-medium px-2 py-0.5 rounded-full shrink-0 <?php echo e($statusColor); ?>"><?php echo e($statusLabel); ?></span>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
            <?php endif; ?>
        ...
        <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between mb-1">
                                <span class="text-xs font-medium text-gray-700 dark:text-slate-300 truncate"><?php echo e($log->tenant?->name ?? 'Tenant #'.$log->tenant_id); ?></span>
                                <span class="text-xs text-gray-400 dark:text-slate-500 shrink-0 ml-2"><?php echo e(number_format($log->total)); ?> pesan</span>
                            </div>
                            <?php $maxAi=$log->tenant?->maxAiMessages()??100; $pct=$maxAi>0?min(100,round(($log->total/$maxAi)*100)):0; ?>
                            <div class="w-full bg-gray-100 dark:bg-white/10 rounded-full h-1.5">
                                <div class="h-1.5 rounded-full <?php echo e($pct>=90?'bg-red-500':($pct>=70?'bg-amber-500':'bg-blue-500')); ?>" style="width:<?php echo e($pct); ?>%"></div>
                            </div>
                        </div>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    ...
                    <a href="<?php echo e(route('super-admin.plans.index')); ?>" class="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10 transition text-sm font-medium text-gray-700 dark:text-slate-300">
                    ...
                    <a href="<?php echo e(route('super-admin.tenants.index')); ?>?status=expired" class="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-red-500/10 hover:bg-red-500/20 transition text-sm font-medium text-red-400">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        Tenant Expired
                        <?php if($expiredTenants>0): ?><span class="ml-auto text-xs bg-red-500/30 text-red-300 px-1.5 py-0.5 rounded-full font-semibold"><?php echo e($expiredTenants); ?></span><?php endif; ?>
                    </a>
                    <a href="<?php echo e(route('super-admin.tenants.index')); ?>?plan=trial" class="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 transition text-sm font-medium text-amber-400">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        Tenant Trial
                        <span class="ml-auto text-xs bg-amber-500/30 text-amber-300 px-1.5 py-0.5 rounded-full font-semibold"><?php echo e($trialTenants); ?></span>
                    </a>
                </div>
            </div>
        </div>
    ...
    requestAnimationFrame(() => setTimeout(() => {
        ...
        new Chart(document.getElementById('growthChart'), {
            type: 'bar',
            data: {
                labels: <?php echo json_encode(array_column($growthChart, 'month')); ?>,
                datasets: [{ label: 'Tenant Baru', data: <?php echo json_encode(array_column($growthChart, 'count')); ?>,
        ...
        new Chart(document.getElementById('planChart'), {
            type: 'doughnut',
            data: {
                labels: <?php echo json_encode($planChartLabels); ?>,
                datasets: [{ data: <?php echo json_encode($planChartData); ?>, backgroundColor: <?php echo json_encode($planChartColors); ?>, borderWidth: 0, hoverOffset: 4 }]
            },
            options: { responsive: true, maintainAspectRatio: false,
