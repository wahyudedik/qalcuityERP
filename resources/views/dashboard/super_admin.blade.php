<x-app-layout>
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500 mt-0.5">{{ now()->translatedFormat('l, d F Y') }} · Platform
            Overview</p>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4 mb-6">
        @foreach ($cards as $card)
            <div
                ...
                <p class="text-base sm:text-xl font-bold text-gray-900 truncate">{{ $card['value'] }}
                </p>
                <p class="text-[10px] sm:text-xs text-gray-400 mt-1 truncate">{{ $card['sub'] }}
                </p>
            </div>
        @endforeach
    </div>
                            ...
                            <div class="flex items-center gap-3">
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between mb-1">
                                        <span
                                            class="text-xs font-medium text-gray-700 truncate">{{ $log->tenant?->name ?? 'Tenant #' . $log->tenant_id }}</span>
                                        <span
                                            class="text-xs text-gray-400 shrink-0 ml-2">{{ number_format($log->total) }}
                                            pesan</span>
                                    </div>
                                    @php
