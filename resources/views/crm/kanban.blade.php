<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            CRM Pipeline
            <a href="{{ route('crm.index') }}" class="text-xs text-gray-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400">Tampilan Tabel →</a>
        </div>
    </x-slot>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            ...
            <p class="text-2xl font-bold text-blue-600 dark:text-blue-400 mt-1">{{ $stageStats->sum('count') }}</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            <p class="text-xs text-gray-500 dark:text-slate-400">Nilai Pipeline</p>
            <p class="text-lg font-bold text-gray-900 dark:text-white mt-1">Rp {{ number_format($stageStats->sum('total_value')/1000000,1) }}jt</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
            <p class="text-xs text-gray-500 dark:text-slate-400">Won Bulan Ini</p>
            <p class="text-lg font-bold text-green-600 dark:text-green-400 mt-1">Rp {{ number_format($wonThisMonth/1000000,1) }}jt</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl p-4 border border-gray-200 dark:border-white/10">
    ...
    <div class="overflow-x-auto pb-4">
        <div class="flex gap-4 min-w-max">
            ...
            <div class="w-64 flex flex-col"
                ...
                {{-- Column Header --}}
                <div class="flex items-center justify-between px-3 py-2 rounded-xl bg-{{ $sc }}-50 dark:bg-{{ $sc }}-500/10 border border-{{ $sc }}-200 dark:border-{{ $sc }}-500/20 mb-3">
                    <div>
                        <p class="text-sm font-semibold text-{{ $sc }}-700 dark:text-{{ $sc }}-400">{{ $stageLabels[$stage] }}</p>
                        @if($stageTotal > 0)
                        <p class="text-xs text-{{ $sc }}-600 dark:text-{{ $sc }}-500">Rp {{ number_format($stageTotal/1000000,1) }}jt</p>
                        @endif
                    </div>
                    <span class="w-6 h-6 flex items-center justify-center rounded-full bg-{{ $sc }}-100 dark:bg-{{ $sc }}-500/20 text-xs font-bold text-{{ $sc }}-700 dark:text-{{ $sc }}-400">
                        {{ $stageLeads->count() }}
                    </span>
                </div>
                    ...
                    <div class="lead-card bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-white/10 p-3 cursor-grab active:cursor-grabbing shadow-sm hover:shadow-md transition"
                        ...
                        <div class="flex items-start justify-between gap-1 mb-2">
                            <div class="flex-1 min-w-0">
                                ...
                                @if($lead->company)
                                <p class="text-xs text-gray-500 dark:text-slate-400 truncate">{{ $lead->company }}</p>
                                @endif
                            </div>
                            <span class="shrink-0 text-xs font-medium text-{{ $sc }}-600 dark:text-{{ $sc }}-400">{{ $lead->probability }}%</span>
                        </div>
                        @if($lead->estimated_value > 0)
                        <p class="text-xs font-semibold text-blue-600 dark:text-blue-400 mb-1">Rp {{ number_format($lead->estimated_value,0,',','.') }}</p>
                        @endif
                        <div class="flex items-center justify-between mt-2">
                            ...
                            <div class="flex gap-1">
                                ...
                                <form method="POST" action="{{ route('crm.destroy', $lead) }}" onsubmit="return confirm('Hapus lead?')">
                                    @csrf @method('DELETE')
            ...
            <div class="w-48 flex flex-col">
                ...
                <div class="flex flex-col gap-2 min-h-[100px]" id="col-{{ $stage }}"
                    ...
                    <div class="bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-white/10 px-3 py-2 text-xs">
                        ...
                        @if($lead->estimated_value > 0)
                        <p class="text-{{ $sc }}-600 dark:text-{{ $sc }}-400">Rp {{ number_format($lead->estimated_value/1000000,1) }}jt</p>
                        @endif
                    </div>
                    @endforeach
                    @if($stageLeads->count() > 5)
                    <p class="text-xs text-center text-gray-400 dark:text-slate-500">+{{ $stageLeads->count()-5 }} lainnya</p>
                    @endif
                </div>
            </div>
            @endforeach
