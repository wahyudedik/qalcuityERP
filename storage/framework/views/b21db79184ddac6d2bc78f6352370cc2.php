...
<div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5 h-full flex flex-col">
    ...
    <div class="flex items-start justify-between mb-4">
        <div>
            <p class="text-xs font-medium text-gray-500 dark:text-slate-400 leading-tight">Achievement & Level</p>
            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-0.5">Lv. <?php echo e($level); ?></p>
        </div>
    ...
    <div class="flex items-center gap-3 mb-4">
        <div class="flex-1 bg-gray-50 dark:bg-white/5 rounded-xl px-3 py-2 text-center">
            <p class="text-xs text-gray-500 dark:text-slate-400">Poin</p>
            <p class="text-base font-bold text-gray-900 dark:text-white"><?php echo e(number_format($totalPoints)); ?></p>
        </div>
        <div class="flex-1 bg-gray-50 dark:bg-white/5 rounded-xl px-3 py-2 text-center">
            <p class="text-xs text-gray-500 dark:text-slate-400">Peringkat</p>
            <p class="text-base font-bold text-gray-900 dark:text-white">#<?php echo e($rank); ?></p>
        </div>
    </div>
    ...
    <div class="mb-4">
        <div class="flex items-center justify-between mb-1.5">
            <span class="text-xs text-gray-500 dark:text-slate-400">Level <?php echo e($level); ?> →
        ...
        <p class="text-[10px] text-gray-400 dark:text-slate-500 mt-1"><?php echo e($progressTo); ?>/<?php echo e($pointsNeeded); ?> poin ke
            level berikutnya</p>
    </div>
    ...
    <?php if($recents->isNotEmpty()): ?>
        <div class="flex-1 mb-4">
            <p class="text-xs font-medium text-gray-500 dark:text-slate-400 mb-2">Achievement Terbaru</p>
            <div class="space-y-2">
                <?php $__currentLoopData = $recents->take(3); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ua): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex items-center gap-2.5">
                        ...
                        <p class="text-xs text-gray-700 dark:text-slate-300 truncate">
    ...
    <?php else: ?>
        <div class="flex-1 mb-4 flex items-center justify-center">
            <p class="text-xs text-gray-400 dark:text-slate-500 text-center">Belum ada achievement.<br>Mulai
