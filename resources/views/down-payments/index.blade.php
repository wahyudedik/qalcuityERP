        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    <td class="px-4 py-3 font-mono font-medium text-slate-800">{{ $dp->number }}</td>
                    <td class="px-4 py-3">
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $dp->type === 'customer' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700' }}">
                            {{ ucfirst($dp->type) }}
                        </span>
                    </td>
                    <td class="px-4 py-3 text-slate-600">{{ $dp->party?->name ?? '-' }}</td>
                    <td class="px-4 py-3 text-slate-500">{{ $dp->payment_date->format('d/m/Y') }}</td>
                    <td class="px-4 py-3 text-right font-medium text-slate-800">Rp {{ number_format($dp->amount, 0, ',', '.') }}</td>
                    <td class="px-4 py-3 text-right text-slate-600">Rp {{ number_format($dp->remaining_amount, 0, ',', '.') }}</td>
                    <td class="px-4 py-3">
