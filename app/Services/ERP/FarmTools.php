...
class FarmTools
{
    ...
    public function updatePlotStatus(array $args): array
    {
        ...
        if (! empty($args['current_crop'])) {
            $updates['current_crop'] = $args['current_crop'];
        }
        if (! empty($args['planted_at'])) {
            $updates['planted_at'] = $args['planted_at'];
        }
        if (! empty($args['expected_harvest'])) {
            $updates['expected_harvest'] = $args['expected_harvest'];
        }
    ...
    public function advanceCropPhase(array $args): array
    {
        ...
        if (! $cycle) {
            return ['status' => 'not_found', 'message' => "Siklus tanam untuk \"{$ref}\" tidak ditemukan."];
        }
        ...
        // Set actual dates
        $dates = [];
        if ($targetPhase === 'land_prep' && ! $cycle->actual_prep_start) {
            $dates['actual_prep_start'] = today();
        }
        if ($targetPhase === 'planting' && ! $cycle->actual_plant_date) {
            $dates['actual_plant_date'] = today();
        }
        if ($targetPhase === 'harvest' && ! $cycle->actual_harvest_date) {
            $dates['actual_harvest_date'] = today();
        }
        if (in_array($targetPhase, ['completed', 'cancelled']) && ! $cycle->actual_end_date) {
            $dates['actual_end_date'] = today();
        }
        if ($dates) {
            $cycle->update($dates);
        }
        ...
        if ($plotStatus) {
            $cycle->plot->update(['status' => $plotStatus]);
        }
    ...
    public function addLivestock(array $args): array
    {
        ...
        $herd = LivestockHerd::create([
            'tenant_id' => $this->tenantId,
            'farm_plot_id' => $plotId,
            'code' => LivestockHerd::generateCode($this->tenantId, $args['animal_type']),
            'name' => $args['name'],
        ...
        $fmt = fn ($v) => number_format($v, $v == (int) $v ? 0 : 1, ',', '.');
        $label = LivestockHerd::ANIMAL_TYPES[$args['animal_type']] ?? $args['animal_type'];
    ...
    {
        $query = LivestockHerd::where('tenant_id', $this->tenantId)
            ->where('status', 'active')
            ->with('plot');
    ...
    {
        $herd = LivestockHerd::where('tenant_id', $this->tenantId)
            ->where('code', $args['herd_code'])
            ->first();

        if (! $herd) {
            return ['status' => 'not_found', 'message' => "Ternak \"{$args['herd_code']}\" tidak ditemukan."];
        }

        $qty = (int) $args['quantity'];
        $isOutbound = in_array($args['type'], LivestockMovement::OUTBOUND_TYPES);
        ...
        $label = LivestockMovement::TYPE_LABELS[$args['type']] ?? $args['type'];
        $fmt = fn ($v) => number_format($v, $v == (int) $v ? 0 : 1, ',', '.');
    ...
    public function getFcr(array $args): array
    {
        ...
        // Compare all herds
        $herds = LivestockHerd::where('tenant_id', $this->tenantId)
            ->where('status', 'active')->get();

        if ($herds->isEmpty()) {
            return ['status' => 'empty', 'message' => 'Belum ada data ternak.'];
        }
    ...
    public function recordLivestockHealth(array $args): array
    {
        ...
        $label = LivestockHealthRecord::TYPE_LABELS[$args['type']] ?? $args['type'];
