<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto space-y-5">
        ...
        <div class="bg-white/5 border border-white/10 rounded-xl p-5">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                    ...
                    <div class="text-white">{{ $journal->postedBy->name }} ({{ $journal->posted_at?->format('d/m/Y H:i') }})</div>
                </div>
                @endif
            </div>
        </div>
            ...
            <table class="w-full text-sm text-gray-300">
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right">{{ $line->debit > 0 ? 'Rp ' . number_format($line->debit, 0, ',', '.') : '-' }}</td>
                        <td class="px-4 py-3 text-right">{{ $line->credit > 0 ? 'Rp ' . number_format($line->credit, 0, ',', '.') : '-' }}</td>
                    </tr>
                    @endforeach
                </tbody>
                <tfoot class="bg-white/5 font-semibold text-white">
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right">Rp {{ number_format($journal->totalDebit(), 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-right">Rp {{ number_format($journal->totalCredit(), 0, ',', '.') }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
        ...
        <div class="flex gap-3">
            <a href="{{ route('journals.index') }}" class="bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-lg text-sm">← Kembali</a>

            @if($journal->status === 'draft')
            <form method="POST" action="{{ route('journals.post', $journal) }}" onsubmit="return confirm('Post jurnal ini? Tidak bisa diubah setelah diposting.')">
                @csrf @method('PATCH')
