@php
    $count = $data['low_stock_count'] ?? 0;
    $bgColor = $count > 0 ? 'bg-red-500/20' : 'bg-green-500/20';
    $icColor = $count > 0 ? 'text-red-400' : 'text-green-400';
@endphp
<div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5 h-full">
    <div class="flex items-start justify-between mb-4">
        <p class="text-xs font-medium text-gray-500 dark:text-slate-400 leading-tight">Stok Menipis</p>
        <div class="w-9 h-9 rounded-xl {{ $bgColor }} flex items-center justify-center shrink-0">
            <svg class="w-5 h-5 {{ $icColor }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
            </svg>
        </div>
    </div>
    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $count }}</p>
    <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">{{ $data['total_products'] ?? 0 }} total produk</p>
</div>
