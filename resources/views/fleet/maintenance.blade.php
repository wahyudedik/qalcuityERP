                    ...
                    @forelse($maintenances as $m)
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 font-mono text-xs text-gray-900">
                                {{ $m->vehicle?->plate_number ?? '-' }}</td>
                            <td class="px-4 py-3 text-xs text-gray-500">{{ $tl }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    @if ($m->status === 'scheduled' || $m->status === 'in_progress')
                                        ...
                                        <button onclick="openCompleteMaint({{ $m->id }})"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="sm:col-span-2"><label
                            ...
                            *</label><input type="text" name="description" required
                            placeholder="Service rutin 10.000 km" class="{{ $cls }}"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Tanggal
                            Jadwal</label><input type="date" name="scheduled_date" class="{{ $cls }}">
                    </div>
            ...
            <form id="form-complete-maint" method="POST" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Tanggal
                            ...
                            value="{{ date('Y-m-d') }}" class="{{ $cls }}"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Odometer</label><input
                            type="number" name="odometer_at" min="0" class="{{ $cls }}"></div>
                    <div><label
