    ...
    public function createJournal(PayrollRun $run, int $userId): JournalEntry
    {
        ...
        return DB::transaction(function () use ($run, $tid, $userId, $accounts, $journalDate, $period, $totalGross, $totalPph21, $totalBpjsEr, $totalBpjs, $totalNet) {
            ...
            if ($journal->isBalanced()) {
                ...
                // BUG-FIN-001 FIX: Throw exception instead of just logging warning
                // Prevent posting imbalanced payroll journal
                $diff = abs($journal->totalDebit() - $journal->totalCredit());
                \Log::error("PayrollGL: jurnal {$number} tidak balance", [
                    'journal_number' => $number,
                    'debit' => $journal->totalDebit(),
                    'credit' => $journal->totalCredit(),
                    'difference' => $diff,
                    'payroll_run' => $run->period,
                ]);
    ...
    public function createPaymentJournal(PayrollRun $run, int $userId): JournalEntry
    {
        ...
        return DB::transaction(function () use ($run, $tid, $userId, $accounts, $journalDate, $period, $totalNet) {
            ...
            if ($periodLockService->isLocked($tid, $journalDate)) {
                $lockInfo = $periodLockService->getLockInfo($tid, $journalDate);
                throw new \RuntimeException(
                    "Periode {$lockInfo} sudah dikunci. Tidak dapat membuat jurnal pembayaran gaji untuk tanggal {$journalDate}."
                );
            }
            ...
            if ($journal->isBalanced()) {
                ...
                // BUG-FIN-001 FIX: Throw exception instead of just logging warning
                \Log::error("PayrollGL payment: jurnal {$number} tidak balance", [
                    'journal_number' => $number,
                    'debit' => $journal->totalDebit(),
                    'credit' => $journal->totalCredit(),
                    'payroll_run' => $run->period,
