<x-app-layout>
    ...
    @endif
    @if(session('error'))
    <div class="mb-4 px-4 py-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-xl text-sm text-red-600 dark:text-red-400">{{ session('error') }}</div>
    @endif
    @if($errors->any())
    <div class="mb-4 px-4 py-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-xl text-sm text-red-600 dark:text-red-400">{{ $errors->first() }}</div>
    @endif
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5 {{ $acc->is_header ? 'font-semibold' : '' }}">
                                <td class="px-4 py-3 font-mono text-xs text-gray-900 dark:text-white">{{ $acc->code }}</td>
                                <td class="px-4 py-3 text-gray-900 dark:text-white">
                                    {{ $acc->is_header ? '' : '↳ ' }}{{ $acc->name }}
                                    @if($acc->parent) <span class="text-xs text-gray-400 dark:text-slate-500">({{ $acc->parent->code }})</span> @endif
                                </td>
                                <td class="px-4 py-3 hidden sm:table-cell">
                                    ...
                                    <span class="px-2 py-0.5 rounded-full text-xs bg-{{ $tc }}-100 text-{{ $tc }}-700 dark:bg-{{ $tc }}-500/20 dark:text-{{ $tc }}-400">
                                        {{ ['asset'=>'Aset','liability'=>'Kewajiban','equity'=>'Ekuitas','revenue'=>'Pendapatan','expense'=>'Beban'][$acc->type] ?? $acc->type }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-center hidden md:table-cell text-xs text-gray-500 dark:text-slate-400">{{ ucfirst($acc->normal_balance) }}</td>
                                <td class="px-4 py-3 text-center">
                                    <span class="px-2 py-0.5 rounded-full text-xs {{ $acc->is_active ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' : 'bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-slate-500' }}">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $ba->bank_name }}</td>
                            <td class="px-4 py-3 font-mono text-xs text-gray-700 dark:text-slate-300">{{ $ba->account_number }}</td>
                            <td class="px-4 py-3 hidden sm:table-cell text-gray-600 dark:text-slate-400">{{ $ba->account_name }}</td>
                            <td class="px-4 py-3 text-right hidden md:table-cell text-gray-900 dark:text-white">Rp {{ number_format($ba->balance,0,',','.') }}</td>
                            <td class="px-4 py-3 text-center">
                                <span class="px-2 py-0.5 rounded-full text-xs {{ $ba->is_active ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' : 'bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-slate-500' }}">
    ...
    <div class="flex flex-col lg:flex-row gap-5">
        ...
        <div class="flex-1">
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $tax->name }}</td>
                            <td class="px-4 py-3 font-mono text-xs text-gray-600 dark:text-slate-400">{{ $tax->code }}</td>
                            <td class="px-4 py-3 hidden sm:table-cell text-gray-600 dark:text-slate-400">{{ $tax->getTypeLabel() }}</td>
            ...
            <div class="mt-3 flex justify-end">
                <a href="{{ route('taxes.efaktur') }}" class="text-xs px-3 py-2 border border-gray-200 dark:border-white/10 rounded-xl text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-white/5">
                    Export e-Faktur CSV
                </a>
            </div>
        </div>
            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            <td class="px-4 py-3 font-mono font-bold text-gray-900 dark:text-white">{{ $cur->code }}</td>
                            <td class="px-4 py-3 text-gray-700 dark:text-slate-300">{{ $cur->name }}</td>
                            <td class="px-4 py-3 text-center text-gray-600 dark:text-slate-400">{{ $cur->symbol }}</td>
                            <td class="px-4 py-3 text-right font-medium text-gray-900 dark:text-white">
                                @if($cur->is_base)
                            ...
                            <td class="px-4 py-3 text-center">
                                <span class="px-2 py-0.5 rounded-full text-xs {{ $cur->is_active ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' : 'bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-slate-500' }}">
