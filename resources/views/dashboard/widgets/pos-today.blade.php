<div class="bg-white rounded-2xl border border-gray-200 p-5 h-full">
    ...
    <p class="text-2xl font-bold text-gray-900">Rp {{ number_format($data['revenue'] ?? 0, 0, ',', '.') }}</p>
    <p class="text-xs text-gray-400 mt-1">{{ $data['count'] ?? 0 }} transaksi · Avg Rp {{ number_format($data['avg_ticket'] ?? 0, 0, ',', '.') }}</p>
</div>
