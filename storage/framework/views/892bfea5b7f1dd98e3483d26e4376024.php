...
<?php $component->withAttributes([]); ?>
    ...
    <div class="max-w-4xl mx-auto space-y-5">
        ...
        <div class="bg-white/5 border border-white/10 rounded-xl p-5">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                    ...
                    <div class="text-white"><?php echo e($journal->postedBy->name); ?> (<?php echo e($journal->posted_at?->format('d/m/Y H:i')); ?>)</div>
                </div>
                <?php endif; ?>
            </div>
        </div>
            ...
            <table class="w-full text-sm text-gray-300">
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right"><?php echo e($line->debit > 0 ? 'Rp ' . number_format($line->debit, 0, ',', '.') : '-'); ?></td>
                        <td class="px-4 py-3 text-right"><?php echo e($line->credit > 0 ? 'Rp ' . number_format($line->credit, 0, ',', '.') : '-'); ?></td>
                    </tr>
                ...
                <tfoot class="bg-white/5 font-semibold text-white">
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right">Rp <?php echo e(number_format($journal->totalDebit(), 0, ',', '.')); ?></td>
                        <td class="px-4 py-3 text-right">Rp <?php echo e(number_format($journal->totalCredit(), 0, ',', '.')); ?></td>
                    </tr>
                </tfoot>
            </table>
        </div>
        ...
        <div class="flex gap-3">
            <a href="<?php echo e(route('journals.index')); ?>" class="bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-lg text-sm">← Kembali</a>

            <?php if($journal->status === 'draft'): ?>
            <form method="POST" action="<?php echo e(route('journals.post', $journal)); ?>" onsubmit="return confirm('Post jurnal ini? Tidak bisa diubah setelah diposting.')">
                <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                <button class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm">✓ Post Jurnal</button>
            </form>
            <?php endif; ?>

            <?php if($journal->status === 'posted'): ?>
            <button onclick="document.getElementById('modal-reverse').classList.remove('hidden')"
                class="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg text-sm">↩ Balik Jurnal</button>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div id="modal-reverse" class="hidden fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
        <div class="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-sm p-6">
            ...
            <form method="POST" action="<?php echo e(route('journals.reverse', $journal)); ?>" class="space-y-4">
                ...
                <div class="flex gap-3">
                    <button type="submit" class="flex-1 bg-orange-600 hover:bg-orange-700 text-white py-2 rounded-lg text-sm">Balik</button>
