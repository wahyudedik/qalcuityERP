...
<?php $component->withAttributes([]); ?>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php $__empty_1 = true; $__currentLoopData = $drivers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 text-gray-900"><?php echo e($d->name); ?>

                                <?php if($d->employee): ?>
                                    <span class="text-xs text-gray-400">(<?php echo e($d->employee->employee_id ?? ''); ?>)</span>
                                <?php endif; ?>
                            </td>
                            <td
                                class="px-4 py-3 hidden sm:table-cell text-gray-700 font-mono text-xs">
                                <?php echo e($d->license_number ?? '-'); ?></td>
                            <td class="px-4 py-3 text-center hidden sm:table-cell text-gray-700">
                                <?php echo e($d->license_type ?? '-'); ?></td>
                            <td class="px-4 py-3 hidden md:table-cell text-gray-500">
                                <?php echo e($d->phone ?? '-'); ?></td>
                            <td class="px-4 py-3 text-center">
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs bg-<?php echo e($dc); ?>-100 text-<?php echo e($dc); ?>-700 $dc }}-500/20 $dc }}-400"><?php echo e($dl); ?></span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <?php if(auth()->check() && app(\App\Services\PermissionService::class)->check(auth()->user(), 'fleet', 'edit')): ?>
                                    <button onclick='openEditDriver(<?php echo json_encode($d, 15, 512) ?>)'
                                    ...
                                    <form method="POST" action="<?php echo e(route('fleet.drivers.destroy', $d)); ?>"
        ...
        <?php if($drivers->hasPages()): ?>
            <div class="px-4 py-3 border-t border-gray-100"><?php echo e($drivers->links()); ?></div>
        <?php endif; ?>
    </div>
            ...
            <form method="POST" action="<?php echo e(route('fleet.drivers.store')); ?>" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2"><label
                            ...
                            *</label><input type="text" name="name" required class="<?php echo e($cls); ?>"></div>
                    <?php if($employees->isNotEmpty()): ?>
                        <div class="sm:col-span-2"><label
                                class="block text-xs font-medium text-gray-600 mb-1">Link
                                Karyawan</label>
                            <select name="employee_id" class="<?php echo e($cls); ?>">
                                <option value="">-- Tanpa Link --</option>
                                <?php $__currentLoopData = $employees; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $e): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($e->id); ?>"><?php echo e($e->name); ?> (<?php echo e($e->employee_id); ?>)
                                    </option>
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">No.
                            SIM</label><input type="text" name="license_number" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Tipe
                            SIM</label>
                        <select name="license_type" class="<?php echo e($cls); ?>">
                            <option value="">-</option>
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">SIM
                            Expired</label><input type="date" name="license_expiry" class="<?php echo e($cls); ?>">
                    </div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Telepon</label><input
                            type="text" name="phone" class="<?php echo e($cls); ?>"></div>
                </div>
        ...
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form id="form-edit-driver" method="POST" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Tipe
                            SIM</label>
                        <select name="license_type" id="ed-license-type" class="<?php echo e($cls); ?>">
                            <option value="">-</option>
                            <option value="A">A</option>
                            <option value="B1">B1</option>
                            <option value="B2">B2</option>
                    ...
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Telepon</label><input
                            type="text" name="phone" id="ed-phone" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Status</label>
                        <select name="status" id="ed-status" class="<?php echo e($cls); ?>">
    ...
    <?php $__env->startPush('scripts'); ?>
        <script>
            function openEditDriver(d) {
                document.getElementById('form-edit-driver').action = '<?php echo e(url('fleet/drivers')); ?>/' + d.id;
