...
<?php $component->withAttributes([]); ?>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <?php $__currentLoopData = [
            ...
            ['label'=>'Terlambat','value'=>$stats['overdue'],'color'=>'text-red-600'],
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        ?>
        <div class="bg-white rounded-2xl border border-gray-200 p-5 flex flex-col gap-3 hover:shadow-md transition">
            <div class="flex items-start justify-between gap-2">
                <div class="flex-1 min-w-0">
                    <a href="<?php echo e(route('projects.show', $project)); ?>" class="font-semibold text-gray-900 hover:text-blue-600 line-clamp-1"><?php echo e($project->name); ?></a>
                    <p class="text-xs text-gray-500 mt-0.5"><?php echo e($project->number); ?> <?php echo e($project->customer ? '· '.$project->customer->name : ''); ?></p>
                </div>
                <span class="shrink-0 px-2 py-0.5 rounded-full text-xs bg-<?php echo e($c); ?>-100 text-<?php echo e($c); ?>-700 $c }}-500/20 $c }}-400">
            ...
            <div class="flex justify-between text-xs">
                ...
                <span class="<?php echo e($overBudget ? 'text-red-500 font-semibold' : 'text-gray-700'); ?>">
            ...
            <a href="<?php echo e(route('projects.show', $project)); ?>"
                class="mt-auto text-center text-xs font-medium text-blue-600 hover:underline">
                Lihat Detail →
            </a>
        </div>
    ...
    <div id="modal-add-project" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
                <h3 class="font-semibold text-gray-900">Proyek Baru</h3>
                <button onclick="document.getElementById('modal-add-project').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form method="POST" action="<?php echo e(route('projects.store')); ?>" class="p-6 space-y-4">
                <?php echo csrf_field(); ?>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-medium text-gray-600 mb-1">Nama Proyek *</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Tipe Proyek</label>
                        <input type="text" name="type" placeholder="misal: Website, Konstruksi..." class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Tanggal Mulai</label>
                        <input type="date" name="start_date" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Deadline</label>
                        <input type="date" name="end_date" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-medium text-gray-600 mb-1">Anggaran (Rp)</label>
                        <input type="number" name="budget" min="0" step="100000" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-medium text-gray-600 mb-1">Deskripsi</label>
                        <textarea name="description" rows="2" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500"></textarea>
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" onclick="document.getElementById('modal-add-project').classList.add('hidden')" class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
