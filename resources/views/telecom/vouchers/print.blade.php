...
<body>
    <div class="no-print" style="text-align: center; padding: 20px; background: #333; color: white;">
        ...
        <p style="margin-top: 10px; font-size: 12px;">{{ count($vouchers) }} vouchers ready to print</p>
    </div>

    <div class="page">
        <div class="header">
            <h1>INTERNET VOUCHER</h1>
            <p>{{ config('app.name') }} - Generated on {{ now()->format('d M Y H:i:s') }}</p>
        </div>
                ...
                <div class="voucher-card">
                    ...
                    <div class="voucher-details">
                        <div class="detail-row">
                            ...
                            <span class="value">{{ $voucher->valid_from->format('d M Y') }}</span>
                        </div>
                        <div class="detail-row">
                        ...
                        @endif
                        @if ($voucher->sale_price)
                            <div class="detail-row">
                                <span class="label">Price:</span>
                                <span class="value">Rp {{ number_format($voucher->sale_price, 0, ',', '.') }}</span>
                            </div>
                        @endif
                        <div class="detail-row">
                            <span class="label">Batch:</span>
                            <span class="value">{{ $voucher->batch_number ?? '-' }}</span>
                        </div>
                    </div>

                    <div class="voucher-footer">
                        <p>Scan QR code or enter code above to connect</p>
                        <p style="margin-top: 5px;">{{ config('app.name') }}</p>
                    </div>
                </div>
            @endforeach
        </div>
