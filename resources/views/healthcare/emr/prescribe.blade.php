<x-app-layout>
    <x-slot name="header">Buat Resep Elektronik - {{ $visit->patient?->full_name ?? 'Pasien' }}</x-slot>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-white/80">RM: {{ $visit->patient?->medical_record_number ?? '-' }} |
                        {{ $visit->visit_date ? \Carbon\Carbon::parse($visit->visit_date)->format('d M Y') : '-' }}</p>
                </div>
            </div>
        </div>

        <form action="{{ route('healthcare.emr.prescriptions.store', $visit) }}" method="POST" class="space-y-6">
            @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-700">Kirim resep ke farmasi</span>
                            </label>
                        </div>
                        ...
                        <div>
                            <label class="flex items-center gap-2">
                                <input type="checkbox" name="print_prescription" value="1"
                                    class="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500">
                                <span class="text-sm text-gray-700">Cetak resep</span>
                            </label>
                        </div>
                    </div>
                </div>
