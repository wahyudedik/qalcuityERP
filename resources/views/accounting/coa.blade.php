                    ...
                    <tr class="hover:bg-gray-50 {{ $acc->is_header ? 'bg-gray-50/50' : '' }}">
                        ...
                        <td class="px-4 py-3" style="padding-left: {{ ($acc->level - 1) * 18 + 16 }}px">
                            <div class="flex items-center gap-1.5">
                                ...
                                @endif
                                <span class="{{ $acc->is_header ? 'font-semibold text-gray-900' : 'text-gray-700' }}">{{ $acc->name }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-3 hidden sm:table-cell">
                            <span class="px-2 py-0.5 rounded-full text-xs {{ $typeColor }}">{{ $acc->getTypeLabel() }}</span>
                        </td>
                        <td class="px-4 py-3 hidden md:table-cell text-xs text-gray-500 capitalize">{{ $acc->normal_balance }}</td>
                        <td class="px-4 py-3 text-center hidden lg:table-cell text-xs text-gray-500">{{ $acc->level }}</td>
                        <td class="px-4 py-3 hidden lg:table-cell font-mono text-xs text-gray-400">{{ $acc->parent?->code ?? '—' }}</td>
