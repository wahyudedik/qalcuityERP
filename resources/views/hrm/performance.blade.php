<x-app-layout>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-5 mb-4">
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-2">
                <select id="career-emp-select"
                    class="px-3 py-1.5 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-violet-500">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($reviews as $review)
                        <tr class="hover:bg-gray-50 cursor-pointer" onclick="openDetail({{ $review->id }})">
                            ...
                            <td class="px-4 py-3 text-gray-700">
                                {{ $review->period }}
                                <span
                                    class="text-xs text-gray-400 ml-1">({{ match ($review->period_type) {'monthly' => 'Bulanan','quarterly' => 'Kuartalan',default => 'Tahunan'} }})</span>
                            </td>
                            <td class="px-4 py-3 hidden md:table-cell text-gray-500">{{ $review->reviewer?->name }}
                            </td>
                            <td class="px-4 py-3 text-center">
                                ...
                                @endphp
                                <span
                                    class="font-bold text-lg {{ $color }}">{{ number_format($score, 1) }}</span>
                            ...
                            <td class="px-4 py-3 text-center hidden lg:table-cell">
                                @if ($review->recommendation)
                                    ...
                                    @endphp
                                    <span
                                        class="px-2 py-0.5 rounded-full text-xs {{ $recColor }}">{{ $review->recommendationLabel() }}</span>
                                @else
                            ...
                            <td class="px-4 py-3 text-center">
                                ...
                                @endphp
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs {{ $sBadge }}">{{ $sLabel }}</span>
                            </td>
                            <td class="px-4 py-3 text-center" onclick="event.stopPropagation()">
                                <div class="flex items-center justify-center gap-1">
                                    @if ($review->status === 'submitted')
                                        <form method="POST"
                                            action="{{ route('hrm.performance.acknowledge', $review) }}">
                                            @csrf @method('PATCH')
                                            <button type="submit"
                                                class="px-2 py-1 text-xs bg-green-600 text-white rounded-lg hover:bg-green-700">Konfirmasi</button>
                                        </form>
                                    @endif
                                    <form method="POST" action="{{ route('hrm.performance.destroy', $review) }}"
                                        data-confirm="Hapus penilaian ini?" data-confirm-type="danger">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center text-gray-400">Belum ada penilaian
                                kinerja.</td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div id="modal-add-review" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-2xl shadow-xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
                ...
                <button onclick="document.getElementById('modal-add-review').classList.add('hidden')"
            ...
            <form method="POST" action="{{ route('hrm.performance.store') }}" class="p-6 space-y-5">
                ...
                {{-- Score sliders --}}
                <div class="space-y-3">
                    <p class="text-xs font-semibold text-gray-600 uppercase tracking-wide">Penilaian (1 = Sangat Buruk,
                        5 = Luar Biasa)</p>
                    @foreach ([['score_work_quality', 'Kualitas Kerja'], ['score_productivity', 'Produktivitas'], ['score_teamwork', 'Kerja Tim'], ['score_initiative', 'Inisiatif'], ['score_attendance', 'Kehadiran']] as [$field, $label])
                                ...
                                @for ($i = 1; $i <= 5; $i++)
                                    <label class="cursor-pointer">
                                        <input type="radio" name="{{ $field }}"
                                            ...
                                            class="sr-only peer">
                                        <span
                                            class="w-9 h-9 flex items-center justify-center rounded-xl border-2 border-gray-200 text-sm font-semibold text-gray-500 peer-checked:border-blue-500 peer-checked:bg-blue-500 peer-checked:text-white transition cursor-pointer hover:border-blue-400">{{ $i }}</span>
                                    </label>
                                @endfor
                            </div>
                        </div>
                ...
                <div class="grid grid-cols-1 gap-4">
                    <div>
                        ...
                        <textarea name="strengths" rows="2"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 resize-none"></textarea>
                    </div>
                    <div>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button"
                        onclick="document.getElementById('modal-add-review').classList.add('hidden')"
    ...
    @push('scripts')
        <script>
            ...
            async function loadCareerPath() {
                ...
                try {
                    ...
                    document.getElementById('career-loading').classList.add('hidden');
                    document.getElementById('career-content').innerHTML =
                        `<p class="text-sm text-red-500">Gagal memuat prediksi. Coba lagi.</p>`;
                } finally {
                    btn.disabled = false;
                }
            }
            ...
            function renderCareerPath(d) {
                ...
                const trendColor = d.trend === 'improving' ? 'text-green-500' : d.trend === 'declining' ? 'text-red-500' :
                    'text-gray-400';
        ...
        <div class="flex flex-col sm:flex-row gap-5 mb-5">
            ...
            <div class="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
                ...
                // Factors
                const posFactors = d.factors.positive.map(f =>
                    `<li class="flex items-start gap-1.5 text-xs text-gray-700"><span class="text-green-500 shrink-0 mt-0.5">?</span>${f}</li>`
                    ).join('');
                const negFactors = d.factors.negative.map(f =>
                    `<li class="flex items-start gap-1.5 text-xs text-gray-700"><span class="text-red-500 shrink-0 mt-0.5">?</span>${f}</li>`
                    ).join('');
