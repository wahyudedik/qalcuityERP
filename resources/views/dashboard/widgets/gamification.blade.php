...
<div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5 h-full flex flex-col">
    ...
    <div class="flex items-start justify-between mb-4">
        <div>
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white mt-0.5">Lv. {{ $level }}</p>
        </div>
    ...
    <div class="flex items-center gap-3 mb-4">
        <div class="flex-1 bg-gray-50 dark:bg-white/5 rounded-xl px-3 py-2 text-center">
            ...
            <p class="text-base font-bold text-gray-900 dark:text-white">{{ number_format($totalPoints) }}</p>
        </div>
        <div class="flex-1 bg-gray-50 dark:bg-white/5 rounded-xl px-3 py-2 text-center">
            <p class="text-xs text-gray-500 dark:text-slate-400">Peringkat</p>
            <p class="text-base font-bold text-gray-900 dark:text-white">#{{ $rank }}</p>
        </div>
    </div>
    ...
    <div class="mb-4">
        <div class="flex items-center justify-between mb-1.5">
            <span class="text-xs text-gray-500 dark:text-slate-400">Level {{ $level }} →
                {{ $level + 1 }}</span>
            <span class="text-xs font-semibold text-indigo-400">{{ $progressPct }}%</span>
        </div>
        ...
        <p class="text-[10px] text-gray-400 dark:text-slate-500 mt-1">{{ $progressTo }}/{{ $pointsNeeded }} poin ke
            level berikutnya</p>
    </div>
