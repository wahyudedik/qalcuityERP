...
<?php $component->withAttributes([]); ?>
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
        <form method="GET" class="flex-1 flex flex-wrap gap-2">
            <select name="vehicle_id" class="px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#1e293b] text-gray-900 dark:text-white">
                ...
                <?php $__currentLoopData = $vehicles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($v->id); ?>" <?php if(request('vehicle_id')==$v->id): echo 'selected'; endif; ?>><?php echo e($v->plate_number); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3 font-mono text-xs text-gray-900 dark:text-white"><?php echo e($m->vehicle->plate_number ?? '-'); ?></td>
                        <td class="px-4 py-3 text-xs text-gray-500 dark:text-slate-400"><?php echo e($tl); ?></td>
                        <td class="px-4 py-3 text-gray-700 dark:text-slate-300"><?php echo e($m->description); ?></td>
                        <td class="px-4 py-3 text-center hidden sm:table-cell text-xs text-gray-500 dark:text-slate-400"><?php echo e($m->scheduled_date?->format('d/m/Y') ?? '-'); ?></td>
                        <td class="px-4 py-3 text-right hidden sm:table-cell text-gray-900 dark:text-white">Rp <?php echo e(number_format($m->cost, 0, ',', '.')); ?></td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-xs bg-<?php echo e($mc); ?>-100 text-<?php echo e($mc); ?>-700 dark:bg-<?php echo e($mc); ?>-500/20 dark:text-<?php echo e($mc); ?>-400"><?php echo e($ml); ?></span>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button onclick="openCompleteMaint(<?php echo e($m->id); ?>)" class="text-xs px-2 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700">Selesai</button>
                                <?php endif; ?>
                                <?php endif; ?>
                                <?php if(auth()->check() && app(\App\Services\PermissionService::class)->check(auth()->user(), 'fleet', 'delete')): ?>
                                <form method="POST" action="<?php echo e(url('fleet/maintenance')); ?>/<?php echo e($m->id); ?>" class="inline" onsubmit="return confirm('Hapus?')">
        ...
        <?php if($maintenances->hasPages()): ?><div class="px-4 py-3 border-t border-gray-100 dark:border-white/5"><?php echo e($maintenances->links()); ?></div><?php endif; ?>
    </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Kendaraan *</label>
                        <select name="vehicle_id" required class="<?php echo e($cls); ?>"><option value="">-- Pilih --</option>
                            <?php $__currentLoopData = $vehicles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($v->id); ?>"><?php echo e($v->plate_number); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                    ...
                    <div class="col-span-2"><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Deskripsi *</label><input type="text" name="description" required placeholder="Service rutin 10.000 km" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Tanggal Jadwal</label><input type="date" name="scheduled_date" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Estimasi Biaya</label><input type="number" name="cost" min="0" step="1000" value="0" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Vendor/Bengkel</label><input type="text" name="vendor" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Next di KM</label><input type="number" name="next_km" min="0" class="<?php echo e($cls); ?>"></div>
                </div>
    ...
    <div id="modal-complete-maint" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-sm shadow-xl">
            ...
            <form id="form-complete-maint" method="POST" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Biaya Aktual *</label><input type="number" name="cost" required min="0" step="1000" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Tanggal Selesai *</label><input type="date" name="completed_date" required value="<?php echo e(date('Y-m-d')); ?>" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Odometer</label><input type="number" name="odometer_at" min="0" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Vendor</label><input type="text" name="vendor" class="<?php echo e($cls); ?>"></div>
                </div>
                <div class="flex justify-end gap-3">
