        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($workCenters as $wc)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-right text-gray-900">Rp
                                {{ number_format($wc->cost_per_hour, 0, ',', '.') }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <button
                                        onclick="openEditWc({{ $wc->id }}, '{{ $wc->code }}', '{{ addslashes($wc->name) }}', {{ $wc->cost_per_hour }}, {{ $wc->capacity_per_day }}, {{ $wc->is_active ? 'true' : 'false' }}, '{{ addslashes($wc->notes ?? '') }}')"
                                    ...
                                    <form method="POST"
                                        action="{{ url('manufacturing/work-centers') }}/{{ $wc->id }}"
                                        class="inline" data-confirm="Hapus work center ini?" data-confirm-type="danger">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center text-gray-400">Belum ada work center.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="modal-create-wc" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-create-wc').classList.add('hidden')"
            ...
            <form method="POST" action="{{ route('manufacturing.work-centers.store') }}" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="code" required maxlength="20" placeholder="WC-01"
                    ...
                    <div>
                        ...
                        <input type="text" name="name" required placeholder="Mesin CNC 1"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Biaya/Jam (Rp)</label>
                        <input type="number" name="cost_per_hour" min="0" step="1000" value="0"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Kapasitas/Hari (jam)</label>
                        <input type="number" name="capacity_per_day" min="1" max="24" value="8"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900">
                    </div>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-create-wc').classList.add('hidden')"
    ...
    <div id="modal-edit-wc" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-edit-wc').classList.add('hidden')"
                    class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form id="form-edit-wc" method="POST" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Kode *</label>
                        <input type="text" name="code" id="edit-wc-code" required maxlength="20"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Nama *</label>
                        <input type="text" name="name" id="edit-wc-name" required
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Biaya/Jam (Rp)</label>
                        <input type="number" name="cost_per_hour" id="edit-wc-cost" min="0" step="1000"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Kapasitas/Hari (jam)</label>
                        <input type="number" name="capacity_per_day" id="edit-wc-cap" min="1"
                            max="24"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900">
                    </div>
                    <div>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="is_active" id="edit-wc-active" value="1"
                                class="rounded">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Catatan</label>
                        <input type="text" name="notes" id="edit-wc-notes"
                            class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900">
                    </div>
                </div>
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-edit-wc').classList.add('hidden')"
                        class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
                    <button type="submit"
