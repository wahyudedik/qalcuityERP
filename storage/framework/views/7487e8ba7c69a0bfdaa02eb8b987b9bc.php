...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> Supplier & Pemasok <?php $__env->endSlot(); ?>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4">
        ...
        <div class="flex gap-2">
            <a href="<?php echo e(route('purchasing.orders')); ?>" class="px-3 py-2 text-sm border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50">Purchase Order</a>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 font-medium text-gray-900"><?php echo e($sup->name); ?></td>
                        ...
                        <td class="px-4 py-3 hidden md:table-cell text-gray-500"><?php echo e($sup->email ?? '-'); ?></td>
                        ...
                        <td class="px-4 py-3 text-center">
                            ...
                            <button onclick="openEditSupplier(<?php echo e($sup->id); ?>, '<?php echo e(addslashes($sup->name)); ?>', '<?php echo e(addslashes($sup->company ?? '')); ?>', '<?php echo e($sup->phone ?? ''); ?>', '<?php echo e($sup->email ?? ''); ?>', '<?php echo e(addslashes($sup->address ?? '')); ?>', <?php echo e($sup->is_active ? 'true' : 'false'); ?>)"
                    ...
                    <tr><td colspan="6" class="px-4 py-12 text-center text-gray-400">Belum ada supplier.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div id="modal-add-supplier" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-add-supplier').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form method="POST" action="<?php echo e(route('purchasing.suppliers.store')); ?>" class="p-6 space-y-3">
                <?php echo csrf_field(); ?>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="company" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" onclick="document.getElementById('modal-add-supplier').classList.add('hidden')" class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
    ...
    <div id="modal-edit-supplier" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button onclick="document.getElementById('modal-edit-supplier').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form id="form-edit-supplier" method="POST" class="p-6 space-y-3">
                <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Nama *</label>
                    <input type="text" id="es-name" name="name" required class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Perusahaan</label>
                    <input type="text" id="es-company" name="company" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Telepon</label>
                        <input type="text" id="es-phone" name="phone" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Email</label>
                        <input type="email" id="es-email" name="email" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="es-active" name="is_active" value="1" class="rounded">
                    <label for="es-active" class="text-sm text-gray-700">Supplier Aktif</label>
                </div>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" onclick="document.getElementById('modal-edit-supplier').classList.add('hidden')" class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
                    <button type="submit" class="px-4 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Simpan</button>
                </div>
            </form>
        </div>
    ...
    function openEditSupplier(id, name, company, phone, email, address, isActive) {
        document.getElementById('form-edit-supplier').action = '<?php echo e(url("purchasing/suppliers")); ?>/' + id;
...
<?php endif; ?>
<?php /**PATH E:\PROJEKU\qalcuityERP\resources\views\purchasing\suppliers.blade.php ENDPATH**/ ?>