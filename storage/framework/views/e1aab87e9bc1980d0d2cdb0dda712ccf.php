...
<?php $component->withAttributes([]); ?>
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
        <a href="<?php echo e(route('commission.index')); ?>" class="px-3 py-2 text-sm text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-white">← Komisi Sales</a>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3 text-gray-900 dark:text-white"><?php echo e($r->name); ?></td>
                        <td class="px-4 py-3 text-center text-xs">
                            <span class="px-2 py-0.5 rounded-full <?php echo e(['flat_pct'=>'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400','tiered'=>'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400','flat_amount'=>'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400'][$r->type] ?? 'bg-gray-100 text-gray-500'); ?>">
                        ...
                        <td class="px-4 py-3 text-center text-xs text-gray-500 dark:text-slate-400"><?php echo e(['revenue'=>'Revenue','profit'=>'Profit','quantity'=>'Qty'][$r->basis] ?? $r->basis); ?></td>
                        <td class="px-4 py-3 text-center">
                            <?php if($r->is_active): ?><span class="px-2 py-0.5 rounded-full text-xs bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400">Aktif</span>
        ...
        <?php if($rules->hasPages()): ?><div class="px-4 py-3 border-t border-gray-100 dark:border-white/5"><?php echo e($rules->links()); ?></div><?php endif; ?>
    </div>
    ...
    <div id="modal-add-rule" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="<?php echo e(route('commission.rules.store')); ?>" class="p-6 space-y-4">
                ...
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Nama *</label><input type="text" name="name" required placeholder="Komisi Sales Standard" class="<?php echo e($cls); ?>"></div>
                ...
                <div id="rate-field"><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Rate (% atau Rp)</label><input type="number" name="rate" min="0" step="0.01" class="<?php echo e($cls); ?>"></div>
