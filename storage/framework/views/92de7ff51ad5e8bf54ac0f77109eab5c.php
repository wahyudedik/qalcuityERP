...
<?php $component->withAttributes([]); ?>
    ...
    <?php if(!isset($invoice)): ?>
        <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-6">
            <p class="text-amber-800 dark:text-amber-200">Invoice tidak ditemukan</p>
        </div>
    <?php else: ?>
        ...
        <div
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white"><?php echo e($invoice->invoice_number ?? '-'); ?>

                        </h3>
                        <p class="text-sm text-gray-600 dark:text-slate-400 mt-1">Tanggal:
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <h4 class="text-sm font-semibold text-gray-500 dark:text-slate-400 uppercase mb-3">Informasi Pasien
                    </h4>
                    <div class="space-y-2">
                        <div>
                            <p class="text-xs text-gray-500 dark:text-slate-400">Nama Pasien</p>
                            <p class="text-sm font-semibold text-gray-900 dark:text-white">
                                <?php echo e($invoice->patient ? $invoice->patient->full_name : '-'); ?></p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500 dark:text-slate-400">No. Rekam Medis</p>
                ...
                <div>
                    <h4 class="text-sm font-semibold text-gray-500 dark:text-slate-400 uppercase mb-3">Informasi Tagihan
                    </h4>
                    <div class="space-y-2">
                        <div>
                            <p class="text-xs text-gray-500 dark:text-slate-400">Layanan</p>
                            <p class="text-sm text-gray-900 dark:text-white"><?php echo e($invoice->service_type ?? '-'); ?></p>
                        </div>
                        <div>
                        ...
                        <div>
                            <p class="text-xs text-gray-500 dark:text-slate-400">Asuransi</p>
                            <p class="text-sm text-gray-900 dark:text-white">
        ...
        <div
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-white/10">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Detail Layanan</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        <?php $__empty_1 = true; $__currentLoopData = $invoice->items ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                <td class="px-4 py-3 text-gray-900 dark:text-white"><?php echo e($item['description'] ?? '-'); ?>
                    ...
                    <tfoot class="bg-gray-50 dark:bg-white/5">
                        <tr>
                            <td colspan="3" class="px-4 py-3 text-right font-semibold text-gray-900 dark:text-white">
                                Subtotal</td>
                            <td class="px-4 py-3 text-right font-semibold text-gray-900 dark:text-white">Rp
                        ...
                        <?php if($invoice->discount > 0): ?>
                            <tr>
                                <td colspan="3" class="px-4 py-3 text-right text-gray-600 dark:text-slate-300">Diskon
                                </td>
                                <td class="px-4 py-3 text-right text-red-600 dark:text-red-400">- Rp
                                    <?php echo e(number_format($invoice->discount, 0, ',', '.')); ?></td>
                            </tr>
                        <?php endif; ?>
                        <?php if($invoice->tax > 0): ?>
                            <tr>
                                <td colspan="3" class="px-4 py-3 text-right text-gray-600 dark:text-slate-300">Pajak
                                    (<?php echo e($invoice->tax_percentage ?? 0); ?>%)</td>
                                <td class="px-4 py-3 text-right text-gray-600 dark:text-slate-300">Rp
                                    <?php echo e(number_format($invoice->tax, 0, ',', '.')); ?></td>
                            </tr>
                        <?php endif; ?>
                        <tr class="border-t-2 border-gray-300 dark:border-white/20">
                            <td colspan="3"
                                class="px-4 py-4 text-right text-lg font-bold text-gray-900 dark:text-white">TOTAL</td>
                            <td class="px-4 py-4 text-right text-lg font-bold text-blue-600 dark:text-blue-400">Rp
                                <?php echo e(number_format($invoice->total_amount, 0, ',', '.')); ?></td>
                        </tr>
                        <tr>
                            <td colspan="3" class="px-4 py-3 text-right text-gray-600 dark:text-slate-300">Dibayar
                            </td>
                            <td class="px-4 py-3 text-right text-green-600 dark:text-green-400">Rp
                                <?php echo e(number_format($invoice->paid_amount ?? 0, 0, ',', '.')); ?></td>
                        </tr>
                        <tr>
                            <td colspan="3" class="px-4 py-3 text-right text-gray-600 dark:text-slate-300">Sisa</td>
                            <td class="px-4 py-3 text-right text-red-600 dark:text-red-400 font-bold">Rp
