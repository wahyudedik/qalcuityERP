<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6">
        ...
        <div class="ml-auto flex gap-2">
            <a href="{{ route('writeoffs.create', ['type' => 'receivable']) }}"
                class="px-4 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">+ Write-off Piutang</a>
            <a href="{{ route('writeoffs.create', ['type' => 'payable']) }}"
                class="px-4 py-2 text-sm bg-purple-600 text-white rounded-xl hover:bg-purple-700">+ Write-off Hutang</a>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($writeoffs as $wo)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 hidden sm:table-cell">
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs font-medium {{ $wo->type === 'receivable' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700' }}">
                                    {{ $wo->typeLabel() }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-gray-700">{{ $wo->reference_number }}</td>
                            <td class="px-4 py-3 text-right font-semibold text-red-600">Rp
                                {{ number_format($wo->writeoff_amount, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 hidden md:table-cell text-gray-500 max-w-xs truncate">
                                {{ $wo->reason }}</td>
                            <td class="px-4 py-3 text-center">
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs font-medium {{ $wo->statusColor() }}">{{ ucfirst($wo->status) }}</span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    @if ($wo->isPending() && in_array(auth()->user()->role, ['admin', 'manager']))
                                        <form method="POST" action="{{ route('writeoffs.approve', $wo) }}">
                                            @csrf
                                            <button type="submit"
                                                class="px-2 py-1 text-xs bg-green-600 text-white rounded-lg hover:bg-green-700">Setujui</button>
                                        </form>
                                        <button onclick="openReject({{ $wo->id }})"
                                            class="px-2 py-1 text-xs bg-red-600 text-white rounded-lg hover:bg-red-700">Tolak</button>
                                    @endif
                                    @if ($wo->isApproved() && in_array(auth()->user()->role, ['admin', 'manager']))
                                        <form method="POST" action="{{ route('writeoffs.post', $wo) }}"
                                            data-confirm="Post jurnal write-off ini?">
                                            @csrf
                                            <button type="submit"
                                                class="px-2 py-1 text-xs bg-blue-600 text-white rounded-lg hover:bg-blue-700">Post
                                                Jurnal</button>
                                        </form>
                                    @endif
                                    @if ($wo->journalEntry)
                                        <a href="{{ route('journals.show', $wo->journalEntry) }}"
                                            class="px-2 py-1 text-xs border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50">Jurnal</a>
                                    @endif
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center text-gray-400">Belum ada data write-off.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
