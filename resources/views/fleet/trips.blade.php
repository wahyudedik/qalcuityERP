<x-app-layout>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    ...
                    @forelse($trips as $t)
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-gray-700">
                                {{ $t->vehicle?->plate_number ?? '-' }}</td>
                            <td class="px-4 py-3 hidden sm:table-cell text-gray-700">
                                {{ $t->driver?->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-900">{{ $t->purpose }}
                                @if ($t->destination)
                                    <span class="text-xs text-gray-400">→ {{ $t->destination }}</span>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3 text-center">
                                @if ($t->status === 'in_progress')
                                    ...
                                    <button
                                        onclick="openComplete({{ $t->id }}, '{{ $t->trip_number }}', {{ $t->odometer_start ?? 0 }})"
        ...
        @if ($trips->hasPages())
            <div class="px-4 py-3 border-t border-gray-100">{{ $trips->links() }}</div>
        @endif
    </div>
            ...
            <form method="POST" action="{{ route('fleet.trips.store') }}" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Kendaraan
                            *</label>
                        <select name="vehicle_id" required class="{{ $cls }}">
                            <option value="">-- Pilih --</option>
                            @foreach ($vehicles as $v)
                                <option value="{{ $v->id }}">{{ $v->plate_number }} — {{ $v->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Driver</label>
                        <select name="driver_id" class="{{ $cls }}">
                    ...
                    <div class="sm:col-span-2"><label
                            ...
                            *</label><input type="text" name="purpose" required
                            placeholder="Kirim barang ke customer" class="{{ $cls }}"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Asal</label><input
                            type="text" name="origin" placeholder="Gudang Pusat" class="{{ $cls }}"></div>
                    <div><label
