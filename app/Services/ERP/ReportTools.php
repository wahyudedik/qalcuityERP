...
class ReportTools
{
    ...
    public function getProfitLoss(array $args): array
    {
        ...
        $hpp = SalesOrderItem::whereHas('salesOrder', fn($q) =>
    ...
    public function getSalesTrend(array $args): array
    {
        ...
        // Produk terlaris
        $topProducts = SalesOrderItem::whereHas('salesOrder', fn($q) =>
            $q->where('tenant_id', $this->tenantId)
              ->whereNotIn('status', ['cancelled'])
              ->whereBetween('date', [$start, $end])
        )->join('products', 'sales_order_items.product_id', '=', 'products.id')
    ...
    public function getInventoryValuation(array $args): array
    {
        ...
        $activeProductIds = SalesOrderItem::whereHas('salesOrder', fn($q) =>
            $q->where('tenant_id', $this->tenantId)->where('date', '>=', now()->subDays(30))
        )->pluck('product_id')->unique();

        $deadStock = $stocks->filter(fn($s) =>
        ...
        return [
            ...
            'data'   => [
                ...
                'dead_stock_nilai'   => 'Rp ' . number_format(
                    $deadStock->sum(fn($s) => $s->quantity * ($s->product->price_buy ?? 0)), 0, ',', '.'
                ),
