    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                <p class="text-2xl font-bold text-gray-900">{{ number_format($totalHours, 1) }} <span
                        class="text-sm font-normal text-gray-400">jam</span></p>
            </div>
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <form method="POST" action="{{ route('timesheets.store') }}"
                ...
                <div>
                    ...
                    <input type="number" name="hours" required min="0.25" max="24" step="0.25"
                        placeholder="8"
                        class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-900 focus:outline-none focus:border-blue-500">
                </div>
                <div>
        ...
        <form method="GET" class="flex flex-wrap gap-3">
            ...
            @if (request()->hasAny(['project_id', 'user_id', 'month']))
                <a href="{{ route('timesheets.index') }}"
                    class="px-4 py-2 border border-gray-200 text-gray-600 text-sm font-medium rounded-xl hover:bg-gray-100 transition">Reset</a>
            @endif
        </form>
                            ...
                            @foreach ($timesheets ?? [] as $ts)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-3 text-gray-500 whitespace-nowrap">
                                        {{ $ts->date->format('d M Y') }}</td>
                                    <td class="px-6 py-3 font-medium text-gray-900">{{ $ts->project?->name ?? '-' }}
                                    </td>
                                    <td class="px-6 py-3 text-gray-500 hidden sm:table-cell">
                                        {{ $ts->user?->name ?? '-' }}</td>
                                    <td class="px-6 py-3 text-gray-600 max-w-xs truncate">{{ $ts->description }}</td>
                                    <td class="px-6 py-3 text-right font-medium text-gray-900">
                                        {{ number_format($ts->hours, 1) }}</td>
                                    ...
                                    <td class="px-6 py-3 text-right">
                                        @if (auth()->user()->hasRole(['admin', 'manager']) || $ts->user_id === auth()->id())
                                            <form method="POST" action="{{ route('timesheets.destroy', $ts) }}"
                                                data-confirm="Hapus entri ini?" data-confirm-type="danger">
                                                @csrf @method('DELETE')
                                                <button type="submit"
