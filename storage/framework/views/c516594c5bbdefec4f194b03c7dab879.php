...
<div x-data="offlineSyncPanel()" x-init="init()" x-show="show" x-cloak
    ...
    <div class="flex items-center justify-between px-4 py-3 cursor-pointer" @click="expanded = !expanded">
        <div class="flex items-center gap-2.5">
            ...
            <span class="text-sm font-medium" :class="online ? 'text-green-700 dark:text-green-400' : 'text-amber-700 dark:text-amber-400'">
    ...
    <div x-show="expanded" x-collapse class="px-4 pb-3 border-t border-white/10">
        <div class="mt-2 space-y-1.5 max-h-40 overflow-y-auto">
            <template x-for="item in items" :key="item.id">
                <div class="flex items-center justify-between text-xs py-1.5 px-2 rounded-lg bg-white/5">
                    <div class="flex items-center gap-2">
                        <span class="px-1.5 py-0.5 rounded bg-slate-700 text-slate-300 uppercase text-[10px] font-bold" x-text="item.method"></span>
...
<?php if (! $__env->hasRenderedOnce('83c4fed1-6f07-4feb-89d3-8e4c52a48e37')): $__env->markAsRenderedOnce('83c4fed1-6f07-4feb-89d3-8e4c52a48e37'); ?>
...
function offlineSyncPanel() {
    return {
        ...
        async init() {
            await this.refresh();
            window.addEventListener('online', () => { this.online = true; this.refresh(); });
            window.addEventListener('offline', () => { this.online = false; this.refresh(); });
