<x-app-layout>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
            ...
            <p class="text-lg font-bold text-gray-900 dark:text-white">Rp {{ number_format($stats['value'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-4">
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('contracts.templates') }}" class="px-3 py-2 text-sm border border-gray-200 dark:border-white/10 rounded-xl text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-white/5">Template</a>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        ...
                        <td class="px-4 py-3 text-gray-700 dark:text-slate-300">{{ Str::limit($c->title, 40) }}</td>
                        ...
                        <td class="px-4 py-3 text-right hidden md:table-cell text-gray-900 dark:text-white">Rp {{ number_format($c->value, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-xs bg-{{ $sc }}-100 text-{{ $sc }}-700 dark:bg-{{ $sc }}-500/20 dark:text-{{ $sc }}-400">{{ $sl }}</span>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <a href="{{ route('contracts.show', $c) }}" class="text-xs px-2 py-1 border border-gray-200 dark:border-white/10 rounded-lg text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-white/5">Detail</a>
                        </td>
                    </tr>
                    @empty
        ...
        @if($contracts->hasPages())<div class="px-4 py-3 border-t border-gray-100 dark:border-white/5">{{ $contracts->links() }}</div>@endif
    </div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-2xl shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" action="{{ route('contracts.store') }}" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2"><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Judul Kontrak *</label><input type="text" name="title" required placeholder="Kontrak Sewa Gudang 2026" class="{{ $cls }}"></div>
                    ...
                    @endif
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Mulai *</label><input type="date" name="start_date" required class="{{ $cls }}"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Berakhir *</label><input type="date" name="end_date" required class="{{ $cls }}"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Nilai Kontrak (Rp) *</label><input type="number" name="value" required min="0" step="1000" class="{{ $cls }}"></div>
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Billing per Siklus (Rp)</label><input type="number" name="billing_amount" min="0" step="1000" class="{{ $cls }}"></div>
                ...
                <details class="text-sm">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-3">
                        <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Response Time (jam)</label><input type="number" name="sla_response_hours" min="1" class="{{ $cls }}"></div>
                        <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Resolution Time (jam)</label><input type="number" name="sla_resolution_hours" min="1" class="{{ $cls }}"></div>
                        <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Uptime (%)</label><input type="number" name="sla_uptime_pct" min="0" max="100" step="0.01" placeholder="99.90" class="{{ $cls }}"></div>
                        <div class="sm:col-span-3"><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Ketentuan SLA</label><textarea name="sla_terms" rows="2" class="{{ $cls }}"></textarea></div>
                        <div class="sm:col-span-3"><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Terms & Conditions</label><textarea name="terms" rows="2" class="{{ $cls }}"></textarea></div>
                    </div>
                </details>
