        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('Detail paket internet') }}</p>
                </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg overflow-hidden">
                        <div class="bg-gradient-to-r from-blue-500 to-blue-600 dark:from-blue-600 dark:to-blue-700 p-6 text-white">
                            <div class="flex justify-between items-start">
                                <div>
                                    ...
                                    <p class="text-blue-100 dark:text-blue-200 text-sm mt-1">{{ ucfirst($package->billing_cycle ?? 'monthly') }}</p>
                                </div>
                                @if ($package->is_active)
                                    <span class="px-2 py-1 text-xs bg-green-500 dark:bg-green-600 rounded-full">{{ __('Aktif') }}</span>
                                @else
                                    <span class="px-2 py-1 text-xs bg-gray-500 dark:bg-gray-600 rounded-full">{{ __('Nonaktif') }}</span>
                                @endif
                            </div>
                            <div class="mt-4">
                        ...
                        <div class="p-6 space-y-4">
                            <div class="flex justify-between">
                                ...
                                <span class="font-bold text-gray-900 dark:text-white">{{ $package->download_speed_mbps }} Mbps</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400 text-sm">{{ __('Upload') }}</span>
                                <span class="font-bold text-gray-900 dark:text-white">{{ $package->upload_speed_mbps }} Mbps</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400 text-sm">{{ __('Kuota') }}</span>
                                <span class="font-bold text-gray-900 dark:text-white">
                                    @if ($package->isUnlimited())
                                        <span class="text-blue-600 dark:text-blue-400">∞ {{ __('Unlimited') }}</span>
                                    @else
                            ...
                            @if ($package->installation_fee > 0)
                                <div class="flex justify-between">
                                    <span class="text-gray-600 dark:text-gray-400 text-sm">{{ __('Biaya Pasang') }}</span>
                                    <span class="font-bold text-gray-900 dark:text-white">Rp {{ number_format($package->installation_fee, 0, ',', '.') }}</span>
                                </div>
                            @endif
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Pelanggan') }}</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Status') }}</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Mulai') }}</th>
                                    ...
                                    @forelse($package->subscriptions as $sub)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $sub->customer?->name ?? '-' }}</div>
                                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ $sub->customer?->email ?? '' }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                                    {{ $sub->status === 'active' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' : ($sub->status === 'suspended' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400') }}">
                                                    {{ ucfirst($sub->status) }}
                                                </span>
                                            </td>
