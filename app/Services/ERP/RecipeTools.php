...
class RecipeTools
{
    ...
    public function createRecipe(array $args): array
    {
        ...
        return DB::transaction(function () use ($product, $args, $ingredientData, $batchSize, $batchUnit) {
            ...
            $ingList = collect($ingredientData)->map(fn($i) =>
                "- **{$i['product']->name}**: {$i['quantity_per_batch']} {$i['unit']}"
            )->implode("\n");
    ...
    public function produceWithRecipe(array $args): array
    {
        ...
        if (!empty($shortage)) {
            $shortageList = collect($shortage)->map(fn($s) =>
                "- **{$s['bahan']}**: butuh {$s['dibutuhkan']}, tersedia {$s['tersedia']}, kurang **{$s['kurang']}**"
            )->implode("\n");
        ...
        if ($dryRun) {
            $checkList = collect($needs)->map(fn($n) =>
                "- **{$n['product']->name}**: butuh " . round($n['qty_needed'], 3) . " {$n['ingredient']->unit}, tersedia {$n['current_stock']} ✅"
            )->implode("\n");
        ...
        return DB::transaction(function () use ($product, $recipe, $qty, $warehouse, $needs, $args) {
            $reference = 'PROD-' . strtoupper(\Illuminate\Support\Str::random(6));
            ...
            $materialCost = collect($needs)->sum(fn($n) =>
                ($n['product']->price_buy ?? 0) * $n['qty_needed']
            );

            $ingDeducted = collect($needs)->map(fn($n) =>
                "- **{$n['product']->name}**: -{$n['qty_needed']} {$n['ingredient']->unit} (sisa: " . ($n['current_stock'] - $n['qty_needed']) . ")"
            )->implode("\n");
