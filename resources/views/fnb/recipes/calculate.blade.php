...
@section('content')
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            @endphp
            <div class="{{ $marginGood ? 'bg-purple-50 border-purple-500' : 'bg-red-50 border-red-500' }} rounded-lg shadow p-4 border-l-4">
                <div class="text-sm {{ $marginGood ? 'text-purple-600' : 'text-red-600' }}">
                    Margin Keuntungan
                </div>
                <div class="text-2xl font-bold {{ $marginGood ? 'text-purple-700' : 'text-red-700' }}">
                    {{ $marginPct }}%
                </div>
            </div>
        </div>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-xs text-gray-600">{{ $ingredient['percentage'] ?? 0 }}%</span>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div class="bg-white rounded-lg shadow p-6 border border-gray-200">
            ...
            <div class="space-y-3">
                ...
                <div class="flex justify-between items-center p-3 {{ $isProfitable ? 'bg-green-50' : 'bg-red-50' }} rounded">
                    <span class="font-medium text-gray-700">Keuntungan per Porsi</span>
                    <span class="font-bold {{ $isProfitable ? 'text-green-700' : 'text-red-700' }}">
                        Rp {{ number_format($costData['profit_margin']['profit_per_serving'] ?? 0, 0, ',', '.') }}
                    </span>
                </div>
                <div class="flex justify-between items-center p-3 {{ $marginGood ? 'bg-green-50' : 'bg-yellow-50' }} rounded">
                    <span class="font-medium text-gray-700">Persentase Margin</span>
                    <span class="font-bold {{ $marginGood ? 'text-green-700' : 'text-yellow-700' }}">
                        {{ $marginPct }}%
                    </span>
                </div>
            </div>
