            ...
            <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-red-500/20 p-4">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="px-1.5 py-0.5 rounded text-xs font-bold {{ $sp->levelColor() }}">{{ $sp->levelLabel() }}</span>
                        <span class="text-xs text-gray-700 dark:text-slate-300 truncate">{{ $sp->employee->name ?? '-' }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                <td class="px-4 py-3">
                                    <p class="font-medium text-gray-900 dark:text-white">{{ $sp->employee->name ?? '-' }}</p>
                                    <p class="text-xs text-gray-400 dark:text-slate-500">{{ $sp->letter_number }}</p>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <span class="px-2 py-0.5 rounded-full text-xs font-semibold {{ $sp->levelColor() }}">{{ $sp->levelLabel() }}</span>
                                    @if($sp->source === 'ai_anomaly')
                                    <p class="text-xs text-purple-400 mt-0.5">AI</p>
                                    @endif
                                </td>
                                <td class="px-4 py-3 hidden sm:table-cell">
                                    <p class="text-xs font-medium text-gray-700 dark:text-slate-300">{{ $sp->violation_type }}</p>
                                    <p class="text-xs text-gray-400 dark:text-slate-500 truncate max-w-[200px]">{{ $sp->violation_description }}</p>
                                </td>
                                <td class="px-4 py-3 hidden md:table-cell text-center text-xs text-gray-500 dark:text-slate-400">
                                    {{ $sp->issued_date->format('d M Y') }}
                                    @if($sp->valid_until)
                                    <p class="text-xs {{ $sp->valid_until->isPast() ? 'text-red-400' : 'text-gray-400 dark:text-slate-500' }}">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        @if($sp->status === 'issued')
                                        <form method="POST" action="{{ route('hrm.disciplinary.acknowledge', $sp) }}">
                                            @csrf @method('PATCH')
