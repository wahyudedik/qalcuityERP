<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Enter Lab Result') }}</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('healthcare.lab-results.store') }}">
                    @csrf
                    <div class="space-y-6">
                        <div>
                        ...
                        <div>
                            <label for="result_value" class="block text-sm font-medium text-gray-700">Result Value
                                *</label>
                            <input type="text" name="result_value" required value="{{ old('result_value') }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                placeholder="e.g., 120">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="interpretation" rows="3"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                placeholder="Clinical interpretation...">{{ old('interpretation') }}</textarea>
                        </div>
                    </div>
