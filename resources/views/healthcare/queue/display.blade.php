...
<body class="min-h-screen text-white">
    ...
    <div class="bg-white/10 backdrop-blur-md border-b border-white/20 p-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black">{{ config('app.name') }}</h1>
                    <p class="text-lg text-white/80">Sistem Antrian Digital</p>
                </div>
            </div>
            <div class="text-right">
                <div id="clock" class="text-4xl font-black">{{ now()->format('H:i:s') }}</div>
                <p class="text-lg text-white/80">{{ now()->format('d F Y') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="p-8">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-6">
                        ...
                        <div>
                            ...
                            <p class="text-7xl font-black">{{ $nowCalling['queue_number'] ?? '-' }}</p>
                            <p class="text-2xl text-white/90 mt-2">{{ $nowCalling['patient_name'] ?? '-' }}</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-xl text-white/80 mb-2">Loket / Ruangan</p>
                        <p class="text-5xl font-black">{{ $nowCalling['counter'] ?? '-' }}</p>
                        <p class="text-xl text-white/80 mt-2">{{ $nowCalling['department'] ?? '-' }}</p>
                    </div>
                </div>
            </div>
        @endif
            ...
            <div class="bg-white/10 backdrop-blur-md rounded-3xl p-6 border border-white/20">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-white/70">{{ count($waitingQueue ?? []) }} antrian</p>
                    </div>
                </div>
                <div class="space-y-3 max-h-96 overflow-y-auto">
                    @forelse($waitingQueue ?? [] as $queue)
                        <div class="queue-item bg-white/10 rounded-xl p-4">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-3xl font-black">{{ $queue['queue_number'] ?? '-' }}</p>
                                    <p class="text-sm text-white/70 mt-1">{{ $queue['patient_name'] ?? '-' }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-white/60">Loket</p>
                                    <p class="text-xl font-bold">{{ $queue['counter'] ?? '-' }}</p>
                                </div>
                            </div>
                        </div>
                    @empty
            ...
            <div class="bg-white/10 backdrop-blur-md rounded-3xl p-6 border border-white/20">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-white/70">{{ count($inProgress ?? []) }} pasien</p>
                    </div>
                </div>
                <div class="space-y-3 max-h-96 overflow-y-auto">
                    @forelse($inProgress ?? [] as $queue)
            ...
            <div class="bg-white/10 backdrop-blur-md rounded-3xl p-6 border border-white/20">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-white/70">{{ count($completedQueue ?? []) }} pasien</p>
                    </div>
                </div>
                <div class="space-y-3 max-h-96 overflow-y-auto">
                    @forelse($completedQueue ?? [] as $queue)
                        <div class="queue-item bg-green-500/20 rounded-xl p-4">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-2xl font-bold text-white/80">{{ $queue['queue_number'] ?? '-' }}
                                    </p>
                                    <p class="text-sm text-white/60 mt-1">{{ $queue['patient_name'] ?? '-' }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-white/50">Selesai</p>
                                    <p class="text-sm text-white/70">{{ $queue['completed_at'] ?? '-' }}</p>
                                </div>
                            </div>
                        </div>
                    @empty
        ...
        <div class="mt-8 bg-white/10 backdrop-blur-md rounded-3xl p-6 border border-white/20">
            <div class="grid grid-cols-4 gap-6 text-center">
                <div>
                    <p class="text-5xl font-black text-amber-400">{{ $stats['total_waiting'] ?? 0 }}</p>
                    <p class="text-lg text-white/70 mt-2">Total Menunggu</p>
                </div>
                <div>
                    <p class="text-5xl font-black text-blue-400">{{ $stats['total_in_progress'] ?? 0 }}</p>
                    <p class="text-lg text-white/70 mt-2">Sedang Dilayani</p>
                </div>
                <div>
                    <p class="text-5xl font-black text-green-400">{{ $stats['total_completed'] ?? 0 }}</p>
                    <p class="text-lg text-white/70 mt-2">Selesai</p>
                </div>
                <div>
                    <p class="text-5xl font-black text-purple-400">{{ $stats['avg_wait_time'] ?? 0 }} min</p>
    ...
    <script>
        ...
        let lastQueueNumber = '{{ $nowCalling['queue_number'] ?? '' }}';
