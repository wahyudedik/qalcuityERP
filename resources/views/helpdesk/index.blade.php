<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('helpdesk.kb') }}" class="px-3 py-2 text-sm border border-gray-200 dark:border-white/10 rounded-xl text-gray-600 dark:text-slate-300">Knowledge Base</a>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    @endphp
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5 {{ $t->isOverdue() ? 'bg-red-50/50 dark:bg-red-500/5' : '' }}">
                        ...
                        <td class="px-4 py-3 text-gray-700 dark:text-slate-300">{{ Str::limit($t->subject, 35) }}</td>
                        <td class="px-4 py-3 hidden sm:table-cell text-xs text-gray-500 dark:text-slate-400">{{ $t->customer->name ?? $t->contact_name ?? '-' }}</td>
                        <td class="px-4 py-3 text-center"><span class="px-2 py-0.5 rounded-full text-xs bg-{{ $pc }}-100 text-{{ $pc }}-700 dark:bg-{{ $pc }}-500/20 dark:text-{{ $pc }}-400">{{ ucfirst($t->priority) }}</span></td>
                        <td class="px-4 py-3 text-center hidden sm:table-cell text-xs text-gray-500 dark:text-slate-400">{{ $t->assignee->name ?? '-' }}</td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <a href="{{ route('helpdesk.show', $t) }}" class="text-xs px-2 py-1 border border-gray-200 dark:border-white/10 rounded-lg text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-white/5">Detail</a>
                        </td>
                    </tr>
                    @empty
        ...
        @if($tickets->hasPages())<div class="px-4 py-3 border-t border-gray-100 dark:border-white/5">{{ $tickets->links() }}</div>@endif
    </div>
    ...
    <div id="modal-add" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" action="{{ route('helpdesk.store') }}" class="p-6 space-y-4">
                ...
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Subjek *</label><input type="text" name="subject" required class="{{ $cls }}"></div>
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Deskripsi *</label><textarea name="description" required rows="3" class="{{ $cls }}"></textarea></div>
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Nama Kontak</label><input type="text" name="contact_name" class="{{ $cls }}"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Email Kontak</label><input type="email" name="contact_email" class="{{ $cls }}"></div>
                </div>
