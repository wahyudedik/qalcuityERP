                    ...
                    <div
                        ...
                        <div>
                            <div class="flex items-center gap-2">
                                <span
                                    class="text-lg font-bold text-gray-900 dark:text-white">{{ $unit->unit_code }}</span>
                                <span
                                    class="text-xs px-2 py-0.5 rounded-full bg-{{ $tempClass }}-100 text-{{ $tempClass }}-700 dark:bg-{{ $tempClass }}-500/20 dark:text-{{ $tempClass }}-400">
                                    {{ $unit->isTemperatureSafe() ? 'Normal' : 'Warning' }}
                                </span>
                            </div>
                            <p class="text-sm text-gray-500 dark:text-slate-400 mt-0.5">{{ $unit->name }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2 text-xs mt-3">
                            @if ($unit->capacity_kg)
                                <div>
                                    ...
                                    <span
                                        class="text-gray-700 dark:text-slate-300">{{ number_format($unit->capacity_kg, 0) }}
                                        kg</span>
                                </div>
                            @endif
                            @if ($unit->utilization_percentage)
                                <div>
                                    ...
                                    <span
                                        class="text-gray-700 dark:text-slate-300">{{ number_format($unit->utilization_percentage, 1) }}%</span>
                                </div>
                            @endif
                            @if ($unit->location)
                                <div class="col-span-2">
                                    <span class="text-gray-400">Lokasi:</span>
                                    <span class="text-gray-700 dark:text-slate-300">{{ $unit->location }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                @foreach ($alerts as $alert)
                    <div
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-600 dark:text-slate-400">{{ $alert['description'] }}</p>
                            <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">{{ $alert['time'] }}</p>
                        </div>
                        <span
                            class="text-xs px-2 py-1 rounded-full bg-{{ $alert['severity_color'] }}-100 text-{{ $alert['severity_color'] }}-700 dark:bg-{{ $alert['severity_color'] }}-500/20 dark:text-{{ $alert['severity_color'] }}-400">
