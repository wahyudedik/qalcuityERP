                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($returns as $return)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3 text-slate-500 font-mono text-xs">{{ $return->invoice?->number ?? '-' }}
                            </td>
                            ...
                            <td class="px-4 py-3 text-slate-500 capitalize">
                                {{ str_replace('_', ' ', $return->refund_method) }}</td>
                            <td class="px-4 py-3">
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    @if ($return->status === 'draft')
                                        <form method="POST" action="{{ route('sales-returns.approve', $return) }}"
                                            ...
                                            @csrf
                                            <button type="submit"
                                                class="text-xs px-2 py-1 bg-blue-100 text-blue-700 hover:bg-blue-200 rounded">Setujui</button>
                                        </form>
                                    @endif
                                    @if ($return->status === 'approved')
                                        <form method="POST" action="{{ route('sales-returns.complete', $return) }}"
                                            class="inline">
                                            @csrf
                                            <button type="submit"
                                                class="text-xs px-2 py-1 bg-green-100 text-green-700 hover:bg-green-200 rounded">Selesaikan</button>
                                        </form>
                                    @endif
                                    @if (in_array($return->status, ['draft', 'approved']))
                                        <form method="POST" action="{{ route('sales-returns.cancel', $return) }}"
