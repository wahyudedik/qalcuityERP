    ...
    <div class="bg-white rounded-2xl border border-gray-200 mb-4">
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4">
            ...
            <div class="flex gap-2">
                <a href="{{ route('inventory.index') }}" class="px-3 py-2 text-sm border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50">Inventori</a>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 hidden sm:table-cell font-mono text-xs text-gray-500">{{ $wh->code }}</td>
                        <td class="px-4 py-3 hidden md:table-cell text-gray-500 text-xs max-w-xs truncate">{{ $wh->address ?? '-' }}</td>
                        <td class="px-4 py-3 text-right hidden lg:table-cell text-gray-700">{{ number_format($wh->product_stocks_count ?? 0) }}</td>
                        <td class="px-4 py-3 text-right font-semibold text-gray-900">{{ number_format($wh->product_stocks_sum_quantity ?? 0) }}</td>
                        <td class="px-4 py-3 text-center hidden sm:table-cell">
                            <span class="px-2 py-0.5 rounded-full text-xs {{ $wh->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <form method="POST" action="{{ route('warehouses.toggle', $wh) }}">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="p-1.5 rounded-lg {{ $wh->is_active ? 'text-yellow-500 hover:bg-yellow-50' : 'text-green-500 hover:bg-green-50' }}" title="{{ $wh->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
