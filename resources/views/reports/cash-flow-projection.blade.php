...
<div class="p-6 space-y-6">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('reports.cash-flow-projection', ['days' => $d]) }}"
               class="px-4 py-2 rounded-lg text-sm font-medium transition-colors
                      {{ $days == $d ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }}">
                {{ $d }} Hari
            </a>
            @endforeach
        </div>
    ...
    <div class="space-y-2">
        ...
        <div class="flex items-start gap-3 p-4 rounded-xl border
                    {{ $alert['type'] === 'deficit' ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200' }}">
            ...
            <p class="text-sm font-medium {{ $alert['type'] === 'deficit' ? 'text-red-700' : 'text-amber-700' }}">
                {{ $alert['message'] }}
            </p>
        </div>
        @endforeach
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p class="text-xl font-bold text-gray-900 mt-1">Rp {{ number_format($data['opening_balance'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500">Total Masuk (AR)</p>
            <p class="text-xl font-bold text-emerald-600 mt-1">Rp {{ number_format($data['totals']['inflow'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500">Total Keluar (AP)</p>
            <p class="text-xl font-bold text-red-600 mt-1">Rp {{ number_format($data['totals']['outflow'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500">Net {{ $days }} Hari</p>
            @php $net = $data['totals']['net']; @endphp
            <p class="text-xl font-bold mt-1 {{ $net >= 0 ? 'text-emerald-600' : 'text-red-600' }}">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        <h2 class="text-sm font-semibold text-gray-700 mb-4">Proyeksi Saldo Kas ({{ $days }} Hari)</h2>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3 text-gray-700 font-medium">{{ $week['label'] }}</td>
                        ...
                        <td class="px-4 py-3 text-right font-medium {{ $week['net'] >= 0 ? 'text-emerald-600' : 'text-red-600' }}">
                            {{ $week['net'] >= 0 ? '+' : '' }}Rp {{ number_format($week['net'], 0, ',', '.') }}
                        </td>
                        <td class="px-4 py-3 text-right font-semibold {{ $week['balance'] >= 0 ? 'text-gray-900' : 'text-red-600' }}">
    ...
    <p class="text-xs text-gray-400 text-right">Dibuat: {{ $data['generated_at'] }}</p>
</div>
