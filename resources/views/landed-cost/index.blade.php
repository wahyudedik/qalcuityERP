<x-app-layout>
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        ...
                        <td class="px-4 py-3 text-gray-700 dark:text-slate-300 text-xs">{{ $lc->purchaseOrder->number ?? '-' }}</td>
                        <td class="px-4 py-3 hidden sm:table-cell text-gray-500 dark:text-slate-400">{{ Str::limit($lc->description, 40) }}</td>
                        <td class="px-4 py-3 text-right font-medium text-gray-900 dark:text-white">Rp {{ number_format($lc->total_additional_cost, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-center text-xs text-gray-500 dark:text-slate-400">{{ $ml }}</td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('landed-cost.show', $lc) }}" class="text-xs px-2 py-1 border border-gray-200 dark:border-white/10 rounded-lg text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-white/5">Detail</a>
                                @if($lc->status !== 'posted')
                                @canmodule('landed_cost', 'delete')
                                <form method="POST" action="{{ route('landed-cost.destroy', $lc) }}" class="inline" onsubmit="return confirm('Hapus?')">
                                    @csrf @method('DELETE')
        ...
        @if($landedCosts->hasPages())<div class="px-4 py-3 border-t border-gray-100 dark:border-white/5">{{ $landedCosts->links() }}</div>@endif
    </div>
            ...
            <form method="POST" action="{{ route('landed-cost.store') }}" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="description" placeholder="Biaya impor PO-xxx" class="{{ $cls }}">
                    </div>
                </div>
