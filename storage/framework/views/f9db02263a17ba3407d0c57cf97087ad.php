...
<?php $component->withAttributes([]); ?>
    ...
    <?php if(session('success')): ?>
        <div class="mb-4 px-4 py-3 bg-green-100 border border-green-200 text-green-600 text-sm rounded-xl">
    ...
    <div x-data="detailModal()" x-cloak>
        ...
        <div x-show="open" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div
                class="bg-white border border-gray-200 rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl">
                <div class="flex items-start justify-between gap-4 px-6 py-4 border-b border-gray-200 shrink-0">
                    <div class="flex items-center gap-3 min-w-0">
                        <span class="text-[11px] font-bold px-2.5 py-1 rounded-full shrink-0 border"
                            :class="badgeClass()">
                            <span x-text="badgeLabel()"></span>
                        </span>
                        <p class="text-sm font-semibold text-gray-900 truncate" x-text="title()"></p>
                    </div>
                    <button @click="close()"
                        class="shrink-0 p-1.5 rounded-lg text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition">
                ...
                <div class="overflow-y-auto flex-1 px-6 py-5 space-y-4">
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <template x-for="m in metaFields()" :key="m.label">
                            <div class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                                <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1" x-text="m.label">
                                </p>
                                <p class="text-xs font-medium break-all" :class="m.color || 'text-gray-800'"
                                    x-text="m.value||'—'"></p>
                            </div>
                        </template>
                    </div>
                    <template x-if="type==='error' && d.file">
                        <div class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                            <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">File</p>
                    ...
                    <template x-if="(type==='error'||type==='activity') && (d.url||d.ip||d.ip_address)">
                        <div class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                            <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Request</p>
                            <template x-if="d.url">
                                <p class="text-xs text-gray-800 font-mono break-all">
                                    <span class="text-blue-600 font-bold mr-2" x-text="d.method"></span><span
                                        x-text="d.url"></span>
                                </p>
                            </template>
                            <template x-if="d.ip||d.ip_address">
                                <p class="text-[10px] text-gray-400 mt-1">IP: <span class="text-gray-500 font-mono"
                                        x-text="d.ip||d.ip_address"></span></p>
                            </template>
                            <template x-if="d.user_agent">
                                <p class="text-[10px] text-gray-400 mt-0.5 break-all">UA: <span class="text-gray-500"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <template x-if="d.old_values">
                                <div class="bg-red-50 border border-red-200 rounded-xl px-4 py-3">
                                    <p class="text-[10px] text-red-600 uppercase tracking-wider mb-2">Nilai Lama</p>
                                    <pre class="text-[11px] text-gray-700 font-mono whitespace-pre-wrap break-all"
                            ...
                            <template x-if="d.new_values">
                                <div class="bg-green-50 border border-green-200 rounded-xl px-4 py-3">
                                    <p class="text-[10px] text-green-600 uppercase tracking-wider mb-2">Nilai Baru</p>
                                    <pre class="text-[11px] text-gray-700 font-mono whitespace-pre-wrap break-all"
                    ...
                    <template x-if="type==='ai' && d.breakdown && d.breakdown.length">
                        <div class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                            <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-3">Riwayat Bulanan</p>
                            <div class="space-y-2">
                                <template x-for="b in d.breakdown" :key="b.month">
                                    <div class="flex items-center gap-3">
                                        <span class="text-[11px] text-gray-500 w-16 shrink-0" x-text="b.month"></span>
                                        <div class="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                            <div class="h-full bg-blue-500 rounded-full"
                                                :style="'width:' + b.pct + '%'">
                                            </div>
                                        </div>
                                        <span class="text-[11px] text-gray-700 w-24 text-right shrink-0"
                    ...
                    <template x-if="type==='anomaly' && d.description">
                        <div class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                            <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-2">Deskripsi Lengkap</p>
                            <p class="text-sm text-gray-800 leading-relaxed" x-text="d.description"></p>
                        </div>
                    </template>
                    <template x-if="type==='anomaly' && d.data">
                        <div class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                            <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-2">Data Payload</p>
                            <pre class="text-[11px] text-gray-700 font-mono whitespace-pre-wrap break-all" x-text="JSON.stringify(d.data,null,2)"></pre>
                        </div>
                    </template>
                    <template x-if="type==='error' && d.context">
                        <div class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                            <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-2">Context</p>
                            <pre class="text-[11px] text-gray-700 font-mono whitespace-pre-wrap break-all"
                                x-text="JSON.stringify(d.context,null,2)"></pre>
                        </div>
                    </template>
                    <template x-if="type==='error' && d.trace">
                        <div class="bg-black/40 border border-gray-200 rounded-xl px-4 py-3">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-[10px] text-gray-400 uppercase tracking-wider">Stack Trace</p>
                                <button @click="copy(d.trace)"
                                    class="text-[10px] text-blue-600 hover:text-blue-300 border border-blue-200 px-2 py-0.5 rounded-lg transition"
                                    x-text="copied?'✓ Tersalin':'Salin'"></button>
                            </div>
                            <pre class="text-[11px] text-gray-500 font-mono whitespace-pre-wrap break-all leading-relaxed" x-text="d.trace"></pre>
                        </div>
                    </template>
                </div>
                <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between gap-3 shrink-0">
                    <span class="text-[11px] text-gray-400" x-text="footerText()"></span>
                    <div class="flex gap-2">
                        <template x-if="type==='error' && !d.is_resolved">
                            <form :action="'/super-admin/monitoring/errors/' + d.id + '/resolve'" method="POST"
                                ...
                                <?php echo csrf_field(); ?>
                                <button type="submit"
                                    class="px-4 py-2 text-xs bg-green-100 hover:bg-green-200 text-green-600 border border-green-200 rounded-xl transition">Tandai
                                    Resolved</button>
                            </form>
                        </template>
                        <button @click="close()"
                            class="px-4 py-2 text-xs bg-gray-50 hover:bg-gray-100 text-gray-700 border border-gray-200 rounded-xl transition">Tutup</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-1 mb-6 bg-gray-50 border border-gray-200 rounded-2xl p-1 w-fit flex-wrap">
        ...
        ['tab' => 'modules', 'label' => 'Module Health', 'icon' => 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10'],
              ...
              <?php echo e($tab === $t['tab'] ? 'bg-blue-600 text-gray-900' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'); ?>">
                    ...
                    <?php if($t['tab'] === 'errors' && $errorStats['unresolved'] > 0): ?>
                        <span
                            class="text-[10px] bg-red-500 text-gray-900 px-1.5 py-0.5 rounded-full font-bold"><?php echo e($errorStats['unresolved']); ?></span>
                    <?php endif; ?>
                    <?php if($t['tab'] === 'anomaly' && $anomalyStats['critical'] > 0): ?>
                        <span
                            class="text-[10px] bg-red-500 text-gray-900 px-1.5 py-0.5 rounded-full font-bold"><?php echo e($anomalyStats['critical']); ?></span>
                    <?php endif; ?>
                </a>
        ...
        <?php if($tab === 'errors'): ?>
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                <?php $__currentLoopData = [['label' => 'Total Error', 'value' => $errorStats['total'], 'color' => 'text-gray-700', 'bg' => 'bg-gray-50'], ['label' => 'Belum Resolved', 'value' => $errorStats['unresolved'], 'color' => 'text-red-600', 'bg' => 'bg-red-50'], ['label' => 'Hari Ini', 'value' => $errorStats['today'], 'color' => 'text-orange-600', 'bg' => 'bg-orange-50'], ['label' => 'Critical', 'value' => $errorStats['critical'], 'color' => 'text-red-600', 'bg' => 'bg-red-50']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="bg-white border border-gray-200 rounded-2xl p-4 text-center <?php echo e($s['bg']); ?>">
                        <p class="text-2xl font-bold <?php echo e($s['color']); ?>"><?php echo e($s['value']); ?></p>
                        <p class="text-xs text-gray-400 mt-1"><?php echo e($s['label']); ?></p>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
            <div class="flex flex-wrap items-center gap-3 mb-4">
                <form method="GET" action="" class="flex gap-2 flex-wrap flex-1">
                    ...
                    <select name="level"
                        class="px-3 py-2 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <button type="submit"
                        class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-gray-900 text-sm rounded-xl transition">Filter</button>
                </form>
                <div class="flex gap-2">
                    <form method="POST" action="<?php echo e(route('super-admin.monitoring.resolve-all')); ?>">
                        <?php echo csrf_field(); ?>
                        <button
                            class="px-3 py-2 text-xs bg-green-100 hover:bg-green-200 text-green-600 border border-green-200 rounded-xl transition">Resolve
                            Semua</button>
                    </form>
                    <form method="POST" action="<?php echo e(route('super-admin.monitoring.clear-errors')); ?>">
                        <?php echo csrf_field(); ?>
                        <button
                            class="px-3 py-2 text-xs bg-red-100 hover:bg-red-200 text-red-600 border border-red-200 rounded-xl transition">Bersihkan
                            Resolved</button>
                    </form>
                </div>
            </div>
            <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php $__empty_1 = true; $__currentLoopData = $errors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $err): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="hover:bg-gray-50 transition cursor-pointer <?php echo e($err->is_resolved ? 'opacity-60' : ''); ?>"
                    ...
                    'tenant_id'=>$err->tenant_id,'user_id'=>$err->user_id,
                                    ...
                                    <td class="px-4 py-3 max-w-xs">
                                        <p class="text-gray-800 text-xs font-medium truncate"><?php echo e($err->message); ?></p>
                                        <?php if($err->file): ?>
                                            <p class="text-gray-400 text-[10px] truncate mt-0.5">
                                                <?php echo e(basename($err->file)); ?>:<?php echo e($err->line); ?></p>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-4 py-3 hidden md:table-cell">
                                        <p class="text-gray-500 text-xs truncate max-w-[180px]"><?php echo e($err->method); ?>

                                            <?php echo e($err->url); ?></p>
                                        <?php if($err->ip): ?>
                                            <p class="text-gray-400 text-[10px]"><?php echo e($err->ip); ?></p>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-4 py-3 hidden lg:table-cell">
                                        <p class="text-gray-500 text-xs"><?php echo e($err->created_at->format('d M Y')); ?></p>
                                        <p class="text-gray-400 text-[10px]"><?php echo e($err->created_at->format('H:i:s')); ?>
                                    ...
                                    <td class="px-4 py-3">
                                        <?php if($err->is_resolved): ?>
                                            <span
                                                class="text-[11px] text-green-600 bg-green-100 px-2 py-0.5 rounded-full">Resolved</span>
                                        <?php else: ?>
                                            <span
                                                class="text-[11px] text-red-600 bg-red-100 px-2 py-0.5 rounded-full">Open</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-4 py-3" @click.stop>
                                        <div class="flex items-center justify-end gap-1">
                                            <button @click.stop="show('error', rowData($el.closest('tr')))"
                                                class="p-1.5 rounded-lg text-gray-500 hover:text-blue-600 hover:bg-blue-50 transition"
                                                title="Detail">
                            ...
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center text-gray-400 text-sm">Tidak ada
                                        error log.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                ...
                <?php if($errors->hasPages()): ?>
                    <div class="px-6 py-4 border-t border-gray-200"><?php echo e($errors->links()); ?></div>
                <?php endif; ?>
            </div>
        <?php endif; ?>
        ...
        <?php if($tab === 'ai'): ?>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
                <?php $__currentLoopData = [['label' => 'Pesan Bulan Ini', 'value' => number_format($aiStats['total_this_month']), 'color' => 'text-blue-600'], ['label' => 'Total Token', 'value' => number_format($aiStats['total_tokens']), 'color' => 'text-purple-600'], ['label' => 'Tenant Aktif AI', 'value' => $aiStats['active_tenants'], 'color' => 'text-green-600']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-5 mb-5">
                <p class="text-sm font-semibold text-gray-800 mb-4">Tren Penggunaan AI (6 Bulan)</p>
                <div class="flex items-end gap-2 h-24">
                    ...
                    <?php $__currentLoopData = $aiMonthly->reverse(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <div class="flex-1 flex flex-col items-center gap-1">
                            <span class="text-[10px] text-gray-500"><?php echo e(number_format($m->total)); ?></span>
                            <div class="w-full rounded-t-lg"
                                style="height:<?php echo e(max(4, $pct * 0.8)); ?>px; background: linear-gradient(to top, #3b82f6, #6366f1);">
                            </div>
                            <span
                                class="text-[10px] text-gray-400"><?php echo e(\Carbon\Carbon::createFromFormat('Y-m', $m->month)->format('M')); ?></span>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </div>
            <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-200">
                    <p class="text-sm font-semibold text-gray-800">Penggunaan per Tenant — <?php echo e(now()->format('F Y')); ?>
                ...
                <div class="overflow-x-auto">
                    ...
                    'breakdown' => $breakdown,
                                    ...
                                    <td class="px-5 py-3 text-right text-gray-700 text-xs font-semibold">
                                    ...
                                    <td class="px-5 py-3 hidden md:table-cell">
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span
                                                class="text-[10px] text-gray-400 w-8 text-right"><?php echo e($pct); ?>%</span>
                                        </div>
                                    </td>
                            ...
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                <tr>
                                    <td colspan="5" class="px-6 py-10 text-center text-gray-400 text-sm">Belum ada
            ...
            <div class="flex flex-wrap gap-3 mb-4">
                <form method="GET" action="" class="flex gap-2 flex-wrap flex-1">
                    ...
                    <input type="text" name="action" value="<?php echo e(request('action')); ?>"
                        ...
                        class="flex-1 min-w-[180px] px-3 py-2 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <select name="tenant_id"
                        class="px-3 py-2 rounded-xl border border-gray-200 bg-white text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <?php if(request()->hasAny(['action', 'tenant_id'])): ?>
                        <a href="?tab=activity"
                            class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm rounded-xl transition">Reset</a>
                    <?php endif; ?>
                </form>
            </div>
            <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php $__empty_1 = true; $__currentLoopData = $activities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $act): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <?php
                                    $actionColor = match (true) {
                                        str_starts_with($act->action, 'ai_') => 'text-purple-600 bg-purple-100',
                                        str_contains($act->action, 'delete') => 'text-red-600 bg-red-100',
                                        str_contains($act->action, 'create') => 'text-green-600 bg-green-100',
                                        str_contains($act->action, 'login') => 'text-blue-600 bg-blue-100',
                                ...
                                <tr class="hover:bg-gray-50 transition cursor-pointer"
                                    ...
                                    <td class="px-4 py-3 max-w-xs">
                                        <p class="text-gray-700 text-xs truncate"><?php echo e($act->description); ?></p>
                                    ...
                                    <td class="px-4 py-3 hidden md:table-cell">
                                        <p class="text-gray-700 text-xs"><?php echo e($act->user?->name ?? 'System'); ?></p>
                                        <p class="text-gray-400 text-[10px]"><?php echo e($act->user?->email); ?></p>
                                    </td>
                                    <td class="px-4 py-3 hidden lg:table-cell">
                                        <p class="text-gray-500 text-xs font-mono"><?php echo e($act->ip_address ?? '-'); ?></p>
                                    </td>
                                    <td class="px-4 py-3">
                                        <p class="text-gray-500 text-xs"><?php echo e($act->created_at->format('d M Y')); ?></p>
                                        <p class="text-gray-400 text-[10px]"><?php echo e($act->created_at->format('H:i:s')); ?>
                            ...
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center text-gray-400 text-sm">Belum ada
                                        activity log.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                ...
                <?php if($activities->hasPages()): ?>
                    <div class="px-6 py-4 border-t border-gray-200"><?php echo e($activities->links()); ?></div>
                <?php endif; ?>
            </div>
        <?php endif; ?>
        ...
        <?php if($tab === 'anomaly'): ?>
            <div class="grid grid-cols-3 gap-3 mb-5">
                <?php $__currentLoopData = [['label' => 'Open', 'value' => $anomalyStats['open'], 'color' => 'text-gray-700', 'bg' => 'bg-gray-50'], ['label' => 'Critical', 'value' => $anomalyStats['critical'], 'color' => 'text-red-600', 'bg' => 'bg-red-50'], ['label' => 'Warning', 'value' => $anomalyStats['warning'], 'color' => 'text-amber-600', 'bg' => 'bg-amber-50']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="bg-white border border-gray-200 rounded-2xl p-4 text-center <?php echo e($s['bg']); ?>">
                        <p class="text-2xl font-bold <?php echo e($s['color']); ?>"><?php echo e($s['value']); ?></p>
                        <p class="text-xs text-gray-400 mt-1"><?php echo e($s['label']); ?></p>
                    </div>
            ...
            <div class="space-y-3">
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-1 flex-wrap">
                                    ...
                                    <span
                                        class="text-[11px] text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full"><?php echo e($a->type); ?></span>
                                    <span class="text-[11px] text-gray-400"><?php echo e($a->tenant?->name); ?></span>
                                </div>
                                <p class="text-sm font-semibold text-gray-800"><?php echo e($a->title); ?></p>
                                <p class="text-xs text-gray-500 mt-0.5 line-clamp-2"><?php echo e($a->description); ?></p>
                            </div>
                            <div class="flex items-center gap-2 shrink-0">
                                <p class="text-[10px] text-gray-400"><?php echo e($a->created_at->diffForHumans()); ?></p>
                                <button
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <div class="bg-white border border-gray-200 rounded-2xl p-12 text-center">
                        <p class="text-gray-400 text-sm">Tidak ada anomali yang open.</p>
                    </div>
                <?php endif; ?>
            </div>
        <?php endif; ?>
        ...
        <?php if($tab === 'health'): ?>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full <?php echo e($health['db_ok'] ? 'text-green-600 bg-green-100' : 'text-red-600 bg-red-100'); ?>">
                            <?php echo e($health['db_ok'] ? 'Online' : 'Error'); ?>

                        </span>
                    </div>
                    <p class="text-2xl font-bold text-gray-900"><?php echo e($health['db_latency_ms']); ?> <span
                            class="text-sm font-normal text-gray-500">ms</span></p>
                    <p class="text-xs text-gray-400 mt-1">Latency koneksi</p>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-sm font-semibold text-gray-800">Disk</p>
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full <?php echo e($health['disk_used_pct'] > 85 ? 'text-red-600 bg-red-100' : 'text-green-600 bg-green-100'); ?>">
                    ...
                    <p class="text-xs text-gray-400"><?php echo e($health['disk_free_gb']); ?> GB bebas dari
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-sm font-semibold text-gray-800">Log File</p>
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full <?php echo e($health['log_size_mb'] > 100 ? 'text-amber-600 bg-amber-100' : 'text-green-600 bg-green-100'); ?>">
                            <?php echo e($health['log_size_mb'] > 100 ? 'Besar' : 'Normal'); ?>

                        </span>
                    </div>
                    <p class="text-2xl font-bold text-gray-900"><?php echo e($health['log_size_mb']); ?> <span
                            class="text-sm font-normal text-gray-500">MB</span></p>
                    <p class="text-xs text-gray-400 mt-1">storage/logs/laravel.log</p>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <p class="text-sm font-semibold text-gray-800 mb-3">Tenant</p>
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Total</span><span
                                class="text-gray-800 font-semibold"><?php echo e($health['total_tenants']); ?></span></div>
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Aktif</span><span
                                class="text-green-600 font-semibold"><?php echo e($health['active_tenants']); ?></span></div>
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Total User</span><span
                                class="text-gray-800 font-semibold"><?php echo e($health['total_users']); ?></span></div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-sm font-semibold text-gray-800">Queue</p>
                        <span
                            class="text-[11px] px-2 py-0.5 rounded-full <?php echo e($health['queue_failed'] > 0 ? 'text-red-600 bg-red-100' : 'text-green-600 bg-green-100'); ?>">
                            <?php echo e($health['queue_failed'] > 0 ? $health['queue_failed'] . ' failed' : 'OK'); ?>

                        </span>
                    </div>
                    <p
                        class="text-2xl font-bold <?php echo e($health['queue_failed'] > 0 ? 'text-red-600' : 'text-gray-900'); ?>">
                        <?php echo e($health['queue_failed']); ?></p>
                    <p class="text-xs text-gray-400 mt-1">Failed jobs</p>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-5 cursor-pointer hover:border-gray-300 transition"
                    ...
                    <p class="text-sm font-semibold text-gray-800 mb-3">Server</p>
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs"><span class="text-gray-500">PHP</span><span
                                class="text-gray-800 font-mono"><?php echo e($health['php_version']); ?></span></div>
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Laravel</span><span
                                class="text-gray-800 font-mono"><?php echo e($health['laravel_version']); ?></span></div>
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Uptime</span><span
                                class="text-gray-800 font-mono"><?php echo e($health['uptime']); ?></span></div>
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Error Hari
                                Ini</span><span
                                class="<?php echo e($health['errors_today'] > 0 ? 'text-red-600' : 'text-green-600'); ?> font-semibold"><?php echo e($health['errors_today']); ?></span>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex justify-end">
                <a href="?tab=health"
                    class="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-900 border border-gray-200 hover:border-gray-300 px-4 py-2 rounded-xl transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <?php if($tab === 'modules'): ?>
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                <div class="bg-white border border-gray-200 rounded-2xl p-4">
                    <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Total Modul</p>
                    <p class="text-2xl font-black text-gray-900"><?php echo e(count($moduleHealth['modules'])); ?></p>
                </div>
                <div class="bg-white border border-gray-200 rounded-2xl p-4">
                    <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Total Alert</p>
                    <p
                        class="text-2xl font-black <?php echo e($moduleHealth['total_alerts'] > 0 ? 'text-amber-600' : 'text-green-600'); ?>">
                        <?php echo e($moduleHealth['total_alerts']); ?></p>
                </div>
                <div class="bg-white border border-gray-200 rounded-2xl p-4">
                    <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Critical</p>
                    <p
                        class="text-2xl font-black <?php echo e($moduleHealth['critical_alerts'] > 0 ? 'text-red-600' : 'text-green-600'); ?>">
                        <?php echo e($moduleHealth['critical_alerts']); ?></p>
                </div>
                <div class="bg-white border border-gray-200 rounded-2xl p-4">
                    <p class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Status</p>
                    <p
                        class="text-2xl font-black <?php echo e($moduleHealth['critical_alerts'] > 0 ? 'text-red-600' : ($moduleHealth['total_alerts'] > 0 ? 'text-amber-600' : 'text-green-600')); ?>">
                    ...
                    <?php
                        ...
                        $borderColor = $hasCritical
                            ...
                            : ($hasAlerts
                                ? 'border-amber-200'
                                : 'border-gray-200');
                        $bgColor = $hasCritical ? 'bg-red-50' : ($hasAlerts ? 'bg-amber-50' : 'bg-white');
                    ?>
                    <div class="<?php echo e($bgColor); ?> border <?php echo e($borderColor); ?> rounded-2xl p-4">
                        <div class="flex items-center justify-between mb-2">
                            <h4 class="text-sm font-semibold text-gray-900"><?php echo e($mod['label']); ?></h4>
                            <?php if($hasCritical): ?>
                                <span
                                    class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-red-100 text-red-600 border border-red-200">CRITICAL</span>
                            <?php elseif($hasAlerts): ?>
                                <span
                                    class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-600 border border-amber-200">WARNING</span>
                            <?php else: ?>
                                <span
                                    class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-green-100 text-green-600 border border-green-200">OK</span>
                            <?php endif; ?>
                        </div>
                        <p class="text-xs text-gray-400 mb-2">Total records: <span
                                class="text-gray-700 font-medium"><?php echo e(number_format($mod['total'])); ?></span></p>
                        <?php if($hasAlerts): ?>
                            <div class="space-y-1">
                ...
                return {
                    ...
                    badgeClass() {
                        ...
                        if (this.type === 'anomaly') {
                            ...
                            return m[this.d.severity] || 'text-blue-600 bg-blue-100 border-blue-200';
                        }
                        if (this.type === 'health') return this.d.ok ? 'text-green-600 bg-green-100 border-green-200' :
