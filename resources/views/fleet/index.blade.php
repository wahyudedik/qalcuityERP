    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            ...
            <p class="text-lg font-bold text-gray-900">Rp
                {{ number_format($stats['fuel_month'], 0, ',', '.') }}</p>
        </div>
    </div>
        ...
        <div
            ...
            <ul class="text-xs text-amber-600 space-y-0.5">
                @foreach ($alerts as $a)
                    <li>{{ $a }}</li>
                @endforeach
                @foreach ($upcomingMaint as $m)
                    <li>Maintenance {{ $m->vehicle?->plate_number ?? '-' }}: {{ $m->description }}
                        ({{ $m->scheduled_date?->format('d/m') }})
                    </li>
                @endforeach
            </ul>
        </div>
                    ...
                    @forelse($vehicles as $v)
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-gray-700">
                                {{ $v->name }}
                                @if ($v->brand)
                                    <span class="text-xs text-gray-400">({{ $v->brand }})</span>
                                @endif
                            </td>
                            <td
                            ...
                            <td class="px-4 py-3 text-right hidden md:table-cell text-gray-900">
                                {{ number_format($v->odometer, 0, ',', '.') }} km</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <button onclick='openEdit(@json($v))'
            ...
            <form method="POST" action="{{ route('fleet.vehicles.store') }}" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div><label
                            ...
                            type="text" name="brand" placeholder="Toyota" class="{{ $cls }}"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Model</label><input
                            type="text" name="model" placeholder="Avanza" class="{{ $cls }}"></div>
                    <div><label
                    ...
                    <div><label
                            ...
                            type="text" name="color" class="{{ $cls }}"></div>
                    ...
                    <div class="sm:col-span-2"><label
                            ...
                            type="text" name="notes" class="{{ $cls }}"></div>
                </div>
        ...
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form id="form-edit" method="POST" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Merek</label><input
                            type="text" name="brand" id="e-brand" class="{{ $cls }}"></div>
                    <div><label
                            class="block text-xs font-medium text-gray-600 mb-1">Model</label><input
                            type="text" name="model" id="e-model" class="{{ $cls }}"></div>
                    <div><label
