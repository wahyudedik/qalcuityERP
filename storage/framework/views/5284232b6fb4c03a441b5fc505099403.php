...
<?php $component->withAttributes([]); ?>
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
        <a href="<?php echo e(route('subscription-billing.index')); ?>" class="px-3 py-2 text-sm text-gray-500 dark:text-slate-400">← Subscriptions</a>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-200 dark:border-white/10 p-5">
            <div class="flex items-start justify-between mb-3">
                <div>
                    <h3 class="font-semibold text-gray-900 dark:text-white"><?php echo e($p->name); ?></h3>
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white mb-1">Rp <?php echo e(number_format($p->price, 0, ',', '.')); ?></p>
            <p class="text-xs text-gray-500 dark:text-slate-400 mb-3">/ <?php echo e($p->cycleLabel()); ?><?php echo e($p->trial_days > 0 ? " · {$p->trial_days} hari trial" : ''); ?></p>
            <?php if($p->features): ?>
            <ul class="text-xs text-gray-600 dark:text-slate-400 space-y-1 mb-3">
        ...
        <div class="col-span-full text-center py-12 text-gray-400 dark:text-slate-500">Belum ada plan.</div>
        <?php endif; ?>
    </div>
    ...
    <div id="modal-add-plan" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-md shadow-xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/10">
                <h3 class="font-semibold text-gray-900 dark:text-white">Buat Plan</h3>
                <button onclick="document.getElementById('modal-add-plan').classList.add('hidden')" class="text-gray-400 hover:text-gray-600 dark:hover:text-white">✕</button>
            </div>
            <form method="POST" action="<?php echo e(route('subscription-billing.plans.store')); ?>" class="p-6 space-y-4">
                ...
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Nama *</label><input type="text" name="name" required placeholder="Plan Premium" class="<?php echo e($cls); ?>"></div>
                <div class="grid grid-cols-2 gap-3">
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Kode</label><input type="text" name="code" placeholder="PREM" maxlength="30" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Harga (Rp) *</label><input type="number" name="price" required min="0" step="1000" class="<?php echo e($cls); ?>"></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Siklus *</label>
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Trial (hari)</label><input type="number" name="trial_days" min="0" value="0" class="<?php echo e($cls); ?>"></div>
                </div>
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Fitur (1 per baris)</label><textarea name="features" rows="3" placeholder="Fitur 1&#10;Fitur 2&#10;Fitur 3" class="<?php echo e($cls); ?>"></textarea></div>
                <div><label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Deskripsi</label><input type="text" name="description" class="<?php echo e($cls); ?>"></div>
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-add-plan').classList.add('hidden')" class="px-4 py-2 text-sm border border-gray-200 dark:border-white/10 rounded-xl text-gray-600 dark:text-slate-300">Batal</button>
