<x-app-layout>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">Approved (Belum Bayar)</p>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">Dibayar Bulan Ini</p>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($reimbursements as $r)
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 hidden sm:table-cell text-xs text-gray-500">{{ $r->categoryLabel() }}
                            </td>
                            ...
                            <td class="px-4 py-3 text-right font-medium text-gray-900">Rp
                                {{ number_format($r->amount, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-center">
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs bg-{{ $sc }}-100 text-{{ $sc }}-700 $sc }}-500/20 $sc }}-400">{{ ucfirst($r->status) }}</span>
                                @if ($r->reject_reason)
                                    <p class="text-xs text-red-400 mt-0.5">{{ Str::limit($r->reject_reason, 20) }}</p>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    @if ($r->status === 'submitted')
                                        ...
                                        <form method="POST" action="{{ route('reimbursement.reject', $r) }}"
                                            ...
                                            <button type="submit"
                                                class="text-xs px-2 py-1 bg-red-600 text-white rounded-lg hover:bg-red-700">Reject</button>
                                        </form>
                                    @elseif($r->status === 'approved')
                                        <form method="POST" action="{{ route('reimbursement.pay', $r) }}"
                                            class="inline" data-confirm="Bayar reimbursement ini?">
                                            @csrf
                                            <select name="payment_method"
                                                class="text-xs px-1 py-0.5 rounded border border-gray-200 bg-white text-gray-900">
                                                <option value="transfer">Transfer</option>
                                                <option value="cash">Cash</option>
                                            </select>
                                            <button type="submit"
                                                class="text-xs px-2 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700">Bayar</button>
                                        </form>
                                    @endif
                                    @endcanmodule
                                    @if (in_array($r->status, ['draft', 'submitted']))
                                        @canmodule('reimbursement', 'delete')
                                        <form method="POST" action="{{ route('reimbursement.destroy', $r) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center text-gray-400">Belum ada reimbursement.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form method="POST" action="{{ route('reimbursement.store') }}" enctype="multipart/form-data"
                ...
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Karyawan *</label>
                    <select name="employee_id" required class="{{ $cls }}">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div><label class="block text-xs font-medium text-gray-600 mb-1">Tanggal *</label><input
                            type="date" name="expense_date" required value="{{ date('Y-m-d') }}"
                            class="{{ $cls }}"></div>
                </div>
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Deskripsi *</label><input
                        type="text" name="description" required placeholder="Ongkos taksi meeting client"
                        class="{{ $cls }}"></div>
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Jumlah (Rp) *</label><input
                        type="number" name="amount" required min="1000" step="500"
                        class="{{ $cls }}"></div>
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Foto Struk/Bukti</label><input
                        type="file" name="receipt_image" accept="image/*" class="{{ $cls }}"></div>
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Catatan</label><input type="text"
                        name="notes" class="{{ $cls }}"></div>
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-add').classList.add('hidden')"
                        class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
                    <button type="submit"
                        class="px-4 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700">Ajukan</button>
                </div>
            </form>
        </div>
    </div>
