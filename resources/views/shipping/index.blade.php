<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                <div>
                    <label class="block text-xs text-gray-500 mb-1.5">Kota Asal</label>
                ...
                <div>
                    <label class="block text-xs text-gray-500 mb-1.5">Kota Tujuan</label>
                    <input id="rate-dest" type="text" placeholder="Contoh: 114"
                        class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 mb-1.5">Berat (kg)</label>
                ...
                <div>
                    <label class="block text-xs text-gray-500 mb-1.5">Kurir</label>
                    <select id="rate-courier" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                <select id="track-courier" class="w-full sm:w-auto bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500">
                    <option value="jne">JNE</option>
                    <option value="jnt">J&T</option>
                    <option value="sicepat">SiCepat</option>
                    <option value="pos">POS Indonesia</option>
                ...
                <input id="track-number" type="text" placeholder="Nomor resi..."
                    class="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-blue-500">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-3 font-medium text-white uppercase">{{ $s->courier }}</td>
                            <td class="px-6 py-3 text-gray-500 font-mono text-xs">{{ $s->tracking_number ?? '—' }}</td>
                            <td class="px-6 py-3 text-gray-700">{{ $s->destination }}</td>
                            <td class="px-6 py-3 text-right text-gray-500 hidden sm:table-cell">{{ $s->weight }} kg</td>
                            ...
                            <td class="px-6 py-3 text-gray-400 hidden md:table-cell">{{ $s->estimated_delivery?->format('d M Y') ?? '—' }}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-gray-400">Belum ada data pengiriman.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div class="px-6 py-4 border-t border-gray-200">{{ $shipments->links() }}</div>
        </div>
    </div>
    ...
    async function checkRate() {
        const res = await fetch('{{ route("shipping.rate") }}', {
            method: 'POST',
        ...
        if (!data.length) { el.innerHTML = '<p class="text-sm text-gray-500">Tidak ada data ongkir.</p>'; return; }
        el.innerHTML = '<div class="grid grid-cols-2 md:grid-cols-3 gap-2">' +
            data.map(r => `<div class="border border-gray-200 bg-gray-50 rounded-xl p-3">
                ...
                <p class="text-xs text-gray-400">ETD: ${r.cost[0]?.etd||'-'} hari</p>
            </div>`).join('') + '</div>';
    }

    async function trackShipment() {
        const res = await fetch('{{ route("shipping.track") }}', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf },
            body: JSON.stringify({
        ...
        el.classList.remove('hidden');
        el.innerHTML = `<pre class="text-xs bg-gray-50 text-gray-700 rounded-xl p-3 overflow-x-auto border border-gray-200">${JSON.stringify(data, null, 2)}</pre>`;
    }
    </script>
    @endpush
</x-app-layout>
