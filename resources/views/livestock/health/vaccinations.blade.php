...
@section('content')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="mt-2 text-3xl font-bold text-gray-900">
                    {{ number_format($stats['total_vaccinations']) }}</div>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <div class="text-sm font-medium text-gray-500">Akan Datang</div>
                <div class="mt-2 text-3xl font-bold text-blue-600">
                    {{ number_format($stats['upcoming_vaccinations']) }}</div>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <div class="text-sm font-medium text-gray-500">Selesai</div>
                <div class="mt-2 text-3xl font-bold text-green-600">
                    {{ number_format($stats['completed_vaccinations']) }}</div>
            </div>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($vaccinations as $vaccination)
                            <tr
                                class="hover:bg-gray-50 @if ($vaccination->isOverdue()) bg-red-50 @endif">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    {{ $vaccination->scheduled_date?->format('d M Y') ?? '-' }}
                                    @if ($vaccination->isOverdue())
                                        <span class="ml-2 text-xs text-red-600">Terlambat</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $vaccination->herd?->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $vaccination->vaccine_name }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    {{ $vaccination->dose_method ?? '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        ...
                                        {{ $vaccination->status === 'scheduled' ? 'Terjadwal' : ($vaccination->status === 'completed' ? 'Selesai' : ucfirst($vaccination->status)) }}
                                    </span>
                                </td>
                            </tr>
                        @empty
    ...
    <div id="addVaccinationModal"
        ...
        <div
            ...
            <form action="{{ route('livestock-enhancement.health.vaccinations.store') }}" method="POST">
                @csrf
                <div class="space-y-4">
                    <div>
