        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    ...
                    <tr>
                        <td class="px-4 py-3 font-mono text-xs text-gray-900 dark:text-white">{{ $s->number }}</td>
                        <td class="px-4 py-3 text-gray-700 dark:text-slate-300">{{ $s->warehouse->name ?? '-' }}</td>
                        <td class="px-4 py-3 text-center text-xs text-gray-500">{{ $s->opname_date->format('d/m/Y') }}</td>
                        <td class="px-4 py-3 text-center"><span class="px-2 py-0.5 rounded-full text-xs bg-{{ $sc }}-100 text-{{ $sc }}-700 dark:bg-{{ $sc }}-500/20 dark:text-{{ $sc }}-400">{{ ucfirst($s->status) }}</span></td>
                        <td class="px-4 py-3 text-center"><a href="{{ route('wms.opname.show', $s) }}" class="text-xs px-2 py-1 border border-gray-200 dark:border-white/10 rounded-lg text-gray-600 dark:text-slate-300">Detail</a></td>
                    </tr>
                    @empty
    ...
    <div id="modal-opname" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-[#1e293b] rounded-2xl w-full max-w-sm shadow-xl">
            ...
            <form method="POST" action="{{ route('wms.opname.store') }}" class="p-6 space-y-3">
                ...
                <div><label class="block text-xs text-gray-600 dark:text-slate-400 mb-1">Tanggal *</label><input type="date" name="opname_date" required value="{{ date('Y-m-d') }}" class="{{ $cls }}"></div>
