...
<?php $component->withAttributes([]); ?>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4">
        ...
        <div class="flex gap-2">
            <a href="<?php echo e(route('suppliers.index')); ?>" class="px-3 py-2 text-sm border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50">Supplier</a>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 font-mono text-xs font-medium text-gray-900"><?php echo e($po->number); ?></td>
                        <td class="px-4 py-3 hidden sm:table-cell text-gray-700"><?php echo e($po->supplier->name ?? '-'); ?></td>
                        <td class="px-4 py-3 hidden md:table-cell text-gray-500"><?php echo e($po->warehouse->name ?? '-'); ?></td>
                        <td class="px-4 py-3 text-right font-medium text-gray-900">Rp <?php echo e(number_format($po->total,0,',','.')); ?></td>
                        <td class="px-4 py-3 text-center">
                            <?php $colors = ['draft'=>'gray','sent'=>'blue','partial'=>'yellow','received'=>'green','cancelled'=>'red']; $c = $colors[$po->status] ?? 'gray'; ?>
                        ...
                        <td class="px-4 py-3 hidden sm:table-cell text-xs text-gray-500"><?php echo e($po->date->format('d M Y')); ?></td>
                        <td class="px-4 py-3 text-center">
                            <?php if($po->status !== 'received' && $po->status !== 'cancelled'): ?>
                            <form method="POST" action="<?php echo e(route('purchasing.orders.status', $po)); ?>" class="inline">
                                <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                <select name="status" onchange="this.form.submit()" class="text-xs px-2 py-1 rounded-lg border border-gray-200 bg-gray-50 text-gray-900">
                                    ...
                                    <option value="<?php echo e($v); ?>" <?php if($po->status===$v): echo 'selected'; endif; ?>><?php echo e($l); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </form>
                            <?php else: ?>
                            ...
                            <form method="POST" action="<?php echo e(route('purchasing.orders.destroy', $po)); ?>" class="inline ml-1"
                                onsubmit="return confirm('Hapus PO <?php echo e($po->number); ?>? Tindakan ini tidak bisa dibatalkan.')">
                                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                <button type="submit" class="p-1.5 rounded-lg text-red-500 hover:bg-red-50" title="Hapus PO">
                    ...
                    <tr><td colspan="7" class="px-4 py-12 text-center text-gray-400">Belum ada purchase order.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div id="modal-create-po" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl w-full max-w-2xl shadow-xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
                ...
                <button onclick="document.getElementById('modal-create-po').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form method="POST" action="<?php echo e(route('purchasing.orders.store')); ?>" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="supplier_id" required class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <div>
                        ...
                        <input type="date" name="date" value="<?php echo e(today()->format('Y-m-d')); ?>" required class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Estimasi Terima</label>
                        <input type="date" name="expected_date" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Pembayaran</label>
                        <select name="payment_type" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <button type="button" onclick="addPoItem()" class="text-xs text-blue-600 hover:underline">+ Tambah Item</button>
                    </div>
                    <div id="po-items" class="space-y-2">
                        <div class="po-item grid grid-cols-12 gap-2 items-center">
                            <div class="col-span-5">
                                <select name="items[0][product_id]" required class="w-full px-2 py-1.5 text-sm rounded-lg border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    <option value="">-- Produk --</option>
                                    <?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($p->id); ?>" data-price="<?php echo e($p->price_buy); ?>"><?php echo e($p->name); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </div>
                            <div class="col-span-3">
                                <input type="number" name="items[0][quantity]" placeholder="Qty" min="1" required class="w-full px-2 py-1.5 text-sm rounded-lg border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div class="col-span-3">
                                <input type="number" name="items[0][price]" placeholder="Harga" min="0" required class="w-full px-2 py-1.5 text-sm rounded-lg border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" onclick="document.getElementById('modal-create-po').classList.add('hidden')" class="px-4 py-2 text-sm border border-gray-200 rounded-xl text-gray-600">Batal</button>
    ...
    <script>
    let poItemCount = 1;
    const productOptions = `<?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($p->id); ?>" data-price="<?php echo e($p->price_buy); ?>"><?php echo e(addslashes($p->name)); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>`;
