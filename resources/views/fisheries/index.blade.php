            ...
            <div class="space-y-3">
                @foreach ($recent_activities as $activity)
                    <div class="flex items-start gap-3 p-3 rounded-lg bg-gray-50 dark:bg-[#0f172a]">
                        <div
                            class="w-8 h-8 rounded-full bg-{{ $activity['color'] ?? 'blue' }}-100 dark:bg-{{ $activity['color'] ?? 'blue' }}-500/20 flex items-center justify-center text-sm">
                            {!! $activity['icon'] ?? '📌' !!}
                        </div>
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">{{ $activity['time'] }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
