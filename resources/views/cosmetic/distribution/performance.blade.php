...
@section('content')
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($performance as $record)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ number_format($record->total_units, 0) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ number_format($record->order_count, 0) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                Rp {{ number_format($record->average_order_value, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-orange-600">
                                Rp {{ number_format($record->total_discount, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-purple-600">
                                Rp {{ number_format($record->total_commission, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-blue-600">
                                Rp {{ number_format($record->net_revenue, 0, ',', '.') }}</td>
                        </tr>
                    @empty
                        <tr>
    ...
    <div id="recordSaleModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
            ...
            <form method="POST" action="{{ route('cosmetic.distribution.sales.record') }}" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <select name="channel_id" required
                        ...
                        @foreach ($channels as $channel)
                            <option value="{{ $channel->id }}">
                                {{ $channel->channel_name }} ({{ $channel->type_label }})
                            </option>
                        @endforeach
                    </select>
                </div>
